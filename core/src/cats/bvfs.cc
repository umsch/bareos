  ...
  while (new_path && *new_path) {
    if (ppathid_cache.lookup(pathid)) {
      ...
      if (SqlNumRows() > 0) {
        ...
        Mmsg(cmd, "INSERT INTO PathHierarchy (PathId, PPathId) VALUES (%s,%lld)", pathid,
             (uint64_t)parent.PathId);
 ...
 *        OK    1
 */
bool BareosDb::UpdatePathHierarchyCache(JobControlRecord *jcr, pathid_cache &ppathid_cache,
                                        JobId_t JobId) {
...
void Bvfs::GetAllFileVersions(DBId_t pathid, const char *fname, const char *client) {
  ...
  db->FillQuery(query, BareosDb::SQL_QUERY_bvfs_versions_6, fname_esc, edit_uint64(pathid, ed1),
                client_esc, filter.c_str(), limit, offset);
...
bool Bvfs::ls_dirs() {
  ...
  *prev_dir = 0;
  ...
  db->FillQuery(special_dirs_query, BareosDb::SQL_QUERY_bvfs_ls_special_dirs_3, pathid, pathid,
                jobids);

  if (*pattern) {
    db->FillQuery(filter, BareosDb::SQL_QUERY_match_query, pattern);
  }
  db->FillQuery(sub_dirs_query, BareosDb::SQL_QUERY_bvfs_ls_sub_dirs_5, pathid, jobids,
                filter.c_str(), jobids, jobids);

  db->FillQuery(union_query, BareosDb::SQL_QUERY_bvfs_lsdirs_4, special_dirs_query.c_str(),
                sub_dirs_query.c_str(), limit, offset);
...
static void build_ls_files_query(JobControlRecord *jcr, BareosDb *db, PoolMem &query,
                                 const char *JobId, const char *PathId, const char *filter,
                                 int64_t limit, int64_t offset) {
  if (db->GetTypeIndex() == SQL_TYPE_POSTGRESQL) {
    db->FillQuery(query, BareosDb::SQL_QUERY_bvfs_list_files, JobId, PathId, JobId, PathId, filter,
                  limit, offset);
  } else {
    db->FillQuery(query, BareosDb::SQL_QUERY_bvfs_list_files, JobId, PathId, JobId, PathId, limit,
                  offset, filter, JobId, JobId);
  }
}
...
static bool CheckTemp(char *output_table) {
  ...
  if (output_table[0] == 'b' && output_table[1] == '2' && IsAnInteger(output_table + 2)) {
    return true;
  }
  return false;
}
...
bool Bvfs::compute_restore_list(char *fileid, char *dirid, char *hardlink, char *output_table) {
  ...
  /* check args */
  if ((*fileid && !Is_a_number_list(fileid)) || (*dirid && !Is_a_number_list(dirid)) ||
  ...
  if (db->GetTypeIndex() == SQL_TYPE_MYSQL) {
    Mmsg(query, "CREATE INDEX idx_%s ON %s (JobId)", output_table, output_table);
    Dmsg1(dbglevel_sql, "q=%s\n", query.c_str());
    if (!db->SqlQuery(query.c_str())) {
      Dmsg0(dbglevel, "Can't execute q\n");
      goto bail_out;
