 ...
 */
enum e_list_type { NF_LIST, RAW_LIST, HORZ_LIST, VERT_LIST };
...
class CATS_IMP_EXP BareosDb : public SmartAlloc, public BareosDbQueryEnum {
 ...
 private:
  ...
  void BuildPathHierarchy(JobControlRecord *jcr, pathid_cache &ppathid_cache, char *org_pathid,
                          char *path);
 ...
 public:
  ...
  bool UpdateDB(const char *file, int line, JobControlRecord *jcr, const char *UpdateCmd,
                int nr_afr);
  ...
  /* sql_find.c */
  bool FindLastJobStartTime(JobControlRecord *jcr, JobDbRecord *jr, POOLMEM *&stime, char *job,
                            int JobLevel);
  ...
  int FindNextVolume(JobControlRecord *jcr, int index, bool InChanger, MediaDbRecord *mr,
  ...
  bool GetBaseFileList(JobControlRecord *jcr, bool use_md5, DB_RESULT_HANDLER *ResultHandler,
                       void *ctx);
  ...
  bool GetFileAttributesRecord(JobControlRecord *jcr, char *filename, JobDbRecord *jr,
                               FileDbRecord *fdbr);
  ...
  bool GetMediaIds(JobControlRecord *jcr, MediaDbRecord *mr, PoolMem &volumes, int *num_ids,
                   DBId_t **ids);
  ...
  bool GetFileList(JobControlRecord *jcr, char *jobids, bool use_md5, bool use_delta,
  ...
  bool GetNdmpEnvironmentString(JobControlRecord *jcr, JobDbRecord *jr,
                                DB_RESULT_HANDLER *ResultHandler, void *ctx);
  bool GetNdmpEnvironmentString(JobControlRecord *jcr, JobId_t JobId,
                                DB_RESULT_HANDLER *ResultHandler, void *ctx);
  bool PrepareMediaSqlQuery(JobControlRecord *jcr, MediaDbRecord *mr, PoolMem *querystring,
                            PoolMem &volumes);
  bool VerifyMediaIdsFromSingleStorage(JobControlRecord *jcr, dbid_list &mediaIds);

  /* sql_list.c */
  void ListPoolRecords(JobControlRecord *jcr, PoolDbRecord *pr, OutputFormatter *sendit,
                       e_list_type type);
  void ListJobRecords(JobControlRecord *jcr, JobDbRecord *jr, const char *range,
                      const char *clientname, int jobstatus, int joblevel, const char *volumename,
                      utime_t since_time, bool last, bool count, OutputFormatter *sendit,
                      e_list_type type);
  ...
  void ListFilesets(JobControlRecord *jcr, JobDbRecord *jr, const char *range,
  ...
  void ListMediaRecords(JobControlRecord *jcr, MediaDbRecord *mdbr, const char *range, bool count,
                        OutputFormatter *sendit, e_list_type type);
  void ListJobmediaRecords(JobControlRecord *jcr, JobId_t JobId, OutputFormatter *sendit,
                           e_list_type type);
  void ListVolumesOfJobid(JobControlRecord *jcr, JobId_t JobId, OutputFormatter *sendit,
                          e_list_type type);
  void ListJoblogRecords(JobControlRecord *jcr, JobId_t JobId, const char *range, bool count,
                         OutputFormatter *sendit, e_list_type type);
  ...
  void ListJobstatisticsRecords(JobControlRecord *jcr, uint32_t JobId, OutputFormatter *sendit,
                                e_list_type type);
  bool ListSqlQuery(JobControlRecord *jcr, const char *query, OutputFormatter *sendit,
                    e_list_type type, bool verbose);
  bool ListSqlQuery(JobControlRecord *jcr, SQL_QUERY_ENUM query, OutputFormatter *sendit,
                    e_list_type type, bool verbose);
  bool ListSqlQuery(JobControlRecord *jcr, const char *query, OutputFormatter *sendit,
                    e_list_type type, const char *description, bool verbose = false);
  bool ListSqlQuery(JobControlRecord *jcr, SQL_QUERY_ENUM query, OutputFormatter *sendit,
                    e_list_type type, const char *description, bool verbose);
  void ListClientRecords(JobControlRecord *jcr, char *clientname, OutputFormatter *sendit,
                         e_list_type type);
  void ListCopiesRecords(JobControlRecord *jcr, const char *range, const char *jobids,
                         OutputFormatter *sendit, e_list_type type);
  ...
  /* Low level methods */
  bool MatchDatabase(const char *db_driver, const char *db_name, const char *db_address,
                     int db_port);
  BareosDb *clone_database_connection(JobControlRecord *jcr, bool mult_db_connections,
                                      bool get_pooled_connection = true, bool need_private = false);
  ...
  virtual bool BigSqlQuery(const char *query, DB_RESULT_HANDLER *ResultHandler, void *ctx) {
...
struct SqlPoolEntry {
  ...
  time_t
      last_update; /**< When was this connection last updated either used or put back on the pool */
...
struct SqlPoolDescriptor {
  ...
  int increment_connections; /**< Increase/Decrease the number of connection in the pool with this
                                value */
