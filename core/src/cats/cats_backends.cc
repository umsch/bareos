...
void DbSetBackendDirs(alist *new_backend_dirs) { backend_dirs = new_backend_dirs; }
...
static inline backend_interface_mapping_t *lookup_backend_interface_mapping(
  ...
  for (backend_interface_mapping = backend_interface_mappings;
       backend_interface_mapping->interface_name != NULL; backend_interface_mapping++) {

    Dmsg3(100,
          "db_init_database: Trying to find mapping of given interfacename %s to mapping "
          "interfacename %s, partly_compare = %s\n",
...
BareosDb *db_init_database(JobControlRecord *jcr, const char *db_driver, const char *db_name,
    ...
    foreach_alist(backend_shared_library, loaded_backends) {
      if (backend_shared_library->interface_type_id ==
        ...
        return backend_shared_library->backend_instantiate(
            jcr, db_driver, db_name, db_user, db_password, db_address, db_port, db_socket,
...
#else
    Mmsg(shared_library_name, "libbareoscats-%s%s", backend_interface_mapping->interface_name,
         DYN_LIB_EXTENSION);
    {
#endif
      ...
      if (backend_instantiate == NULL) {
        PmStrcpy(error, dlerror());
        Jmsg(jcr, M_ERROR, 0,
             _("Lookup of backend_instantiate in shared library %s failed: ERR=%s\n"),
             shared_library_name.c_str(), error.c_str());
  ...
  if (dl_handle) {
    /*
     * Create a new loaded shared library entry and tack it onto the list of loaded backend shared
     * libs.
     */
