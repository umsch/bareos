...
BareosDbDBI::BareosDbDBI(JobControlRecord *jcr, const char *db_driver, const char *db_name,
                         const char *db_user, const char *db_password, const char *db_address,
                         int db_port, const char *db_socket, bool mult_db_connections,
                         bool disable_batch_insert, bool need_private, bool try_reconnect,
                         bool exit_on_fatal) {
  char *p;
 ...
 */
bool BareosDbDBI::SqlQueryWithHandler(const char *query, DB_RESULT_HANDLER *ResultHandler,
                                      void *ctx) {
  bool retval = true;
  SQL_ROW row;
  ...
  if (dbi_result_seek_row(result, row_number)) {
    ...
    switch (dbitype) {
      ...
      case DBI_TYPE_STRING:
        if (field_length) {
          field_length =
              Bsnprintf(buf, field_length + 1, "%s", dbi_result_get_string(result, field_name));
        } else {
          buf[0] = 0;
        }
        break;
      case DBI_TYPE_BINARY:
        ...
        if (field_length) {
          field_length =
              Bsnprintf(buf, field_length + 1, "%s", dbi_result_get_binary(result, field_name));
        } else {
          buf[0] = 0;
        }
        break;
      case DBI_TYPE_DATETIME:
        ...
        if (last == -1) {
          ...
          Blocaltime(&last, &tm);
          field_length = Bsnprintf(buf, 20, "%04d-%02d-%02d %02d:%02d:%02d", (tm.tm_year + 1900),
...
bool BareosDbDBI::SqlBatchInsert(JobControlRecord *jcr, AttributesDbRecord *ar) {
  ...
  switch (db_type_) {
    ...
    case SQL_TYPE_POSTGRESQL:
      ...
      len = Mmsg(cmd, "%u\t%s\t%s\t%s\t%s\t%s\t%u\n", ar->FileIndex, edit_int64(ar->JobId, ed1),
                 esc_path, esc_name, ar->attr, digest, ar->DeltaSeq);
      ...
      if (custom_function != NULL) {
         ...
         */
        custom_function_error = (custom_function_error_t)dbi_driver_specific_function(
...
#endif
{
  ...
  mdb = New(BareosDbDBI(jcr, db_driver, db_name, db_user, db_password, db_address, db_port,
