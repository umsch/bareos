...
BareosDbMysql::BareosDbMysql(JobControlRecord *jcr, const char *db_driver, const char *db_name,
                             const char *db_user, const char *db_password, const char *db_address,
                             int db_port, const char *db_socket, bool mult_db_connections,
                             bool disable_batch_insert, bool try_reconnect, bool exit_on_fatal,
                             bool need_private) {
  /*
 ...
 */
bool BareosDbMysql::SqlQueryWithHandler(const char *query, DB_RESULT_HANDLER *ResultHandler,
                                        void *ctx) {
  int status;
  SQL_ROW row;
  bool send = true;
...
const char *BareosDbMysql::sql_strerror(void) { return mysql_error(db_handle_); }

void BareosDbMysql::SqlDataSeek(int row) { return mysql_data_seek(result_, row); }

int BareosDbMysql::SqlAffectedRows(void) { return mysql_affected_rows(db_handle_); }
...
bool BareosDbMysql::SqlFieldIsNotNull(int field_type) { return IS_NOT_NULL(field_type); }

bool BareosDbMysql::SqlFieldIsNumeric(int field_type) { return IS_NUM(field_type); }
...
bool BareosDbMysql::SqlBatchInsert(JobControlRecord *jcr, AttributesDbRecord *ar) {
  ...
  if (changes == 0) {
    Mmsg(cmd,
         ...
         "(%u,%s,'%s','%s','%s','%s',%u,'%s','%s')",
         ar->FileIndex, edit_int64(ar->JobId, ed1), esc_path, esc_name, ar->attr, digest,
         ar->DeltaSeq, edit_uint64(ar->Fhinfo, ed2), edit_uint64(ar->Fhnode, ed3));
    changes++;
  } else {
    /*
    ...
    Mmsg(esc_obj, ",(%u,%s,'%s','%s','%s','%s',%u,%u,%u)", ar->FileIndex,
         edit_int64(ar->JobId, ed1), esc_path, esc_name, ar->attr, digest, ar->DeltaSeq, ar->Fhinfo,
         ar->Fhnode);
...
extern "C" BareosDb CATS_IMP_EXP *backend_instantiate(
    JobControlRecord *jcr, const char *db_driver, const char *db_name, const char *db_user,
    ...
    bool need_private)
#else
BareosDb *db_init_database(JobControlRecord *jcr, const char *db_driver, const char *db_name,
                           const char *db_user, const char *db_password, const char *db_address,
                           int db_port, const char *db_socket, bool mult_db_connections,
                           bool disable_batch_insert, bool try_reconnect, bool exit_on_fatal,
                           bool need_private)
#endif
{
  ...
  mdb = New(BareosDbMysql(jcr, db_driver, db_name, db_user, db_password, db_address, db_port,
