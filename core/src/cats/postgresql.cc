...
BareosDbPostgresql::BareosDbPostgresql(JobControlRecord *jcr, const char *db_driver,
 ...
 */
bool BareosDbPostgresql::BigSqlQuery(const char *query, DB_RESULT_HANDLER *ResultHandler,
                                     void *ctx) {
  SQL_ROW row;
  bool retval = false;
 ...
 * the ResultHandler is called with the ctx.
 */
bool BareosDbPostgresql::SqlQueryWithHandler(const char *query, DB_RESULT_HANDLER *ResultHandler,
                                             void *ctx) {
  SQL_ROW row;
  bool retval = true;
...
const char *BareosDbPostgresql::sql_strerror(void) { return PQerrorMessage(db_handle_); }
...
bool BareosDbPostgresql::SqlBatchInsert(JobControlRecord *jcr, AttributesDbRecord *ar) {
  ...
  len = Mmsg(cmd, "%u\t%s\t%s\t%s\t%s\t%s\t%u\t%s\t%s\n", ar->FileIndex, edit_int64(ar->JobId, ed1),
             esc_path, esc_name, ar->attr, digest, ar->DeltaSeq, edit_uint64(ar->Fhinfo, ed2),
...
#endif
{
  ...
  mdb = New(BareosDbPostgresql(jcr, db_driver, db_name, db_user, db_password, db_address, db_port,
