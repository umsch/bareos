...
int BareosDb::ListResult(list_result_handler* handler)
{
  auto num_fields = SqlNumFields();

  SqlFieldSeek(0);
  for (int i = 0; i < num_fields; i++) {
    Dmsg1(800, "ListResult processing field %d\n", i);

    auto* field = SqlFetchField();
    if (!field) { break; }

    handler->add_field(field, {.numeric = SqlFieldIsNumeric(field->type),
                               .nonnull = SqlFieldIsNotNull(field->type)});
  }
