...
DBId_t dbid_list::get(int i) const {
  if (i >= size()) {
    Emsg2(M_ERROR_TERM, 0, _("Unable to access dbid_list entry %d. Only %d entries available.\n"),
          i, size());
    return (DBId_t)0;
  }
  return DBId[i];
 ...
 *          true on success
 */
bool BareosDb::UpdateDB(const char *file, int line, JobControlRecord *jcr, const char *UpdateCmd,
                        int nr_afr) {
  int num_rows;
...
void BareosDb::DbDebugPrint(FILE *fp) {
  ...
  fprintf(fp, "BareosDb=%p db_name=%s db_user=%s connected=%s\n", this, NPRTB(get_db_name()),
          NPRTB(get_db_user()), IsConnected() ? "true" : "false");
