...
bool BareosDb::CreateJobRecord(JobControlRecord *jcr, JobDbRecord *jr) {
  ...
  Mmsg(cmd,
       ...
       esc_ujobname, esc_jobname, (char)(jr->JobType), (char)(jr->JobLevel), (char)(jr->JobStatus),
       dt, edit_uint64(JobTDate, ed1), edit_int64(jr->ClientId, ed2), buf.c_str());
...
bool BareosDb::CreateJobmediaRecord(JobControlRecord *jcr, JobMediaDbRecord *jm) {
   ...
   */
  Mmsg(cmd, "SELECT count(*) from JobMedia WHERE JobId=%s", edit_int64(jm->JobId, ed1));
  ...
  Mmsg(cmd,
       ...
       edit_int64(jm->JobId, ed1), edit_int64(jm->MediaId, ed2), jm->FirstIndex, jm->LastIndex,
  ...
  if (!INSERT_DB(jcr, cmd)) {
     ...
     */
    Mmsg(cmd, "UPDATE Media SET EndFile=%u, EndBlock=%u WHERE MediaId=%u", jm->EndFile,
         jm->EndBlock, jm->MediaId);
...
bool BareosDb::CreatePoolRecord(JobControlRecord *jcr, PoolDbRecord *pr) {
  ...
  Mmsg(cmd,
       ...
       esc_poolname, pr->NumVols, pr->MaxVols, pr->UseOnce, pr->UseCatalog, pr->AcceptAnyVolume,
       pr->AutoPrune, pr->Recycle, edit_uint64(pr->VolRetention, ed1),
       edit_uint64(pr->VolUseDuration, ed2), pr->MaxVolJobs, pr->MaxVolFiles,
       edit_uint64(pr->MaxVolBytes, ed3), pr->PoolType, pr->LabelType, esc_lf,
       edit_int64(pr->RecyclePoolId, ed4), edit_int64(pr->ScratchPoolId, ed5), pr->ActionOnPurge,
...
bool BareosDb::CreateDeviceRecord(JobControlRecord *jcr, DeviceDbRecord *dr) {
  ...
  Mmsg(cmd, "SELECT DeviceId,Name FROM Device WHERE Name='%s' AND StorageId = %s", esc,
       edit_int64(dr->StorageId, ed1));
   ...
   * Must create it
   */
  Mmsg(cmd, "INSERT INTO Device (Name,MediaTypeId,StorageId) VALUES ('%s',%s,%s)", esc,
       edit_uint64(dr->MediaTypeId, ed1), edit_int64(dr->StorageId, ed2));
...
bool BareosDb::CreateMediaRecord(JobControlRecord *jcr, MediaDbRecord *mr) {
  ...
  Mmsg(cmd,
       ...
       edit_uint64(mr->VolCapacityBytes, ed2), mr->Recycle, edit_uint64(mr->VolRetention, ed3),
       edit_uint64(mr->VolUseDuration, ed4), mr->MaxVolJobs, mr->MaxVolFiles, esc_status, mr->Slot,
       edit_uint64(mr->VolBytes, ed5), mr->InChanger, edit_int64(mr->VolReadTime, ed6),
       edit_int64(mr->VolWriteTime, ed7), mr->LabelType, edit_int64(mr->StorageId, ed8),
       edit_int64(mr->DeviceId, ed9), edit_int64(mr->LocationId, ed10),
       edit_int64(mr->ScratchPoolId, ed11), edit_int64(mr->RecyclePoolId, ed12), mr->Enabled,
...
bool BareosDb::CreateClientRecord(JobControlRecord *jcr, ClientDbRecord *cr) {
  ...
  Mmsg(cmd,
       ...
       "('%s','%s',%d,%s,%s)",
       esc_clientname, esc_uname, cr->AutoPrune, edit_uint64(cr->FileRetention, ed1),
...
bool BareosDb::CreateCounterRecord(JobControlRecord *jcr, CounterDbRecord *cr) {
   ...
   * Must create it
   */
  FillQuery(SQL_QUERY_insert_counter_values, esc, cr->MinValue, cr->MaxValue, cr->CurrentValue,
            cr->WrapCounter);
 ...
 *  - insert missing paths into path with another single query (lock Path table to avoid
 * duplicates).
...
bool BareosDb::CreateFileRecord(JobControlRecord *jcr, AttributesDbRecord *ar) {
  ...
  Mmsg(cmd,
       ...
       ar->FileIndex, ar->JobId, ar->PathId, esc_name, ar->attr, digest, ar->DeltaSeq, ar->Fhinfo,
       ar->Fhnode);
...
bool BareosDb::CreateAttributesRecord(JobControlRecord *jcr, AttributesDbRecord *ar) {
   ...
   */
  if (!(ar->Stream == STREAM_UNIX_ATTRIBUTES || ar->Stream == STREAM_UNIX_ATTRIBUTES_EX)) {
...
bool BareosDb::CreateBaseFileAttributesRecord(JobControlRecord *jcr, AttributesDbRecord *ar) {
  ...
  Mmsg(cmd, "INSERT INTO basefile%lld (Path, Name) VALUES ('%s','%s')", (uint64_t)jcr->JobId,
       esc_path, esc_name);
...
bool BareosDb::CreateRestoreObjectRecord(JobControlRecord *jcr, RestoreObjectDbRecord *ro) {
  ...
  Mmsg(cmd,
       ...
       esc_name, esc_plug_name, esc_obj, ro->object_len, ro->object_full_len, ro->object_index,
...
bool BareosDb::CreateQuotaRecord(JobControlRecord *jcr, ClientDbRecord *cr) {
  ...
  DbLock(this);
  Mmsg(cmd, "SELECT ClientId FROM Quota WHERE ClientId='%s'", edit_uint64(cr->ClientId, ed1));
 ...
 *          true on success
 */
bool BareosDb::CreateNdmpEnvironmentString(JobControlRecord *jcr, JobDbRecord *jr, char *name,
                                           char *value) {
  bool retval = false;
  char ed1[50], ed2[50];
...
bool BareosDb::CreateDeviceStatistics(JobControlRecord *jcr, DeviceStatisticsDbRecord *dsr) {
  ...
  Mmsg(cmd,
       ...
       edit_uint64(dsr->VolCatFiles, ed11), edit_uint64(dsr->VolCatBlocks, ed12));
