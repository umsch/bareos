 ...
 */
bool BareosDb::FindJobStartTime(JobControlRecord *jcr, JobDbRecord *jr, POOLMEM *&stime,
                                char *job) {
  bool retval = false;
  SQL_ROW row;
  char ed1[50], ed2[50];
  ...
  if (jr->JobId == 0) {
    ...
    Mmsg(cmd, "SELECT StartTime, Job FROM Job WHERE Job.JobId=%s", edit_int64(jr->JobId, ed1));
  }
 ...
 */
bool BareosDb::FindLastJobStartTime(JobControlRecord *jcr, JobDbRecord *jr, POOLMEM *&stime,
 ...
 */
bool BareosDb::FindFailedJobSince(JobControlRecord *jcr, JobDbRecord *jr, POOLMEM *stime,
                                  int &JobLevel) {
  bool retval = false;
  SQL_ROW row;
  char ed1[50], ed2[50];
  ...
  Mmsg(cmd,
       ...
       "ORDER BY StartTime DESC LIMIT 1",
       jr->JobType, L_FULL, L_DIFFERENTIAL, esc_jobname, edit_int64(jr->ClientId, ed1),
       edit_int64(jr->FileSetId, ed2), stime);
  if (!QUERY_DB(jcr, cmd)) {
    goto bail_out;
  }
 ...
 *          false on failure
 */
bool BareosDb::FindLastJobid(JobControlRecord *jcr, const char *Name, JobDbRecord *jr) {
  ...
  if (jr->JobLevel == L_VERIFY_CATALOG) {
    ...
    Mmsg(cmd,
         ...
         L_VERIFY_INIT, esc_jobname, edit_int64(jr->ClientId, ed1));
             ...
             jr->JobLevel == L_VERIFY_DISK_TO_CATALOG || jr->JobType == JT_BACKUP) {
    if (Name) {
      EscapeString(jcr, esc_jobname, (char *)Name, MIN(strlen(Name), sizeof(esc_jobname)));
      Mmsg(cmd,
 ...
 *          numrows on success
 */
int BareosDb::FindNextVolume(JobControlRecord *jcr, int item, bool InChanger, MediaDbRecord *mr,
                             const char *unwanted_volumes) {
  char ed1[50];
  int num_rows = 0;
  SQL_ROW row = NULL;
...
retry_fetch:
  if (find_oldest) {
    ...
    if (bstrcmp(mr->VolStatus, "Recycle") || bstrcmp(mr->VolStatus, "Purged")) {
      PmStrcpy(
          order,
          "AND Recycle=1 ORDER BY LastWritten ASC,MediaId"); /* Take oldest that can be recycled */
    } else {
      FillQuery(order,
                SQL_QUERY_sql_media_order_most_recently_written); /* Take most recently written */
    }
    ...
    Mmsg(cmd,
         ...
         "%s "
         "%s LIMIT %d",
         edit_int64(mr->PoolId, ed1), esc_type, esc_status, changer.c_str(), order.c_str(), item);
  }
