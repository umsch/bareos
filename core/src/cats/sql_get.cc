 ...
 */
bool BareosDb::GetFileAttributesRecord(JobControlRecord *jcr, char *filename, JobDbRecord *jr,
                                       FileDbRecord *fdbr) {
  bool retval;
...
bool BareosDb::GetFileRecord(JobControlRecord *jcr, JobDbRecord *jr, FileDbRecord *fdbr) {
  ...
  if (jcr->getJobLevel() == L_VERIFY_DISK_TO_CATALOG) {
    Mmsg(cmd,
         ...
         edit_int64(fdbr->PathId, ed1), esc_name, edit_int64(jr->ClientId, ed3));
  } else if (jcr->getJobLevel() == L_VERIFY_VOLUME_TO_CATALOG) {
    Mmsg(cmd,
         "SELECT FileId, LStat, MD5, Fhinfo, Fhnode FROM File WHERE File.JobId=%s AND "
         "File.PathId=%s AND "
         "File.Name='%s' AND File.FileIndex=%u",
         edit_int64(fdbr->JobId, ed1), edit_int64(fdbr->PathId, ed2), esc_name, jr->FileIndex);
  } else {
    Mmsg(cmd,
         "SELECT FileId, LStat, MD5, Fhinfo, Fhnode FROM File WHERE File.JobId=%s AND "
         "File.PathId=%s AND "
 ...
 * Returns: number of volumes on success
 */
int BareosDb::GetJobVolumeParameters(JobControlRecord *jcr, JobId_t JobId,
  ...
  if (QUERY_DB(jcr, cmd)) {
    ...
    if (num_rows <= 0) {
      ...
      for (i = 0; i < retval; i++) {
        if (SId[i] != 0) {
          Mmsg(cmd, "SELECT Name from Storage WHERE StorageId=%s", edit_int64(SId[i], ed1));
          if (QUERY_DB(jcr, cmd)) {
...
bool BareosDb::GetPoolRecord(JobControlRecord *jcr, PoolDbRecord *pdbr) {
  ...
  if (pdbr->PoolId != 0) { /* find by id */
    ...
    Mmsg(cmd,
         ...
         "AutoPrune,Recycle,VolRetention,VolUseDuration,MaxVolJobs,MaxVolFiles,"
         "MaxVolBytes,PoolType,LabelType,LabelFormat,RecyclePoolId,ScratchPoolId,"
         "ActionOnPurge,MinBlocksize,MaxBlocksize FROM Pool WHERE Pool.Name='%s'",
         esc);
  }
  if (QUERY_DB(jcr, cmd)) {
    num_rows = SqlNumRows();
...
bool BareosDb::GetStorageRecord(JobControlRecord *jcr, StorageDbRecord *sdbr) {
  ...
  if (sdbr->StorageId != 0) { /* find by id */
    Mmsg(cmd, "SELECT StorageId,Name,AutoChanger FROM Storage WHERE Storage.StorageId=%s",
         edit_int64(sdbr->StorageId, ed1));
  } else { /* find by name */
    EscapeString(jcr, esc, sdbr->Name, strlen(sdbr->Name));
    Mmsg(cmd, "SELECT StorageId,Name,Autochanger FROM Storage WHERE Storage.Name='%s'", esc);
  }
  if (QUERY_DB(jcr, cmd)) {
    num_rows = SqlNumRows();
    if (num_rows > 1) {
...
bool BareosDb::PrepareMediaSqlQuery(JobControlRecord *jcr, MediaDbRecord *mr, PoolMem &volumes) {
  ...
  Mmsg(cmd, "SELECT DISTINCT MediaId FROM Media WHERE Recycle=%d AND Enabled=%d ", mr->Recycle,
       mr->Enabled);
 ...
 */
bool BareosDb::GetMediaIds(JobControlRecord *jcr, MediaDbRecord *mr, PoolMem &volumes, int *num_ids,
                           DBId_t *ids[]) {
  bool ok = false;
  SQL_ROW row;
  int i = 0;
...
bool BareosDb::GetFileList(JobControlRecord *jcr, char *jobids, bool use_md5, bool use_delta,
  ...
  if (use_delta) {
    FillQuery(query2, SQL_QUERY_select_recent_version_with_basejob_and_delta, jobids, jobids,
              jobids, jobids);
  } else {
...
bool BareosDb::GetBaseFileList(JobControlRecord *jcr, bool use_md5,
                               DB_RESULT_HANDLER *ResultHandler, void *ctx) {
  PoolMem query(PM_MESSAGE);
...
bool BareosDb::GetVolumeJobids(JobControlRecord *jcr, MediaDbRecord *mr, db_list_ctx *lst) {
  ...
  DbLock(this);
  Mmsg(cmd, "SELECT DISTINCT JobId FROM JobMedia WHERE MediaId=%s", edit_int64(mr->MediaId, ed1));
 ...
 *         true: on success
 */
bool BareosDb::GetQuotaJobbytesNofailed(JobControlRecord *jcr, JobDbRecord *jr,
  ...
  FillQuery(SQL_QUERY_get_quota_jobbytes_nofailed, edit_uint64(jr->ClientId, ed1),
            edit_uint64(jr->JobId, ed2), dt);
  if (QUERY_DB(jcr, cmd)) {
    num_rows = SqlNumRows();
    if (num_rows == 1) {
 ...
 *         true: on success
 */
bool BareosDb::GetNdmpEnvironmentString(JobControlRecord *jcr, JobId_t JobId,
                                        DB_RESULT_HANDLER *ResultHandler, void *ctx) {
 ...
 *         true: on success
 */
bool BareosDb::GetNdmpEnvironmentString(JobControlRecord *jcr, JobDbRecord *jr,
                                        DB_RESULT_HANDLER *ResultHandler, void *ctx) {
  PoolMem query(PM_MESSAGE);
  char ed1[50], ed2[50];
  db_int64_ctx lctx;
  ...
  Mmsg(query,
       ...
       edit_uint64(jr->VolSessionId, ed1), edit_uint64(jr->VolSessionTime, ed2));
  if (!SqlQueryWithHandler(query.c_str(), db_int64_handler, &lctx)) {
    goto bail_out;
  }
 ...
 * over VolumeName. The caller must free ids if non-NULL.
 */
bool BareosDb::PrepareMediaSqlQuery(JobControlRecord *jcr, MediaDbRecord *mr, PoolMem *querystring,
                                    PoolMem &volumes) {
  bool ok = true;
  char ed1[50];
  char esc[MAX_NAME_LENGTH * 2 + 1];
