...
bool BareosDb::ListSqlQuery(JobControlRecord *jcr, BareosDb::SQL_QUERY_ENUM query,
                            OutputFormatter *sendit, e_list_type type, bool verbose) {
  return ListSqlQuery(jcr, query, sendit, type, "query", verbose);
}
...
bool BareosDb::ListSqlQuery(JobControlRecord *jcr, BareosDb::SQL_QUERY_ENUM query,
                            OutputFormatter *sendit, e_list_type type, const char *description,
                            bool verbose) {
  return ListSqlQuery(jcr, get_predefined_query(query), sendit, type, description, verbose);
}

void BareosDb::ListPoolRecords(JobControlRecord *jcr, PoolDbRecord *pdbr, OutputFormatter *sendit,
                               e_list_type type) {
...
void BareosDb::ListClientRecords(JobControlRecord *jcr, char *clientname, OutputFormatter *sendit,
                                 e_list_type type) {
  DbLock(this);
...
void BareosDb::ListStorageRecords(JobControlRecord *jcr, OutputFormatter *sendit,
                                  e_list_type type) {
  DbLock(this);
 ...
 */
void BareosDb::ListMediaRecords(JobControlRecord *jcr, MediaDbRecord *mdbr, const char *range,
...
void BareosDb::ListJobmediaRecords(JobControlRecord *jcr, uint32_t JobId, OutputFormatter *sendit,
                                   e_list_type type) {
  char ed1[50];
...
void BareosDb::ListVolumesOfJobid(JobControlRecord *jcr, uint32_t JobId, OutputFormatter *sendit,
                                  e_list_type type) {
  char ed1[50];
...
void BareosDb::ListCopiesRecords(JobControlRecord *jcr, const char *range, const char *JobIds,
  ...
  if (JobIds && JobIds[0]) {
    Mmsg(str_jobids, " AND (Job.PriorJobId IN (%s) OR Job.JobId IN (%s)) ", JobIds, JobIds);
  }
...
void BareosDb::ListJoblogRecords(JobControlRecord *jcr, uint32_t JobId, const char *range,
                                 bool count, OutputFormatter *sendit, e_list_type type) {
  char ed1[50];
 ...
 *
 */
void BareosDb::ListJobstatisticsRecords(JobControlRecord *jcr, uint32_t JobId,
 ...
 */
void BareosDb::ListJobRecords(JobControlRecord *jcr, JobDbRecord *jr, const char *range,
                              const char *clientname, int jobstatus, int joblevel,
 ...
 * List fileset
 */
void BareosDb::ListFilesets(JobControlRecord *jcr, JobDbRecord *jr, const char *range,
                            OutputFormatter *sendit, e_list_type type) {
  char esc[MAX_ESCAPE_NAME_LENGTH];
  ...
  if (jr->Name[0] != 0) {
    EscapeString(jcr, esc, jr->Name, strlen(jr->Name));
    Mmsg(cmd,
         "SELECT DISTINCT FileSet.FileSetId AS FileSetId, FileSet, MD5, CreateTime, FileSetText "
         "FROM Job, FileSet "
