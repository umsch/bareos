 ...
 */
BareosDb *db_sql_get_non_pooled_connection(JobControlRecord *jcr, const char *db_drivername,
                                           ...
                                           bool try_reconnect, bool exit_on_fatal,
                                           bool need_private) {
  BareosDb *mdb;

#if defined(HAVE_DYNAMIC_CATS_BACKENDS)
  Dmsg2(100,
        "db_sql_get_non_pooled_connection allocating 1 new non pooled database connection to "
  ...
  Dmsg1(100,
        ...
        db_name);
#endif
  mdb = db_init_database(jcr, db_drivername, db_name, db_user, db_password, db_address, db_port,
  ...
  while (spe) {
    ...
    if (!flush_only || spe->reference_count == 0) {
      Dmsg3(100, "DbSqlPoolDestroy destroy db pool connection %d to %s, backend type %s\n", spe->id,
            spe->db_handle->get_db_name(), spe->db_handle->GetType());
 ...
 */
bool db_sql_pool_initialize(const char *db_drivername, const char *db_name, const char *db_user,
                            const char *db_password, const char *db_address, int db_port,
                            const char *db_socket, bool disable_batch_insert, bool try_reconnect,
                            bool exit_on_fatal, int min_connections, int max_connections,
                            int increment_connections, int idle_timeout, int validate_timeout) {
  int cnt;
  BareosDb *mdb;
  time_t now;
   ...
   */
  if (min_connections <= 0 || max_connections <= 0 || increment_connections <= 0 ||
      min_connections > max_connections) {
    Jmsg(NULL, M_FATAL, 0,
         _("Illegal values for sql pool initialization, min_connections = %d, max_connections = "
           "%d, increment_connections = %d"),
  ...
  for (cnt = 0; cnt < min_connections; cnt++) {
    mdb = db_init_database(NULL, db_drivername, db_name, db_user, db_password, db_address, db_port,
                           db_socket, true, disable_batch_insert, try_reconnect, exit_on_fatal);
    if (mdb == NULL) {
...
static inline void SqlPoolGrow(SqlPoolDescriptor *spd) {
  ...
  if (spe != NULL) {
     ...
     */
    cnt = 0;
    foreach_dlist(spe, spd->pool_entries) { spe->id = cnt++; }
...
static inline void SqlPoolShrink(SqlPoolDescriptor *spd) {
  ...
  if (spe) {
    Dmsg3(100,
          "SqlPoolShrink shrinking connection pool with %d connections to database %s, backend "
          "type %s\n",
...
static inline SqlPoolEntry *sql_find_first_connection(SqlPoolDescriptor *spd) {
  ...
  foreach_dlist(spe, spd->pool_entries) { return spe; }
  return NULL;
}
 ...
 */
BareosDb *db_sql_get_pooled_connection(JobControlRecord *jcr, const char *db_drivername,
                                       const char *db_name, const char *db_user,
                                       const char *db_password, const char *db_address, int db_port,
                                       const char *db_socket, bool mult_db_connections,
                                       bool disable_batch_insert, bool try_reconnect,
                                       bool exit_on_fatal, bool need_private) {
  int cnt = 0;
...
#if defined(HAVE_DYNAMIC_CATS_BACKENDS)
  Dmsg2(100,
        "db_sql_get_pooled_connection get new connection for connection to database %s, backend "
        "type %s\n",
        db_name, db_drivername);
#else
   ...
   */
  if (!db_pooling_descriptors) {
    return db_sql_get_non_pooled_connection(
        jcr, db_drivername, db_name, db_user, db_password, db_address, db_port, db_socket,
        mult_db_connections, disable_batch_insert, try_reconnect, exit_on_fatal, need_private);
  }
      ...
      if (use_connection) {
        ...
        if (mult_db_connections || need_private) {
          ...
          if (wanted_pool->nr_connections >= wanted_pool->max_connections) {
            db_handle = db_sql_get_non_pooled_connection(
                jcr, db_drivername, db_name, db_user, db_password, db_address, db_port, db_socket,
                mult_db_connections, disable_batch_insert, try_reconnect, exit_on_fatal,
                need_private);
            goto bail_out;
          }

          Dmsg0(100,
                "db_sql_get_pooled_connection trying to grow connection pool for getting free "
                "connection\n");
    ...
    while (spe) {
      ...
      if (spe->db_handle == mdb) {
        ...
        if (!abort) {
          ...
          Dmsg3(100,
                "DbSqlClosePooledConnection decrementing reference count of connection %d now %d, "
                "backend type %s\n",
          ...
          Dmsg3(100,
                "DbSqlClosePooledConnection aborting connection to database %s reference count %d, "
                "backend type %s\n",
