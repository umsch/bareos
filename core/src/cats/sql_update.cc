...
bool BareosDb::AddDigestToFileRecord(JobControlRecord *jcr, FileId_t FileId, char *digest,
                                     int type) {
  bool retval;
  char ed1[50];
  ...
  Mmsg(cmd, "UPDATE File SET MD5='%s' WHERE FileId=%s", esc_name, edit_int64(FileId, ed1));
 ...
 */
bool BareosDb::MarkFileRecord(JobControlRecord *jcr, FileId_t FileId, JobId_t JobId) {
...
bool BareosDb::UpdateJobStartRecord(JobControlRecord *jcr, JobDbRecord *jr) {
  ...
  Mmsg(cmd,
       ...
       (char)(jcr->JobStatus), (char)(jr->JobLevel), dt, edit_int64(jr->ClientId, ed1),
       edit_uint64(JobTDate, ed2), edit_int64(jr->PoolId, ed3), edit_int64(jr->FileSetId, ed4),
...
bool BareosDb::UpdateJobEndRecord(JobControlRecord *jcr, JobDbRecord *jr) {
  ...
  Mmsg(cmd,
       ...
       edit_uint64(jr->JobBytes, ed1), edit_uint64(jr->ReadBytes, ed4), jr->JobFiles, jr->JobErrors,
       jr->VolSessionId, jr->VolSessionTime, jr->PoolId, jr->FileSetId, edit_uint64(JobTDate, ed2),
       rdt, PriorJobId, jr->HasBase, jr->PurgedFiles, edit_int64(jr->JobId, ed3));
...
bool BareosDb::UpdateClientRecord(JobControlRecord *jcr, ClientDbRecord *cr) {
  ...
  Mmsg(cmd,
       ...
       cr->AutoPrune, edit_uint64(cr->FileRetention, ed1), edit_uint64(cr->JobRetention, ed2),
...
bool BareosDb::UpdateCounterRecord(JobControlRecord *jcr, CounterDbRecord *cr) {
  ...
  FillQuery(SQL_QUERY_update_counter_values, cr->MinValue, cr->MaxValue, cr->CurrentValue,
            cr->WrapCounter, esc);
  retval = UPDATE_DB(jcr, cmd);
...
bool BareosDb::UpdatePoolRecord(JobControlRecord *jcr, PoolDbRecord *pr) {
  ...
  Mmsg(cmd, "SELECT count(*) from Media WHERE PoolId=%s", edit_int64(pr->PoolId, ed4));
  ...
  Mmsg(cmd,
       ...
       pr->NumVols, pr->MaxVols, pr->UseOnce, pr->UseCatalog, pr->AcceptAnyVolume,
       edit_uint64(pr->VolRetention, ed1), edit_uint64(pr->VolUseDuration, ed2), pr->MaxVolJobs,
       pr->MaxVolFiles, edit_uint64(pr->MaxVolBytes, ed3), pr->Recycle, pr->AutoPrune,
       pr->LabelType, esc, edit_int64(pr->RecyclePoolId, ed5), edit_int64(pr->ScratchPoolId, ed6),
       pr->ActionOnPurge, pr->MinBlocksize, pr->MaxBlocksize, ed4);
  retval = UPDATE_DB(jcr, cmd);
  DbUnlock(this);
  return retval;
}
...
bool BareosDb::UpdateStorageRecord(JobControlRecord *jcr, StorageDbRecord *sr) {
  ...
  DbLock(this);
  Mmsg(cmd, "UPDATE Storage SET AutoChanger=%d WHERE StorageId=%s", sr->AutoChanger,
       edit_int64(sr->StorageId, ed1));
...
bool BareosDb::UpdateMediaRecord(JobControlRecord *jcr, MediaDbRecord *mr) {
  ...
  Mmsg(cmd,
       ...
       " WHERE VolumeName='%s'",
       mr->VolJobs, mr->VolFiles, mr->VolBlocks, edit_uint64(mr->VolBytes, ed1), mr->VolMounts,
       mr->VolErrors, mr->VolWrites, edit_uint64(mr->MaxVolBytes, ed2), esc_status, mr->Slot,
       mr->InChanger, edit_int64(mr->VolReadTime, ed3), edit_int64(mr->VolWriteTime, ed4),
       mr->LabelType, edit_int64(mr->StorageId, ed5), edit_int64(mr->PoolId, ed6),
       edit_uint64(mr->VolRetention, ed7), edit_uint64(mr->VolUseDuration, ed8), mr->MaxVolJobs,
       mr->MaxVolFiles, mr->Enabled, edit_uint64(mr->LocationId, ed9),
       edit_uint64(mr->ScratchPoolId, ed10), edit_uint64(mr->RecyclePoolId, ed11), mr->RecycleCount,
       mr->Recycle, mr->ActionOnPurge, mr->MinBlocksize, mr->MaxBlocksize, esc_medianame);
...
bool BareosDb::UpdateMediaDefaults(JobControlRecord *jcr, MediaDbRecord *mr) {
  ...
  if (mr->VolumeName[0]) {
    ...
    Mmsg(cmd,
         ...
         edit_uint64(mr->MaxVolBytes, ed3), edit_uint64(mr->RecyclePoolId, ed4), mr->MinBlocksize,
         mr->MaxBlocksize, esc);
  } else {
    Mmsg(cmd,
         "UPDATE Media SET "
         ...
         mr->ActionOnPurge, mr->Recycle, edit_uint64(mr->VolRetention, ed1),
         edit_uint64(mr->VolUseDuration, ed2), mr->MaxVolJobs, mr->MaxVolFiles,
         edit_uint64(mr->MaxVolBytes, ed3), edit_int64(mr->RecyclePoolId, ed4), mr->MinBlocksize,
         mr->MaxBlocksize, edit_int64(mr->PoolId, ed5));
  }
  ...
  if (mr->InChanger != 0 && mr->Slot != 0 && mr->StorageId != 0) {
    ...
    if (mr->MediaId != 0) {
      Mmsg(cmd,
           ...
           mr->Slot, edit_int64(mr->StorageId, ed1), edit_int64(mr->MediaId, ed2));
...
bool BareosDb::UpdateQuotaGracetime(JobControlRecord *jcr, JobDbRecord *jr) {
  ...
  Mmsg(cmd, "UPDATE Quota SET GraceTime=%s WHERE ClientId='%s'", edit_uint64(now, ed1),
...
bool BareosDb::UpdateQuotaSoftlimit(JobControlRecord *jcr, JobDbRecord *jr) {
  ...
  Mmsg(cmd, "UPDATE Quota SET QuotaLimit=%s WHERE ClientId='%s'",
       edit_uint64((jr->JobSumTotalBytes + jr->JobBytes), ed1), edit_uint64(jr->ClientId, ed2));
...
bool BareosDb::ResetQuotaRecord(JobControlRecord *jcr, ClientDbRecord *cr) {
  ...
  Mmsg(cmd, "UPDATE Quota SET GraceTime='0', QuotaLimit='0' WHERE ClientId='%s'",
 ...
 *          true on success
 */
bool BareosDb::UpdateNdmpLevelMapping(JobControlRecord *jcr, JobDbRecord *jr, char *filesystem,
                                      int level) {
  bool retval;
  ...
  Mmsg(cmd,
       ...
       edit_uint64(level, ed1), edit_uint64(jr->ClientId, ed2), edit_uint64(jr->FileSetId, ed3),
       esc_name);
