...
BareosDbSqlite::BareosDbSqlite(JobControlRecord *jcr, const char *db_driver, const char *db_name,
 ...
 */
bool BareosDbSqlite::SqlQueryWithHandler(const char *query, DB_RESULT_HANDLER *ResultHandler,
                                         void *ctx) {
  bool retval = false;
  int status;
  ...
  status =
      sqlite3_exec(db_handle_, query, SqliteResultHandler, (void *)&rh_data, &lowlevel_errmsg_);
...
bool BareosDbSqlite::SqlQueryWithoutHandler(const char *query, int flags) {
  ...
  status = sqlite3_get_table(db_handle_, (char *)query, &result_, &num_rows_, &num_fields_,
                             &lowlevel_errmsg_);
...
int BareosDbSqlite::SqlAffectedRows(void) { return sqlite3_changes(db_handle_); }

uint64_t BareosDbSqlite::SqlInsertAutokeyRecord(const char *query, const char *table_name) {
  /*
 ...
 *         false if failed
 */
bool BareosDbSqlite::SqlBatchInsert(JobControlRecord *jcr, AttributesDbRecord *ar) {
...
#endif
{
  ...
  mdb = New(BareosDbSqlite(jcr, db_driver, db_name, db_user, db_password, db_address, db_port,
