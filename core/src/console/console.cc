...
static void ReadAndProcessInput(FILE *input, BareosSocket *UA_sock) {
  ...
  while (1) {
    ...
    while ((status = UA_sock->recv()) >= 0 ||
           ((status == BNET_SIGNAL) &&
            ((UA_sock->msglen != BNET_EOD) && (UA_sock->msglen != BNET_MAIN_PROMPT) &&
...
static bool SelectDirector(const char *director, DirectorResource **ret_dir,
  ...
  *ret_dir = NULL;
  ...
  LockRes();
  numdir = 0;
  foreach_res(dir, R_DIRECTOR) { numdir++; }
  numcon = 0;
  foreach_res(cons, R_CONSOLE) { numcon++; }
  UnlockRes();
 ...
 */
static int dir_psk_client_callback(char *identity, unsigned int max_identity_len,
...
int main(int argc, char *argv[]) {
  ...
  if (list_directors) {
    LockRes();
    foreach_res(dir, R_DIRECTOR) { senditf("%s\n", dir->name()); }
    UnlockRes();
  }
  ...
  UA_sock = New(BareosSocketTCP);
  if (!UA_sock->connect(NULL, 5, 15, heart_beat, "Director daemon", dir->address, NULL,
                        dir->DIRport, false)) {
    delete UA_sock;
    TerminateConsole(0);
    return 1;
