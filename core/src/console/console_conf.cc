...
static ResourceItem cons_items[] = {
    {"Name", CFG_TYPE_NAME, ITEM(res_cons.hdr.name), 0, CFG_ITEM_REQUIRED, NULL, NULL,
    ...
    {"HistoryLength", CFG_TYPE_PINT32, ITEM(res_cons.history_length), 0, CFG_ITEM_DEFAULT, "100",
     NULL, NULL},
    {"Password", CFG_TYPE_MD5PASSWORD, ITEM(res_cons.password), 0, CFG_ITEM_REQUIRED, NULL, NULL,
     NULL},
    {"Director", CFG_TYPE_STR, ITEM(res_cons.director), 0, 0, NULL, NULL, NULL},
    {"HeartbeatInterval", CFG_TYPE_TIME, ITEM(res_cons.heartbeat_interval), 0, CFG_ITEM_DEFAULT,
     "0", NULL, NULL},
...
bool SaveResource(int type, ResourceItem *items, int pass) {
  ...
  if (pass == 2) {
    switch (type) {
      case R_CONSOLE:
        if ((res = (UnionOfResources *)GetResWithName(R_CONSOLE, res_all.res_cons.name())) ==
            NULL) {
