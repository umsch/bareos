...
bool AuthenticateWithStorageDaemon(JobControlRecord *jcr, StorageResource *store) {
  ...
  auth_success =
      sd->AuthenticateOutboundConnection(jcr, "Storage daemon", dirname, store->password, store);
  if (!auth_success) {
...
bool AuthenticateWithFileDaemon(JobControlRecord *jcr) {
  ...
  if (fd->recv() <= 0) {
    Dmsg1(debuglevel, _("Bad response from File daemon to Hello command: ERR=%s\n"),
          bnet_strerror(fd));
    Jmsg(jcr, M_FATAL, 0,
         _("Bad response from File daemon at \"%s:%d\" to Hello command: ERR=%s\n"), fd->host(),
...
bool AuthenticateFileDaemon(BareosSocket *fd, char *client_name) {
  ...
  if (client) {
    if (IsConnectFromClientAllowed(client)) {
      auth_success = fd->AuthenticateInboundConnection(NULL, "File Daemon", client_name,
...
bool AuthenticateUserAgent(UaContext *uac) {
  ...
  if (bstrcmp(name, "*UserAgent*")) { /* default console */
    auth_success =
        ua->AuthenticateInboundConnection(NULL, "Console", "*UserAgent*", me->password, me);
  } else {
    UnbashSpaces(name);
