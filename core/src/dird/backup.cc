...
static int AccurateListHandler(void *ctx, int num_fields, char **row) {
  ...
  if (jcr->use_accurate_chksum && num_fields == 9 && row[6][0] && /* skip checksum = '0' */
      row[6][1]) {
    jcr->file_bsock->fsend("%s%s%c%s%c%s%c%s", row[0], row[1], 0, row[4], 0, row[6], 0, row[5]);
  } else {
    jcr->file_bsock->fsend("%s%s%c%s%c%c%s", row[0], row[1], 0, row[4], 0, 0, row[5]);
  }
  return 0;
}
...
bool SendAccurateCurrentFiles(JobControlRecord *jcr) {
  ...
  if (jcr->HasBase) {
    ...
    if (!jcr->db->GetBaseFileList(jcr, jcr->use_accurate_chksum, AccurateListHandler,
                                  (void *)jcr)) {
    ...
    jcr->db_batch->GetFileList(jcr, jobids.list, jcr->use_accurate_chksum, false /* no delta */,
...
int WaitForJobTermination(JobControlRecord *jcr, int timeout) {
  ...
  if (fd) {
    ...
    while ((n = BgetDirmsg(fd)) >= 0) {
      if (!fd_ok && sscanf(fd->msg, EndJob, &jcr->FDJobStatus, &JobFiles, &ReadBytes, &JobBytes,
                           &JobErrors, &VSS, &Encrypt) == 7) {
        fd_ok = true;
...
void NativeBackupCleanup(JobControlRecord *jcr, int TermCode) {
  ...
  if (jcr->is_JobStatus(JS_Terminated) && (jcr->JobErrors || jcr->SDErrors || jcr->JobWarnings)) {
...
void UpdateBootstrapFile(JobControlRecord *jcr) {
   ...
   */
  if (jcr->IsTerminatedOk() && jcr->jr.JobBytes && jcr->res.job->WriteBootstrap) {
    FILE *fd;
    int VolCount;
    int got_pipe = 0;
    Bpipe *bpipe = NULL;
    ...
    if (fd) {
      ...
      fprintf(fd, "# %s - %s - %s%s\n", edt, jcr->jr.Job, level_to_str(jcr->getJobLevel()),
              jcr->since);
...
void GenerateBackupSummary(JobControlRecord *jcr, ClientDbRecord *cr, int msg_type,
  ...
  utime_t RunTime;
  MediaDbRecord mr;
  PoolMem temp, level_info, statistics, quota_info, client_options, daemon_status,
      secure_erase_status, compress_algo_list;
  ...
  bstrftimes(gdt, sizeof(gdt), jcr->res.client->GraceTime + jcr->res.client->SoftQuotaGracePeriod);
        ...
        if (jcr->HasBase) {
          Mmsg(client_options,
               _("  Software Compression:   %s%s\n"
                 ...
                 "  Encryption:             %s\n"
                 "  Accurate:               %s\n"),
               compress, compress_algo_list.c_str(), jcr->nb_base_files, jcr->nb_base_files_used,
               jcr->nb_base_files_used * 100.0 / jcr->nb_base_files, jcr->VSS ? _("yes") : _("no"),
               jcr->Encrypt ? _("yes") : _("no"), jcr->accurate ? _("yes") : _("no"));
        } else {
          Mmsg(client_options,
               _("  Software Compression:   %s%s\n"
                 "  VSS:                    %s\n"
                 "  Encryption:             %s\n"
                 "  Accurate:               %s\n"),
               compress, compress_algo_list.c_str(), jcr->VSS ? _("yes") : _("no"),
               jcr->Encrypt ? _("yes") : _("no"), jcr->accurate ? _("yes") : _("no"));
        }
  ...
  Jmsg(jcr, msg_type, 0,
       _("%s %s %s (%s):\n"
         ...
         "  Termination:            %s\n\n"),
       BAREOS, my_name, VERSION, LSMDATE, HOST_OS, DISTNAME, DISTVER, jcr->jr.JobId, jcr->jr.Job,
       level_info.c_str(), jcr->res.client->name(), cr->Uname, jcr->res.fileset->name(),
       jcr->FSCreateTime, jcr->res.pool->name(), jcr->res.pool_source, jcr->res.catalog->name(),
       jcr->res.catalog_source, jcr->res.wstore->name(), jcr->res.wstore_source, schedt, sdt, edt,
       edit_utime(RunTime, elapsed, sizeof(elapsed)), jcr->JobPriority, statistics.c_str(),
       quota_info.c_str(), kbps, client_options.c_str(), jcr->VolumeName, jcr->VolSessionId,
