...
RestoreBootstrapRecordFileIndex *new_findex() {
  ...
  RestoreBootstrapRecordFileIndex *fi =
      (RestoreBootstrapRecordFileIndex *)bmalloc(sizeof(RestoreBootstrapRecordFileIndex));
 ...
 * for each Volume.
 */
static inline uint32_t write_findex(RestoreBootstrapRecordFileIndex *fi, int32_t FirstIndex,
                                    int32_t LastIndex, PoolMem *buffer) {
 ...
 */
static inline bool is_volume_selected(RestoreBootstrapRecordFileIndex *fi, int32_t FirstIndex,
                                      int32_t LastIndex) {
  while (fi) {
    if ((fi->findex >= FirstIndex && fi->findex <= LastIndex) ||
        (fi->findex2 >= FirstIndex && fi->findex2 <= LastIndex) ||
...
bool CompleteBsr(UaContext *ua, RestoreBootstrapRecord *bsr) {
  ...
  for (; bsr; bsr = bsr->next) {
    ...
    if ((bsr->VolCount = ua->db->GetJobVolumeParameters(ua->jcr, bsr->JobId, &(bsr->VolParams))) ==
        0) {
  ...
  for (bsr = rx.bsr; bsr; bsr = bsr->next) {
    ...
    for (i = 0; i < bsr->VolCount; i++) {
      if (bsr->VolParams[i].VolumeName[0]) {
        ...
        Mmsg(volmsg, "%c%-25s %-25s %-25s", online, bsr->VolParams[i].VolumeName,
 ...
 */
static uint32_t write_bsr_item(RestoreBootstrapRecord *bsr, UaContext *ua, RestoreContext &rx,
                               PoolMem *buffer, bool &first, uint32_t &LastIndex) {
  int i;
  char ed1[50], ed2[50];
  uint32_t count = 0;
   ...
   */
  for (i = 0; i < bsr->VolCount; i++) {
    if (!is_volume_selected(bsr->fi, bsr->VolParams[i].FirstIndex, bsr->VolParams[i].LastIndex)) {
      bsr->VolParams[i].VolumeName[0] = 0; /* zap VolumeName */
      continue;
    }

    if (!rx.store) {
      FindStorageResource(ua, rx, bsr->VolParams[i].Storage, bsr->VolParams[i].MediaType);
    }
    ...
    count =
        write_findex(bsr->fi, bsr->VolParams[i].FirstIndex, bsr->VolParams[i].LastIndex, buffer);
    if (count) {
