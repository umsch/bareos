...
static int SendVolumeInfoToStorageDaemon(JobControlRecord *jcr, BareosSocket *sd,
                                         MediaDbRecord *mr) {
  int status;
  ...
  status = sd->fsend(OK_media, mr->VolumeName, mr->VolJobs, mr->VolFiles, mr->VolBlocks,
                     edit_uint64(mr->VolBytes, ed1), mr->VolMounts, mr->VolErrors, mr->VolWrites,
                     edit_uint64(mr->MaxVolBytes, ed2), edit_uint64(mr->VolCapacityBytes, ed3),
                     mr->VolStatus, mr->Slot, mr->MaxVolJobs, mr->MaxVolFiles, mr->InChanger,
...
void CatalogRequest(JobControlRecord *jcr, BareosSocket *bs) {
  ...
  if (sscanf(bs->msg, Find_media, &Job, &index, &pool_name, &mr.MediaType,
    ...
    if (ok) {
      ...
      ok = FindNextVolumeForAppend(jcr, &mr, index, unwanted_volumes.c_str(), fnv_create_vol,
                                   fnv_prune);
  ...
  } else if (sscanf(bs->msg, Update_media, &Job, &sdmr.VolumeName, &sdmr.VolJobs, &sdmr.VolFiles,
                    &sdmr.VolBlocks, &sdmr.VolBytes, &sdmr.VolMounts, &sdmr.VolErrors,
                    &sdmr.VolWrites, &sdmr.MaxVolBytes, &VolLastWritten, &sdmr.VolStatus,
                    &sdmr.Slot, &label, &sdmr.InChanger, &sdmr.VolReadTime, &sdmr.VolWriteTime,
                    &VolFirstWritten) == 18) {
    /*
  ...
  bail_out:
    ...
    return;
  } else if (sscanf(bs->msg, Create_job_media, &Job, &jm.FirstIndex, &jm.LastIndex, &jm.StartFile,
                    &jm.EndFile, &jm.StartBlock, &jm.EndBlock, &Copy, &Stripe, &MediaId) == 10) {
    /*
    ...
    Dmsg6(400, "create_jobmedia JobId=%d MediaId=%d SF=%d EF=%d FI=%d LI=%d\n", jm.JobId,
          jm.MediaId, jm.StartFile, jm.EndFile, jm.FirstIndex, jm.LastIndex);
...
static void UpdateAttribute(JobControlRecord *jcr, char *msg, int32_t msglen) {
  ...
  Dmsg5(400, "UpdCat VolSessId=%d VolSessT=%d FI=%d Strm=%d reclen=%d\n", VolSessionId,
        VolSessionTime, FileIndex, Stream, reclen);
  ...
  switch (Stream) {
    ...
    case STREAM_RESTORE_OBJECT: {
      ...
      Dmsg7(100, "oname=%s stream=%d FT=%d FI=%d JobId=%d, obj_len=%d\nobj=\"%s\"\n",
            ro.object_name, ro.Stream, ro.FileType, ro.FileIndex, ro.JobId, ro.object_len,
            ro.object);
        ...
        if (ar->FileIndex != FileIndex) {
          ...
          switch (Stream) {
            ...
            default:
               ...
               */
              Jmsg(jcr, M_ERROR, 0,
                   _("Catalog error updating file digest. Unsupported digest stream type: %d"),
                   Stream);
          }
