...
static inline void StartNewConsolidationJob(JobControlRecord *jcr, char *jobname) {
  ...
  ua->batch = true;
  Mmsg(ua->cmd, "run job=\"%s\" jobid=%s level=VirtualFull %s", jobname, jcr->vf_jobids,
    ...
    if (job->AlwaysIncremental) {
      ...
      if (job->AlwaysIncrementalJobRetention) {
        ...
        Jmsg(jcr, M_INFO, 0, _("%s: considering jobs older than %s for consolidation.\n"),
             job->name(), sdt);
        Dmsg4(10,
              _("%s: considering jobs with ClientId %d and FilesetId %d older than %s for "
                "consolidation.\n"),
      ...
      if ((max_incrementals_to_consolidate + 1) > 1) {
        ...
        Dmsg3(10, "total: %d, to_consolidate: %d, limit: %d.\n", incrementals_total,
              max_incrementals_to_consolidate, jcr->jr.limit);
        jobids_ctx.reset();
        jcr->db->AccurateGetJobids(jcr, &jcr->jr, &jobids_ctx);
        ...
        if (incrementals_to_consolidate < 1) {
          Jmsg(jcr, M_INFO, 0,
               _("%s: After limited query: less incrementals than required, not consolidating\n"),
               job->name());
          continue;
        }
      } else {
      ...
      if (job->AlwaysIncrementalMaxFullAge) {
        ...
        if (incrementals_to_consolidate < 2) {
          Jmsg(jcr, M_INFO, 0,
               _("%s: less incrementals than required to consolidate without full, not "
                 "consolidating\n"),
               job->name());
          continue;
        }
        ...
        if (starttime > oldest_allowed_starttime) {
          Jmsg(jcr, M_INFO, 0,
               _("Full is newer than AlwaysIncrementalMaxFullAge -> skipping first jobid %s "
                 "because of age\n"),
               jobids);
          if (p) {
          ...
          Jmsg(jcr, M_INFO, 0,
               _("%d AlwaysIncrementalFullConsolidations reached -> skipping first jobid %s "
                 "independent of age\n"),
          ...
          Jmsg(jcr, M_INFO, 0,
               _("Full is older than AlwaysIncrementalMaxFullAge -> also consolidating Full jobid "
                 "%s\n"),
               jobids);
          if (p) {
            *p = ',';   /* Restore ,*/
