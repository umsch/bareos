...
static void usage() {
  ...
  fprintf(stderr,
          "Usage: bareos-dbcheck [ options ] <working-directory> <bareos-database> <user> "
          "<password> [<dbhost>] [<dbport>]\n"
          "       -b                batch mode\n"
          "       -B                print catalog configuration and exit\n"
          "       -c  <config>      Director configuration filename or configuration directory "
          "(e.g. /etc/bareos)\n"
          "       -C  <catalog>     catalog name in the director configuration file\n"
          "       -d  <nnn>         set debug level to <nnn>\n"
          "       -dt               print a timestamp in debug output\n"
          "       -D  <driver name> specify the database driver name (default NULL) "
...
static void PrintCatalogDetails(CatalogResource *catalog, const char *working_dir) {
  ...
  if (db) {
    printf("%sdb_type=%s\nworking_dir=%s\n", catalog->display(catalog_details), db->GetType(),
           working_directory);
...
static bool CheckIdx(const char *col_name) {
  ...
  switch (db->GetTypeIndex()) {
    case SQL_TYPE_MYSQL:
      ...
      for (i = 0; (idx_list[i].key_name != NULL) && (i < (MAXIDX - 1)); i++) {
        /*
         * NOTE : if (idx_list[i].count_key > 1) then index idx_list[i].key_name is
 ...
 */
static bool create_tmp_idx(const char *idx_name, const char *table_name, const char *col_name) {
...
static void repair_bad_filenames() {
  ...
  if (fix && id_list.num_ids > 0) {
    ...
    for (i = 0; i < id_list.num_ids; i++) {
      ...
      Bsnprintf(buf, sizeof(buf), "UPDATE File SET Name='%s' WHERE FileId=%s", esc_name,
                edit_int64(id_list.Id[i], ed1));
      if (verbose > 1) {
        printf("%s\n", buf);
      }
...
static void repair_bad_paths() {
  ...
  if (fix && id_list.num_ids > 0) {
    ...
    for (i = 0; i < id_list.num_ids; i++) {
      ...
      len = PmStrcat(name, "/");
      db->EscapeString(NULL, esc_name, name, len);
      Bsnprintf(buf, sizeof(buf), "UPDATE Path SET Path='%s' WHERE PathId=%s", esc_name,
                edit_int64(id_list.Id[i], ed1));
      if (verbose > 1) {
        printf("%s\n", buf);
      }
...
int main(int argc, char *argv[]) {
   ...
   * Open database
   */
  db = db_init_database(NULL, db_driver, db_name, user, password, dbhost, dbport, NULL, false,
                        false, false, false);
