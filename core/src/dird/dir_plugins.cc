...
static bRC bareosJobMsg(bpContext *ctx, const char *file, int line, int type, utime_t mtime,
                        const char *fmt, ...);
static bRC bareosDebugMsg(bpContext *ctx, const char *file, int line, int level, const char *fmt,
                          ...);
...
#ifdef needed
static inline bool IsPluginDisabled(JobControlRecord *jcr) { return IsPluginDisabled(jcr->ctx); }
#endif
...
static inline bool trigger_plugin_event(JobControlRecord *jcr, bDirEventType eventType,
                                        bDirEvent *event, bpContext *ctx, void *value,
                                        alist *plugin_ctx_list, int *index, bRC *rc) {
  bool stop = false;
...
void LoadDirPlugins(const char *plugin_dir, alist *plugin_names) {
  ...
  if (!LoadPlugins((void *)&binfo, (void *)&bfuncs, dird_plugin_list, plugin_dir, plugin_names,
...
static bool IsPluginCompatible(Plugin *plugin) {
  ...
  if (info->version != DIR_PLUGIN_INTERFACE_VERSION) {
    ...
    Dmsg3(50, "Plugin version incorrect. Plugin=%s wanted=%d got=%d\n", plugin->file,
          DIR_PLUGIN_INTERFACE_VERSION, info->version);
    return false;
  }
 ...
 */
static inline bpContext *instantiate_plugin(JobControlRecord *jcr, Plugin *plugin,
                                            uint32_t instance) {
  bpContext *ctx;
  b_plugin_ctx *b_ctx;
...
void DispatchNewPluginOptions(JobControlRecord *jcr) {
  ...
  if (jcr->res.job && jcr->res.job->DirPluginOptions && jcr->res.job->DirPluginOptions->size()) {
    ...
    foreach_alist_index(i, plugin_options, jcr->res.job->DirPluginOptions) {
      ...
      *bp++ = '\0';
      ...
      if (instance < LOWEST_PLUGIN_INSTANCE || instance > HIGHEST_PLUGIN_INSTANCE) {
        Jmsg(NULL, M_ERROR, 0,
             _("Illegal DIR plugin options encountered, %s instance %d skipping\n"), plugin_options,
             instance);
        continue;
      }
...
bool DbGetPoolRecord(JobControlRecord *jcr, BareosDb *db, PoolDbRecord *pdbr) { return true; }
