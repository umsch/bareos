...
int main(int argc, char *argv[]) {
  ...
  if (!test_config) { /* we don't need to do this block in test mode */
    ...
    CreatePidFile(me->pid_directory, "bareos-dir", GetFirstPortHostOrder(me->DIRaddrs));
    ReadStateFile(me->working_directory, "bareos-dir", GetFirstPortHostOrder(me->DIRaddrs));
  }

  SetJcrInTsd(INVALID_JCR);
  SetThreadConcurrency(me->MaxConcurrentJobs * 2 + 4 /* UA */ + 5 /* sched+watchdog+jobsvr+misc */);
...
static int me_psk_server_callback(const char *identity, unsigned char *psk,
...
static int ConsPskServerCallback(const char *identity, unsigned char *psk,
                                 unsigned int max_psk_len) {
...
static int client_psk_client_callback(char *identity, unsigned int max_identity_len,
...
static int store_psk_client_callback(char *identity, unsigned int max_identity_len,
                                     unsigned char *psk, unsigned int max_psk_len) {
...
static inline bool IsSameStorageDaemon(StorageResource *store1, StorageResource *store2) {
  ...
  return store1->SDport == store2->SDport && Bstrcasecmp(store1->address, store2->address) &&
...
static bool check_resources() {
  ...
  if (!me) {
    ...
    if (((me->tls_cert.CaCertfile == nullptr || me->tls_cert.CaCertfile->empty()) &&
      ...
      Jmsg(NULL, M_FATAL, 0,
           _("Neither \"TLS CA Certificate\" or \"TLS CA"
  ...
  foreach_res(job, R_JOB) {
    if (job->MaxFullConsolidations && job->JobType != JT_CONSOLIDATE) {
      Jmsg(NULL, M_FATAL, 0,
           _("MaxFullConsolidations configured in job %s which is not of job type \"consolidate\" "
             "in file %s\n"),
           job->name(), configfile);
      OK = false;
      goto bail_out;
    ...
    if (job->JobType != JT_BACKUP &&
        (job->AlwaysIncremental || job->AlwaysIncrementalJobRetention ||
         job->AlwaysIncrementalKeepNumber || job->AlwaysIncrementalMaxFullAge)) {
      Jmsg(NULL, M_FATAL, 0,
           _("AlwaysIncremental configured in job %s which is not of job type \"backup\" in file "
             "%s\n"),
           job->name(), configfile);
      OK = false;
      goto bail_out;
  ...
  foreach_res(client, R_CLIENT) {
    ...
    if ((client->tls_cert.CaCertfile == nullptr || client->tls_cert.CaCertfile->empty()) &&
        (client->tls_cert.CaCertdir == nullptr || client->tls_cert.CaCertdir->empty()) &&
        need_tls) {
      Jmsg(NULL, M_FATAL, 0,
...
static bool InitializeSqlPooling(void) {
  ...
  foreach_res(catalog, R_CATALOG) {
    if (!db_sql_pool_initialize(catalog->db_driver, catalog->db_name, catalog->db_user,
      ...
      Jmsg(NULL, M_FATAL, 0,
           _("Could not setup sql pooling for Catalog \"%s\", database \"%s\".\n"), catalog->name(),
           catalog->db_name);
      retval = false;
      goto bail_out;
    }
...
static bool CheckCatalog(cat_op mode) {
  ...
  foreach_res(catalog, R_CATALOG) {
     ...
     */
    db = db_init_database(
        NULL, catalog->db_driver, catalog->db_name, catalog->db_user, catalog->db_password.value,
        catalog->db_address, catalog->db_port, catalog->db_socket, catalog->mult_db_connections,
        catalog->disable_batch_insert, catalog->try_reconnect, catalog->exit_on_fatal);
    ...
    foreach_res(client, R_CLIENT) {
      ...
      if (client->catalog != catalog) {
        Dmsg3(500, "Skip client=%s with cat=%s not catalog=%s\n", client->name(),
              client->catalog->name(), catalog->name());
        continue;
      }
