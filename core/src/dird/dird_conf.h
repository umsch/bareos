 ...
 */
enum { R_NAME = 1020, R_ADDRESS, R_PASSWORD, R_TYPE, R_BACKUP };
...
class DirectorResource : public TlsResource {
 public:
  ...
  uint32_t jcr_watchdog_time;  /* Absolute time after which a Job gets terminated regardless of its
                                  progress */
...
class CatalogResource : public BareosResource {
 public:
  ...
  bool
      exit_on_fatal; /**< Make any fatal error in the connection to the database exit the program */
  uint32_t pooling_min_connections; /**< When using sql pooling start with this number of
                                       connections to the database */
  uint32_t pooling_max_connections; /**< When using sql pooling maximum number of connections to the
                                       database */
  uint32_t pooling_increment_connections; /**< When using sql pooling increment the pool with this
                                             amount when its to small */
  uint32_t pooling_idle_timeout; /**< When using sql pooling set this to the number of seconds to
                                    keep an idle connection */
  uint32_t pooling_validate_timeout; /**< When using sql pooling set this to the number of seconds
                                        after a idle connection should be validated */
...
class JobResource : public BareosResource {
 public:
  ...
  utime_t
      AlwaysIncrementalJobRetention;   /**< Timeinterval where incrementals are not consolidated */
  utime_t AlwaysIncrementalMaxFullAge; /**< If Full Backup is older than this age the consolidation
                                          job will include also the full */
  ...
  int32_t AlwaysIncrementalKeepNumber; /**< Number of incrementals that are always left and not
                                          consolidated */
  int32_t MaxFullConsolidations; /**< Number of consolidate jobs to be started that will include a
                                    full */
...
bool print_datatype_schema_json(PoolMem &buffer, int level, const int type, ResourceItem items[],
