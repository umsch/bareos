...
static int date_item(JobControlRecord *jcr, int code, const char **val_ptr, int *val_len,
                     int *val_size) {
  int val = 0;
  char buf[10];
  struct tm tm;
...
static int job_item(JobControlRecord *jcr, int code, const char **val_ptr, int *val_len,
                    int *val_size) {
  const char *str = " ";
  char buf[20];
 ...
 */
static var_rc_t lookup_built_in_var(var_t *ctx, void *my_ctx, const char *var_ptr, int var_len,
                                    int var_index, const char **val_ptr, int *val_len,
                                    int *val_size) {
  ...
  for (i = 0; _(built_in_vars[i].var_name); i++) {
    if (bstrncmp(_(built_in_vars[i].var_name), var_ptr, var_len)) {
      status = (*built_in_vars[i].func)(jcr, built_in_vars[i].code, val_ptr, val_len, val_size);
      if (status) {
        return VAR_OK;
      }
      break;
 ...
 */
static var_rc_t lookup_counter_var(var_t *ctx, void *my_ctx, const char *var_ptr, int var_len,
                                   int var_inc, int var_index, const char **val_ptr, int *val_len,
                                   int *val_size) {
  ...
  LockRes();
  for (counter = NULL;
       (counter = (CounterResource *)GetNextRes(R_COUNTER, (CommonResourceHeader *)counter));) {
 ...
 */
static var_rc_t lookup_var(var_t *ctx, void *my_ctx, const char *var_ptr, int var_len, int var_inc,
                           int var_index, const char **val_ptr, int *val_len, int *val_size) {
  PoolMem buf(PM_NAME);
  char *val, *p, *v;
  var_rc_t status;
  int count;
   ...
   */
  if ((status = lookup_built_in_var(ctx, my_ctx, var_ptr, var_len, var_index, val_ptr, val_len,
                                    val_size)) == VAR_OK) {
    return VAR_OK;
  }

  if ((status = lookup_counter_var(ctx, my_ctx, var_ptr, var_len, var_inc, var_index, val_ptr,
                                   val_len, val_size)) == VAR_OK) {
    return VAR_OK;
  }
 ...
 */
static var_rc_t operate_var(var_t *var, void *my_ctx, const char *op_ptr, int op_len,
                            const char *arg_ptr, int arg_len, const char *val_ptr, int val_len,
