...
int BgetDirmsg(BareosSocket *bs, bool allow_any_message) {
  ...
  for (; !bs->IsStop() && !bs->IsTimedOut();) {
    ...
    if (bs->msg[0] == 'J') { /* Job message */
      if (sscanf(bs->msg, "Jmsg Job=%127s type=%d level=%lld", Job, &type, &mtime) != 3) {
        Jmsg1(jcr, M_ERROR, 0, _("Malformed message: %s\n"), bs->msg);
        continue;
      }
      Dmsg1(900, "Got msg: %s\n", bs->msg);
    ...
    if (bs->msg[0] == 'B') { /* SD sending file spool attributes */
      ...
      char filename[256];
      if (sscanf(bs->msg, "BlastAttr Job=%127s File=%255s", Job, filename) != 2) {
        Jmsg1(jcr, M_ERROR, 0, _("Malformed message: %s\n"), bs->msg);
        continue;
      }
    ...
    if (bs->msg[0] = 'D') { /* Device update */
      ...
      if (sscanf(bs->msg, Device_update, &Job, dev_name.c_str(), &dev_append, &dev_read,
                 &dev_num_writers, &dev_open, &dev_labeled, &dev_offline, &dev_reserved,
                 &dev_max_writers, &dev_autoselect, &dev_autochanger, changer_name.c_str(),
                 media_type.c_str(), volume_name.c_str(), &dev_read_time, &dev_write_time,
 ...
 *           true  on success
 */
bool response(JobControlRecord *jcr, BareosSocket *bs, char *resp, const char *cmd,
              e_prtmsg prtmsg) {
  int n;
