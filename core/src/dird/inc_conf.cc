 ...
 * name handler value code flags default_value
 */
ResourceItem options_items[] = {{"Compression", CFG_TYPE_OPTION, {0}, 0, 0, NULL, NULL, NULL},
...
static inline void IsInPermittedSet(LEX *lc, const char *SetType, const char *permitted_set) {
  ...
  for (p = lc->str; *p; p++) {
    ...
    if (!found) {
      scan_err3(lc, _("Illegal %s option %c, got option string: %s:"), SetType, *p, lc->str);
    }
  }
}
...
#ifdef HAVE_JANSSON
json_t *json_incexc(const int type) { return json_datatype(type, newinc_items); }

json_t *json_options(const int type) { return json_datatype(type, options_items); }
#endif
