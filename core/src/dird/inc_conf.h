 ...
 * options given above.
 */
static struct s_kw FS_option_kw[] = {{"compression", INC_KW_COMPRESSION},
 ...
 * included files.
 */
static struct s_fs_opt FS_options[] = {{"md5", INC_KW_DIGEST, "M"},
...
bool print_incexc_schema_json(PoolMem &buffer, int level, const int type, const bool last = false);
bool print_options_schema_json(PoolMem &buffer, int level, const int type, const bool last = false);
#ifdef HAVE_JANSSON
