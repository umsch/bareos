...
void term_job_server() { JobqDestroy(&job_queue); /* ignore any errors */ }
...
bool IsConnectingToClientAllowed(ClientResource *res) { return res->conn_from_dir_to_fd; }
...
bool IsConnectFromClientAllowed(ClientResource *res) { return res->conn_from_fd_to_dir; }
...
static void *job_thread(void *arg) {
  ...
  if (jcr->res.job->MaxStartDelay != 0 &&
      jcr->res.job->MaxStartDelay < (utime_t)(jcr->start_time - jcr->sched_time)) {
...
static bool JobCheckMaxwaittime(JobControlRecord *jcr) {
  ...
  if (job->MaxWaitTime != 0 && (current + jcr->wait_time_sum) >= job->MaxWaitTime) {
    cancel = true;
  }
...
static bool JobCheckMaxruntime(JobControlRecord *jcr) {
  ...
  if (job->MaxRunTime == 0 && job->FullMaxRunTime == 0 && job->IncMaxRunTime == 0 &&
      job->DiffMaxRunTime == 0) {
    return false;
  }
  run_time = watchdog_time - jcr->start_time;
  Dmsg7(200, "check_maxruntime %llu-%u=%llu >= %llu|%llu|%llu|%llu\n", watchdog_time,
        jcr->start_time, run_time, job->MaxRunTime, job->FullMaxRunTime, job->IncMaxRunTime,
        job->DiffMaxRunTime);

  if (jcr->getJobLevel() == L_FULL && job->FullMaxRunTime != 0 && run_time >= job->FullMaxRunTime) {
    ...
    if (bstrcmp(job->name(), djcr->res.job->name())) {
      ...
      if (job->CancelLowerLevelDuplicates && djcr->is_JobType(JT_BACKUP) &&
        ...
        switch (jcr->getJobLevel()) {
          case L_FULL:
            if (djcr->getJobLevel() == L_DIFFERENTIAL || djcr->getJobLevel() == L_INCREMENTAL) {
              cancel_dup = true;
            }
            break;
          case L_DIFFERENTIAL:
            ...
            break;
          case L_INCREMENTAL:
            if (djcr->getJobLevel() == L_FULL || djcr->getJobLevel() == L_DIFFERENTIAL) {
              cancel_me = true;
            }
        }
        /*
  ...
  switch (JobLevel) {
    ...
    case L_INCREMENTAL:
      ...
      if (do_full) {
        ...
        Jmsg(jcr, M_INFO, 0,
             _("No prior or suitable Full backup found in catalog. Doing FULL backup.\n"));
        ...
        Jmsg(jcr, M_INFO, 0, "%s", jcr->db->strerror());
        Jmsg(jcr, M_INFO, 0,
             _("No prior or suitable Full backup found in catalog. Doing Virtual FULL backup.\n"));
        ...
        /*
         * If we get upgraded to a Virtual Full we will be using a read pool so make sure we have a
         * rpool_source.
         */
        if (!jcr->res.rpool_source) {
          jcr->res.rpool_source = GetPoolMemory(PM_MESSAGE);
         ...
         */
        Jmsg(jcr, M_INFO, 0,
             _("No prior or suitable Differential backup found in catalog. Doing Differential "
               "backup.\n"));
        Bsnprintf(jcr->since, sizeof(jcr->since), _(" (upgraded from %s)"), level_to_str(JobLevel));
      ...
      case L_FULL:
        if (jcr->res.full_pool) {
          ...
          if (jcr->res.run_full_pool_override) {
            ...
            Dmsg2(100, "Pool set to '%s' because of %s", jcr->res.full_pool->name(),
      ...
      case L_VIRTUAL_FULL:
        if (jcr->res.vfull_pool) {
          ...
          if (jcr->res.run_vfull_pool_override) {
            ...
            Dmsg2(100, "Pool set to '%s' because of %s", jcr->res.vfull_pool->name(),
      ...
      case L_INCREMENTAL:
        if (jcr->res.inc_pool) {
          ...
          if (jcr->res.run_inc_pool_override) {
            ...
            Dmsg2(100, "Pool set to '%s' because of %s", jcr->res.inc_pool->name(),
      ...
      case L_DIFFERENTIAL:
        if (jcr->res.diff_pool) {
          ...
          if (jcr->res.run_diff_pool_override) {
            ...
            Dmsg2(100, "Pool set to '%s' because of %s", jcr->res.diff_pool->name(),
...
bool GetOrCreateClientRecord(JobControlRecord *jcr) {
  ...
  Dmsg2(100, "Created Client %s record %d\n", jcr->res.client->hdr.name, jcr->jr.ClientId);
  return true;
}
...
bool GetOrCreateFilesetRecord(JobControlRecord *jcr) {
  ...
  if (!jcr->res.fileset->ignore_fs_changes || !jcr->db->GetFilesetRecord(jcr, &fsr)) {
