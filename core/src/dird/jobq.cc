  ...
  for (;;) {
    ...
    if (!jq->waiting_jobs->empty() && !jq->quit) {
      ...
      for (; je;) {
        ...
        Dmsg4(2300, "Examining Job=%d JobPri=%d want Pri=%d (%s)\n", jcr->JobId, jcr->JobPriority,
              Priority, jcr->res.job->allow_mixed_priority ? "mix" : "no mix");

        /*
         * Take only jobs of correct Priority
         */
        if (!(jcr->JobPriority == Priority ||
              (jcr->JobPriority < Priority && jcr->res.job->allow_mixed_priority &&
               running_allow_mix))) {
...
static bool RescheduleJob(JobControlRecord *jcr, jobq_t *jq, jobq_item_t *je) {
   ...
   */
  if (jcr->res.job->RescheduleTimes == 0 || jcr->reschedule_count < jcr->res.job->RescheduleTimes) {
    resched =
        /*
  ...
  if (resched) {
    ...
    Jmsg(jcr, M_INFO, 0, _("Rescheduled Job %s at %s to re-run in %d seconds (%s).\n"), jcr->Job,
         dt, (int)jcr->res.job->RescheduleInterval, dt2);
...
static bool AcquireResources(JobControlRecord *jcr) {
  ...
  if (jcr->res.rstore && jcr->res.rstore == jcr->res.wstore) { /* possible deadlock */
    Jmsg(jcr, M_FATAL, 0,
         ...
         jcr->res.rstore->name(), jcr->res.rstore_source, jcr->res.wstore->name(),
