...
static char replicatecmd[] = "replicate Job=%s address=%s port=%d ssl=%d Authorization=%s\n";
 ...
 */
static const char *sql_job_bytes = "SELECT SUM(JobBytes) FROM Job WHERE JobId IN (%s)";
...
static inline bool IsSameStorageDaemon(StorageResource *rstore, StorageResource *wstore) {
  ...
  return rstore->SDport == wstore->SDport && Bstrcasecmp(rstore->address, wstore->address) &&
 ...
 */
static inline bool SetMigrationNextPool(JobControlRecord *jcr, PoolResource **retpool) {
...
static inline void StartNewMigrationJob(JobControlRecord *jcr) {
  ...
  ua->batch = true;
  Mmsg(ua->cmd, "run job=\"%s\" jobid=%s ignoreduplicatecheck=yes", jcr->res.job->name(),
 ...
 */
static bool FindJobidsFromMediaidList(JobControlRecord *jcr, idpkt *ids, const char *type) {
...
static bool find_mediaid_then_jobids(JobControlRecord *jcr, idpkt *ids, const char *query1,
                                     const char *type) {
  bool ok = false;
  PoolMem query(PM_MESSAGE);
 ...
 *    ids.count   number of jobids found (may be zero)
 */
static inline bool FindJobidsOfPoolUncopiedJobs(JobControlRecord *jcr, idpkt *ids) {
  ...
  if (!jcr->is_JobType(JT_COPY)) {
    Jmsg(jcr, M_FATAL, 0, _("Selection Type 'pooluncopiedjobs' only applies to Copy Jobs"));
    goto bail_out;
  }
...
static bool regex_find_jobids(JobControlRecord *jcr, idpkt *ids, const char *query1,
...
static inline bool getJobs_to_migrate(JobControlRecord *jcr) {
  ...
  switch (jcr->res.job->selection_type) {
    ...
    case MT_SQLQUERY:
      ...
      if (!jcr->db->SqlQuery(jcr->res.job->selection_pattern, UniqueDbidHandler, (void *)&ids)) {
        Jmsg(jcr, M_FATAL, 0, _("SQL failed. ERR=%s\n"), jcr->db->strerror());
        goto bail_out;
      }
      break;
    ...
    case MT_POOL_OCCUPANCY: {
      ...
      if (!FindJobidsFromMediaidList(jcr, &ids, "Volume")) {
        goto bail_out;
      }
    ...
    case MT_POOL_UNCOPIED_JOBS:
      if (!FindJobidsOfPoolUncopiedJobs(jcr, &ids)) {
        goto bail_out;
      }
      break;
    default:
  ...
  Jmsg(jcr, M_INFO, 0, _("The following %u JobId%s chosen to be %s: %s\n"), ids.count,
       (ids.count < 2) ? _(" was") : _("s were"), jcr->get_ActionName(true), ids.list);
...
bail_out:
  ...
  if (mid.list) {
    FreePoolMemory(mid.list);
  }

  if (jids.list) {
    FreePoolMemory(jids.list);
  }
...
bool DoMigrationInit(JobControlRecord *jcr) {
   ...
   * one of the worker Jobs that do the actual Migration or Copy. If jcr->MigrateJobId is
   * set we know that its an actual Migration or Copy Job.
   */
  if (jcr->MigrateJobId != 0) {
    ...
    if (!jcr->db->GetJobRecord(jcr, &jcr->previous_jr)) {
      Jmsg(jcr, M_FATAL, 0, _("Could not get job record for JobId %s to %s. ERR=%s"),
           edit_int64(jcr->previous_jr.JobId, ed1), jcr->get_ActionName(), jcr->db->strerror());
      return false;
    }
    ...
    Dmsg5(dbglevel, "JobId=%d: Current: Name=%s JobId=%d Type=%c Level=%c\n", (int)jcr->JobId,
          jcr->jr.Name, (int)jcr->jr.JobId, jcr->jr.JobType, jcr->jr.JobLevel);
    ...
    Dmsg4(dbglevel, "mig_jcr: Name=%s JobId=%d Type=%c Level=%c\n", mig_jcr->jr.Name,
          (int)mig_jcr->jr.JobId, mig_jcr->jr.JobType, mig_jcr->jr.JobLevel);
  }
...
static inline bool DoActualMigration(JobControlRecord *jcr) {
   ...
   */
  if (jcr->previous_jr.JobType != JT_BACKUP && jcr->previous_jr.JobType != JT_JOB_COPY) {
  ...
  if (jcr->remote_replicate) {
    ...
    if (!StartStorageDaemonMessageThread(mig_jcr)) {
      goto bail_out;
    }
    ...
    Mmsg(command, replicatecmd, mig_jcr->Job, connection_target_address, wstore->SDDport, tls_need,
         mig_jcr->sd_auth_key);
...
bool DoMigration(JobControlRecord *jcr) {
   ...
   * See if this is a control job e.g. the one that selects the Jobs to Migrate or Copy or
   * one of the worker Jobs that do the actual Migration or Copy. If jcr->MigrateJobId is
   * unset we know that its the control job.
   */
  if (jcr->MigrateJobId == 0) {
...
static inline void GenerateMigrateSummary(JobControlRecord *jcr, MediaDbRecord *mr, int msg_type,
                                          const char *TermMsg) {
  double kbps;
  utime_t RunTime;
  JobControlRecord *mig_jcr = jcr->mig_jcr;
  ...
  Bsnprintf(term_code, sizeof(term_code), TermMsg, jcr->get_OperationName(), jcr->get_ActionName());
  ...
  if (jcr->previous_jr.JobId != 0) {
    ...
    Jmsg(jcr, msg_type, 0,
         ...
         edit_uint64(jcr->jr.JobId, ec8), jcr->jr.Job, level_to_str(jcr->getJobLevel()),
         jcr->res.client ? jcr->res.client->name() : _("*None*"),
         jcr->res.fileset ? jcr->res.fileset->name() : _("*None*"), jcr->res.rpool->name(),
         jcr->res.rpool_source, jcr->res.rstore ? jcr->res.rstore->name() : _("*None*"),
         NPRT(jcr->res.rstore_source), jcr->res.pool->name(), jcr->res.pool_source,
         jcr->res.wstore ? jcr->res.wstore->name() : _("*None*"), NPRT(jcr->res.wstore_source),
         jcr->res.next_pool ? jcr->res.next_pool->name() : _("*None*"), NPRT(jcr->res.npool_source),
         jcr->res.catalog->name(), jcr->res.catalog_source, sdt, edt,
         ...
         mig_jcr ? mig_jcr->VolumeName : _("*None*"), jcr->VolSessionId, jcr->VolSessionTime,
         edit_uint64_with_commas(mr->VolBytes, ec4), edit_uint64_with_suffix(mr->VolBytes, ec5),
         jcr->SDErrors, sd_term_msg, term_code);
  } else {
    /*
    ...
    Jmsg(jcr, msg_type, 0,
         ...
         jcr->res.catalog_source, sdt, edt, edit_utime(RunTime, elapsed, sizeof(elapsed)),
         jcr->JobPriority, term_code);
  }
}
...
void MigrationCleanup(JobControlRecord *jcr, int TermCode) {
  ...
  if (mig_jcr) {
    ...
    if (jcr->IsTerminatedOk()) {
      ...
      switch (jcr->getJobType()) {
        case JT_MIGRATE:
           ...
           */
          Mmsg(query, "UPDATE Job SET Type='%c' WHERE JobId=%s", (char)JT_MIGRATED_JOB, old_jobid);
          mig_jcr->db->SqlQuery(query.c_str());

          /*
           * Move JobLog to new JobId
           */
          Mmsg(query, "UPDATE Log SET JobId=%s WHERE JobId=%s", new_jobid, old_jobid);
          mig_jcr->db->SqlQuery(query.c_str());
        ...
        case JT_COPY:
          ...
          Mmsg(query, "UPDATE Job SET Type='%c' WHERE JobId=%s", (char)JT_JOB_COPY, new_jobid);
          mig_jcr->db->SqlQuery(query.c_str());
          break;
        default:
          break;
    ...
    if (!mig_jcr->db->GetJobVolumeNames(mig_jcr, mig_jcr->jr.JobId, mig_jcr->VolumeName)) {
      /*
