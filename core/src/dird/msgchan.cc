...
bool StartStorageDaemonJob(JobControlRecord *jcr, alist *rstore, alist *wstore, bool send_bsr) {
  ...
  /*
   * Before actually starting a new Job on the SD make sure we send any specific plugin options for
   * this Job.
   */
  ...
  sd->fsend(jobcmd, edit_int64(jcr->JobId, ed1), jcr->Job, job_name.c_str(), client_name.c_str(),
            jcr->getJobType(), jcr->getJobLevel(), fileset_name.c_str(),
            !jcr->res.pool->catalog_files, jcr->res.job->SpoolAttributes, fileset_md5,
            jcr->spool_data, jcr->res.job->PreferMountedVolumes, edit_int64(jcr->spool_size, ed2),
  ...
  if (BgetDirmsg(sd) > 0) {
    Dmsg1(100, "<stored: %s", sd->msg);
    if (sscanf(sd->msg, OK_job, &jcr->VolSessionId, &jcr->VolSessionTime, &auth_key) != 3) {
      Dmsg1(100, "BadJob=%s\n", sd->msg);
  ...
  if (ok && rstore) {
    ...
    foreach_alist(storage, rstore) {
      ...
      sd->fsend(use_storage, StoreName.c_str(), media_type.c_str(), pool_name.c_str(),
                pool_type.c_str(), 0, copy, stripe);
...
extern "C" void *msg_thread(void *arg) {
  ...
  while (!JobCanceled(jcr) && (n = BgetDirmsg(sd)) >= 0) {
     ...
     */
    if (sscanf(sd->msg, Job_end, Job, &JobStatus, &JobFiles, &JobBytes, &JobErrors) == 5) {
