...
static inline bool extract_post_backup_stats_ndmp_native(JobControlRecord *jcr, char *filesystem,
...
int NdmpLoadNext(struct ndm_session *sess) {
  ...
  if (FindNextVolumeForAppend(jcr, &mr, index, unwanted_volumes, create, prune)) {
    ...
    slot_number_t slotnumber =
        LookupStorageMapping(store, slot_type_normal, LOGICAL_TO_PHYSICAL, mr.Slot);
    /*
...
bool DoNdmpBackupNdmpNative(JobControlRecord *jcr) {
   ...
   */
  if (!NdmpBuildClientAndStorageJob(jcr, jcr->res.wstore, jcr->res.client, true, /* init_tape */
                                    true,                                        /* init_robot */
  ...
  for (i = 0; i < fileset->num_includes; i++) {
    ...
    for (j = 0; j < ie->name_list.size(); j++) {
      ...
      if (!FillBackupEnvironment(jcr, ie, nis->filesystem, &ndmp_sess.control_acb->job)) {
        goto cleanup;
      }
...
static inline bool extract_post_backup_stats_ndmp_native(JobControlRecord *jcr, char *filesystem,
  ...
  for (media = sess->control_acb->job.media_tab.head; media; media = media->next) {
    ...
    media->slot_addr = LookupStorageMapping(jcr->res.wstore, slot_type_normal, PHYSICAL_TO_LOGICAL,
                                            media->slot_addr);
#if 0
     ...
     */
    if (media->media_open_error || media->media_io_error || media->label_io_error ||
        media->label_mismatch || media->fmark_error) {
      retval = false;
    }
  }
