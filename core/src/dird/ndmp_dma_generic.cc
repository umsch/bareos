 ...
 */
static inline bool fill_ndmp_agent_config(JobControlRecord *jcr, struct ndmagent *agent,
                                          uint32_t protocol, uint32_t authtype, char *address,
                                          uint32_t port, char *username, char *password) {
...
bool NdmpBuildClientJob(JobControlRecord *jcr, ClientResource *client, StorageResource *store,
  ...
  if (!fill_ndmp_agent_config(jcr, &job->data_agent, client->Protocol, client->AuthType,
                              client->address, client->FDport, client->username,
  ...
  if (!fill_ndmp_agent_config(jcr, &job->tape_agent, store->Protocol, store->AuthType,
                              store->address, store->SDport, store->username,
  ...
  if (Bstrcasecmp(jcr->backup_format, "smtape")) {
    ...
    if (jcr->res.client->ndmp_blocksize < SMTAPE_MIN_BLOCKSIZE ||
      ...
      Jmsg(jcr, M_FATAL, 0,
           _("For SMTAPE NDMP jobs the NDMP blocksize needs to be between %d and %d, but is set to "
             "%d\n"),
    ...
    if ((jcr->res.client->ndmp_blocksize % SMTAPE_BLOCKSIZE_INCREMENTS) != 0) {
      Jmsg(jcr, M_FATAL, 0,
           _("For SMTAPE NDMP jobs the NDMP blocksize needs to be in increments of %d bytes, but "
             "is set to %d\n"),
...
bool NdmpBuildStorageJob(JobControlRecord *jcr, StorageResource *store, bool init_tape,
  ...
  if (!fill_ndmp_agent_config(jcr, &job->data_agent, store->Protocol, store->AuthType,
                              store->address, store->SDport, store->username,
                              store->password.value)) {
    goto bail_out;
  }
  ...
  if (init_robot) {
     ...
     */
    if (!fill_ndmp_agent_config(jcr, &job->robot_agent, store->Protocol, store->AuthType,
                                store->address, store->SDport, store->username,
                                store->password.value)) {
      goto bail_out;
    }
...
void DoNdmpClientStatus(UaContext *ua, ClientResource *client, char *cmd) {
   ...
   */
  ASSERT(client->password.encoding == p_encoding_clear);
  if (!fill_ndmp_agent_config(ua->jcr, &ndmp_job.data_agent, client->Protocol, client->AuthType,
                              client->address, client->FDport, client->username,
                              client->password.value)) {
    return;
  }
  ...
  NdmpDoQuery(
      ua, &ndmp_job,
      (client->ndmp_loglevel > me->ndmp_loglevel) ? client->ndmp_loglevel : me->ndmp_loglevel);
}
#else
void DoNdmpClientStatus(UaContext *ua, ClientResource *client, char *cmd) {
