 ...
 */
static inline void AddToNamelist(struct ndm_job_param *job, char *filename,
                                 const char *restore_prefix, char *name, char *other_name,
                                 int64_t node) {
  ndmp9_name nl;
 ...
 */
static inline int set_files_to_restore(JobControlRecord *jcr, struct ndm_job_param *job,
                                       int32_t FileIndex, const char *restore_prefix,
    ...
    if (node->FileIndex == FileIndex && node->extract) {
      ...
      if (!bstrncmp(restore_pathname.c_str(), "/@NDMP/", 7)) {
        ...
        if (bstrncmp(restore_pathname.c_str(), ndmp_filesystem, len)) {
          AddToNamelist(job, restore_pathname.c_str() + len, restore_prefix, (char *)"", (char *)"",
                        NDMP_INVALID_U_QUAD, NDMP_INVALID_U_QUAD);
        } else {
          AddToNamelist(job, restore_pathname.c_str(), restore_prefix, (char *)"", (char *)"",
                        NDMP_INVALID_U_QUAD, NDMP_INVALID_U_QUAD);
        }
        cnt++;
      }
 ...
 */
static inline bool fill_restore_environment(JobControlRecord *jcr, int32_t current_fi,
  ...
  if (!nbf_options || nbf_options->needs_namelist) {
    if (set_files_to_restore(jcr, job, current_fi, destination_path.c_str(), ndmp_filesystem) ==
        0) {
      /*
       * There is no specific filename selected so restore everything.
       */
      AddToNamelist(job, (char *)"", destination_path.c_str(), (char *)"", (char *)"",
                    NDMP_INVALID_U_QUAD, NDMP_INVALID_U_QUAD);
    }
  }
    ...
    for (bsr = jcr->bsr; bsr; bsr = bsr->next) {
      ...
      for (fileindex = bsr->FileIndex; fileindex; fileindex = fileindex->next) {
        ...
        Dmsg4(20, "sessionid:sesstime : first_fi/last_fi : %d:%d %d/%d \n", current_sessionid,
              current_sessiontime, first_fi, last_fi);
      }
      ...
      if (next_sessid | next_fi) {
        ...
        Jmsg(jcr, M_INFO, 0, _("Run restore for sesstime %s (%d), sessionid %d, fileindex %d\n"),
             bstrftime(dt, sizeof(dt), current_sessiontime, NULL), current_sessiontime,
             current_sessionid, current_fi);
        ...
        if (!fill_restore_environment(jcr, current_fi, &ndmp_sess.control_acb->job)) {
