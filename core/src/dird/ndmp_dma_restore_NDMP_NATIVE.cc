 ...
 */
static inline bool fill_restore_environment_ndmp_native(JobControlRecord *jcr, int32_t current_fi,
                                                        struct ndm_job_param *job) {
  int i;
  char *bp;
  ndmp9_pval pv;
  FilesetResource *fileset;
  char *restore_pathname, *ndmp_filesystem, *restore_prefix;
  PoolMem tape_device;
  ...
  if (jcr->db->GetNdmpEnvironmentString(jcr, JobId, NdmpEnvHandler, &job->env_tab)) {
    /*
  ...
  if (ndmp_filesystem && SetFilesToRestoreNdmpNative(jcr, job, current_fi, destination_path.c_str(),
                                                     ndmp_filesystem) == 0) {
...
int SetFilesToRestoreNdmpNative(JobControlRecord *jcr, struct ndm_job_param *job, int32_t FileIndex,
  ...
  while (node) {
     ...
     * itself, the subdirs and subfiles are not automaticaly marked for extraction ( i.e. set
     * node->extract)
     *
...
static bool DoNdmpNativeRestore(JobControlRecord *jcr) {
  ...
  if (!NdmpBuildClientAndStorageJob(jcr, jcr->res.rstore, jcr->res.client, true, /* init_tape */
                                    true,                                        /* init_robot */
  ...
  if (!fill_restore_environment_ndmp_native(jcr, current_fi, &ndmp_sess.control_acb->job)) {
  ...
  for (ndmmedia *media = ndmp_sess.control_acb->job.media_tab.head; media; media = media->next) {
    ...
    ndmp_slot = LookupStorageMapping(jcr->res.rstore, slot_type_normal, LOGICAL_TO_PHYSICAL,
                                     media->slot_addr);
