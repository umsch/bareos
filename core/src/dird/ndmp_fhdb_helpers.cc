...
void NdmpStoreAttributeRecord(JobControlRecord *jcr, char *fname, char *linked_fname,
  ...
  /*
   * When we do NDMP_BAREOS backup, we only update some fields of this structure the rest is already
   * filled before by initial attributes saved by the tape agent in the storage daemon.
   *
   * With NDMP_NATIVE Backup, we do not get any attributes before so we need to fill everything we
   * need here
   */
...
void NdmpConvertFstat(ndmp9_file_stat *fstat, int32_t FileIndex, int8_t *FileType,
  ...
  Dmsg11(100,
         "ftype:%d mtime:%lu atime:%lu ctime:%lu uid:%lu gid:%lu mode:%lu size:%llu links:%lu "
         "node:%llu fh_info:%llu \n",
         fstat->ftype, fstat->mtime.value, fstat->atime.value, fstat->ctime.value, fstat->uid.value,
         fstat->gid.value, fstat->mode.value, fstat->size.value, fstat->links.value,
