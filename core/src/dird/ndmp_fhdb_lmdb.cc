...
extern "C" int bndmp_fhdb_lmdb_add_node(struct ndmlog *ixlog, int tagc, ndmp9_u_quad node,
                                        ndmp9_file_stat *ndmp_fstat) {
  NIS *nis;
  nis = (NIS *)ixlog->ctx;
...
void NdmpFhdbLmdbRegister(struct ndmlog *ixlog) {
  ...
  if (nis->save_filehist) {
    ...
    if (result) {
      Dmsg2(debuglevel,
            _("Unable to create LMDB database %s: %s. Check OS ulimit settings or adapt "
              "FileHistorySize\n"),
    ...
    result = mdb_dbi_open(fhdb_state->db_rw_txn, NULL, MDB_CREATE | MDB_INTEGERKEY | MDB_DUPSORT,
                          &fhdb_state->db_dbi);
    if (result) {
      ...
      case 0:
        ...
        if (ndmp_fstat.node.valid == NDMP9_VALIDITY_VALID) {
          ...
          NdmpStoreAttributeRecord(
              nis->jcr, full_path.c_str(), nis->virtual_filename, attribs.c_str(), FileType, 0,
              (ndmp_fstat.fh_info.valid == NDMP9_VALIDITY_VALID) ? ndmp_fstat.fh_info.value : 0);
        } else {
