...
static N_TREE_NODE *search_and_insert_tree_node(N_TREE_NODE *node, N_TREE_ROOT *root,
                                                N_TREE_NODE *parent) {
...
static N_TREE_NODE *find_tree_node(N_TREE_ROOT *root, uint64_t inode) {
  ...
  if (root->cached_parent && root->cached_parent->parent) {
    found_node =
        (N_TREE_NODE *)root->cached_parent->parent->child.search(&match_node, NodeCompareById);
    if (found_node) {
      return found_node;
    }
  }
 ...
 */
static inline void add_out_of_order_metadata(NIS *nis, N_TREE_ROOT *fhdb_root, const char *raw_name,
  ...
  Dmsg2(
      100,
      "bndmp_fhdb_mem_add_dir: Added out of order metadata entry for node %llu with parent %llu\n",
      node, dir_node);
}
  ...
  if (nis->save_filehist) {
    ...
    if (!fhdb_root) {
      Jmsg(nis->jcr, M_FATAL, 0,
           _("NDMP protocol error, FHDB add_dir call before add_dirnode_root.\n"));
      return 1;
    }
     ...
     */
    if (fhdb_root->cached_parent && fhdb_root->cached_parent->inode == dir_node) {
      search_and_insert_tree_node(raw_name, fhdb_root->FileIndex, node, fhdb_root,
 ...
 */
static N_TREE_NODE *insert_metadata_parent_node(htable *meta_data, N_TREE_ROOT *fhdb_root,
                                                uint64_t dir_node) {
  N_TREE_NODE *parent;
  OOO_MD *md_entry;
...
static inline bool ProcessOutOfOrderMetadata(htable *meta_data, N_TREE_ROOT *fhdb_root) {
  ...
  foreach_htable(md_entry, meta_data) {
     ...
     * See if this entry is in the cached parent.
     */
    if (fhdb_root->cached_parent && fhdb_root->cached_parent->inode == md_entry->dir_node) {
...
extern "C" int bndmp_fhdb_mem_add_node(struct ndmlog *ixlog, int tagc, ndmp9_u_quad node,
  ...
  if (nis->save_filehist) {
    ...
    if (meta_data) {
      if (!ProcessOutOfOrderMetadata(meta_data, fhdb_root)) {
        Jmsg(nis->jcr, M_FATAL, 0,
             _("NDMP protocol error, FHDB unable to process out of order metadata.\n"));
        meta_data->destroy();
        free(meta_data);
    ...
    if (!wanted_node) {
      Jmsg(nis->jcr, M_FATAL, 0,
           _("NDMP protocol error, FHDB add_node request for unknown node %llu.\n"), node);
      return 1;
    }
...
extern "C" int bndmp_fhdb_mem_add_dirnode_root(struct ndmlog *ixlog, int tagc,
  ...
  if (nis->save_filehist) {
    ...
    if (fhdb_root) {
      Jmsg(nis->jcr, M_FATAL, 0,
           _("NDMP protocol error, FHDB add_dirnode_root call more then once.\n"));
      return 1;
    }
...
void NdmpFhdbMemProcessDb(struct ndmlog *ixlog) {
  ...
  if (fhdb_root) {
    if (nis->jcr->ar) {
      ...
      NdmpStoreAttributeRecord(nis->jcr, fhdb_root->fname, nis->virtual_filename, fhdb_root->attr,
      ...
      for (node = fhdb_root->first; node; node = node->next) {
        ...
        if (node->attr) {
          ...
          NdmpStoreAttributeRecord(nis->jcr, fname.c_str(), nis->virtual_filename, node->attr,
