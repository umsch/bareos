...
bool StoreNdmmediaInfoInDatabase(ndmmedia *media, JobControlRecord *jcr) {
  ...
  if (!jcr->db->GetMediaRecord(jcr, &mr)) {
    Jmsg(jcr, M_FATAL, 0, _("Catalog error getting Media record for Medium %s: %s"), mr.VolumeName,
         jcr->db->strerror());
    return false;
  }
  ...
  if (!jcr->db->UpdateMediaRecord(jcr, &mr)) {
    Jmsg(jcr, M_FATAL, 0, _("Catalog error updating Media record for Medium %s: %s"), mr.VolumeName,
         jcr->db->strerror());
    return false;
  }
  return true;
