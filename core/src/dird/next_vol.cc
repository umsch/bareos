...
int FindNextVolumeForAppend(JobControlRecord *jcr, MediaDbRecord *mr, int index,
  ...
  /*
   * If we are using an Autochanger, restrict Volume search to the Autochanger on the first pass
   */
  ...
  while (1) {
    ...
    if (!ok) {
       ...
       */
      Dmsg4(debuglevel, "after find_next_vol ok=%d index=%d InChanger=%d Vstat=%s\n", ok, index,
       ...
       */
      if (!ok && (jcr->res.pool->purge_oldest_volume || jcr->res.pool->recycle_oldest_volume)) {
    ...
    if (ok) {
      ...
      if (bstrcmp(mr->VolStatus, "Append") && HasVolumeExpired(jcr, mr)) {
        if (retry++ < 200) { /* sanity check */
          ...
          Jmsg(jcr, M_ERROR, 0,
               _("We seem to be looping trying to find the next volume. I give up.\n"));
        }
      }
    }
...
bool HasVolumeExpired(JobControlRecord *jcr, MediaDbRecord *mr) {
  ...
  /*
   * Check limits and expirations if "Append" and it has been used i.e. mr->VolJobs > 0
   */
  if (bstrcmp(mr->VolStatus, "Append") && mr->VolJobs > 0) {
    ...
    if ((mr->MaxVolBytes > 0 && mr->VolBytes >= mr->MaxVolBytes)) {
      Jmsg(jcr, M_INFO, 0, _("Max Volume bytes=%s exceeded. Marking Volume \"%s\" as Full.\n"),
       ...
       */
      Jmsg(jcr, M_INFO, 0, _("Volume used once. Marking Volume \"%s\" as Used.\n"), mr->VolumeName);
       ...
       */
      Jmsg(jcr, M_INFO, 0, _("Max Volume jobs=%s exceeded. Marking Volume \"%s\" as Used.\n"),
           edit_uint64_with_commas(mr->MaxVolJobs, ed1), mr->VolumeName);
      Dmsg3(debuglevel, "MaxVolJobs=%d JobId=%d Vol=%s\n", mr->MaxVolJobs, (uint32_t)jcr->JobId,
            mr->VolumeName);
      bstrncpy(mr->VolStatus, "Used", sizeof(mr->VolStatus));
      expired = true;
       ...
       */
      Jmsg(jcr, M_INFO, 0, _("Max Volume files=%s exceeded. Marking Volume \"%s\" as Used.\n"),
      ...
      if (mr->VolUseDuration <= (now - mr->FirstWritten)) {
        Jmsg(jcr, M_INFO, 0,
             _("Max configured use duration=%s sec. exceeded. Marking Volume \"%s\" as Used.\n"),
  ...
  if (expired) {
    ...
    if (!jcr->db->UpdateMediaRecord(jcr, mr)) {
      Jmsg(jcr, M_ERROR, 0, _("Catalog error updating volume \"%s\". ERR=%s"), mr->VolumeName,
           jcr->db->strerror());
    }
  }
...
void CheckIfVolumeValidOrRecyclable(JobControlRecord *jcr, MediaDbRecord *mr, const char **reason) {
  ...
  *reason = _("but should be Append, Purged or Recycle");
  ...
  /*
   * Check retention period from last written, but recycle to within a minute to try to catch close
   * calls ...
   */
...
bool GetScratchVolume(JobControlRecord *jcr, bool InChanger, MediaDbRecord *mr,
