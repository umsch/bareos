...
uint64_t FetchRemainingQuotas(JobControlRecord *jcr) {
  ...
  if (jcr->res.client->StrictQuotas && jcr->res.client->SoftQuota &&
    ...
    remaining = jcr->res.client->SoftQuota - jcr->jr.JobSumTotalBytes;
             ...
             jcr->res.client->GraceTime > 0 && jcr->res.client->SoftQuotaGracePeriod > 0 &&
             (now - (uint64_t)jcr->res.client->GraceTime) >
                 (uint64_t)jcr->res.client->SoftQuotaGracePeriod) {
    /*
  ...
  Dmsg4(debuglevel, "Quota for %s is %llu. Remainder is %llu, QuotaLimit: %llu\n", jcr->jr.Name,
        jcr->jr.JobSumTotalBytes, remaining, jcr->res.client->QuotaLimit);
    ...
    if (jcr->res.client->GraceTime == 0 && jcr->res.client->SoftQuotaGracePeriod) {
      ...
      goto bail_out;
               ...
               (now - (uint64_t)jcr->res.client->GraceTime) <
                   (uint64_t)jcr->res.client->SoftQuotaGracePeriod) {
      Jmsg(jcr, M_ERROR, 0,
           _("Softquota Exceeded, will be enforced after Grace Period expires.\n"));
    } else if (jcr->res.client->SoftQuotaGracePeriod &&
               (now - (uint64_t)jcr->res.client->GraceTime) >
                   (uint64_t)jcr->res.client->SoftQuotaGracePeriod) {
      /*
       * If gracetime has expired update else check more if not set softlimit yet then set and bail
       * out.
       */
      if (jcr->res.client->QuotaLimit < 1) {
        ...
        /*
         * If gracetime has expired update else check more if not set softlimit yet then set and
         * bail out.
         */
        if (jcr->res.client->QuotaLimit < 1) {
          if (!jcr->db->UpdateQuotaSoftlimit(jcr, &jcr->jr)) {
