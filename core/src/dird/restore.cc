...
static inline bool DoNativeRestoreBootstrap(JobControlRecord *jcr) {
  ...
  while (!feof(info.bs)) {
    ...
    if (!jcr->passive_client) {
      ...
      fd->fsend(storaddrcmd, connection_target_address, store->SDDport, tls_need, jcr->sd_auth_key);
  ...
  switch (jcr->getJobProtocol()) {
    ...
    default:
      ...
      Jmsg(jcr, msg_type, 0,
           ...
           edit_uint64_with_commas((uint64_t)jcr->ExpectedFiles, ec1),
           edit_uint64_with_commas((uint64_t)jcr->jr.JobFiles, ec2),
           edit_uint64_with_commas(jcr->jr.JobBytes, ec3), (float)kbps, jcr->JobErrors, fd_term_msg,
           sd_term_msg, secure_erase_status.c_str(), TermMsg);
      break;
  }
}
