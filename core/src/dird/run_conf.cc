...
struct s_kw RunFields[] = {
    {"pool", 'P'},     {"fullpool", 'f'},  {"incrementalpool", 'i'}, {"differentialpool", 'd'},
    {"nextpool", 'n'}, {"level", 'L'},     {"storage", 'S'},         {"messages", 'M'},
    {"priority", 'p'}, {"spooldata", 's'}, {"maxrunschedtime", 'm'}, {"accurate", 'a'},
    {NULL, 0}};
        ...
        switch (RunFields[i].token) {
          ...
          case 'n': /* NextPool */
            ...
            if (pass == 2) {
              ...
              if (res == NULL) {
                scan_err1(lc, _("Could not find specified Pool Resource: %s"), lc->str);
                /* NOT REACHED */
              }
              switch (RunFields[i].token) {
                case 'P':
          ...
          case 'S': /* Storage */
            ...
            if (pass == 2) {
              ...
              if (res == NULL) {
                scan_err1(lc, _("Could not find specified Storage Resource: %s"), lc->str);
                /* NOT REACHED */
              }
          ...
          case 'M': /* Messages */
            ...
            if (pass == 2) {
              ...
              if (res == NULL) {
                scan_err1(lc, _("Could not find specified Messages Resource: %s"), lc->str);
                /* NOT REACHED */
              }
      ...
      case s_modulo:
        ...
        *p++ = 0; /* Separate two halves */
        ...
        if (IsAnInteger(lc->str) && IsAnInteger(p)) {
          ...
          for (i = 0; i < 31; i++) {
            if (i % code2 == 0) {
              SetBit(i + code, lrun.mday);
                   ...
                   (lc->str[0] == 'w' || lc->str[0] == 'W') && (p[0] == 'w' || p[0] == 'W') &&
                   IsAnInteger(lc->str + 1) && IsAnInteger(p + 1)) {
          /*
          ...
          if (code > code2) {
            scan_err0(lc, _("Bad week number specification in modulo, offset must always be <= "
                            "than modulo."));
          }
          if (!have_woy) {
            ClearBits(0, 53, lrun.woy);
          ...
          scan_err0(lc, _("Bad modulo time specification. Format for weekdays is '01/02', for "
                          "yearweeks is 'w01/w02'."));
        }
        break;
      case s_range:
        p = strchr(lc->str, '-');
