...
#ifdef list_chain
  job_item *je;
  foreach_dlist(je, jobs_to_run) { dump_job(je, _("Walk queue")); }
#endif
  /*
...
static void find_runs() {
  ...
  Dmsg8(debuglevel, "nh = %x: h=%d m=%d md=%d wd=%d wom=%d woy=%d yday=%d\n", next_hour, nh_hour,
        nh_month, nh_mday, nh_wday, nh_wom, nh_woy, nh_yday);
  ...
  foreach_res(job, R_JOB) {
    ...
    for (run = sched->run; run; run = run->next) {
...
#ifdef xxxx
      Dmsg0(000, "\n");
      Dmsg7(000, "run h=%d m=%d md=%d wd=%d wom=%d woy=%d yday=%d\n", hour, month, mday, wday, wom,
            woy, yday);
      Dmsg6(000, "bitset bsh=%d bsm=%d bsmd=%d bswd=%d bswom=%d bswoy=%d\n",
            BitIsSet(hour, run->hour), BitIsSet(month, run->month), BitIsSet(mday, run->mday),
            BitIsSet(wday, run->wday), BitIsSet(wom, run->wom), BitIsSet(woy, run->woy));

      Dmsg7(000, "nh_run h=%d m=%d md=%d wd=%d wom=%d woy=%d yday=%d\n", nh_hour, nh_month, nh_mday,
            nh_wday, nh_wom, nh_woy, nh_yday);
      Dmsg6(000, "nh_bitset bsh=%d bsm=%d bsmd=%d bswd=%d bswom=%d bswoy=%d\n",
            BitIsSet(nh_hour, run->hour), BitIsSet(nh_month, run->month),
            BitIsSet(nh_mday, run->mday), BitIsSet(nh_wday, run->wday), BitIsSet(nh_wom, run->wom),
      ...
      run_nh = BitIsSet(nh_hour, run->hour) && BitIsSet(nh_mday, run->mday) &&
...
static void add_job(JobResource *job, RunResource *run, time_t now, time_t runtime) {
...
#ifdef SCHED_DEBUG
  Dmsg4(000, "Add: Job=\"%s\" run=%lld last_run=%lld now=%lld\n", job->hdr.name, (utime_t)runtime,
        (utime_t)run->last_run, (utime_t)now);
#endif
  ...
  foreach_dlist(ji, jobs_to_run) {
    if (ji->runtime > je->runtime || (ji->runtime == je->runtime && ji->Priority > je->Priority)) {
...
#ifdef SCHED_DEBUG
  foreach_dlist(ji, jobs_to_run) { dump_job(ji, _("Run queue")); }
