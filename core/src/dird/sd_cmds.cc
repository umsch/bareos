 ...
 */
bool ConnectToStorageDaemon(JobControlRecord *jcr, int retry_interval, int max_retry_time,
                            bool verbose) {
  BareosSocket *sd;
      ...
      switch (vl->Type) {
        case slot_type_drive:
          ...
          if (vl->Index >= INDEX_MAX_DRIVES) {
            ua->ErrorMsg(
                _("Drive number %hd greater then INDEX_MAX_DRIVES(%hd) please increase define\n"),
 ...
 */
bool NativeTransferVolume(UaContext *ua, StorageResource *store, slot_number_t src_slot,
...
bool DoStoragePluginOptions(JobControlRecord *jcr) {
  ...
  if (jcr->res.job && jcr->res.job->SdPluginOptions && jcr->res.job->SdPluginOptions->size()) {
