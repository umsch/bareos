...
static char DevStats[] =
    "Devicestats [%lld]: Device=%s Read=%llu, Write=%llu, SpoolSize=%llu, NumWaiting=%lu, "
    "NumWriters=%lu, "
...
static inline bool LookupDevice(JobControlRecord *jcr, const char *device_name, DBId_t StorageId,
                                DBId_t *DeviceId) {
  DeviceDbRecord dr;

  memset(&dr, 0, sizeof(dr));

  if (cached_device.StorageId == StorageId && bstrcmp(cached_device.device_name, device_name)) {
        ...
        while (BnetRecv(sd) >= 0) {
          ...
          if (bstrncmp(sd->msg, "Devicestats", 10)) {
            ...
            if (sscanf(sd->msg, DevStats, &dsr.SampleTime, DevName.c_str(), &dsr.ReadBytes,
                       ...
                       &dsr.ReadTime, &dsr.WriteTime, &dsr.MediaId, &dsr.VolCatBytes,
              ...
              Dmsg4(200, "MediaId=%ld, VolBytes=%llu, VolFiles=%llu, VolBlocks=%llu\n", dsr.MediaId,
                    dsr.VolCatBytes, dsr.VolCatFiles, dsr.VolCatBlocks);
            ...
            if (sscanf(sd->msg, TapeAlerts, &tsr.SampleTime, DevName.c_str(), &tsr.AlertFlags) ==
                3) {
              UnbashSpaces(DevName);

              Dmsg3(200, "New stats [%lld]: Device %s TapeAlert %llu\n", tsr.SampleTime,
                    DevName.c_str(), tsr.AlertFlags);
            ...
            if (sscanf(sd->msg, JobStats, &jsr.SampleTime, &jsr.JobId, &jsr.JobFiles, &jsr.JobBytes,
                       DevName.c_str()) == 5) {
              UnbashSpaces(DevName);

              Dmsg5(200,
                    "New Jobstats [%lld]: JobId %ld, JobFiles %lu, JobBytes %llu, DevName %s\n",
