...
void CopyRstorage(JobControlRecord *jcr, alist *storage, const char *where) {
  ...
  if (storage) {
    ...
    foreach_alist(store, storage) { jcr->res.rstorage->append(store); }
  ...
  switch (jcr->getJobType()) {
    case JT_BACKUP:
      ...
      if (jcr->res.wstorage) {
        ...
        Jmsg(jcr, M_FATAL, 0, _("No write storage, don't know how to setup paired storage\n"));
      }
      break;
    case JT_RESTORE:
      /*
      ...
      if (jcr->res.rstorage) {
        ...
        Jmsg(jcr, M_FATAL, 0, _("No read storage, don't know how to setup paired storage\n"));
      }
      break;
    case JT_MIGRATE:
    case JT_COPY:
      ...
      break;
    default:
      Jmsg(jcr, M_FATAL, 0, _("Unknown Job Type %s, don't know how to setup paired storage\n"),
  ...
  if (jcr->res.pstorage) {
    switch (jcr->getJobType()) {
      ...
      case JT_COPY:
        ...
        break;
      default:
        Jmsg(jcr, M_FATAL, 0, _("Unknown Job Type %s, don't know how to free paired storage\n"),
             job_type_to_str(jcr->getJobType()));
        break;
    }
  }
  ...
  switch (jcr->getJobType()) {
    case JT_BACKUP:
      ...
      if (jcr->res.wstorage) {
        ...
        Jmsg(jcr, M_FATAL, 0, _("No write storage, don't know how to check for paired storage\n"));
        return false;
      }
      break;
    case JT_RESTORE:
    ...
    case JT_COPY:
      if (jcr->res.rstorage) {
        ...
        Jmsg(jcr, M_FATAL, 0, _("No read storage, don't know how to check for paired storage\n"));
        return false;
      }
      break;
    default:
...
bool SelectNextRstore(JobControlRecord *jcr, bootstrap_info &info) {
   ...
   */
  FreeRstorage(jcr);
  Jmsg(jcr, M_FATAL, 0, _("Could not acquire read storage lock for \"%s\""), info.storage);
  return false;
}
 ...
 */
changer_vol_list_t *get_vol_list_from_storage(UaContext *ua, StorageResource *store, bool listall,
                                              bool scan, bool cached) {
  vol_list_type type;
  dlist *contents = NULL;
...
bool transfer_volume(UaContext *ua, StorageResource *store, slot_number_t src_slot,
 ...
 */
vol_list_t *vol_is_loaded_in_drive(StorageResource *store, changer_vol_list_t *vol_list,
                                   slot_number_t slot) {
  vol_list_t *vl;
 ...
 */
slot_number_t LookupStorageMapping(StorageResource *store, slot_type slot_type, s_mapping_type type,
                                   slot_number_t slot) {
