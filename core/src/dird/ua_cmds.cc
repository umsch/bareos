...
static struct ua_cmdstruct commands[] = {
    ...
    {NT_(".actiononpurge"), DotAopCmd, _("List possible actions on purge"), NULL, true, false},
    ...
    {NT_(".authorized"), DotAuthorizedCmd, _("Check for authorization"),
     ...
     false, false},
    {NT_(".catalogs"), DotCatalogsCmd, _("List all catalog resources"), NULL, false, false},
    {NT_(".clients"), DotClientsCmd, _("List all client resources"), NT_("[enabled | disabled]"),
     true, false},
    {NT_(".consoles"), DotConsolesCmd, _("List all console resources"), NULL, true, false},
    {NT_(".defaults"), DotDefaultsCmd, _("Get default settings"),
     NT_("job=<job-name> | client=<client-name> | storage=<storage-name | pool=<pool-name>"), false,
     false},
#ifdef DEVELOPER
    ...
    {NT_(".exit"), DotAdminCmds, _("Close connection"),
     NT_("[dir | director] [client=<client>] [storage=<storage>]"), false, true},
#endif
    {NT_(".filesets"), DotFilesetsCmd, _("List all filesets"), NULL, false, false},
    {NT_(".help"), DotHelpCmd, _("Print parsable information about a command"),
     NT_("[ all | item=cmd ]"), false, false},
    {NT_(".jobdefs"), DotJobdefsCmd, _("List all job defaults resources"), NULL, true, false},
    {NT_(".jobs"), DotJobsCmd, _("List all job resources"),
     NT_("[type=<jobtype>] | [enabled | disabled]"), true, false},
    {NT_(".jobstatus"), DotJobstatusCmd, _("List jobstatus information"), NT_("[ =<jobstatus> ]"),
     true, false},
    {NT_(".levels"), DotLevelsCmd, _("List all backup levels"), NULL, false, false},
    {NT_(".locations"), DotLocationsCmd, NULL, NULL, true, false},
    {NT_(".messages"), DotGetmsgsCmd, _("Display pending messages"), NULL, false, false},
    {NT_(".media"), DotMediaCmd, _("List all medias"), NULL, true, false},
    {NT_(".mediatypes"), DotMediatypesCmd, _("List all media types"), NULL, true, false},
    {NT_(".msgs"), DotMsgsCmd, _("List all message resources"), NULL, false, false},
    {NT_(".pools"), DotPoolsCmd, _("List all pool resources"), NT_("type=<pooltype>"), true, false},
    {NT_(".profiles"), DotProfilesCmd, _("List all profile resources"), NULL, true, false},
    {NT_(".quit"), quit_cmd, _("Close connection"), NULL, false, false},
    {NT_(".sql"), DotSqlCmd, _("Send an arbitrary SQL command"), NT_("query=<sqlquery>"), false,
     true},
    ...
    {NT_(".status"), DotStatusCmd, _("Report status"),
     NT_("dir ( current | last | header | scheduled | running | terminated ) |\n"
         "storage=<storage> [ header | waitreservation | devices | volumes | spooling | running | "
         "terminated ] |\n"
         "client=<client> [ header | terminated | running ]"),
     false, true},
    {NT_(".storages"), DotStorageCmd, _("List all storage resources"), NT_("[enabled | disabled]"),
     true, false},
    {NT_(".types"), DotTypesCmd, _("List all job types"), NULL, false, false},
    {NT_(".volstatus"), DotVolstatusCmd, _("List all volume status"), NULL, true, false},
    {NT_(".bvfs_lsdirs"), DotBvfsLsdirsCmd, _("List directories using BVFS"),
     NT_("jobid=<jobid> path=<path> | pathid=<pathid> [limit=<limit>] [offset=<offset>]"), true,
     true},
    {NT_(".bvfs_lsfiles"), DotBvfsLsfilesCmd, _("List files using BVFS"),
     NT_("jobid=<jobid> path=<path> | pathid=<pathid> [limit=<limit>] [offset=<offset>]"), true,
     true},
    {NT_(".bvfs_update"), DotBvfsUpdateCmd, _("Update BVFS cache"), NT_("[jobid=<jobid>]"), true,
     true},
    ...
    {NT_(".bvfs_versions"), DotBvfsVersionsCmd, _("List versions of a file"),
     NT_("jobid=0 client=<client-name> pathid=<path-id> filename=<file-name> [copies] [versions]"),
     true, true},
    {NT_(".bvfs_restore"), DotBvfsRestoreCmd,
     _("Mark BVFS files/directories for restore. Stored in handle."),
     NT_("path=<handle> jobid=<jobid> [fileid=<file-id>] [dirid=<dirid>] [hardlink=<hardlink>]"),
     true, true},
    {NT_(".bvfs_cleanup"), DotBvfsCleanupCmd, _("Cleanup BVFS cache for a certain handle"),
     NT_("path=<handle>"), true, true},
    {NT_(".bvfs_clear_cache"), DotBvfsClearCacheCmd, _("Clear BVFS cache"), NT_("yes"), false,
     true},
    {NT_("add"), add_cmd, _("Add media to a pool"),
     NT_("pool=<pool-name> storage=<storage-name> jobid=<jobid>"), false, true},
    {NT_("autodisplay"), AutodisplayCmd, _("Autodisplay console messages"), NT_("on | off"), false,
     false},
    {NT_("automount"), AutomountCmd, _("Automount after label"), NT_("on | off"), false, true},
    {NT_("cancel"), CancelCmd, _("Cancel a job"),
     NT_("storage=<storage-name> | jobid=<jobid> | job=<job-name> | ujobid=<unique-jobid> | "
         "state=<job_state> | all yes"),
     false, true},
    {NT_("configure"), ConfigureCmd, _("Configure director resources"),
     NT_(get_configure_usage_string()), false, true},
    {NT_("create"), CreateCmd, _("Create DB Pool from resource"), NT_("pool=<pool-name>"), false,
     true},
    ...
    {NT_("estimate"), EstimateCmd, _("Performs FileSet estimate, listing gives full listing"),
     NT_("fileset=<fileset-name> client=<client-name> level=<level> accurate=<yes/no> "
         "job=<job-name> listing"),
     true, true},
    {NT_("exit"), quit_cmd, _("Terminate Bconsole session"), NT_(""), false, false},
    {NT_("export"), ExportCmd, _("Export volumes from normal slots to import/export slots"),
     NT_("storage=<storage-name> srcslots=<slot-selection> [ dstslots=<slot-selection> "
         "volume=<volume-name> scan ]"),
     true, true},
    {NT_("gui"), gui_cmd,
     _("Switch between interactive (gui off) and non-interactive (gui on) mode"), NT_("on | off"),
     false, false},
    ...
    {NT_("label"), LabelCmd, _("Label a tape"),
     NT_("storage=<storage-name> volume=<volume-name> pool=<pool-name> slot=<slot> [ drive = "
         "<drivenum>] [ barcodes ] [ encrypt ] [ yes ]"),
     false, true},
    {NT_("list"), list_cmd, _("List objects from catalog"),
     NT_("basefiles jobid=<jobid> | basefiles ujobid=<complete_name> |\n"
         "backups client=<client-name> [fileset=<fileset-name>] [jobstatus=<status>] "
         "[level=<level>] [order=<asc|desc>] [limit=<number>] |\n"
         ...
         "jobs [job=<job-name>] [client=<client-name>] [jobstatus=<status>] [joblevel=<joblevel>] "
         "[volume=<volumename>] [days=<number>] [hours=<number>] [last] [count] |\n"
         "job=<job-name> [client=<client-name>] [jobstatus=<status>] [volume=<volumename>] "
    ...
    {NT_("llist"), LlistCmd, _("Full or long list like list command"),
     NT_("basefiles jobid=<jobid> | basefiles ujobid=<complete_name> |\n"
         "backups client=<client-name> [fileset=<fileset-name>] [jobstatus=<status>] "
         "[level=<level>] [order=<asc|desc>] [limit=<number>] [days=<number>] [hours=<number>]|\n"
         "clients | copies jobid=<jobid> |\n"
         "files jobid=<jobid> | files ujobid=<complete_name> |\n"
         "filesets |\n"
         "fileset jobid=<jobid> | fileset ujobid=<complete_name> |\n"
         "fileset [ filesetid=<filesetid> ] | fileset [ jobid=<jobid> ] |\n"
         "jobs [job=<job-name>] [client=<client-name>] [jobstatus=<status>] [volume=<volumename>] "
         "[days=<number>] [hours=<number>] [last] [count] |\n"
         "job=<job-name> [client=<client-name>] [jobstatus=<status>] [joblevel=<joblevel>] "
     ...
     true, true},
    {NT_("messages"), MessagesCmd, _("Display pending messages"), NT_(""), false, false},
    {NT_("memory"), MemoryCmd, _("Print current memory usage"), NT_(""), true, false},
    ...
    {NT_("purge"), PurgeCmd, _("Purge records from catalog"),
     NT_("[files [job=<job> | jobid=<jobid> | client=<client> | volume=<volume>]] |\n"
         ...
         "[volume[=<volume>] [storage=<storage>] [pool=<pool> | allpools] [devicetype=<type>] "
         "[drive=<drivenum>] [action=<action>]] |\n"
         "[quota [client=<client>]]"),
     true, true},
    {NT_("quit"), quit_cmd, _("Terminate Bconsole session"), NT_(""), false, false},
    ...
    {NT_("rerun"), reRunCmd, _("Rerun a job"),
     NT_("jobid=<jobid> | since_jobid=<jobid> [ until_jobid=<jobid> ] | days=<nr_days> | "
         "hours=<nr_hours> | yes"),
     false, true},
    ...
    {NT_("run"), RunCmd, _("Run a job"),
     NT_("job=<job-name> client=<client-name> fileset=<fileset-name> level=<level>\n"
         ...
         "\tpool=<pool-name> pluginoptions=<plugin-options-string> accurate=<yes|no> "
         "comment=<text>\n"
         "\tspooldata=<yes|no> priority=<number> jobid=<jobid> catalog=<catalog> "
         "migrationjob=<job-name>\n"
    ...
    {NT_("status"), StatusCmd, _("Report status"),
     NT_("all | dir=<dir-name> | director | scheduler | schedule=<schedule-name> | "
         "client=<client-name> |\n"
    ...
    {NT_("setdebug"), SetdebugCmd, _("Sets debug level"),
     NT_("level=<nn> trace=0/1 timestamp=0/1 client=<client-name> | dir | storage=<storage-name> | "
         "all"),
     true, true},
    {NT_("setip"), SetipCmd, _("Sets new client address -- if authorized"), NT_(""), false, true},
    {NT_("show"), show_cmd, _("Show resource records"),
     NT_("catalog=<catalog-name> | client=<client-name> | console=<console-name> | "
         ...
         "all [verbose]"),
     true, true},
    {NT_("sqlquery"), SqlqueryCmd, _("Use SQL to query catalog"), NT_(""), false, true},
    {NT_("time"), time_cmd, _("Print current time"), NT_(""), true, false},
    {NT_("trace"), TraceCmd, _("Turn on/off trace to file"), NT_("on | off"), true, true},
    ...
    {NT_("update"), UpdateCmd, _("Update volume, pool, slots, job or statistics"),
     NT_("[volume=<volume-name> "
         ...
         "[days=<number>]]"),
     true, true},
    {NT_("use"), use_cmd, _("Use specific catalog"), NT_("catalog=<catalog>"), false, true},
    {NT_("var"), var_cmd, _("Does variable expansion"), NT_(""), false, true},
    {NT_("version"), VersionCmd, _("Print Director version"), NT_(""), true, false},
    {NT_("wait"), wait_cmd, _("Wait until no jobs are running"),
     NT_("jobname=<name> | jobid=<jobid> | ujobid=<complete_name> | mount [timeout=<number>]"),
     false, false}};
...
bool Do_a_command(UaContext *ua) {
  ...
  for (i = 0; i < comsize; i++) { /* search for command */
    if (bstrncasecmp(ua->argk[0], commands[i].key, len)) {
       ...
       */
      if (!bstrcmp(ua->argk[0], NT_("quit")) && !ua->AclAccessOk(Command_ACL, ua->argk[0], true)) {
        break;
      }
...
static bool add_cmd(UaContext *ua, const char *cmd) {
  ...
  for (;;) {
    ...
    Bsnprintf(buf, sizeof(buf), _("Enter number of Volumes to create. 0=>fixed name. Max=%d: "),
              max);
    if (!GetPint(ua, buf)) {
      return true;
    }
...
static inline bool SetbwlimitFiled(UaContext *ua, ClientResource *client, int64_t limit,
                                   char *Job) {
  /*
...
static inline bool setbwlimit_stored(UaContext *ua, StorageResource *store, int64_t limit,
                                     char *Job) {
  /*
...
static void do_storage_setdebug(UaContext *ua, StorageResource *store, int level, int trace_flag,
                                int timestamp_flag) {
  BareosSocket *sd;
...
static void DoDirectorSetdebug(UaContext *ua, int level, int trace_flag, int timestamp_flag) {
  ...
  ua->SendMsg("level=%d trace=%d hangup=%d timestamp=%d tracefilename=%s\n", level, GetTrace(),
              GetHangup(), GetTimestamp(), tracefilename.c_str());
}

static void do_all_setdebug(UaContext *ua, int level, int trace_flag, int hangup_flag,
                            int timestamp_flag) {
...
static bool SetdebugCmd(UaContext *ua, const char *cmd) {
  ...
  for (i = 1; i < ua->argc; i++) {
    ...
    if (Bstrcasecmp(ua->argk[i], "dir") || Bstrcasecmp(ua->argk[i], "director")) {
      DoDirectorSetdebug(ua, level, trace_flag, timestamp_flag);
      return true;
    }
    ...
    if (Bstrcasecmp(ua->argk[i], NT_("store")) || Bstrcasecmp(ua->argk[i], NT_("storage")) ||
...
static bool ResolveCmd(UaContext *ua, const char *cmd) {
  ...
  for (int i = 1; i < ua->argc; i++) {
    if (Bstrcasecmp(ua->argk[i], NT_("client")) || Bstrcasecmp(ua->argk[i], NT_("fd"))) {
      if (ua->argv[i]) {
        client = ua->GetClientResWithName(ua->argv[i]);
        if (!client) {
          ua->ErrorMsg(_("Client \"%s\" not found.\n"), ua->argv[i]);
...
static bool TruncateCmd(UaContext *ua, const char *cmd) {
  ...
  ua->db->ListSqlQuery(ua->jcr, tmp.c_str(), ua->send, HORZ_LIST, "volumes", true);
  ...
  if (ua->GetStoreResWithId(mr.StorageId)->Protocol != APT_NATIVE) {
    ua->WarningMsg(_(
        "Storage uses a non-native protocol. Truncate is only supported for native protocols.\n"));
    goto bail_out;
  }
...
static bool DeleteCmd(UaContext *ua, const char *cmd) {
  ...
  static const char *keywords[] = {NT_("volume"), NT_("pool"), NT_("jobid"), NULL};
  ...
  *tok2 = '\0';
  ...
  if (Is_a_number(tok) && Is_a_number(tok2)) {
    ...
    if (j2 > j1) {
       ...
       */
      if ((j2 - j1) > 25) {
        Bsnprintf(buf, sizeof(buf), _("Are you sure you want to delete %d JobIds ? (yes/no): "),
                  j2 - j1);
...
static void DoMountCmd(UaContext *ua, const char *cmd) {
  ...
  if ((bstrcmp(cmd, "release") || bstrcmp(cmd, "unmount")) && FindArg(ua, "alldrives") >= 0) {
    do_alldrives = true;
  }
...
static bool wait_cmd(UaContext *ua, const char *cmd) {
  ...
  for (int i = 1; i < ua->argc; i++) {
    if (Bstrcasecmp(ua->argk[i], "jobid")) {
      ...
      break;
    } else if (Bstrcasecmp(ua->argk[i], "jobname") || Bstrcasecmp(ua->argk[i], "job")) {
      if (!ua->argv[i]) {
        break;
      }
