...
static inline bool configure_create_resource_string(UaContext *ua, int first_parameter,
                                                    ResourceTable *res_table, PoolMem &resourcename,
                                                    PoolMem &resource) {
...
static inline bool ConfigureCreateFdResourceString(UaContext *ua, PoolMem &resource,
...
static inline bool ConfigureCreateFdResource(UaContext *ua, const char *clientname) {
  ...
  if (!my_config->GetPathOfNewResource(filename, temp, basedir.c_str(), "director", dirname,
 ...
 */
static inline bool ConfigureAddResource(UaContext *ua, int first_parameter,
  ...
  if (!configure_write_resource(filename_tmp.c_str(), res_table->name, name.c_str(),
                                resource.c_str())) {
...
bool ConfigureCmd(UaContext *ua, const char *cmd) {
  ...
  if (!(my_config->IsUsingConfigIncludeDir())) {
    ua->WarningMsg(
        _("It seems that the configuration is not adapted to the include directory structure. "
