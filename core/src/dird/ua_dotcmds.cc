...
static authorization_mapping authorization_mappings[] = {
    {"job", Job_ACL},           {"client", Client_ACL},   {"storage", Storage_ACL},
    {"schedule", Schedule_ACL}, {"pool", Pool_ACL},       {"cmd", Command_ACL},
    {"fileset", FileSet_ACL},   {"catalog", Catalog_ACL}, {NULL, 0},
};
  ...
  for (int i = 1; i < ua->argc; i++) {
    for (int j = 0; authorization_mappings[j].type; j++) {
      if (Bstrcasecmp(ua->argk[i], authorization_mappings[j].type)) {
        if (ua->argv[i] &&
            ua->AclAccessOk(authorization_mappings[j].acl_type, ua->argv[i], false)) {
          retval = true;
        } else {
          retval = false;
        }
...
static inline bool BvfsParseArgVersion(UaContext *ua, char **client, char **fname, bool *versions,
                                       bool *copies) {
  *fname = NULL;
  *client = NULL;
  *versions = false;
  *copies = false;
  ...
  for (int i = 1; i < ua->argc; i++) {
    if (Bstrcasecmp(ua->argk[i], NT_("name")) || Bstrcasecmp(ua->argk[i], NT_("fname")) ||
...
static bool BvfsParseArg(UaContext *ua, DBId_t *pathid, char **path, char **jobid, int *limit,
                         int *offset) {
  *pathid = 0;
  *limit = 2000;
  *offset = 0;
 ...
 */
static bool BvfsValidateJobids(UaContext *ua, const char *jobids, PoolMem &filtered_jobids,
                               bool audit_event) {
  int cnt = 0;
  char *cur_id, *bp;
...
/**
 * .bvfs_versions jobid=0 client=<client-name> filename=<file-name> pathid=<number> [copies]
 * [versions]
 *
 * jobid isn't used.
...
bool DotMediatypesCmd(UaContext *ua, const char *cmd) {
  ...
  if (!ua->db->SqlQuery("SELECT DISTINCT MediaType FROM MediaType ORDER BY MediaType", OneHandler,
                        (void *)ua)) {
...
bool DotMediaCmd(UaContext *ua, const char *cmd) {
  ...
  if (!ua->db->SqlQuery("SELECT DISTINCT Media.VolumeName FROM Media ORDER BY VolumeName",
                        OneHandler, (void *)ua)) {
...
bool DotLocationsCmd(UaContext *ua, const char *cmd) {
  ...
  if (!ua->db->SqlQuery("SELECT DISTINCT Location FROM Location ORDER BY Location", OneHandler,
                        (void *)ua)) {
...
bool DotDefaultsCmd(UaContext *ua, const char *cmd) {
  ...
  if ((pos = FindArgWithValue(ua, "job")) >= 0) {
    ...
    if (job) {
      ...
      ua->send->SendBuffer();
      ua->send->ObjectKeyValue("client", "%s=", ((job->client) ? job->client->name() : _("*None*")),
                               "%s\n");
      ua->send->SendBuffer();
      GetJobStorage(&store, job, NULL);
      ua->send->ObjectKeyValue("storage", "%s=", store.store->name(), "%s\n");
      ua->send->SendBuffer();
      ua->send->ObjectKeyValue("where", "%s=", (job->RestoreWhere ? job->RestoreWhere : ""),
                               "%s\n");
      ua->send->SendBuffer();
      ...
      ua->send->ObjectKeyValue(
          "fileset", "%s=", ((job->fileset) ? job->fileset->name() : _("*None*")), "%s\n");
      ua->send->SendBuffer();
      ua->send->ObjectKeyValue("enabled", "%s=", job->enabled, "%d\n");
      ua->send->SendBuffer();
      ua->send->ObjectKeyValue(
          "catalog", "%s=", ((job->client) ? job->client->catalog->name() : _("*None*")), "%s\n");
      ua->send->SendBuffer();
    }
    ...
    if (client) {
      ...
      ua->send->ObjectKeyValue("file_retention", "%s=", edit_uint64(client->FileRetention, ed1),
                               "%s\n");
      ua->send->ObjectKeyValue("job_retention", "%s=", edit_uint64(client->JobRetention, ed1),
                               "%s\n");
    ...
    if (pool) {
      ...
      ua->send->ObjectKeyValue("label_format",
                               "%s=", (pool->label_format ? pool->label_format : ""), "%s\n");
      ...
      ua->send->ObjectKeyValue("vol_retention", "%s=", edit_uint64(pool->VolRetention, ed1),
                               "%s\n");
      ua->send->ObjectKeyValue("vol_use_duration", "%s=", edit_uint64(pool->VolUseDuration, ed1),
                               "%s\n");
      ...
      ua->send->ObjectKeyValue("file_retention", "%s=", edit_uint64(pool->FileRetention, ed1),
                               "%s\n");
      ua->send->ObjectKeyValue("job_retention", "%s=", edit_uint64(pool->JobRetention, ed1),
                               "%s\n");
    }
  } else {
