 ...
 * the list.
 */
static inline slot_number_t count_enabled_slots(char *slot_list, slot_number_t max_slots) {

  slot_number_t i;
  slot_number_t cnt = 0;

  for (i = 0; i < max_slots; i++) {
    if (BitIsSet(i, slot_list)) {
      cnt++;
    }
  }
  return cnt;
 ...
 */
static inline void validate_slot_list(UaContext *ua, StorageResource *store,
                                      changer_vol_list_t *vol_list, char *slot_list,
        ...
        if (BitIsSet(vl->Slot - 1, slot_list)) {
          switch (content) {
            case slot_content_full:
               ...
               */
              if (vl->Type == slot_type_normal && vl->Content == slot_content_empty &&
                  vol_is_loaded_in_drive(store, vol_list, vl->Slot) != NULL) {
                continue;
              }
              Dmsg1(100, "Deselecting slot %hd doesn't have wanted content.\n", vl->Slot);
              ua->WarningMsg(_("Deselecting slot %hd doesn't have wanted content.\n"), vl->Slot);
 ...
 */
static inline vol_list_t *find_slot_in_list(changer_vol_list_t *vol_list, slot_number_t slotnr) {
 ...
 * it.
 */
static inline bool slot_lists_overlap(char *src_slot_list, char *dst_slot_list,
  ...
  for (i = 0; i < max_slots; i++) {
     ...
     */
    if (BitIsSet(i, src_slot_list) && BitIsSet(i, dst_slot_list) && !other_slot_enabled) {
 ...
 */
static inline changer_vol_list_t *scan_slots_for_volnames(UaContext *ua, StorageResource *store,
   ...
   */
  new_vol_list = get_vol_list_from_storage(
      ua, store, true /* listall */, true /* want to see all slots */, false /* non cached list */);
  if (!new_vol_list) {
    /*
      ...
      case slot_type_drive:
        switch (vl1->Content) {
          case slot_content_full:
             ...
             */
            vls.Index = vl1->Index;
            vl2 = (vol_list_t *)vol_list->contents->binary_search((void *)&vls,
                                                                  StorageCompareVolListEntry);
            if (vl2 && vl2->Content == slot_content_full && vl2->Loaded == vl1->Loaded) {
              /*
      ...
      case slot_type_import:
        ...
        switch (vl1->Content) {
          case slot_content_full:
            ...
            vl2 = (vol_list_t *)vol_list->contents->binary_search((void *)&vls,
                                                                  StorageCompareVolListEntry);
            if (vl2 && vl2->Content == slot_content_full && vl2->Slot == vl1->Slot) {
              /*
 ...
 */
static inline bool get_slot_list_using_volname(UaContext *ua, StorageResource *store,
                                               const char *volumename, changer_vol_list_t *vol_list,
                                               char *wanted_slot_list, char *selected_slot_list,
                                               slot_number_t max_slots) {
  vol_list_t *vl1, *vl2;
  bool found = false;
  ...
  if (IsNameValid(volumename)) {
          ...
          switch (vl1->Content) {
            case slot_content_full:
              ...
              Dmsg3(100, "Checking for volume name in slot %hd, wanted %s, found %s\n", vl1->Slot,
                    volumename, (vl1->VolName) ? vl1->VolName : "NULL");
    ...
    if (found) {
      ...
      ua->WarningMsg(_("No volume named %s in changer or in selected source slots.\n"), volumename);
    }
  } else {
 ...
 */
static inline slot_number_t get_slot_list_using_volnames(UaContext *ua, StorageResource *store,
                                                         int arg, changer_vol_list_t *vol_list,
 ...
 */
static inline slot_number_t auto_fill_slot_selection(StorageResource *store,
                                                     changer_vol_list_t *vol_list, char *slot_list,
                                                     slot_type type, slot_content content) {
  slot_number_t cnt = 0;
  vol_list_t *vl;
 ...
 */
static inline bool verify_slot_list(StorageResource *store, changer_vol_list_t *vol_list,
                                    char *slot_list, slot_type type, slot_content content) {
  vol_list_t *vl;
            ...
            switch (content) {
              case slot_content_empty:
                if (vol_is_loaded_in_drive(store, vol_list, vl->Slot) != NULL) {
                  Dmsg3(100,
                        "Skipping slot %hd, Type %hd, Content %hd is empty but loaded in drive\n",
                        vl->Slot, vl->Type, vl->Content);
                  return false;
                }
                break;
 ...
 */
static inline bool update_internal_slot_list(changer_vol_list_t *vol_list, slot_number_t source,
  ...
  if (vl1 && vl2) {
    ...
    Dmsg5(100,
          "Update internal slotlist slot %hd with volname %s, content %hd and slot %hd with "
          "content %hd and volname NULL\n",
 ...
 */
static bool release_loaded_volume(UaContext *ua, StorageResource *store, drive_number_t drive,
                                  changer_vol_list_t *vol_list) {
  bool found;
  vol_list_t *vl1, *vl2;
 ...
 */
static char *move_volumes_in_autochanger(UaContext *ua, enum e_move_op operation,
                                         StorageResource *store, changer_vol_list_t *vol_list,
                                         char *src_slot_list, char *dst_slot_list,
                                         slot_number_t max_slots) {
  vol_list_t *vl;
  ...
  for (transfer_from = 1; transfer_from <= max_slots; transfer_from++) {
    ...
    if (BitIsSet(transfer_from - 1, src_slot_list)) {
      ...
      while (transfer_to <= max_slots) {
        if (BitIsSet(transfer_to - 1, dst_slot_list)) {
          break;
        }
        transfer_to++;
      }
      ...
      if (transfer_to > max_slots) {
        ...
        ua->WarningMsg(_("Failed to find suitable destination slot in slot range.\n"));
        break;
      }
      ...
      if (transfer_volume(ua, store, transfer_from, transfer_to)) {
        ...
        Dmsg2(100, "Failed to move volume from source slot %hd to destination slot %hd\n",
              transfer_from, transfer_to);
        ua->WarningMsg(_("Failed to move volume from source slot %hd to destination slot %hd\n"),
                       transfer_from, transfer_to);
        switch (operation) {
          case VOLUME_EXPORT:
            /*
...
static bool PerformMoveOperation(UaContext *ua, enum e_move_op operation) {
  ...
  if (i >= 0) {
    ...
    if (!GetUserSlotList(ua, src_slot_list, "srcslots", max_slots)) {
       ...
       */
      if (scan) {
        vol_list = scan_slots_for_volnames(ua, store.store, drive, vol_list, src_slot_list);
        if (!vol_list) {
          goto bail_out;
        }
      }
    ...
    case VOLUME_EXPORT:
      ...
      if (i > 0) {
        ...
        nr_enabled_src_slots = get_slot_list_using_volnames(
            ua, store.store, i, vol_list, src_slot_list, tmp_slot_list, max_slots);
        if (src_slot_list) {
          free(src_slot_list);
        }
   ...
   */
  if (nr_enabled_src_slots && nr_enabled_dst_slots && nr_enabled_src_slots > nr_enabled_dst_slots) {
    ua->WarningMsg(_("Source slot selection doesn't fit into destination slot selection.\n"));
    goto bail_out;
  }
  ...
  if (nr_enabled_src_slots && nr_enabled_dst_slots &&
    ...
    ua->WarningMsg(_("Source slot selection and destination slot selection overlap.\n"));
    goto bail_out;
  }
  ...
  switch (operation) {
    case VOLUME_EXPORT:
      if (nr_enabled_src_slots == 0) {
        ua->WarningMsg(_("Cannot perform an export operation without source slot selection\n"));
        goto bail_out;
      }
      break;
    case VOLUME_MOVE:
      if (nr_enabled_src_slots == 0 || nr_enabled_dst_slots == 0) {
        ua->WarningMsg(
            _("Cannot perform a move operation without source and/or destination selection\n"));
        goto bail_out;
      }
      break;
    default:
  ...
  switch (operation) {
    case VOLUME_IMPORT:
      ...
      if (nr_enabled_src_slots == 0) {
        src_slot_list = (char *)malloc(NbytesForBits(max_slots));
        ClearAllBits(max_slots, src_slot_list);
        nr_enabled_src_slots = auto_fill_slot_selection(store.store, vol_list, src_slot_list,
                                                        slot_type_import, slot_content_full);
      } else {
        /*
         * All slots in the source selection need to be import/export slots and filled.
         */
        if (!verify_slot_list(store.store, vol_list, src_slot_list, slot_type_import,
                              slot_content_full)) {
          ua->WarningMsg(_("Not all slots in source selection are import slots and filled.\n"));
          goto bail_out;
        }
      }
      /*
      ...
      if (nr_enabled_dst_slots == 0) {
        dst_slot_list = (char *)malloc(NbytesForBits(max_slots));
        ClearAllBits(max_slots, dst_slot_list);
        nr_enabled_dst_slots = auto_fill_slot_selection(store.store, vol_list, dst_slot_list,
         ...
         */
        if (!verify_slot_list(store.store, vol_list, dst_slot_list, slot_type_normal,
                              slot_content_empty)) {
    ...
    case VOLUME_EXPORT:
       ...
       */
      if (!verify_slot_list(store.store, vol_list, src_slot_list, slot_type_normal,
                            slot_content_full)) {
      ...
      if (nr_enabled_dst_slots == 0) {
        ...
        if (nr_enabled_src_slots > nr_enabled_dst_slots) {
          ua->WarningMsg(_("Not enough free export slots available to export %hd volume%s\n"),
         ...
         */
        if (!verify_slot_list(store.store, vol_list, dst_slot_list, slot_type_import,
                              slot_content_empty)) {
          ua->WarningMsg(_("Not all slots in destination selection are export slots and empty.\n"));
          goto bail_out;
        }
      }
      visited_slot_list = move_volumes_in_autochanger(ua, operation, store.store, vol_list,
 ...
 */
bool ImportCmd(UaContext *ua, const char *cmd) { return PerformMoveOperation(ua, VOLUME_IMPORT); }

/**
 * Export volumes from normal slots to Import/Export Slots.
 */
bool ExportCmd(UaContext *ua, const char *cmd) { return PerformMoveOperation(ua, VOLUME_EXPORT); }

/**
 * Move volume from one slot to another.
 */
bool move_cmd(UaContext *ua, const char *cmd) { return PerformMoveOperation(ua, VOLUME_MOVE); }
