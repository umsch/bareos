...
static inline bool update_database(UaContext *ua, MediaDbRecord *mr, PoolDbRecord *pr,
 ...
 */
static inline bool native_send_label_request(UaContext *ua, MediaDbRecord *mr, MediaDbRecord *omr,
                                             PoolDbRecord *pr, bool relabel, drive_number_t drive) {
  BareosSocket *sd;
  ...
  if (relabel) {
    ...
    sd->fsend(
        ...
        dev_name, omr->VolumeName, mr->VolumeName, pr->Name, mr->MediaType, mr->Slot, drive,
        /*
    ...
    Dmsg8(100,
          "label %s VolumeName=%s PoolName=%s MediaType=%s "
          "Slot=%hd drive=%hd MinBlocksize=%d MaxBlocksize=%d\n",
          dev_name, mr->VolumeName, pr->Name, mr->MediaType, mr->Slot, drive, pr->MinBlocksize,
          pr->MaxBlocksize);
  }
...
bool SendLabelRequest(UaContext *ua, StorageResource *store, MediaDbRecord *mr, MediaDbRecord *omr,
                      PoolDbRecord *pr, bool media_record_exists, bool relabel,
...
static inline bool IsCleaningTape(UaContext *ua, MediaDbRecord *mr, PoolDbRecord *pr) {
  ...
  Dmsg4(100, "CLNprefix=%s: Vol=%s: len=%d bstrncmp=%s\n", ua->jcr->res.pool->cleaning_prefix,
        mr->VolumeName, strlen(ua->jcr->res.pool->cleaning_prefix), retval ? "true" : "false");
 ...
 */
static void label_from_barcodes(UaContext *ua, drive_number_t drive, bool label_encrypt, bool yes) {
...
static int do_label(UaContext *ua, const char *cmd, bool relabel) {
  ...
  checkVol:
    if (!bstrcmp(omr.VolStatus, "Purged") && !bstrcmp(omr.VolStatus, "Recycle")) {
      ua->ErrorMsg(
          _("Volume \"%s\" has VolStatus %s. It must be Purged or Recycled before relabeling.\n"),
  ...
  ok = SendLabelRequest(ua, store.store, &mr, &omr, &pr, media_record_exists, relabel, drive,
                        mr.Slot);
  if (ok) {
    ...
    if (relabel) {
      ...
      if (!ua->db->DeleteMediaRecord(ua->jcr, &omr)) {
        ua->ErrorMsg(_("Delete of Volume \"%s\" failed. ERR=%s"), omr.VolumeName,
                     ua->db->strerror());
      } else {
