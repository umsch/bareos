...
static void ShowDisabledClients(UaContext *ua) {
  ...
  if (first) {
    ua->SendMsg(_("No disabled Clients.\n"));
  }
}
...
static void ShowDisabledSchedules(UaContext *ua) {
  ...
  if (first) {
    ua->SendMsg(_("No disabled Schedules.\n"));
  }
}
          ...
          switch (j) {
            ...
            default:
              if (my_config->res_head_[j - my_config->r_first_]) {
                DumpResource(j, my_config->res_head_[j - my_config->r_first_], bsendmsg, ua,
...
bool LlistCmd(UaContext *ua, const char *cmd) { return DoListCmd(ua, cmd, VERT_LIST); }

/* Do short or summary listing */
bool list_cmd(UaContext *ua, const char *cmd) { return DoListCmd(ua, cmd, HORZ_LIST); }
...
static inline void SetHiddenColumn(UaContext *ua, int column) { ua->send->AddHiddenColumn(column); }
...
static bool DoListCmd(UaContext *ua, const char *cmd, e_list_type llist) {
  ...
  if ((Bstrcasecmp(ua->argk[1], NT_("jobs")) && (ua->argv[1] == NULL)) ||
      ((Bstrcasecmp(ua->argk[1], NT_("job")) || Bstrcasecmp(ua->argk[1], NT_("jobname"))) &&
       ua->argv[1])) {
    /*
    ...
    ua->db->ListJobRecords(ua->jcr, &jr, query_range.c_str(), clientname, jobstatus, joblevel,
                           volumename, schedtime, last, count, ua->send, llist);
  } else if (Bstrcasecmp(ua->argk[1], NT_("jobtotals"))) {
    /*
     * List JOBTOTALS
     */
    ua->db->ListJobTotals(ua->jcr, &jr, ua->send);
  } else if ((Bstrcasecmp(ua->argk[1], NT_("jobid")) || Bstrcasecmp(ua->argk[1], NT_("ujobid"))) &&
             ua->argv[1]) {
    /*
     * List JOBID=nn
     * List UJOBID=xxx
    ...
    if (jobid > 0) {
      ...
      ua->ErrorMsg(
          _("jobid not found in db, access to job or client denied by ACL, or client not found in "
            "db\n"));
    }
  ...
  } else if (Bstrcasecmp(ua->argk[1], NT_("pool")) || Bstrcasecmp(ua->argk[1], NT_("pools"))) {
    PoolDbRecord pr;
  ...
  } else if (Bstrcasecmp(ua->argk[1], NT_("media")) || Bstrcasecmp(ua->argk[1], NT_("volume")) ||
            ...
            for (i = 0; i < num_pools; i++) {
              ...
              if (ua->db->GetPoolRecord(ua->jcr, &pr)) {
                if (ua->AclAccessOk(Pool_ACL, pr.Name, false)) {
                  ...
                  mr.PoolId = ids[i];
                  ua->db->ListMediaRecords(ua->jcr, &mr, query_range.c_str(), count, ua->send,
                                           llist);
    ...
    if (i >= 0) {
      if (Is_a_number_list(ua->argv[i])) {
        ua->db->ListCopiesRecords(ua->jcr, query_range.c_str(), ua->argv[i], ua->send, llist);
      }
    } else {
...
static inline bool parse_jobstatus_selection_param(PoolMem &selection, UaContext *ua,
...
static inline bool parse_level_selection_param(PoolMem &selection, UaContext *ua,
                                               const char *default_selection) {
  int pos;
    ...
    while (cur_level) {
      ...
      for (int i = 0; joblevels[i].level_name; i++) {
        if (joblevels[i].job_type == JT_BACKUP &&
            bstrncasecmp(joblevels[i].level_name, cur_level, strlen(cur_level))) {
...
static inline bool parse_fileset_selection_param(PoolMem &selection, UaContext *ua, bool listall) {
...
static bool ParseListBackupsCmd(UaContext *ua, const char *range, e_list_type llist) {
  ...
  if (llist == VERT_LIST) {
    ua->db->FillQuery(ua->cmd, BareosDb::SQL_QUERY_list_jobs_long, selection.c_str(),
                      criteria.c_str());
  } else {
    ua->db->FillQuery(ua->cmd, BareosDb::SQL_QUERY_list_jobs, selection.c_str(), criteria.c_str());
  }
...
static bool ListNextvol(UaContext *ua, int ndays) {
  ...
  for (run = NULL; (run = find_next_run(run, job, runtime, ndays));) {
    ...
    if (!FindNextVolumeForAppend(jcr, &mr, 1, NULL, fnv_no_create_vol, fnv_prune)) {
      ua->ErrorMsg(_("Could not find next Volume for Job %s (Pool=%s, Level=%s).\n"), job->name(),
                   pr.Name, level_to_str(run->level));
    } else {
      ua->SendMsg(_("The next Volume to be used by Job \"%s\" (Pool=%s, Level=%s) will be %s\n"),
...
bool CompleteJcrForJob(JobControlRecord *jcr, JobResource *job, PoolResource *pool) {
  ...
  jcr->db = db_sql_get_pooled_connection(
      jcr, jcr->res.catalog->db_driver, jcr->res.catalog->db_name, jcr->res.catalog->db_user,
      jcr->res.catalog->db_password.value, jcr->res.catalog->db_address, jcr->res.catalog->db_port,
...
bool DotMessagesCmd(UaContext *ua, const char *cmd) {
  ...
  if (console_msg_pending && ua->AclNoRestrictions(Command_ACL) && ua->auto_display_messages) {
    DoMessages(ua, cmd);
  }
  return true;
}
...
of_filter_state filterit(void *ctx, void *data, of_filter_tuple *tuple) {
  ...
  switch (tuple->type) {
    ...
    case OF_FILTER_OFFSET:
      break;
    case OF_FILTER_ACL:
      if (!row[tuple->u.acl_filter.column] || strlen(row[tuple->u.acl_filter.column]) == 0) {
        retval = OF_FILTER_STATE_UNKNOWN;
      } else {
        if (!ua->AclAccessOk(tuple->u.acl_filter.acltype, row[tuple->u.acl_filter.column], false)) {
    ...
    case OF_FILTER_RESOURCE:
      if (!row[tuple->u.res_filter.column] || strlen(row[tuple->u.res_filter.column]) == 0) {
        retval = OF_FILTER_STATE_UNKNOWN;
      } else {
        if (!GetResWithName(tuple->u.res_filter.restype, row[tuple->u.res_filter.column], false)) {
