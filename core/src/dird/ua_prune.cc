...
bool PruneCmd(UaContext *ua, const char *cmd) {
  ...
  switch (kw) {
    ...
    case 1: { /* prune jobs */
       ...
       */
      if (!GetUserJobTypeSelection(ua, &jobtype)) {
        return false;
      }
      ...
      if (pool && pool->JobRetention > 0) {
        if (!ConfirmRetention(ua, &pool->JobRetention, "Job")) {
          return false;
        }
    ...
    case 2: /* prune volume */
      ...
      if (!SelectPoolAndMediaDbr(ua, &pr, &mr)) {
        return false;
      }
      ...
      if (!ConfirmRetention(ua, &mr.VolRetention, "Volume")) {
        return false;
      }
    ...
    case 3: /* prune stats */
      ...
      if (!ConfirmRetention(ua, &retention, "Statistics")) {
        return false;
      }
...
static bool PruneDirectory(UaContext *ua, ClientResource *client) {
  ...
  if (i >= 0) {
    ...
    if (recursive) {
      if (!GetCmd(ua, _("Please enter the full path prefix to remove: "), false)) {
        return false;
      }
    } else {
   ...
   */
  if (!client) {
    if (!GetYesno(ua, _("Cleanup orphaned path records (yes/no):")) || !ua->pint32_val) {
      retval = true;
      goto bail_out;
    }
 ...
 */
static bool prune_set_filter(UaContext *ua, ClientResource *client, PoolResource *pool,
                             utime_t period, PoolMem *add_from, PoolMem *add_where) {
  utime_t now;
...
bool PruneFiles(UaContext *ua, ClientResource *client, PoolResource *pool) {
  ...
  Mmsg(query, "SELECT COUNT(1) FROM Job %s WHERE PurgedFiles=0 %s", sql_from.c_str(),
       sql_where.c_str());
  ...
  if (cnt.count == 0) {
    if (ua->verbose) {
      ua->WarningMsg(_("No Files found to prune.\n"));
    }
    goto bail_out;
  }
  ...
  Mmsg(query, "SELECT JobId FROM Job %s WHERE PurgedFiles=0 %s", sql_from.c_str(),
       sql_where.c_str());
  Dmsg1(050, "select sql=%s\n", query.c_str());
...
static void DropTempTables(UaContext *ua) { ua->db->SqlQuery(BareosDb::SQL_QUERY_drop_deltabs); }
...
int GetPruneListForVolume(UaContext *ua, MediaDbRecord *mr, del_ctx *del) {
  ...
  now = (utime_t)time(NULL);
  ua->db->FillQuery(query, BareosDb::SQL_QUERY_sel_JobMedia, edit_int64(mr->MediaId, ed1),
