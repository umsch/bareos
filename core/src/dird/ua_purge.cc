...
bool PurgeCmd(UaContext *ua, const char *cmd) {
  ...
  static const char *keywords[] = {NT_("files"), NT_("jobs"), NT_("volume"), NT_("quota"), NULL};

  static const char *files_keywords[] = {NT_("Job"), NT_("JobId"), NT_("Client"), NT_("Volume"),
                                         NULL};
    ...
    case 2:
      ...
      while ((i = FindArg(ua, NT_("volume"))) >= 0) {
        ...
        *ua->argk[i] = 0; /* zap keyword already seen */
        ...
        /*
         * Add volume=mr.VolumeName to cmd_holder if we have a new volume name from interactive
         * selection. In certain cases this can produce duplicates, which we don't prevent as there
         * are no side effects.
         */
...
static bool PurgeFilesFromClient(UaContext *ua, ClientResource *client) {
  ...
  if (del.num_ids == 0) {
    ...
    ua->InfoMsg(_("Found Files in %d Jobs for client \"%s\" in catalog \"%s\".\n"), del.num_ids,
...
static bool PurgeQuotaFromClient(UaContext *ua, ClientResource *client) {
  ...
  if (!ua->db->CreateQuotaRecord(ua->jcr, &cr)) {
    return false;
  }
  if (!ua->db->ResetQuotaRecord(ua->jcr, &cr)) {
    return false;
  }
...
void UpgradeCopies(UaContext *ua, char *jobs) {
  ...
  ua->db->FillQuery(query, BareosDb::SQL_QUERY_uap_upgrade_copies_oldest_job, JT_JOB_COPY, jobs,
                    jobs);
...
void PurgeFilesFromVolume(UaContext *ua, MediaDbRecord *mr) {} /* ***FIXME*** implement */
...
bool IsVolumePurged(UaContext *ua, MediaDbRecord *mr, bool force) {
  ...
  cnt.count = 0;
  Mmsg(query, "SELECT 1 FROM JobMedia WHERE MediaId=%s LIMIT 1", edit_int64(mr->MediaId, ed1));
  ...
  if (cnt.count == 0) {
    ua->WarningMsg(_("There are no more Jobs associated with Volume \"%s\". Marking it purged.\n"),
                   mr->VolumeName);
 ...
 */
static void do_truncate_on_purge(UaContext *ua, MediaDbRecord *mr, char *pool, char *storage,
  ...
  sd->fsend(
      ...
      storage, mr->VolumeName, mr->VolumeName, pool, mr->MediaType, mr->Slot, drive,
      /*
...
static bool ActionOnPurgeCmd(UaContext *ua, const char *cmd) {
  ...
  for (int i = 1; i < ua->argc; i++) {
    if (Bstrcasecmp(ua->argk[i], NT_("allpools"))) {
      allpools = true;
    } else if (Bstrcasecmp(ua->argk[i], NT_("volume")) && IsNameValid(ua->argv[i])) {
    ...
    } else if (Bstrcasecmp(ua->argk[i], NT_("action")) && IsNameValid(ua->argv[i])) {
      action = ua->argv[i];
    }
  }
...
bool MarkMediaPurged(UaContext *ua, MediaDbRecord *mr) {
  ...
  if (status) {
    ...
    if (!ua->db->UpdateMediaRecord(jcr, mr)) {
      return false;
    }
    ...
    if (mr->RecyclePoolId && mr->RecyclePoolId != mr->PoolId) {
      ...
      if (ua->db->GetPoolRecord(jcr, &oldpr) && ua->db->GetPoolRecord(jcr, &newpr)) {
        /*
