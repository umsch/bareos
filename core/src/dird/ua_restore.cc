...
bool RestoreCmd(UaContext *ua, const char *cmd) {
  ...
  UnlockRes();
  if (!rx.restore_jobs) {
    ua->ErrorMsg(_("No Restore Job Resource found in %s.\n"
  ...
  Mmsg(ua->cmd,
       ...
       job->name(), rx.ClientName, rx.RestoreClientName, rx.store ? rx.store->name() : "",
       escaped_bsr_name ? escaped_bsr_name : jcr->RestoreBootstrap, rx.selected_files,
       ua->catalog->name());
  ...
  if (rx.RegexWhere) {
    ...
    Mmsg(buf, " regexwhere=\"%s\"", escaped_where_name ? escaped_where_name : rx.RegexWhere);

  } else if (rx.where) {
    escaped_where_name = escape_filename(rx.where);
    Mmsg(buf, " where=\"%s\"", escaped_where_name ? escaped_where_name : rx.where);
  }
  PmStrcat(ua->cmd, buf);
...
static int UserSelectJobidsOrFiles(UaContext *ua, RestoreContext *rx) {
  ...
  bool done = false;
  int i, j;
  const char *list[] = {_("List last 20 Jobs run"),
  ...
  for (; !done;) {
    ...
    switch (DoPrompt(ua, "", _("Select item: "), NULL, 0)) {
      ...
      case 11: /* Choose a jobid and select jobs */
        if (!GetCmd(ua, _("Enter JobId to get the state to restore: ")) || !IsAnInteger(ua->cmd)) {
          return 0;
        }
...
static bool InsertFileIntoFindexList(UaContext *ua, RestoreContext *rx, char *file, char *date) {
  ...
  if (*rx->JobIds == 0) {
    ua->db->FillQuery(rx->query, BareosDb::SQL_QUERY_uar_jobid_fileindex, date, rx->path, rx->fname,
                      rx->ClientName);
  } else {
    ua->db->FillQuery(rx->query, BareosDb::SQL_QUERY_uar_jobids_fileindex, rx->JobIds, date,
...
static bool InsertDirIntoFindexList(UaContext *ua, RestoreContext *rx, char *dir, char *date) {
  ...
  if (*rx->JobIds == 0) {
    ...
    ua->db->FillQuery(rx->query, BareosDb::SQL_QUERY_uar_jobid_fileindex_from_dir, rx->JobIds, dir,
                      rx->ClientName);
  }
...
static bool BuildDirectoryTree(UaContext *ua, RestoreContext *rx) {
  ...
  if (!ua->db->GetFileList(ua->jcr, rx->JobIds, false /* do not use md5 */, true /* get delta */,
                           InsertTreeHandler, (void *)&tree)) {
...
static bool SelectBackupsBeforeDate(UaContext *ua, RestoreContext *rx, char *date) {
  ...
  if (i < 0) { /* fileset not found */
    ua->db->FillQuery(rx->query, BareosDb::SQL_QUERY_uar_sel_fileset, edit_int64(cr.ClientId, ed1),
                      ed1);
   ...
   */
  if (pool_select[0]) {
    ua->db->FillQuery(rx->query, BareosDb::SQL_QUERY_uar_last_full, edit_int64(cr.ClientId, ed1),
      ...
      if (bstrcmp(MediaType, store->media_type)) {
        if (ua->AclAccessOk(Storage_ACL, store->name())) {
          ...
          if (Storage == NULL) {
            ...
            ua->WarningMsg(
                _("Storage \"%s\" not found, using Storage \"%s\" from MediaType \"%s\".\n"),
