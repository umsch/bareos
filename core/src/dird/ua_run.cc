  ...
  if (timeframe || since_jobid_given) {
    ...
    if (since_jobid_given) {
      if (until_jobid_given) {
        Mmsg(query,
             "SELECT JobId FROM Job WHERE JobStatus = 'f' AND JobId >= %s AND JobId <= %s ORDER BY "
             "JobId",
      ...
      Mmsg(query, "SELECT JobId FROM Job WHERE JobStatus = 'f' AND SchedTime > '%s' ORDER BY JobId",
           dt);
    }
    ...
    if (!yes && (!GetYesno(ua, _("rerun these jobids? (yes/no): ")) || !ua->pint32_val)) {
      goto bail_out;
    }
    /*
     * Job Query End
...
try_again:
  ...
  do {
    ...
    if (ua->cmd[0] == 0 || bstrncasecmp(ua->cmd, ".mod ", MIN(length, 5)) ||
        ...
        bstrncasecmp(ua->cmd, NT_("yes"), length) || bstrncasecmp(ua->cmd, _("yes"), length) ||
        bstrncasecmp(ua->cmd, NT_("no"), length) || bstrncasecmp(ua->cmd, _("no"), length)) {
      valid_response = true;
    }
...
bool RunCmd(UaContext *ua, const char *cmd) { return (DoRunCmd(ua, ua->cmd) != 0); }
...
int ModifyJobParameters(UaContext *ua, JobControlRecord *jcr, RunContext &rc) {
  ...
  if (ua->cmd[0] != 0 && bstrncasecmp(ua->cmd, _("mod"), strlen(ua->cmd))) {
    ...
    if (jcr->is_JobType(JT_BACKUP) || jcr->is_JobType(JT_COPY) || jcr->is_JobType(JT_MIGRATE) ||
    ...
    switch (DoPrompt(ua, "", _("Select parameter to modify"), NULL, 0)) {
      ...
      case 6:
        ...
        if (GetCmd(
                ua,
                _("Please enter desired start time as YYYY-MM-DD HH:MM:SS (return for now): "))) {
          if (ua->cmd[0] == 0) {
      ...
      case 12:
        ...
        if (jcr->RestoreBootstrap) {
          ua->SendMsg(
              _("You must set the bootstrap file to NULL to be able to specify a JobId.\n"));
        }
        goto try_again;
      case 13:
        /* Plugin Options */
...
try_again_reg:
  ua->SendMsg(_("strip_prefix=%s add_prefix=%s add_suffix=%s\n"), NPRT(strip_prefix),
              NPRT(add_prefix), NPRT(add_suffix));
    ...
    case JT_ADMIN:
      if (ua->api) {
        ...
        ua->SendMsg(
            ...
            "When:     %s\n"
            "Priority: %d\n",
            job->name(), jcr->res.fileset->name(), NPRT(jcr->res.client->name()),
      ...
      if (jcr->is_JobType(JT_BACKUP)) {
        ...
        if (ua->api) {
          ...
          ua->SendMsg(
              ...
              "Priority: %d\n"
              "%s%s%s",
              job->name(), level_to_str(jcr->getJobLevel()), jcr->res.client->name(),
              jcr->backup_format, jcr->res.fileset->name(), NPRT(jcr->res.pool->name()),
              is_virtual ? "NextPool: " : "",
              is_virtual ? (jcr->res.next_pool ? jcr->res.next_pool->name() : _("*None*")) : "",
              is_virtual ? "\n" : "", jcr->res.wstore ? jcr->res.wstore->name() : _("*None*"),
              bstrutime(dt, sizeof(dt), jcr->sched_time), jcr->JobPriority,
              jcr->plugin_options ? "Plugin Options: " : "",
              jcr->plugin_options ? jcr->plugin_options : "", jcr->plugin_options ? "\n" : "");
        } else {
          ua->SendMsg(
              _("Run Backup job\n"
                "JobName:  %s\n"
              ...
              is_virtual ? (jcr->res.next_pool ? jcr->res.next_pool->name() : _("*None*")) : "",
              is_virtual ? " (From " : "", is_virtual ? jcr->res.npool_source : "",
              is_virtual ? ")\n" : "", jcr->res.wstore ? jcr->res.wstore->name() : _("*None*"),
              jcr->res.wstore_source, bstrutime(dt, sizeof(dt), jcr->sched_time), jcr->JobPriority,
              jcr->plugin_options ? "Plugin Options: " : "",
              jcr->plugin_options ? jcr->plugin_options : "", jcr->plugin_options ? "\n" : "");
        }
      } else { /* JT_VERIFY */
        ...
        if (jcr->res.verify_job) {
          ...
          if (!ua->db->GetJobRecord(jcr, &jr)) {
            ua->ErrorMsg(_("Could not get job record for selected JobId. ERR=%s"),
                         ua->db->strerror());
            return false;
          }
          Name = jr.Job;
        ...
        if (ua->api) {
          ...
          ua->SendMsg(
              ...
              "Priority:    %d\n",
              job->name(), level_to_str(jcr->getJobLevel()), jcr->res.client->name(),
              jcr->res.fileset->name(), NPRT(jcr->res.pool->name()), jcr->res.pool_source,
              jcr->res.rstore->name(), jcr->res.rstore_source, Name, verify_list,
              bstrutime(dt, sizeof(dt), jcr->sched_time), jcr->JobPriority);
        } else {
        ...
        if (jcr->RegexWhere || (job->RegexWhere && !jcr->where)) {
          if (ua->api) {
            ...
            ua->SendMsg(
                ...
                jcr->res.fileset->name(), rc.client_name, jcr->res.client->name(),
                jcr->backup_format, jcr->res.rstore->name(),
                bstrutime(dt, sizeof(dt), jcr->sched_time), jcr->res.catalog->name(),
                jcr->JobPriority, NPRT(jcr->plugin_options));
          } else {
    ...
    case JT_MIGRATE:
      ...
      if (ua->api) {
        ...
        ua->SendMsg(
            ...
            jcr->res.rstore ? jcr->res.rstore->name() : _("*None*"), NPRT(jcr->res.pool->name()),
            jcr->res.next_pool ? jcr->res.next_pool->name() : _("*None*"),
            jcr->res.wstore ? jcr->res.wstore->name() : _("*None*"),
            (jcr->MigrateJobId == 0) ? _("*None*") : edit_uint64(jcr->MigrateJobId, ec1),
            bstrutime(dt, sizeof(dt), jcr->sched_time), jcr->res.catalog->name(), jcr->JobPriority);
      } else {
        if (jcr->is_JobType(JT_COPY)) {
        ...
        ua->SendMsg(_("%s"
                      ...
                      "Priority:      %d\n"),
                    prt_type, job->name(), NPRT(jcr->RestoreBootstrap),
                    jcr->res.rstore ? jcr->res.rstore->name() : _("*None*"), jcr->res.rstore_source,
                    NPRT(jcr->res.pool->name()), jcr->res.pool_source,
                    jcr->res.wstore ? jcr->res.wstore->name() : _("*None*"), jcr->res.wstore_source,
                    jcr->res.next_pool ? jcr->res.next_pool->name() : _("*None*"),
