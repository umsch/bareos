...
ClientResource *select_client_resource(UaContext *ua) {
  ...
  if (DoPrompt(ua, _("Client"), _("Select Client (File daemon) resource"), name, sizeof(name)) <
      0) {
    return NULL;
  }
...
bool GetClientDbr(UaContext *ua, ClientDbRecord *cr) {
  ...
  for (int i = 1; i < ua->argc; i++) {
    if ((Bstrcasecmp(ua->argk[i], NT_("client")) || Bstrcasecmp(ua->argk[i], NT_("fd"))) &&
      ...
      if (!ua->db->GetClientRecord(ua->jcr, cr)) {
        ua->ErrorMsg(_("Could not find Client \"%s\": ERR=%s"), ua->argv[i], ua->db->strerror());
        cr->ClientId = 0;
        break;
      }
      return true;
...
bool SelectClientDbr(UaContext *ua, ClientDbRecord *cr) {
  ...
  for (int i = 0; i < num_clients; i++) {
    ocr.ClientId = ids[i];
    if (!ua->db->GetClientRecord(ua->jcr, &ocr) || !ua->AclAccessOk(Client_ACL, ocr.Name)) {
      continue;
    }
...
bool GetStorageDbr(UaContext *ua, StorageDbRecord *sr, const char *argk) {
  ...
  if (sr->Name[0]) { /* If name already supplied */
    if (ua->db->GetStorageRecord(ua->jcr, sr) && ua->AclAccessOk(Pool_ACL, sr->Name)) {
      return true;
    }
...
bool GetPoolDbr(UaContext *ua, PoolDbRecord *pr, const char *argk) {
  ...
  if (pr->Name[0]) { /* If name already supplied */
    if (ua->db->GetPoolRecord(ua->jcr, pr) && ua->AclAccessOk(Pool_ACL, pr->Name)) {
      return true;
    }
...
bool SelectPoolDbr(UaContext *ua, PoolDbRecord *pr, const char *argk) {
  ...
  for (int i = 1; i < ua->argc; i++) {
    if (Bstrcasecmp(ua->argk[i], argk) && ua->argv[i] && ua->AclAccessOk(Pool_ACL, ua->argv[i])) {
  ...
  for (int i = 0; i < num_pools; i++) {
    opr.PoolId = ids[i];
    if (!ua->db->GetPoolRecord(ua->jcr, &opr) || !ua->AclAccessOk(Pool_ACL, opr.Name)) {
      continue;
    }
...
bool SelectStorageDbr(UaContext *ua, StorageDbRecord *sr, const char *argk) {
  ...
  for (int i = 0; i < num_storages; i++) {
    osr.StorageId = ids[i];
    if (!ua->db->GetStorageRecord(ua->jcr, &osr) || !ua->AclAccessOk(Storage_ACL, osr.Name)) {
      continue;
    }
...
int GetJobDbr(UaContext *ua, JobDbRecord *jr) {
  ...
  for (i = 1; i < ua->argc; i++) {
    ...
    if (!ua->db->GetJobRecord(ua->jcr, jr)) {
      ua->ErrorMsg(_("Could not find Job \"%s\": ERR=%s"), ua->argv[i], ua->db->strerror());
      jr->JobId = 0;
      break;
    }
    return jr->JobId;
  ...
  for (i = 1; i < ua->argc; i++) {
    if ((Bstrcasecmp(ua->argk[i], NT_("jobname")) || Bstrcasecmp(ua->argk[i], NT_("job"))) &&
        ua->argv[i]) {
      jr->JobId = 0;
...
int DoPrompt(UaContext *ua, const char *automsg, const char *msg, char *prompt, int max_prompt) {
  ...
  if (ua->batch) {
     ...
     */
    ua->SendMsg(_("Your request has multiple choices for \"%s\". Selection is not possible in "
                  "batch mode.\n"),
                automsg);
    item = -1;
    goto done;
...
StorageResource *get_storage_resource(UaContext *ua, bool use_default, bool autochangers_only) {
  ...
  for (i = 1; i < ua->argc; i++) {
    ...
    if (use_default && !ua->argv[i]) {
       ...
       */
      if (Bstrcasecmp("barcode", ua->argk[i]) || Bstrcasecmp("barcodes", ua->argk[i]) ||
          Bstrcasecmp("encrypt", ua->argk[i]) || Bstrcasecmp("scan", ua->argk[i]) ||
      ...
      if (Bstrcasecmp(ua->argk[i], NT_("storage")) || Bstrcasecmp(ua->argk[i], NT_("sd"))) {
        StoreName = ua->argv[i];
        break;
        ...
        FreeJcr(jcr);
        break;
      } else if (Bstrcasecmp(ua->argk[i], NT_("job")) || Bstrcasecmp(ua->argk[i], NT_("jobname"))) {
        if (!ua->argv[i]) {
...
int GetMediaType(UaContext *ua, char *MediaType, int max_media) {
  ...
  return (DoPrompt(ua, _("Media Type"), _("Select the Media Type"), MediaType, max_media) < 0) ? 0
                                                                                               : 1;
}
