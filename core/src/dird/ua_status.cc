...
void ListDirStatusHeader(UaContext *ua) {
  ...
  ua->SendMsg(_("Daemon started %s. Jobs: run=%d, running=%d mode=%d db=%s\n"), dt, num_jobs_run,
              JobCount(), (int)DEVELOPER_MODE, dbdrivers.c_str());
...
static bool show_scheduled_preview(UaContext *ua, ScheduleResource *sched, PoolMem &overview,
                                   int *max_date_len, struct tm tm, time_t time_to_check) {
  ...
  for (run = sched->run; run; run = run->next) {
    ...
    run_now = BitIsSet(hour, run->hour) && BitIsSet(mday, run->mday) && BitIsSet(wday, run->wday) &&
...
start_again:
  ...
  while (time_to_check < stop) {
    ...
    if (client || job) {
      ...
      if (job) {
        if (job->schedule) {
          if (!show_scheduled_preview(ua, job->schedule, overview, &max_date_len, tm,
                                      time_to_check)) {
            goto start_again;
          }
        }
      ...
      foreach_res(sched, R_SCHEDULE) {
        ...
        if (!show_scheduled_preview(ua, sched, overview, &max_date_len, tm, time_to_check)) {
          UnlockRes();
          goto start_again;
        }
      }
...
static void PrtRuntime(UaContext *ua, sched_pkt *sp) {
  ...
  if (ua->api) {
    ua->SendMsg(_("%-14s\t%-8s\t%3d\t%-18s\t%-18s\t%s\n"), level_ptr,
                job_type_to_str(sp->job->JobType), sp->priority, dt, sp->job->name(),
                mr.VolumeName);
  } else {
    ua->SendMsg(_("%-14s %-8s %3d  %-18s %-18s %s\n"), level_ptr, job_type_to_str(sp->job->JobType),
                sp->priority, dt, sp->job->name(), mr.VolumeName);
  }
  if (CloseDb) {
...
static void ListConnectedClients(UaContext *ua) {
  ...
  ua->send->Decoration("%-20s%-20s%-20s%-40s\n", "Connect time", "Protocol", "Authenticated",
                       "Name");
...
static void ContentSendInfoApi(UaContext *ua, char type, int Slot, char *vol_name) {
  ...
  if (ua->db->GetMediaRecord(ua->jcr, &mr)) {
    ...
    ua->SendMsg(slot_api_full_format, type, Slot, mr.Slot, mr.VolumeName,
                edit_uint64(mr.VolBytes, ed1), mr.VolStatus, mr.MediaType, pr.Name,
                edit_uint64(mr.LastWritten, ed2),
                edit_uint64(mr.LastWritten + mr.VolRetention, ed3));
  } else { /* Media unknown */
    ua->SendMsg(slot_api_full_format, type, Slot, 0, mr.VolumeName, "?", "?", "?", "?", "0", "0");
  }
}
...
static void StatusContentApi(UaContext *ua, StorageResource *store) {
  ...
  vol_list =
      get_vol_list_from_storage(ua, store, true /* listall */, true /* want to see all slots */);
  if (!vol_list) {
          ...
          case slot_content_full:
            ...
            if (vl1->Content == slot_content_full) {
              if (!vl1->VolName) {
                ...
                ua->SendMsg(slot_hformat, vl1->Slot, (vl1->Type == slot_type_import) ? '@' : '*',
                            "?", "?", "?", "?");
                continue;
              }
            ...
            if (mr.VolumeName[0] && ua->db->GetMediaRecord(ua->jcr, &mr)) {
              ...
              if (vl1->Type == slot_type_import) {
                ua->SendMsg(slot_hformat, vl1->Slot, '@', mr.VolumeName, mr.VolStatus, mr.MediaType,
                            pr.Name);
              } else {
                ua->SendMsg(slot_hformat, vl1->Slot,
                            ((vl1->Slot == mr.Slot) ? (vl2 ? '%' : ' ') : '*'), mr.VolumeName,
