...
int InsertTreeHandler(void *ctx, int num_fields, char **row) {
  ...
  Dmsg8(150,
        "node=0x%p JobId=%s FileIndex=%s Delta=%s node.delta=%d LinkFI=%d, fhinfo=%d, fhnode=%d\n",
  ...
  if (!node->inserted && JobId == node->JobId) {
    if ((hard_link && FileIndex > node->FileIndex) || (!hard_link && FileIndex < node->FileIndex)) {
      ok = false;
    }
  }
  if (ok) {
...
static int countcmd(UaContext *ua, TreeContext *tree) {
  ...
  ua->SendMsg(_("%s total files/dirs. %s marked to be restored.\n"),
              edit_uint64_with_commas(total, ec1), edit_uint64_with_commas(num_extract, ec2));
  return 1;
}
 ...
 */
static inline void ls_output(guid_list *guid, POOLMEM *&buf, const char *fname, const char *tag,
  ...
  if (dot_cmd) {
    ...
    Mmsg(buf, "%s,%d,%s,%s,%s,%s,%c,%s", mode_str, (uint32_t)statp->st_nlink,
         ...
         guid->gid_to_name(statp->st_gid, en2, sizeof(en2)), edit_int64(statp->st_size, ec1),
         time_str, *tag, fname);
  } else {
    time_t time;
    ...
    Mmsg(buf, "%s  %2d %-8.8s %-8.8s  %12.12s  %s %c%s", mode_str, (uint32_t)statp->st_nlink,
         guid->uid_to_name(statp->st_uid, en1, sizeof(en1)),
         guid->gid_to_name(statp->st_gid, en2, sizeof(en2)), edit_int64(statp->st_size, ec1),
         time_str, *tag, fname);
  }
...
int DotDircmd(UaContext *ua, TreeContext *tree) { return DoDircmd(ua, tree, true /*dot command*/); }
...
static int Estimatecmd(UaContext *ua, TreeContext *tree) {
  ...
  ua->SendMsg(_("%d total files; %d marked to be restored; %s bytes.\n"), total, num_extract,
              edit_uint64_with_commas(total_bytes, ec1));
  return 1;
}
