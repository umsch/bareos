...
static void UpdateVolstatus(UaContext *ua, const char *val, MediaDbRecord *mr) {
  ...
  const char *kw[] = {NT_("Append"),   NT_("Archive"), NT_("Disabled"),  NT_("Full"),  NT_("Used"),
                      NT_("Cleaning"), NT_("Recycle"), NT_("Read-Only"), NT_("Error"), NULL};
  bool found = false;
  int i;
...
static void UpdateVolmaxjobs(UaContext *ua, char *val, MediaDbRecord *mr) {
  ...
  Mmsg(query, "UPDATE Media SET MaxVolJobs=%s WHERE MediaId=%s", val, edit_int64(mr->MediaId, ed1));
  if (!ua->db->SqlQuery(query.c_str())) {
    ua->ErrorMsg("%s", ua->db->strerror());
  } else {
...
static void UpdateVolmaxbytes(UaContext *ua, char *val, MediaDbRecord *mr) {
  ...
  Mmsg(query, "UPDATE Media SET MaxVolBytes=%s WHERE MediaId=%s", edit_uint64(maxbytes, ed1),
...
static void UpdateVolinchanger(UaContext *ua, char *val, MediaDbRecord *mr) {
  ...
  Mmsg(query, "UPDATE Media SET InChanger=%d WHERE MediaId=%s", InChanger ? 1 : 0,
       edit_int64(mr->MediaId, ed1));
...
void UpdateVolPool(UaContext *ua, char *val, MediaDbRecord *mr, PoolDbRecord *opr) {
  ...
  DbLock(ua->db);
  Mmsg(query, "UPDATE Media SET PoolId=%s WHERE MediaId=%s", edit_int64(mr->PoolId, ed1),
       edit_int64(mr->MediaId, ed2));
  if (!ua->db->SqlQuery(query.c_str())) {
    ua->ErrorMsg("%s", ua->db->strerror());
  } else {
...
void UpdateVolStorage(UaContext *ua, char *val, MediaDbRecord *mr) {
  ...
  DbLock(ua->db);
  Mmsg(query, "UPDATE Media SET StorageId=%s WHERE MediaId=%s", edit_int64(mr->StorageId, ed1),
       edit_int64(mr->MediaId, ed2));
  if (!ua->db->SqlQuery(query.c_str())) {
    ua->ErrorMsg("%s", ua->db->strerror());
  }
...
static void UpdateVolFromPool(UaContext *ua, MediaDbRecord *mr) {
  ...
  memset(&pr, 0, sizeof(pr));
  pr.PoolId = mr->PoolId;
  if (!ua->db->GetPoolRecord(ua->jcr, &pr) || !ua->AclAccessOk(Pool_ACL, pr.Name, true)) {
    return;
  }
...
static void UpdateVolActiononpurge(UaContext *ua, char *val, MediaDbRecord *mr) {
  ...
  SetStorageidInMr(NULL, mr);
  if (!ua->db->UpdateMediaRecord(ua->jcr, mr)) {
    ua->ErrorMsg(_("Error updating media record ActionOnPurge: ERR=%s"), ua->db->strerror());
  } else {
...
static bool UpdateJob(UaContext *ua) {
  ...
  if (start_time) {
    ...
    Dmsg3(200, "ST=%lld jr.ST=%lld delta=%lld\n", StartTime, (utime_t)jr.StartTime, delta_start);
  ...
  Mmsg(cmd,
       ...
       jr.Name, edit_int64(jr.ClientId, ed1), jr.cStartTime, jr.cSchedTime, jr.cEndTime,
       edit_uint64(jr.JobTDate, ed2), edit_uint64(jr.FileSetId, ed3), jr.JobType,
 ...
 */
void UpdateSlotsFromVolList(UaContext *ua, StorageResource *store, changer_vol_list_t *vol_list,
                            char *slot_list) {
  vol_list_t *vl;
  MediaDbRecord mr;
 ...
 * The vol_list passed here needs to be from an "autochanger listall" cmd.
 */
void UpdateInchangerForExport(UaContext *ua, StorageResource *store, changer_vol_list_t *vol_list,
                              char *slot_list) {
  vol_list_t *vl;
  MediaDbRecord mr;
