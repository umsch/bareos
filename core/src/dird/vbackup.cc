...
bool DoNativeVbackupInit(JobControlRecord *jcr) {
  ...
  if (!SetMigrationWstorage(jcr, jcr->res.pool, jcr->res.next_pool, storage_source)) {
    return false;
  }
...
void NativeVbackupCleanup(JobControlRecord *jcr, int TermCode, int JobLevel) {
  ...
  switch (jcr->JobStatus) {
    ...
    case JS_Warnings:
      jcr->jr.JobLevel =
          JobLevel; /* We want this to appear as what the first consolidated job was */
  ...
  if (jcr->getJobStatus() == JS_Terminated && (jcr->JobErrors || jcr->SDErrors)) {
  ...
  Mmsg(query,
       ...
       edit_uint64(jcr->previous_jr.JobTDate, ec1), edit_uint64(jcr->JobId, ec2));
...
static bool CreateBootstrapFile(JobControlRecord *jcr, char *jobids) {
  ...
  if (!jcr->db_batch->GetFileList(jcr, jobids, false /* don't use md5 */, true /* use delta */,
