  ...
  switch (JobLevel) {
    ...
    case L_VERIFY_DISK_TO_CATALOG:
      ...
      if (jcr->res.verify_job &&
          (JobLevel == L_VERIFY_VOLUME_TO_CATALOG || JobLevel == L_VERIFY_DISK_TO_CATALOG)) {
      ...
      if (jcr->VerifyJobId) {
        ...
        if (!jcr->db->FindLastJobid(jcr, Name, &jr)) {
          if (JobLevel == L_VERIFY_CATALOG) {
            ...
            Jmsg(jcr, M_FATAL, 0, _("Unable to find JobId of previous Job for this client.\n"));
          }
          return false;
        }
...
void VerifyCleanup(JobControlRecord *jcr, int TermCode) {
  ...
  JobLevel = jcr->getJobLevel();
  Dmsg3(900, "JobLevel=%c Expected=%u JobFiles=%u\n", JobLevel, jcr->ExpectedFiles, jcr->JobFiles);
  if (JobLevel == L_VERIFY_VOLUME_TO_CATALOG && jcr->ExpectedFiles != jcr->JobFiles) {
  ...
  switch (TermCode) {
    ...
    default:
      TermMsg = term_code;
      Bsnprintf(term_code, sizeof(term_code), _("Inappropriate term code: %d %c\n"), TermCode,
                TermCode);
      break;
  }
    ...
    case L_VERIFY_VOLUME_TO_CATALOG:
      ...
      Jmsg(jcr, msg_type, 0,
           _("%s %s %s (%s):\n"
             ...
             "  Termination:            %s\n\n"),
           BAREOS, my_name, VERSION, LSMDATE, HOST_OS, DISTNAME, DISTVER, jcr->jr.JobId,
           jcr->jr.Job, jcr->res.fileset->hdr.name, level_to_str(JobLevel),
           jcr->res.client->hdr.name, jcr->previous_jr.JobId, Name, sdt, edt,
           edit_uint64_with_commas(jcr->ExpectedFiles, ec1),
           edit_uint64_with_commas(jcr->JobFiles, ec2), jcr->JobErrors, fd_term_msg, sd_term_msg,
           TermMsg);
      break;
    default:
      Jmsg(jcr, msg_type, 0,
           _("%s %s %s (%s):\n"
             ...
             "  Termination:            %s\n\n"),
           BAREOS, my_name, VERSION, LSMDATE, HOST_OS, DISTNAME, DISTVER, jcr->jr.JobId,
           jcr->jr.Job, jcr->res.fileset->hdr.name, level_to_str(JobLevel), jcr->res.client->name(),
           jcr->previous_jr.JobId, Name, sdt, edt, edit_uint64_with_commas(jcr->JobFiles, ec1),
           jcr->JobErrors, fd_term_msg, TermMsg);
      break;
  }
...
void GetAttributesAndCompareToCatalog(JobControlRecord *jcr, JobId_t JobId) {
  ...
  while ((n = BgetDirmsg(fd)) >= 0 && !JobCanceled(jcr)) {
    ...
    if ((len = sscanf(fd->msg, "%ld %d %100s", &file_index, &stream, fname)) != 3) {
      Jmsg3(jcr, M_FATAL, 0,
    ...
    *fn = *p++; /* term filename and point to attribs */
    ...
    switch (stream) {
      ...
      case STREAM_UNIX_ATTRIBUTES_EX:
         ...
         */
        fdbr.FileId = 0;
        if (!jcr->db->GetFileAttributesRecord(jcr, jcr->fname, &jcr->previous_jr, &fdbr)) {
