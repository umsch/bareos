...
BareosAccurateFilelistHtable::BareosAccurateFilelistHtable(JobControlRecord *jcr,
...
bool BareosAccurateFilelistHtable::AddFile(char *fname, int fname_length, char *lstat,
                                           int lstat_length, char *chksum, int chksum_length,
                                           int32_t delta_seq) {
  CurFile *item;
  int total_length;
  bool retval = true;
...
bool BareosAccurateFilelistHtable::SendDeletedList() {
  ...
  foreach_htable(elt, file_list_) {
    if (BitIsSet(elt->payload.filenr, seen_bitmap_) || PluginCheckFile(jcr_, elt->fname)) {
      continue;
    }
