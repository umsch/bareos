...
bool BareosAccurateFilelistLmdb::AddFile(char *fname, int fname_length, char *lstat,
                                         int lstat_length, char *chksum, int chksulength_,
                                         int32_t delta_seq) {
...
bool BareosAccurateFilelistLmdb::SendDeletedList() {
  ...
  if (result == 0) {
    while ((result = mdb_cursor_get(cursor, &key, &data, MDB_NEXT)) == 0) {
      ...
      if (BitIsSet(payload->filenr, seen_bitmap_) || PluginCheckFile(jcr_, (char *)key.mv_data)) {
        continue;
      }
