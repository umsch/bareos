...
static int send_data(JobControlRecord *jcr, int stream, FindFilesPacket *ff_pkt, DIGEST *digest,
...
static inline bool SaveRsrcAndFinder(b_save_ctx &bsctx) {
  ...
  if (bsctx.ff_pkt->hfsinfo.rsrclength > 0) {
    if (BopenRsrc(&bsctx.ff_pkt->bfd, bsctx.ff_pkt->fname, O_RDONLY | O_BINARY, 0) < 0) {
      ...
      berrno be;
      Jmsg(bsctx.jcr, M_NOTSAVED, -1, _("     Cannot open resource fork for \"%s\": ERR=%s.\n"),
      ...
      rsrc_stream =
          BitIsSet(FO_ENCRYPT, flags) ? STREAM_ENCRYPTED_MACOS_FORK_DATA : STREAM_MACOS_FORK_DATA;

      status = send_data(bsctx.jcr, rsrc_stream, bsctx.ff_pkt, bsctx.digest, bsctx.signing_digest);
...
int SaveFile(JobControlRecord *jcr, FindFilesPacket *ff_pkt, bool top_level) {
  ...
  switch (ff_pkt->type) {
    ...
    case FT_NOFSCHG:
      ...
      if (!IsInFileset(ff_pkt)) {
        Jmsg(jcr, M_INFO, 1,
             _("     %s is a different filesystem. Will not descend from %s into it.\n"),
...
#endif
    } else if (ff_pkt->type == FT_RAW || ff_pkt->type == FT_FIFO || ff_pkt->type == FT_REPARSE ||
  ...
  if (do_read) {
    ...
    ff_pkt->bfd.reparse_point = (ff_pkt->type == FT_REPARSE || ff_pkt->type == FT_JUNCTION);
  ...
  if (have_darwin_os) {
    ...
    if (ff_pkt->type != FT_LNKSAVED &&
        (S_ISREG(ff_pkt->statp.st_mode) && BitIsSet(FO_HFSPLUS, ff_pkt->flags))) {
...
static inline bool SendDataToSd(b_ctx *bctx) {
  ...
  if (BitIsSet(FO_COMPRESS, bctx->ff_pkt->flags)) {
    if (!CompressData(bctx->jcr, bctx->ff_pkt->Compress_algo, bctx->rbuf,
                      bctx->jcr->store_bsock->msglen, bctx->cbuf, bctx->max_compress_len,
                      &bctx->compress_len)) {
      return false;
    }
...
bool EncodeAndSendAttributes(JobControlRecord *jcr, FindFilesPacket *ff_pkt, int &data_stream) {
  ...
  switch (ff_pkt->type) {
    ...
    case FT_LNKSAVED:
      ...
      status =
          sd->fsend("%ld %d %s%c%s%c%s%c%s%c%u%c", jcr->JobFiles, ff_pkt->type, ff_pkt->fname, 0,
                    attribs.c_str(), 0, ff_pkt->link, 0, attribsEx, 0, ff_pkt->delta_seq, 0);
      break;
    case FT_DIREND:
    case FT_REPARSE:
      /* Here link is the canonical filename (i.e. with trailing slash) */
      status = sd->fsend("%ld %d %s%c%s%c%c%s%c%u%c", jcr->JobFiles, ff_pkt->type, ff_pkt->link, 0,
                         attribs.c_str(), 0, 0, attribsEx, 0, ff_pkt->delta_seq, 0);
      break;
    case FT_PLUGIN_CONFIG:
    case FT_RESTORE_FIRST:
      ...
      sd->msglen = Mmsg(sd->msg, "%d %d %d %d %d %d %s%c%s%c", jcr->JobFiles, ff_pkt->type,
      ...
      break;
    case FT_REG:
      status = sd->fsend("%ld %d %s%c%s%c%c%s%c%d%c", jcr->JobFiles, ff_pkt->type, ff_pkt->fname, 0,
                         attribs.c_str(), 0, 0, attribsEx, 0, ff_pkt->delta_seq, 0);
      break;
    default:
      status = sd->fsend("%ld %d %s%c%s%c%c%s%c%u%c", jcr->JobFiles, ff_pkt->type, ff_pkt->fname, 0,
                         attribs.c_str(), 0, 0, attribsEx, 0, ff_pkt->delta_seq, 0);
      break;
  }
