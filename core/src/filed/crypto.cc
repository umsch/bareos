...
bool CryptoSessionStart(JobControlRecord *jcr, crypto_cipher_t cipher) {
  ...
  if (jcr->crypto.pki_encrypt) {
    ...
    if (!jcr->crypto.pki_session) {
      Jmsg(jcr, M_FATAL, 0,
           _("Cannot create a new crypto session probably unsupported cipher configured.\n"));
      return false;
    }
     ...
     * Encode session data
     */
    if (!CryptoSessionEncode(jcr->crypto.pki_session, (uint8_t *)jcr->crypto.pki_session_encoded,
                             &size)) {
      Jmsg(jcr, M_FATAL, 0, _("An error occurred while encrypting the stream.\n"));
      return false;
    }
 ...
 */
bool FlushCipher(JobControlRecord *jcr, BareosWinFilePacket *bfd, uint64_t *addr, char *flags,
                 int32_t stream, RestoreCipherContext *cipher_ctx) {
...
again:
   ...
   */
  cipher_ctx->buf =
      CheckPoolMemorySize(cipher_ctx->buf, cipher_ctx->buf_len + cipher_ctx->block_size);
...
void DeallocateCipher(r_ctx &rctx) {
  ...
  if (rctx.cipher_ctx.cipher) {
    FlushCipher(rctx.jcr, &rctx.bfd, &rctx.fileAddr, rctx.flags, rctx.comp_stream,
                &rctx.cipher_ctx);
...
void DeallocateForkCipher(r_ctx &rctx) {
  ...
  if (rctx.fork_cipher_ctx.cipher) {
    FlushCipher(rctx.jcr, &rctx.forkbfd, &rctx.fork_addr, rctx.fork_flags, rctx.comp_stream,
...
bool SetupEncryptionContext(b_ctx &bctx) {
  ...
  if (BitIsSet(FO_ENCRYPT, bctx.ff_pkt->flags)) {
     ...
     */
    if ((bctx.cipher_ctx =
             crypto_cipher_new(bctx.jcr->crypto.pki_session, true, &cipher_block_size)) == NULL) {
      /*
       * Shouldn't happen!
       */
     ...
     */
    bctx.jcr->crypto.crypto_buf = CheckPoolMemorySize(
...
bool DecryptData(JobControlRecord *jcr, char **data, uint32_t *length,
   ...
   */
  cipher_ctx->buf =
      CheckPoolMemorySize(cipher_ctx->buf, cipher_ctx->buf_len + *length + cipher_ctx->block_size);

  /*
   * Decrypt the input block
   */
  if (!CryptoCipherUpdate(cipher_ctx->cipher, (const u_int8_t *)*data, *length,
                          (u_int8_t *)&cipher_ctx->buf[cipher_ctx->buf_len], &decrypted_len)) {
    /*
