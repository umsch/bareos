...
static BareosSocket *connect_to_director(JobControlRecord *jcr, DirectorResource *dir_res,
                                         bool verbose);
...
static bool ValidateCommand(JobControlRecord *jcr, const char *cmd, alist *allowed_job_cmds) {
  ...
  if (!allowed) {
    Jmsg(jcr, M_FATAL, 0,
         _("Illegal \"%s\" command not allowed by Allowed Job Cmds setting of this filed.\n"), cmd);
  }
...
bool StartConnectToDirectorThreads() {
  ...
  foreach_res(dir_res, R_DIRECTOR) {
    if (dir_res->conn_from_fd_to_dir) {
      if (!dir_res->address) {
        Emsg1(M_ERROR, 0,
              "Failed to connect to Director \"%s\". The address config directive is missing.\n",
              dir_res->name());
      } else if (!dir_res->port) {
        Emsg1(M_ERROR, 0,
              "Failed to connect to Director \"%s\". The port config directive is missing.\n",
              dir_res->name());
      } else {
        Dmsg3(120, "Connecting to Director \"%s\", address %s:%d.\n", dir_res->name(),
              dir_res->address, dir_res->port);
        thread = (pthread_t *)bmalloc(sizeof(pthread_t));
        if ((pthread_create_result = pthread_create(thread, NULL, handle_connection_to_director,
                                                    (void *)dir_res)) == 0) {
...
static bool SetdebugCmd(JobControlRecord *jcr) {
  ...
  if (scan == 4) {
    ...
    return dir->fsend(OKsetdebugv1, level, GetTrace(), GetHangup(), GetTimestamp(),
                      tracefilename.c_str());
  } else {
...
#else
  return dir->fsend(OKjob, VERSION, LSMDATE, os_version, DISTNAME, DISTVER, OBS_DISTRIBUTION,
                    OBS_ARCH);
#endif
}
...
static bool RunscriptCmd(JobControlRecord *jcr) {
   ...
   */
  if (sscanf(dir->msg, runscriptcmd, &on_success, &on_failure, &fail_on_error, &cmd->when, msg) !=
      5) {
    PmStrcpy(jcr->errmsg, dir->msg);
...
static bool RestoreObjectCmd(JobControlRecord *jcr) {
  ...
  *rop.plugin_name = 0;
  ...
  if (sscanf(dir->msg, restoreobjcmd, &rop.JobId, &rop.object_len, &rop.object_full_len,
             &rop.object_index, &rop.object_type, &rop.object_compression, &FileIndex,
             rop.plugin_name) != 8) {

    /*
     * Old version, no plugin_name
     */
    if (sscanf(dir->msg, restoreobjcmd1, &rop.JobId, &rop.object_len, &rop.object_full_len,
               &rop.object_index, &rop.object_type, &rop.object_compression, &FileIndex) != 7) {
  ...
  Dmsg7(100,
        ...
        "FI=%d plugin_name=%s\n",
        rop.JobId, rop.object_len, rop.object_full_len, rop.object_index, rop.object_type,
        FileIndex, rop.plugin_name);
...
static bool LevelCmd(JobControlRecord *jcr) {
  ...
  if (bstrcmp(level, "base")) {
    ...
    if (sscanf(dir->msg, "level = since_utime %s mtime_only=%d prev_job=%127s", buf, &mtime_only,
               jcr->PrevJob) != 3) {
      if (sscanf(dir->msg, "level = since_utime %s mtime_only=%d", buf, &mtime_only) != 2) {
        goto bail_out;
      }
    }
    ...
    /*
     * Sync clocks by polling him for the time. We take 10 samples of his time throwing out the
     * first two.
     */
    for (int i = 0; i < 10; i++) {
    ...
    if (adj > 3 || adj < -3) {
      ...
      Jmsg(jcr, type, 0,
           _("DIR and FD clocks differ by %lld seconds, FD automatically compensating.\n"), adj);
    }
    dir->signal(BNET_EOD);
...
static bool SessionCmd(JobControlRecord *jcr) {
  ...
  if (sscanf(dir->msg, sessioncmd, jcr->VolumeName, &jcr->VolSessionId, &jcr->VolSessionTime,
             &jcr->StartFile, &jcr->EndFile, &jcr->StartBlock, &jcr->EndBlock) != 7) {
    PmStrcpy(jcr->errmsg, dir->msg);
...
static bool StorageCmd(JobControlRecord *jcr) {
  ...
  sd_auth_key.check_size(dir->msglen);
  if (sscanf(dir->msg, storaddrv1cmd, stored_addr, &stored_port, &enable_ssl,
             sd_auth_key.c_str()) != 4) {
          ...
          switch (fo->Compress_algo) {
            ...
            default:
              ...
              Jmsg(jcr, M_WARNING, 0,
                   "%s compression support requested in fileset but not available on this "
                   "platform. Disabling ...\n",
  ...
  if (fileset) {
    for (int i = 0; i < fileset->include_list.size(); i++) {
      ...
      for (int j = 0; j < incexe->opts_list.size(); j++) {
        ...
        if (BitIsSet(FO_FORCE_ENCRYPT, fo->flags)) {
          force_encrypt = true;
        }
...
static bool BackupCmd(JobControlRecord *jcr) {
  ...
  if (!have_xattr) {
    ClearFlagInFileset(jcr, FO_XATTR,
                       _("XATTR support requested in fileset but not available on this platform. "
                         "Disabling ...\n"));
  }

  if (!have_encryption) {
    ClearFlagInFileset(jcr, FO_ENCRYPT,
                       _("Encryption support requested in fileset but not available on this "
                         "platform. Disabling ...\n"));
  }
...
static BareosSocket *connect_to_director(JobControlRecord *jcr, DirectorResource *dir_res,
  ...
  if (!dir->connect(jcr, retry_interval, max_retry_time, heart_beat, dir_res->name(),
                    dir_res->address, NULL, dir_res->port, verbose)) {
    delete dir;
    dir = NULL;
    return NULL;
  }
  ...
  *args = 0;
  ...
  if (jcr->pVSSClient) {
    if (!jcr->pVSSClient->InitializeForRestore(jcr)) {
      berrno be;
      Jmsg(jcr, M_WARNING, 0,
           _("VSS was not initialized properly. VSS support is disabled. ERR=%s\n"),
           be.bstrerror());
    }
...
static bool OpenSdReadSession(JobControlRecord *jcr) {
   ...
   */
  sd->fsend(read_open, "DummyVolume", jcr->VolSessionId, jcr->VolSessionTime, jcr->StartFile,
            jcr->EndFile, jcr->StartBlock, jcr->EndBlock);
  Dmsg1(110, ">stored: %s", sd->msg);
