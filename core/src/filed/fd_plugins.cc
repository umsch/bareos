 ...
 */
extern DLL_IMP_EXP int (*plugin_bopen)(BareosWinFilePacket *bfd, const char *fname, int flags,
                                       mode_t mode);
...
extern DLL_IMP_EXP boffset_t (*plugin_blseek)(BareosWinFilePacket *bfd, boffset_t offset,
                                              int whence);
...
static bRC bareosJobMsg(bpContext *ctx, const char *fname, int line, int type, utime_t mtime,
                        const char *fmt, ...);
static bRC bareosDebugMsg(bpContext *ctx, const char *fname, int line, int level, const char *fmt,
                          ...);
 ...
 */
static inline bool trigger_plugin_event(JobControlRecord *jcr, bEventType eventType, bEvent *event,
                                        bpContext *ctx, void *value, alist *plugin_ctx_list,
                                        int *index, bRC *rc)
...
/**
 * Create a plugin event When receiving bEventCancelCommand, this function is called by another
 * thread.
 */
...
int PluginSave(JobControlRecord *jcr, FindFilesPacket *ff_pkt, bool top_level) {
  ...
  foreach_alist(ctx, plugin_ctx_list) {
    ...
    /*
     * We put the current plugin pointer, and the plugin context into the jcr, because during
     * SaveFile(), the plugin will be called many times and these values are needed.
     */
    if (!IsEventEnabled(ctx, eventType)) {
      Dmsg1(debuglevel, "Event %d disabled for this plugin.\n", eventType);
      continue;
    ...
    while (!jcr->IsJobCanceled()) {
      ...
      if (IS_FT_OBJECT(sp.type)) {
        if (!sp.object_name) {
          Jmsg1(jcr, M_FATAL, 0,
                _("Command plugin \"%s\": no object_name in startBackupFile packet.\n"), cmd);
          goto bail_out;
        }
 ...
 */
int PluginCreateFile(JobControlRecord *jcr, Attributes *attr, BareosWinFilePacket *bfd,
                     int replace) {
  int flags;
  int retval;
  int status;
...
/**
 * Reset the file attributes after all file I/O is done -- this allows the previous access
 * time/dates to be set properly, and it also allows us to properly set directory permissions.
 */
 ...
 */
bacl_exit_code PluginBuildAclStreams(JobControlRecord *jcr, acl_data_t *acl_data,
      ...
      case bRC_OK:
        if (ap.content_length && ap.content) {
          acl_data->u.build->content =
              CheckPoolMemorySize(acl_data->u.build->content, ap.content_length);
...
/**
 * This entry point is called internally by Bareos to ensure that the plugin IO calls come into this
 * code.
 */
void LoadFdPlugins(const char *plugin_dir, alist *plugin_names) {
  ...
  if (!LoadPlugins((void *)&binfo, (void *)&bfuncs, fd_plugin_list, plugin_dir, plugin_names,
...
/**
 * Check if a plugin is compatible.  Called by the load_plugin function to allow us to verify the
 * plugin.
 */
static bool IsPluginCompatible(Plugin *plugin) {
  ...
  if (info->version != FD_PLUGIN_INTERFACE_VERSION) {
    ...
    Dmsg3(50, "Plugin version incorrect. Plugin=%s wanted=%d got=%d\n", plugin->file,
          FD_PLUGIN_INTERFACE_VERSION, info->version);
    return false;
  }
...
/**
 * Entry point for reading from the file this is a wrapper around the pluginIO entry point in the
 * plugin.
 */
static ssize_t MyPluginBread(BareosWinFilePacket *bfd, void *buf, size_t count) {
...
/**
 * Entry point for writing to the file this is a wrapper around the pluginIO entry point in the
 * plugin.
 */
static ssize_t MyPluginBwrite(BareosWinFilePacket *bfd, void *buf, size_t count) {
...
/**
 * Entry point for seeking in the file this is a wrapper around the pluginIO entry point in the
 * plugin.
 */
static boffset_t MyPluginBlseek(BareosWinFilePacket *bfd, boffset_t offset, int whence) {
...
static bRC bareosClearSeenBitmap(bpContext *ctx, bool all, char *fname) {
  ...
  if (all && fname) {
    Dmsg0(debuglevel,
          "fd-plugin: API error in call to ClearSeenBitmap, both all and fname set!!!\n");
    goto bail_out;
  }
...
int SaveFile(JobControlRecord *jcr, FindFilesPacket *ff_pkt, bool top_level) { return 0; }

bool AccurateMarkFileAsSeen(JobControlRecord *jcr, char *fname) { return true; }

bool SetCmdPlugin(BareosWinFilePacket *bfd, JobControlRecord *jcr) { return true; }
