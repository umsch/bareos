...
enum { IO_OPEN = 1, IO_READ = 2, IO_WRITE = 3, IO_CLOSE = 4, IO_SEEK = 5 };
...
bRC GeneratePluginEvent(JobControlRecord *jcr, bEventType event, void *value = NULL,
                        bool reverse = false);
...
typedef struct s_bareosFuncs {
  ...
  bRC (*JobMessage)(bpContext *ctx, const char *file, int line, int type, utime_t mtime,
                    const char *fmt, ...);
...
typedef enum { pVarName = 1, pVarDescription = 2 } pVariable;
