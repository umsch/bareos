...
static void append_file(JobControlRecord *jcr, findIncludeExcludeItem *incexe, const char *buf,
                        bool IsFile) {
  if (IsFile) {
    /*
    ...
    Jmsg(jcr, M_FATAL, 0, _("Plugin Directory not defined. Cannot use plugin: \"%s\"\n"), buf);
  }
}
...
bool TermFileset(JobControlRecord *jcr) {
  ...
  for (int i = 0; i < fileset->include_list.size(); i++) {
    ...
    foreach_dlist(node, &incexe->name_list) { Dmsg1(400, "F %s\n", node->c_str()); }

    foreach_dlist(node, &incexe->plugin_list) { Dmsg1(400, "P %s\n", node->c_str()); }
  }
    ...
    switch (*p) {
      ...
      case 'S':
        switch (*(p + 1)) {
          ...
          default:
            /*
             * If 2 or 3 is seen here, SHA2 is not configured, so eat the option, and drop back to
             * SHA-1.
             */
            if (p[1] == '2' || p[1] == '3') {
              p++;
