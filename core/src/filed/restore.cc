...
static int BcloseChksize(JobControlRecord *jcr, BareosWinFilePacket *bfd, boffset_t osize) {
  ...
  if (fsize > 0 && fsize != osize) {
    Qmsg3(jcr, M_WARNING, 0,
          _("Size of data or stream of %s not correct. Original %s, restored %s.\n"),
...
static inline void DropDelayedDataStreams(r_ctx &rctx, bool reuse) {
  ...
  if (!rctx.delayed_streams || rctx.delayed_streams->empty()) {
    return;
  }
 ...
 */
static inline bool do_reStoreAcl(JobControlRecord *jcr, int stream, char *content,
 ...
 * This can either be a delayed restore or direct restore.
 */
static inline bool do_restore_xattr(JobControlRecord *jcr, int stream, char *content,
                                    uint32_t content_length) {
...
void DoRestore(JobControlRecord *jcr) {
  ...
  crypto_digest_t signing_algorithm = have_sha2 ? CRYPTO_DIGEST_SHA256 : CRYPTO_DIGEST_SHA1;
  /*
  ...
  while (BgetMsg(sd) >= 0 && !JobCanceled(jcr)) {
     ...
     */
    if (sscanf(sd->msg, rec_header, &VolSessionId, &VolSessionTime, &file_index, &rctx.full_stream,
               &rctx.size) != 5) {
    ...
    Dmsg5(150, "Got hdr: Files=%d FilInx=%d size=%d Stream=%d, %s.\n", jcr->JobFiles, file_index,
          rctx.size, rctx.stream, stream_to_ascii(rctx.stream));
    ...
    switch (rctx.stream) {
      ...
      case STREAM_ENCRYPTED_SESSION_DATA:
        if (rctx.extract) {
          ...
          if (!jcr->crypto.pki_recipients) {
            Jmsg(jcr, M_ERROR, 0,
                 _("No private decryption keys have been defined to decrypt encrypted backup "
                   "data.\n"));
            rctx.extract = false;
            bclose(&rctx.bfd);
            break;
          ...
          switch (cryptoerr) {
            ...
            case CRYPTO_ERROR_NORECIPIENT:
              Jmsg(jcr, M_ERROR, 0,
                   _("Missing private key required to decrypt encrypted backup data.\n"));
              break;
            ...
            default:
               ...
               */
              Jmsg1(jcr, M_ERROR, 0,
                    _("An error occurred while decoding encrypted session data stream: %s\n"),
      ...
      case STREAM_ENCRYPTED_WIN32_COMPRESSED_DATA:
        if (rctx.extract) {
          ...
          if (process_data) {
            ...
            if (ExtractData(jcr, &rctx.bfd, sd->msg, sd->msglen, &rctx.fileAddr, rctx.flags,
                            rctx.stream, &rctx.cipher_ctx) < 0) {
              rctx.extract = false;
              bclose(&rctx.bfd);
              continue;
            }
      ...
      case STREAM_MACOS_FORK_DATA:
        if (have_darwin_os) {
          ...
          if (rctx.extract) {
            ...
            if (ExtractData(jcr, &rctx.forkbfd, sd->msg, sd->msglen, &rctx.fork_addr,
                            rctx.fork_flags, rctx.stream, &rctx.fork_cipher_ctx) < 0) {
              rctx.extract = false;
              bclose(&rctx.forkbfd);
              continue;
            }
      ...
      case STREAM_SIGNED_DIGEST:
         ...
         * Save signature.
         */
        if (rctx.extract && (rctx.sig = crypto_sign_decode(jcr, (uint8_t *)sd->msg,
                                                           (uint32_t)sd->msglen)) == NULL) {
...
ok_out:
   ...
   */
  Dmsg2(10, "End Do Restore. Files=%d Bytes=%s\n", jcr->JobFiles, edit_uint64(jcr->JobBytes, ec1));
  ...
  if (non_support_data > 1 || non_support_attr > 1) {
    Jmsg(jcr, M_WARNING, 0,
         _("%d non-supported data streams and %d non-supported attrib streams ignored.\n"),
...
bool SparseData(JobControlRecord *jcr, BareosWinFilePacket *bfd, uint64_t *addr, char **data,
                uint32_t *length) {
  unser_declare;
  uint64_t faddr;
  char ec1[50];
...
bool StoreData(JobControlRecord *jcr, BareosWinFilePacket *bfd, char *data, const int32_t length,
               bool win32_decomp) {
  if (jcr->crypto.digest) {
