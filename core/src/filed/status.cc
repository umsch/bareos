  ...
  foreach_jcr(njcr) {
    ...
    if (njcr->JobId > 0) {
      ...
      len = Mmsg(msg, _("    %s%s %s Job started: %s\n"),
                 ...
                 level_to_str(njcr->getJobLevel()), job_type_to_str(njcr->getJobType()), dt);
#else
    ...
    len = Mmsg(msg,
               ...
               edit_uint64_with_commas(njcr->JobBytes, b2), edit_uint64_with_commas(bps, b3),
    ...
    found = true;
    if (njcr->store_bsock) {
      len = Mmsg(msg, "    SDReadSeqNo=%" lld " fd=%d\n", njcr->store_bsock->read_seqno,
                 njcr->store_bsock->fd_);
      sendit(msg, len, sp);
    } else {
...
static void ListRunningJobsApi(StatusPacket *sp) {
  ...
  foreach_jcr(njcr) {
    ...
    len = Mmsg(msg,
               ...
               edit_uint64(bps, b3), njcr->JobErrors, edit_int64(njcr->max_bandwidth, b4));
    sendit(msg, len, sp);
