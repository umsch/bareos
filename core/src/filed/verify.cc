...
static int VerifyFile(JobControlRecord *jcr, FindFilesPacket *ff_pkt, bool top_level) {
  ...
  if (ff_pkt->type == FT_LNK || ff_pkt->type == FT_LNKSAVED) {
    status = dir->fsend("%d %d %s %s%c%s%c%s%c", jcr->JobFiles, STREAM_UNIX_ATTRIBUTES,
                        ff_pkt->VerifyOpts, ff_pkt->fname, 0, attribs.c_str(), 0, ff_pkt->link, 0);
  ...
  if (ff_pkt->type != FT_LNKSAVED && S_ISREG(ff_pkt->statp.st_mode) &&
      (BitIsSet(FO_MD5, ff_pkt->flags) || BitIsSet(FO_SHA1, ff_pkt->flags) ||
       BitIsSet(FO_SHA256, ff_pkt->flags) || BitIsSet(FO_SHA512, ff_pkt->flags))) {
    ...
    if (calculate_file_chksum(jcr, ff_pkt, &digest, &digest_stream, &digest_buf, &digest_name)) {
      ...
      if (digest_stream != STREAM_NONE && digest == NULL) {
        ...
        dir->fsend("%d %d %s *%s-%d*", jcr->JobFiles, digest_stream, digest_buf, digest_name,
                   jcr->JobFiles);
