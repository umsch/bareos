...
void DoVerifyVolume(JobControlRecord *jcr) {
  ...
  while (BgetMsg(sd) >= 0 && !JobCanceled(jcr)) {
     ...
     */
    if (sscanf(sd->msg, rec_header, &VolSessionId, &VolSessionTime, &file_index, &stream, &size) !=
        5) {
    ...
    switch (stream) {
      ...
      case STREAM_UNIX_ATTRIBUTES_EX:
        ...
        *fp = *ap++; /* Terminate filename & point to attribs */
        ...
        if (type == FT_LNK || type == FT_LNKSAVED) {
          status = dir->fsend("%d %d %s %s%c%s%c%s%c", jcr->JobFiles, STREAM_UNIX_ATTRIBUTES,
                              "pinsug5", fname, 0, ap, 0, lname, 0);
          /* for a deleted record, we set fileindex=0 */
        } else if (type == FT_DELETED) {
          status = dir->fsend("%d %d %s %s%c%s%c%c", 0, STREAM_UNIX_ATTRIBUTES, "pinsug5", fname, 0,
                              ap, 0, 0);
        } else {
          status = dir->fsend("%d %d %s %s%c%s%c%c", jcr->JobFiles, STREAM_UNIX_ATTRIBUTES,
      ...
      case STREAM_MD5_DIGEST:
        ...
        dir->fsend("%d %d %s *MD5-%d*", jcr->JobFiles, STREAM_MD5_DIGEST, digest, jcr->JobFiles);
      ...
      case STREAM_SHA1_DIGEST:
        ...
        dir->fsend("%d %d %s *SHA1-%d*", jcr->JobFiles, STREAM_SHA1_DIGEST, digest, jcr->JobFiles);
      ...
      case STREAM_SHA256_DIGEST:
        ...
        dir->fsend("%d %d %s *SHA256-%d*", jcr->JobFiles, STREAM_SHA256_DIGEST, digest,
                   jcr->JobFiles);
      ...
      case STREAM_SHA512_DIGEST:
        ...
        dir->fsend("%d %d %s *SHA512-%d*", jcr->JobFiles, STREAM_SHA512_DIGEST, digest,
                   jcr->JobFiles);
      ...
      case STREAM_RESTORE_OBJECT:
        ...
        dir->fsend("%d %d %s %s%c%c%c", jcr->JobFiles, STREAM_RESTORE_OBJECT, "ReStOrEObJeCt",
                   fname, 0, 0, 0);
        break;
