 ...
 */
bacl_exit_code BuildAclStreams(JobControlRecord *jcr, acl_data_t *acl_data,
                               FindFilesPacket *ff_pkt) {
  return bacl_exit_fatal;
}

bacl_exit_code parse_acl_streams(JobControlRecord *jcr, acl_data_t *acl_data, int stream,
...
static bool AclIsTrivial(struct acl *acl) { return (acl_last(acl) != acl->acl_ext ? false : true); }
...
static bool acl_nfs4_is_trivial(nfs4_acl_int_t *acl) {
  ...
  for (i = 0; i < count; i++) {
    ...
    if (!((ace->flags & ACE4_ID_SPECIAL) != 0 &&
          ...
          (ace->aceMask & ~(ACE4_READ_DATA | ACE4_LIST_DIRECTORY | ACE4_WRITE_DATA | ACE4_ADD_FILE |
                            ACE4_EXECUTE)) == 0)) {
      return false;
    }
  }
...
static bacl_exit_code aix_build_acl_streams(JobControlRecord *jcr, acl_data_t *acl_data,
                                            FindFilesPacket *ff_pkt) {
  mode_t mode;
  acl_type_t type;
  ...
  if (aclx_printStr(acl_data->u.build->content, &acltxtsize, aclbuf, aclsize, type,
                    acl_data->last_fname, 0) < 0) {
    switch (errno) {
      case ENOSPC:
        /*
 ...
 */
static inline bool aix_query_acl_support(JobControlRecord *jcr, acl_data_t *acl_data,
                                         uint64_t aclType, acl_type_t *pacl_type_info) {
  unsigned int i;
...
static bacl_exit_code aix_parse_acl_streams(JobControlRecord *jcr, acl_data_t *acl_data, int stream,
                                            char *content, uint32_t content_length) {
  int cnt;
  acl_type_t type;
  size_t aclsize;
  ...
  switch (stream) {
    ...
    case STREAM_ACL_AIX_AIXC:
      if (!aix_query_acl_support(jcr, acl_data, ACL_AIXC, &type)) {
        Mmsg1(jcr->errmsg,
              _("Trying to restore POSIX acl on file \"%s\" on filesystem without AIXC acl "
                "support\n"),
              acl_data->last_fname);
        goto bail_out;
      }
    ...
    case STREAM_ACL_AIX_NFS4:
      if (!aix_query_acl_support(jcr, acl_data, ACL_NFS4, &type)) {
        Mmsg1(jcr->errmsg,
              _("Trying to restore NFSv4 acl on file \"%s\" on filesystem without NFS4 acl "
                "support\n"),
              acl_data->last_fname);
        goto bail_out;
      }
...
static bacl_exit_code aix_build_acl_streams(JobControlRecord *jcr, acl_data_t *acl_data,
  ...
  if ((acl_text = acl_get(acl_data->last_fname)) != NULL) {
    acl_data->u.build->content_length = PmStrcpy(acl_data->u.build->content, acl_text);
 ...
 */
static bacl_exit_code (*os_build_acl_streams)(JobControlRecord *jcr, acl_data_t *acl_data,
                                              FindFilesPacket *ff_pkt) = aix_build_acl_streams;
static bacl_exit_code (*os_parse_acl_streams)(JobControlRecord *jcr, acl_data_t *acl_data,
                                              int stream, char *content,
                                              uint32_t content_length) = aix_parse_acl_streams;

#elif defined(HAVE_DARWIN_OS) || defined(HAVE_FREEBSD_OS) || defined(HAVE_IRIX_OS) || \
    defined(HAVE_OSF1_OS) || defined(HAVE_LINUX_OS) || defined(HAVE_HURD_OS)
...
static bool AclIsTrivial(acl_t acl) {
  ...
  while (entry_available == 1) {
     ...
     */
    if (tag != ACL_USER_OBJ && tag != ACL_GROUP_OBJ && tag != ACL_OTHER) return false;
    entry_available = acl_get_entry(acl, ACL_NEXT_ENTRY, &ace);
  }
  return true;
#elif defined(HAVE_IRIX_OS)
  ...
  for (n = 0; n < acl->acl_cnt; n++) {
     ...
     * Anything other the ACL_USER_OBJ, ACL_GROUP_OBJ or ACL_OTHER breaks the spell.
     */
    if (tag != ACL_USER_OBJ && tag != ACL_GROUP_OBJ && tag != ACL_OTHER_OBJ) return false;
  }
  return true;
#elif defined(HAVE_OSF1_OS)
  int count;
  ...
  while (count > 0) {
     ...
     */
    if ((ace->entry->acl_perm & ~(ACL_READ | ACL_WRITE | ACL_EXECUTE))) return false;
    ace = ace->next;
    count--;
  }
  return true;
 ...
 */
static bacl_exit_code generic_get_acl_from_os(JobControlRecord *jcr, acl_data_t *acl_data,
                                              bacl_type acltype) {
  acl_t acl;
  acl_type_t ostype;
  char *acl_text;
 ...
 */
static bacl_exit_code generic_set_acl_on_os(JobControlRecord *jcr, acl_data_t *acl_data,
                                            bacl_type acltype, char *content,
...
static bacl_exit_code darwin_build_acl_streams(JobControlRecord *jcr, acl_data_t *acl_data,
                                               FindFilesPacket *ff_pkt) {
...
static bacl_exit_code darwin_parse_acl_streams(JobControlRecord *jcr, acl_data_t *acl_data,
                                               int stream, char *content, uint32_t content_length) {
#if defined(HAVE_ACL_TYPE_EXTENDED)
  return generic_set_acl_on_os(jcr, acl_data, BACL_TYPE_EXTENDED, content, content_length);
#else
  return generic_set_acl_on_os(jcr, acl_data, BACL_TYPE_ACCESS, content, content_length);
#endif
}
 ...
 */
static bacl_exit_code (*os_build_acl_streams)(JobControlRecord *jcr, acl_data_t *acl_data,
                                              FindFilesPacket *ff_pkt) = darwin_build_acl_streams;
static bacl_exit_code (*os_parse_acl_streams)(JobControlRecord *jcr, acl_data_t *acl_data,
                                              int stream, char *content,
                                              uint32_t content_length) = darwin_parse_acl_streams;
...
static bacl_exit_code freebsd_build_acl_streams(JobControlRecord *jcr, acl_data_t *acl_data,
                                                FindFilesPacket *ff_pkt) {
  int acl_enabled = 0;
...
static bacl_exit_code freebsd_parse_acl_streams(JobControlRecord *jcr, acl_data_t *acl_data,
                                                int stream, char *content,
                                                uint32_t content_length) {
  int acl_enabled = 0;
  ...
  switch (stream) {
    ...
    case STREAM_UNIX_DEFAULT_ACL:
    case STREAM_ACL_FREEBSD_DEFAULT_ACL:
      return generic_set_acl_on_os(jcr, acl_data, BACL_TYPE_DEFAULT, content, content_length);
    case STREAM_ACL_FREEBSD_NFS4_ACL:
      return generic_set_acl_on_os(jcr, acl_data, BACL_TYPE_NFS4, content, content_length);
    default:
      break;
  }
  return bacl_exit_error;
 ...
 */
static bacl_exit_code (*os_build_acl_streams)(JobControlRecord *jcr, acl_data_t *acl_data,
                                              FindFilesPacket *ff_pkt) = freebsd_build_acl_streams;
static bacl_exit_code (*os_parse_acl_streams)(JobControlRecord *jcr, acl_data_t *acl_data,
                                              int stream, char *content,
                                              uint32_t content_length) = freebsd_parse_acl_streams;
...
static bacl_exit_code generic_build_acl_streams(JobControlRecord *jcr, acl_data_t *acl_data,
                                                FindFilesPacket *ff_pkt) {
  /*
   * Read access ACLs for files, dirs and links
   */
...
static bacl_exit_code generic_parse_acl_streams(JobControlRecord *jcr, acl_data_t *acl_data,
                                                int stream, char *content,
                                                uint32_t content_length) {
  unsigned int cnt;
 ...
 */
static bacl_exit_code (*os_build_acl_streams)(JobControlRecord *jcr, acl_data_t *acl_data,
                                              FindFilesPacket *ff_pkt) = generic_build_acl_streams;
static bacl_exit_code (*os_parse_acl_streams)(JobControlRecord *jcr, acl_data_t *acl_data,
                                              int stream, char *content,
                                              uint32_t content_length) = generic_parse_acl_streams;
...
static bacl_exit_code tru64_build_acl_streams(JobControlRecord *jcr, acl_data_t *acl_data,
                                              FindFilesPacket *ff_pkt) {
  /*
   * Read access ACLs for files, dirs and links
   */
  if (generic_get_acl_from_os(jcr, acl_data, BACL_TYPE_ACCESS) == bacl_exit_fatal) {
    return bacl_exit_error;
    if (acl_data->u.build->content_length > 0) {
      if (!SendAclStream(jcr, acl_data, STREAM_ACL_TRU64_ACCESS_ACL)) return bacl_exit_error;
    }
    /*
     * Directories can have default ACLs too
     */
    if (acl_data->filetype == FT_DIREND) {
      if (generic_get_acl_from_os(jcr, acl_data, BACL_TYPE_DEFAULT) == bacl_exit_fatal) {
        return bacl_exit_error;
        if (acl_data->u.build->content_length > 0) {
          if (!SendAclStream(jcr, acl_data, STREAM_ACL_TRU64_DEFAULT_ACL)) return bacl_exit_error;
        }
        /**
      ...
      static bacl_exit_code tru64_parse_acl_streams(JobControlRecord * jcr, acl_data_t * acl_data,
                                                    int stream, char *content,
                                                    uint32_t content_length) {
        switch (stream) {
          case STREAM_UNIX_ACCESS_ACL:
          ...
          case STREAM_ACL_TRU64_DEFAULT_DIR_ACL:
            return generic_set_acl_on_os(jcr, acl_data, BACL_TYPE_DEFAULT_DIR, content,
                                         content_length);
        }
        ...
        static bacl_exit_code (*os_parse_acl_streams)(
            JobControlRecord * jcr, acl_data_t * acl_data, int stream, char *content,
            uint32_t content_length) = tru64_parse_acl_streams;
...
static bacl_exit_code hpux_build_acl_streams(JobControlRecord *jcr,
                                             acl_data_t *acl_data FindFilesPacket *ff_pkt) {
  int n;
...
static bacl_exit_code hpux_parse_acl_streams(JobControlRecord *jcr, acl_data_t *acl_data,
                                             int stream, char *content, uint32_t content_length) {
  int n;
  struct acl_entry acls[NACLENTRIES];
 ...
 */
static bacl_exit_code (*os_build_acl_streams)(JobControlRecord *jcr, acl_data_t *acl_data,
                                              FindFilesPacket *ff_pkt) = hpux_build_acl_streams;
static bacl_exit_code (*os_parse_acl_streams)(JobControlRecord *jcr, acl_data_t *acl_data,
                                              int stream, char *content,
                                              uint32_t content_length) = hpux_parse_acl_streams;
...
typedef enum acl_type { ACLENT_T = 0, ACE_T = 1 } acl_type_t;
#endif
 ...
 */
static bacl_exit_code solaris_build_acl_streams(JobControlRecord *jcr, acl_data_t *acl_data,
                                                FindFilesPacket *ff_pkt) {
...
static bacl_exit_code solaris_parse_acl_streams(JobControlRecord *jcr, acl_data_t *acl_data,
                                                int stream, char *content,
                                                uint32_t content_length) {
  acl_t *aclp;
      ...
      switch (acl_enabled) {
        ...
        default:
          ...
          switch (stream) {
            case STREAM_ACL_SOLARIS_ACLENT:
              /*
               * An aclent can be restored on filesystems with _ACL_ACLENT_ENABLED or
               * _ACL_ACE_ENABLED support.
               */
              if ((acl_enabled & (_ACL_ACLENT_ENABLED | _ACL_ACE_ENABLED)) == 0) {
                Mmsg1(jcr->errmsg,
                      _("Trying to restore POSIX acl on file \"%s\" on filesystem without aclent "
                        "acl support\n"),
                      acl_data->last_fname);
                return bacl_exit_error;
              }
            ...
            case STREAM_ACL_SOLARIS_ACE:
              ...
              if ((acl_enabled & _ACL_ACE_ENABLED) == 0) {
                Mmsg1(jcr->errmsg,
                      _("Trying to restore NFSv4 acl on file \"%s\" on filesystem without ace acl "
                        "support\n"),
                      acl_data->last_fname);
                return bacl_exit_error;
              }
 ...
 */
static bacl_exit_code (*os_build_acl_streams)(JobControlRecord *jcr, acl_data_t *acl_data,
                                              FindFilesPacket *ff_pkt) = solaris_build_acl_streams;
static bacl_exit_code (*os_parse_acl_streams)(JobControlRecord *jcr, acl_data_t *acl_data,
                                              int stream, char *content,
                                              uint32_t content_length) = solaris_parse_acl_streams;
...
static bacl_exit_code afs_build_acl_streams(JobControlRecord *jcr, acl_data_t *acl_data,
                                            FindFilesPacket *ff_pkt) {
  int error;
...
static bacl_exit_code afs_parse_acl_stream(JobControlRecord *jcr, acl_data_t *acl_data, int stream,
                                           char *content, uint32_t content_length) {
  int error;
  struct ViceIoctl vip;
