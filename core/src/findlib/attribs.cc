 ...
 */
static inline bool RestoreFileAttributes(JobControlRecord *jcr, Attributes *attr,
...
bool SetAttributes(JobControlRecord *jcr, Attributes *attr, BareosWinFilePacket *ofd) {
...
#if defined(HAVE_WIN32)
  if (attr->stream == STREAM_UNIX_ATTRIBUTES_EX && set_win32_attributes(jcr, attr, ofd)) {
    if (IsBopen(ofd)) {
      bclose(ofd);
    }
  ...
  if (attr->data_stream == STREAM_WIN32_DATA || attr->data_stream == STREAM_WIN32_GZIP_DATA ||
  ...
  if (IsBopen(ofd)) {
    ...
    if (attr->type == FT_REG && fsize > 0 && attr->statp.st_size > 0 &&
      ...
      Jmsg3(jcr, M_ERROR, 0,
            _("File size of restored file %s not correct. Original %s, restored %s.\n"),
  ...
  /**
   * For link, change owner of link using lchown, but don't try to do a chmod as that will update
   * the file behind it.
   */
  if (attr->type == FT_LNK) {
    /*
...
int encode_attribsEx(JobControlRecord *jcr, char *attribsEx, FindFilesPacket *ff_pkt) {
  ...
  if (p_GetFileAttributesExW) {
    ...
    if (!p_GetFileAttributesExA(ff_pkt->sys_fname, GetFileExInfoStandard, (LPVOID)&atts)) {
 ...
 *           false on failure
 */
static bool set_win32_attributes(JobControlRecord *jcr, Attributes *attr,
                                 BareosWinFilePacket *ofd) {
  char *p = attr->attrEx;
  int64_t val;
  WIN32_FILE_ATTRIBUTE_DATA atts;
...
void WinError(JobControlRecord *jcr, const char *prefix, POOLMEM *win32_ofile) {
  ...
  LPTSTR msg;
  FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, NULL, lerror, 0,
                (LPTSTR)&msg, 0, NULL);
