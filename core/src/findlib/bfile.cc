...
DLL_IMP_EXP int (*plugin_bopen)(BareosWinFilePacket *bfd, const char *fname, int flags,
                                mode_t mode) = NULL;
...
DLL_IMP_EXP boffset_t (*plugin_blseek)(BareosWinFilePacket *bfd, boffset_t offset,
                                       int whence) = NULL;
...
static inline int BopenEncrypted(BareosWinFilePacket *bfd, const char *fname, int flags,
                                 mode_t mode) {
  bool is_dir;
  ULONG ulFlags = 0;
  POOLMEM *win32_fname;
  ...
  if (p_OpenEncryptedFileRawW && p_MultiByteToWideChar) {
    ...
    if (p_OpenEncryptedFileRawW((LPCWSTR)win32_fname_wchar, ulFlags, &(bfd->pvContext))) {
      bfd->mode = BF_CLOSED;
    }
  } else {
    /*
    ...
    if (p_OpenEncryptedFileRawA(win32_fname_wchar, ulFlags, &(bfd->pvContext))) {
      bfd->mode = BF_CLOSED;
    }
  }
...
static inline int BopenNonencrypted(BareosWinFilePacket *bfd, const char *fname, int flags,
                                    mode_t mode) {
  POOLMEM *win32_fname;
  POOLMEM *win32_fname_wchar;
  ...
  if (flags & O_CREAT) { /* Create */
    ...
    if (p_CreateFileW && p_MultiByteToWideChar) {
      ...
      bfd->fh = p_CreateFileW((LPCWSTR)win32_fname_wchar, dwaccess, /* Requested access */
                              0,                                    /* Shared mode */
...
ssize_t bread(BareosWinFilePacket *bfd, void *buf, size_t count) {
  ...
  if (bfd->use_backup_api) {
    if (!p_BackupRead(bfd->fh, (BYTE *)buf, count, &bfd->rw_bytes, 0, /* no Abort */
                      1,                                              /* Process Security */
...
ssize_t bwrite(BareosWinFilePacket *bfd, void *buf, size_t count) {
  ...
  if (bfd->use_backup_api) {
    if (!p_BackupWrite(bfd->fh, (BYTE *)buf, count, &bfd->rw_bytes, 0, /* No abort */
                       1,                                              /* Process Security */
                       &bfd->lpContext)) {                             /* Context */
      bfd->lerror = GetLastError();
      bfd->berrno = b_errno_win32;
 ...
 */
bool set_win32_backup(BareosWinFilePacket *bfd) { return false; /* no can do */ }

bool SetPortableBackup(BareosWinFilePacket *bfd) { return true; /* no problem */ }

/**
 * Return true  if we are writing in portable format
 * return false if not
 */
bool IsPortableBackup(BareosWinFilePacket *bfd) { return true; /* portable by definition */ }

bool set_prog(BareosWinFilePacket *bfd, char *prog, JobControlRecord *jcr) { return false; }
...
#else
int BopenRsrc(BareosWinFilePacket *bfd, const char *fname, int flags, mode_t mode) { return -1; }
#endif
