      ...
      switch (attr->type) {
        ...
        case FT_SPEC:
          ...
          if (S_ISFIFO(attr->statp.st_mode)) {
...
#endif
          } else if ((S_ISBLK(attr->statp.st_mode) || S_ISCHR(attr->statp.st_mode)) && !exists &&
                     isOnRoot) {
            /*
            ...
            if (mknod(attr->ofname, attr->statp.st_mode, attr->statp.st_rdev) != 0 &&
                errno != EEXIST) {
              berrno be;
        ...
        case FT_LNK:
          ...
          if (win32_symlink(attr->olname, attr->ofname, attr->statp.st_rdev) != 0 &&
              errno != EEXIST) {
            berrno be;
