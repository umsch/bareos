...
int EnableBackupPrivileges(JobControlRecord *jcr, int ignore_errors) { return 0; }
#endif
...
static int enable_priv(JobControlRecord *jcr, HANDLE hToken, const char *name, int ignore_errors) {
...
int EnableBackupPrivileges(JobControlRecord *jcr, int ignore_errors) {
  ...
  if (!p_OpenProcessToken(hProcess, TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, &hToken)) {
    if (!ignore_errors) {
