  ...
  if (fileset) {
    ...
    for (i = 0; i < fileset->include_list.size(); i++) {
      ...
      Dmsg4(50, "Verify=<%s> Accurate=<%s> BaseJob=<%s> flags=<%d>\n", ff->VerifyOpts,
            ff->AccurateOpts, ff->BaseJobOpts, ff->flags);
...
bool AcceptFile(FindFilesPacket *ff) {
  ...
  for (j = 0; j < incexe->opts_list.size(); j++) {
     ...
     */
    if (BitIsSet(FO_EXCLUDE, ff->flags) && fo->regex.size() == 0 && fo->wild.size() == 0 &&
        fo->regexdir.size() == 0 && fo->wilddir.size() == 0 && fo->regexfile.size() == 0 &&
  ...
  for (i = 0; i < fileset->exclude_list.size(); i++) {
    ...
    fnm_flags =
        (incexe->current_opts != NULL && BitIsSet(FO_IGNORECASE, incexe->current_opts->flags))
            ? FNM_CASEFOLD
            : 0;
    foreach_dlist(node, &incexe->name_list) {
      char *fname = node->c_str();
