...
#else
static inline bool AcceptDrivetype(FindFilesPacket *ff, void *dummy) { return true; }
#endif
...
static inline bool no_dump(JobControlRecord *jcr, FindFilesPacket *ff_pkt) {
  ...
  if (BitIsSet(FO_HONOR_NODUMP, ff_pkt->flags) && (ff_pkt->statp.st_flags & UF_NODUMP)) {
 ...
 * check for sizes
 */
static inline bool CheckSizeMatching(JobControlRecord *jcr, FindFilesPacket *ff_pkt) {
...
bool CheckChanges(JobControlRecord *jcr, FindFilesPacket *ff_pkt) {
  ...
  if (ff_pkt->incremental &&
      (ff_pkt->statp.st_mtime < ff_pkt->save_time &&
       (BitIsSet(FO_MTIMEONLY, ff_pkt->flags) || ff_pkt->statp.st_ctime < ff_pkt->save_time))) {
    return false;
  }
...
static inline int process_hfsattributes(JobControlRecord *jcr, FindFilesPacket *ff_pkt,
                                        int HandleFile(JobControlRecord *jcr, FindFilesPacket *ff,
                                                       bool top_level),
  ...
  if (getattrlist(fname, &attrList, &ff_pkt->hfsinfo, sizeof(ff_pkt->hfsinfo), FSOPT_NOFOLLOW) !=
      0) {
...
static inline int process_directory(JobControlRecord *jcr, FindFilesPacket *ff_pkt,
  ...
  if (!top_level && BitIsSet(FO_NO_RECURSION, ff_pkt->flags)) {
    ...
    recurse = false;
  } else if (!top_level && (parent_device != ff_pkt->statp.st_dev || is_win32_mount_point)) {
...
static inline int process_special_file(JobControlRecord *jcr, FindFilesPacket *ff_pkt,
  ...
  } else if (top_level && S_ISFIFO(ff_pkt->statp.st_mode) && BitIsSet(FO_READFIFO, ff_pkt->flags)) {
    ff_pkt->type = FT_FIFO;
  } else {
    /*
 ...
 */
static inline bool NeedsProcessing(JobControlRecord *jcr, FindFilesPacket *ff_pkt, char *fname) {
...
int FindOneFile(JobControlRecord *jcr, FindFilesPacket *ff_pkt,
   ...
   */
  if (top_level) {
    if (!NeedsProcessing(jcr, ff_pkt, fname)) {
      return 1;
    }
  }
  ...
  if (!BitIsSet(FO_NO_HARDLINK, ff_pkt->flags) && ff_pkt->statp.st_nlink > 1) {
    switch (ff_pkt->statp.st_mode & S_IFMT) {
      ...
      case S_IFSOCK:
         ...
         */
        rtn_stat = process_hardlink(jcr, ff_pkt, HandleFile, fname, top_level, &done);
        if (done) {
          return rtn_stat;
        }
        break;
...
#endif
    case S_IFDIR:
      return process_directory(jcr, ff_pkt, HandleFile, fname, parent_device, top_level);
    default:
