...
CurLink *new_hardlink(JobControlRecord *jcr, FindFilesPacket *ff_pkt, char *fname, ino_t ino,
                      dev_t dev) {
  int len;
  CurLink *hl = NULL;
  uint64_t binary_search_key[2];
 ...
 */
void FfPktSetLinkDigest(FindFilesPacket *ff_pkt, int32_t digest_stream, const char *digest,
                        uint32_t len) {
