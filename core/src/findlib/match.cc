...
bool MatchFiles(JobControlRecord *jcr, FindFilesPacket *ff,
...
void AddFnameToIncludeList(FindFilesPacket *ff, int prefixed, const char *fname) {
  ...
  Dmsg4(100, "add_fname_to_include prefix=%d compres=%d alg= %d fname=%s\n", prefixed,
        BitIsSet(FO_COMPRESS, inc->options), inc->algo, inc->fname);
}
...
bool FileIsIncluded(FindFilesPacket *ff, const char *file) {
  ...
  for (; inc; inc = inc->next) {
    ...
    if (inc->len < len && IsPathSeparator(file[inc->len]) && bstrncmp(inc->fname, file, inc->len)) {
      return true;
    }
 ...
 */
bool ParseSizeMatch(const char *size_match_pattern, struct s_sz_matching *size_matching) {
