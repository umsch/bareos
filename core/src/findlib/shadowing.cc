 ...
 */
static inline bool check_include_pattern_shadowing(JobControlRecord *jcr, const char *pattern1,
                                                   const char *pattern2, bool recursive) {
  int len1, len2;
  bool retval = false;
  struct stat st1, st2;
 ...
 */
static void check_local_fileset_shadowing(JobControlRecord *jcr, findIncludeExcludeItem *incexe,
                                          bool remove) {
  ...
  while (str1) {
    ...
    while (str1 && str2) {
      if (check_include_pattern_shadowing(jcr, str1->c_str(), str2->c_str(), recursive)) {
        /*
        ...
        if (strlen(str1->c_str()) < strlen(str2->c_str())) {
          if (remove) {
            ...
            Jmsg(jcr, M_WARNING, 0, _("Fileset include block entry %s shadows %s\n"), str2->c_str(),
                 str1->c_str());
          }
        } else {
          if (remove) {
 ...
 */
static inline void check_global_fileset_shadowing(JobControlRecord *jcr, findFILESET *fileset,
                                                  bool remove) {
  int i, j;
  ...
  for (i = 0; i < fileset->include_list.size(); i++) {
    ...
    for (j = i + 1; j < fileset->include_list.size(); j++) {
       ...
       */
      global_recursive = (local_recursive && IncludeBlockIsRecursive(compare_against));
  ...
  for (i = 0; i < fileset->include_list.size(); i++) {
    ...
    switch (shadow_type) {
      ...
      case check_shadow_local_remove:
         ...
         */
        check_local_fileset_shadowing(jcr, incexe, shadow_type == check_shadow_local_remove);
        break;
      ...
      case check_shadow_global_remove:
         ...
         * from the function.
         */
        check_global_fileset_shadowing(jcr, fileset, shadow_type == check_shadow_global_remove);
        return;
    }
  }
}
