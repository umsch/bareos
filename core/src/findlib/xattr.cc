 ...
 */
bxattr_exit_code BuildXattrStreams(JobControlRecord *jcr, xattr_data_t *xattr_data,
                                   FindFilesPacket *ff_pkt) {
  return bxattr_exit_fatal;
}

bxattr_exit_code ParseXattrStreams(JobControlRecord *jcr, xattr_data_t *xattr_data, int stream,
...
uint32_t SerializeXattrStream(JobControlRecord *jcr, xattr_data_t *xattr_data,
   ...
   */
  xattr_data->u.build->content =
      CheckPoolMemorySize(xattr_data->u.build->content, expected_serialize_len + 10);
...
bxattr_exit_code UnSerializeXattrStream(JobControlRecord *jcr, xattr_data_t *xattr_data,
                                        char *content, uint32_t content_length,
...
static bxattr_exit_code aix_build_xattr_streams(JobControlRecord *jcr, xattr_data_t *xattr_data,
                                                FindFilesPacket *ff_pkt) {
  char *bp;
  bool skip_xattr;
   ...
   */
  for (bp = xattr_list; (bp - xattr_list) + 1 < xattr_list_len; bp = strchr(bp, '\0') + 1) {
    skip_xattr = false;
  ...
  if (xattr_count > 0) {
     ...
     */
    if (SerializeXattrStream(jcr, xattr_data, expected_serialize_len, xattr_value_list) <
...
static bxattr_exit_code aix_parse_xattr_streams(JobControlRecord *jcr, xattr_data_t *xattr_data,
                                                int stream, char *content,
  ...
  if (UnSerializeXattrStream(jcr, xattr_data, content, content_length, xattr_value_list) !=
      bxattr_exit_ok) {
    goto bail_out;
  }

  foreach_alist(current_xattr, xattr_value_list) {
    if (lsetea(xattr_data->last_fname, current_xattr->name, current_xattr->value,
 ...
 */
static bxattr_exit_code (*os_build_xattr_streams)(JobControlRecord *jcr, xattr_data_t *xattr_data,
                                                  FindFilesPacket *ff_pkt) =
    aix_build_xattr_streams;
static bxattr_exit_code (*os_parse_xattr_streams)(
    JobControlRecord *jcr, xattr_data_t *xattr_data, int stream, char *content,
    uint32_t content_length) = aix_parse_xattr_streams;
...
static xattr_naming_space xattr_naming_spaces[] = {
    {"user.", ATTR_DONTFOLLOW}, {"root.", ATTR_ROOT | ATTR_DONTFOLLOW}, {NULL, 0}};

static bxattr_exit_code irix_build_xattr_streams(JobControlRecord *jcr, xattr_data_t *xattr_data,
                                                 FindFilesPacket *ff_pkt) {
  char dummy[32];
  ...
  for (cnt = 0; xattr_naming_spaces[cnt].name != NULL; cnt++) {
    ...
    while (1) {
      ...
      for (cnt = 0; cnt < attrlist->al_count; cnt++) {
         ...
         */
        length = sizeof(dummy);
        if (attr_get(xattr_data->last_fname, attrlist_ent->a_name, dummy, &length,
        ...
        current_xattr->name_length =
            strlen(xattr_naming_spaces[cnt].name) + strlen(attrlist_ent->a_name) + 1;
        current_xattr->name = (char *)malloc(current_xattr->name_length);
         ...
         */
        if (attr_get(xattr_data->last_fname, attrlist_ent->a_name, current_xattr->value, &length,
                     xattr_naming_spaces[cnt].flags) != 0) {
          berrno be;
...
static bxattr_exit_code irix_parse_xattr_streams(JobControlRecord *jcr, xattr_data_t *xattr_data,
                                                 int stream, char *content,
                                                 uint32_t content_length) {
  char *bp;
  ...
  foreach_alist(current_xattr, xattr_value_list) {
    ...
    for (cnt = 0; xattr_naming_spaces[cnt].name != NULL; cnt++) {
      ...
      if (bstrncasecmp(current_xattr->name, xattr_naming_spaces[cnt].name, cmp_size)) {
    ...
    if (attr_set(xattr_data->last_fname, ++bp, current_xattr->value, current_xattr->value_length,
                 flags) != 0) {
      berrno be;
...
static bxattr_exit_code (*os_parse_xattr_streams)(
    JobControlRecord *jcr, xattr_data_t *xattr_data, int stream, char *content,
    uint32_t content_length) = irix_parse_xattr_streams;
...
static const char *xattr_acl_skiplist[3] = {"system.posix_acl_access", "system.posix_acl_default",
                                            NULL};
static const char *xattr_skiplist[1] = {NULL};
...
static bxattr_exit_code generic_build_xattr_streams(JobControlRecord *jcr, xattr_data_t *xattr_data,
                                                    FindFilesPacket *ff_pkt) {
  char *bp;
  bool skip_xattr;
  char *xattr_list = NULL;
  ...
  for (bp = xattr_list; (bp - xattr_list) + 1 < xattr_list_len; bp = strchr(bp, '\0') + 1) {
    ...
    switch (xattr_value_len) {
      ...
      default:
        ...
        xattr_value_len =
            lgetxattr(xattr_data->last_fname, bp, current_xattr->value, xattr_value_len);
        if (xattr_value_len < 0) {
          berrno be;
...
static bxattr_exit_code generic_parse_xattr_streams(JobControlRecord *jcr, xattr_data_t *xattr_data,
                                                    int stream, char *content,
                                                    uint32_t content_length) {
  xattr_t *current_xattr;
  alist *xattr_value_list;
  ...
  foreach_alist(current_xattr, xattr_value_list) {
    if (lsetxattr(xattr_data->last_fname, current_xattr->name, current_xattr->value,
                  current_xattr->value_length, 0) != 0) {
      berrno be;
...
static bxattr_exit_code (*os_parse_xattr_streams)(
    JobControlRecord *jcr, xattr_data_t *xattr_data, int stream, char *content,
    uint32_t content_length) = generic_parse_xattr_streams;

#elif defined(HAVE_FREEBSD_OS) || defined(HAVE_NETBSD_OS) || defined(HAVE_OPENBSD_OS)
    ...
    !defined(HAVE_EXTATTR_NAMESPACE_TO_STRING) || !defined(HAVE_EXTATTR_STRING_TO_NAMESPACE)
...
static const char *xattr_acl_skiplist[4] = {"system.posix1e.acl_access",
                                            "system.posix1e.acl_default", "system.nfs4.acl", NULL};
static const char *xattr_skiplist[1] = {NULL};
...
static bxattr_exit_code bsd_build_xattr_streams(JobControlRecord *jcr, xattr_data_t *xattr_data,
                                                FindFilesPacket *ff_pkt) {
  bool skip_xattr;
  char *xattr_list = NULL;
   ...
   */
  for (namespace_index = 0; namespace_index < sizeof(os_default_xattr_namespaces) / sizeof(int);
       namespace_index++) {
     ...
     */
    xattr_list_len =
        extattr_list_link(xattr_data->last_fname, attrnamespace, xattr_list, xattr_list_len);
    switch (xattr_list_len) {
      case -1: {
        berrno be;
    ...
    for (index = 0; index < xattr_list_len; index += xattr_list[index] + 1) {
       ...
       */
      Bsnprintf(current_attrtuple, sizeof(current_attrtuple), "%s.%s", current_attrnamespace,
                current_attrname);
       ...
       */
      xattr_value_len =
          extattr_get_link(xattr_data->last_fname, attrnamespace, current_attrname, NULL, 0);
      switch (xattr_value_len) {
        case -1: {
          berrno be;
      ...
      switch (xattr_value_len) {
        ...
        default:
          ...
          xattr_value_len =
              extattr_get_link(xattr_data->last_fname, attrnamespace, current_attrname,
...
static bxattr_exit_code bsd_parse_xattr_streams(JobControlRecord *jcr, xattr_data_t *xattr_data,
                                                int stream, char *content,
                                                uint32_t content_length) {
  xattr_t *current_xattr;
  alist *xattr_value_list;
  ...
  foreach_alist(current_xattr, xattr_value_list) {
    ...
    *attrname++ = '\0';
     ...
     */
    cnt = extattr_set_link(xattr_data->last_fname, current_attrnamespace, attrname,
...
static bxattr_exit_code (*os_parse_xattr_streams)(
    JobControlRecord *jcr, xattr_data_t *xattr_data, int stream, char *content,
    uint32_t content_length) = bsd_parse_xattr_streams;

#elif defined(HAVE_OSF1_OS)

#if !defined(HAVE_GETPROPLIST) || !defined(HAVE_GET_PROPLIST_ENTRY) ||           \
    !defined(HAVE_SIZEOF_PROPLIST_ENTRY) || !defined(HAVE_ADD_PROPLIST_ENTRY) || \
...
static bxattr_exit_code tru64_build_xattr_streams(JobControlRecord *jcr, xattr_data_t *xattr_data,
                                                  FindFilesPacket *ff_pkt) {
  int cnt;
  ...
  xattrbuf_min_size = 0;
  xattr_list_len = getproplist(xattr_data->last_fname, 1, &prop_args, xattrbuf_size, xattrbuf,
                               &xattrbuf_min_size);
...
static bxattr_exit_code tru64_parse_xattr_streams(JobControlRecord *jcr, xattr_data_t *xattr_data,
                                                  int stream, char *content,
                                                  uint32_t content_length) {
...
static bxattr_exit_code (*os_parse_xattr_streams)(
    JobControlRecord *jcr, xattr_data_t *xattr_data, int stream, char *content,
    uint32_t content_length) = tru64_parse_xattr_streams;
...
#if !defined(HAVE_OPENAT) || !defined(HAVE_UNLINKAT) || !defined(HAVE_FCHOWNAT) || \
...
static inline void add_xattr_link_cache_entry(xattr_data_t *xattr_data, ino_t inum, char *target) {
...
static bool AclIsTrivial(int count, aclent_t *entries) {
  ...
  for (n = 0; n < count; n++) {
    ace = &entries[n];
    if (!(ace->a_type == USER_OBJ || ace->a_type == GROUP_OBJ || ace->a_type == OTHER_OBJ ||
...
static bxattr_exit_code solaris_save_xattr_acl(JobControlRecord *jcr, xattr_data_t *xattr_data,
                                               int fd, const char *attrname, char **acl_text) {
  bxattr_exit_code retval = bxattr_exit_error;
#ifdef HAVE_ACL
   ...
   */
  if ((fd != -1 && fpathconf(fd, _PC_ACL_ENABLED) > 0) || pathconf(attrname, _PC_ACL_ENABLED) > 0) {
    /*
  ...
  if (n >= MIN_ACL_ENTRIES) {
    ...
    if ((fd != -1 && facl(fd, GETACL, n, acls) != n) || acl(attrname, GETACL, n, acls) != n) {
      berrno be;
 ...
 */
static bxattr_exit_code solaris_save_xattrs(JobControlRecord *jcr, xattr_data_t *xattr_data,
                                            const char *xattr_namespace, const char *attr_parent);
 ...
 */
static bxattr_exit_code solaris_save_xattr(JobControlRecord *jcr, xattr_data_t *xattr_data, int fd,
                                           const char *xattr_namespace, const char *attrname,
                                           bool toplevel_hidden_dir, int stream) {
  int cnt;
  int attrfd = -1;
  struct stat st;
  ...
  switch (st.st_mode & S_IFMT) {
    ...
    case S_IFBLK:
      ...
      cnt = Bsnprintf(buffer, sizeof(buffer), "%s%c%s%c%s%c", target_attrname, 0, attribs, 0,
                      (acl_text) ? acl_text : "", 0);
      break;
    case S_IFDIR:
      /*
      ...
      if (st.st_size > 0) {
        ...
        while ((cnt = read(attrfd, buffer, sizeof(buffer))) > 0) {
          xattr_data->u.build->content = CheckPoolMemorySize(
              xattr_data->u.build->content, xattr_data->u.build->content_length + cnt);
          memcpy(xattr_data->u.build->content + xattr_data->u.build->content_length, buffer, cnt);
          xattr_data->u.build->content_length += cnt;
        }
...
static bxattr_exit_code solaris_save_xattrs(JobControlRecord *jcr, xattr_data_t *xattr_data,
  ...
  if (attr_parent) {
    name = attr_parent;
    if (xattr_namespace) {
      Bsnprintf(current_xattr_namespace, sizeof(current_xattr_namespace), "%s%s/", xattr_namespace,
                attr_parent);
    } else {
   ...
   */
  if (!attr_parent)
    solaris_save_xattr(jcr, xattr_data, attrdirfd, current_xattr_namespace, ".", true,
  ...
  while ((dp = readdir(dirp)) != NULL) {
    ...
    Dmsg3(400, "processing extended attribute %s%s on file \"%s\"\n", current_xattr_namespace,
          dp->d_name, xattr_data->last_fname);
    ...
    if (bstrcmp(dp->d_name, VIEW_READWRITE)) {
       ...
       * Save the xattr.
       */
      solaris_save_xattr(jcr, xattr_data, attrdirfd, current_xattr_namespace, dp->d_name, false,
...
#ifdef HAVE_ACL
static bxattr_exit_code solaris_restore_xattr_acl(JobControlRecord *jcr, xattr_data_t *xattr_data,
                                                  int fd, const char *attrname, char *acl_text) {
#ifdef HAVE_EXTENDED_ACL
  int error;
  acl_t *aclp = NULL;
  ...
  if (!acls) {
    if ((fd != -1 && facl(fd, SETACL, n, acls) != 0) || acl(attrname, SETACL, n, acls) != 0) {
      berrno be;
...
static bxattr_exit_code solaris_restore_xattrs(JobControlRecord *jcr, xattr_data_t *xattr_data,
                                               bool is_extensible, char *content,
                                               uint32_t content_length)

{
  ...
  switch (st.st_mode & S_IFMT) {
    ...
    case S_IFREG:
      ...
      if (inum != 0) {
        ...
        if (link(linked_target, target_attrname) < 0) {
          ...
          Dmsg4(100, "Unable to link xattr %s to %s on file \"%s\": ERR=%s\n", target_attrname,
                linked_target, xattr_data->last_fname, be.bstrerror());
          goto bail_out;
        }
        ...
        if ((attrfd = openat(attrdirfd, target_attrname, O_RDWR | O_CREAT | O_TRUNC, st.st_mode)) <
            0) {
          berrno be;
      ...
      if (st.st_size > 0) {
        ...
        if (cnt != st.st_size) {
          Mmsg2(jcr->errmsg,
                _("Unable to restore data of xattr %s on file \"%s\": Not all data available in "
                  "xattr stream\n"),
                target_attrname, xattr_data->last_fname);
          Dmsg2(100,
    ...
    case S_IFLNK:
      /*
       * The current implementation of xattr on Solaris doesn't support this, but if it ever does we
       * are prepared.
       */
      linked_target = bp;
  ...
  if (acl_text && *acl_text)
    if (solaris_restore_xattr_acl(jcr, xattr_data, attrfd, target_attrname, acl_text) !=
        bxattr_exit_ok)
      goto bail_out;
#endif /* HAVE_ACL */
...
static bxattr_exit_code solaris_build_xattr_streams(JobControlRecord *jcr, xattr_data_t *xattr_data,
                                                    FindFilesPacket *ff_pkt) {
  char cwd[PATH_MAX];
...
static bxattr_exit_code solaris_parse_xattr_streams(JobControlRecord *jcr, xattr_data_t *xattr_data,
                                                    int stream, char *content,
                                                    uint32_t content_length) {
  char cwd[PATH_MAX];
    ...
    case STREAM_XATTR_SOLARIS_SYS:
      if (pathconf(xattr_data->last_fname, _PC_SATTR_ENABLED) <= 0) {
        Mmsg1(jcr->errmsg, _("Failed to restore extensible attributes on file \"%s\"\n"),
              xattr_data->last_fname);
        Dmsg1(100,
              "Unable to restore extensible attributes on file \"%s\", filesystem doesn't support "
              "this\n",
              xattr_data->last_fname);
        goto bail_out;
      }
    ...
    case STREAM_XATTR_SOLARIS:
      if (pathconf(xattr_data->last_fname, _PC_XATTR_ENABLED) <= 0) {
        Mmsg1(jcr->errmsg, _("Failed to restore extended attributes on file \"%s\"\n"),
              xattr_data->last_fname);
        Dmsg1(100,
              "Unable to restore extended attributes on file \"%s\", filesystem doesn't support "
              "this\n",
              xattr_data->last_fname);
        goto bail_out;
      }
...
static bxattr_exit_code (*os_parse_xattr_streams)(
    JobControlRecord *jcr, xattr_data_t *xattr_data, int stream, char *content,
    uint32_t content_length) = solaris_parse_xattr_streams;
