 ...
 */
enum { APT_NATIVE = 0, APT_NDMPV2, APT_NDMPV3, APT_NDMPV4 };

/**
 * Authentication types
 */
enum { AT_NONE = 0, AT_CLEAR, AT_MD5, AT_VOID };
...
#define JobWaiting(jcr)                                                                           \
  (jcr->job_started && (jcr->JobStatus == JS_WaitFD || jcr->JobStatus == JS_WaitSD ||             \
...
#define foreach_jcr(jcr) for (jcr = jcr_walk_start(); jcr; (jcr = jcr_walk_next(jcr)))
...
class DLL_IMP_EXP JobControlRecord {
 ...
 public:
  ...
  bool NoClientUsed() const {
    return (JobType_ == JT_MIGRATE || JobType_ == JT_COPY || JobLevel_ == L_VIRTUAL_FULL);
  }
  ...
  JobControlRecord
      *cjcr; /**< Controlling JobControlRecord when this is a slave JobControlRecord being
