...
const char *IPADDR::get_address(char *outputbuf, int outlen) {
  ...
  inet_ntop(
      saddr->sa_family,
      saddr->sa_family == AF_INET ? (void *)&(saddr4->sin_addr) : (void *)&(saddr6->sin6_addr),
      outputbuf, outlen);
#else
...
const char *IPADDR::build_address_str(char *buf, int blen, bool print_port /*=true*/) {
  ...
  if (print_port) {
    switch (GetFamily()) {
      case AF_INET:
        Bsnprintf(buf, blen, "host[ipv4;%s;%hu] ", get_address(tmp, sizeof(tmp) - 1),
                  GetPortHostOrder());
        break;
      case AF_INET6:
        Bsnprintf(buf, blen, "host[ipv6;%s;%hu] ", get_address(tmp, sizeof(tmp) - 1),
                  GetPortHostOrder());
        break;
      default:
        break;
    ...
    switch (GetFamily()) {
      case AF_INET:
        Bsnprintf(buf, blen, "host[ipv4;%s] ", get_address(tmp, sizeof(tmp) - 1));
        break;
      case AF_INET6:
        Bsnprintf(buf, blen, "host[ipv6;%s] ", get_address(tmp, sizeof(tmp) - 1));
        break;
      default:
        break;
    }
...
int AddAddress(dlist **out, IPADDR::i_type type, unsigned short defaultport, int family,
  ...
  type = (type == IPADDR::R_SINGLE_PORT || type == IPADDR::R_SINGLE_ADDR) ? IPADDR::R_SINGLE : type;
  if (type != IPADDR::R_DEFAULT) {
    ...
    foreach_dlist(iaddr, addrs) {
      if (iaddr->GetType() == IPADDR::R_DEFAULT) {
        ...
        Bsnprintf(buf, buflen, _("the old style addresses cannot be mixed with new style"));
        return 0;
      }
    }
    if (def) {
  ...
  if (!hostaddrs) {
    Bsnprintf(buf, buflen, _("can't resolve hostname(%s) %s"), hostname_str, myerrstr);
    return 0;
  }
    ...
    foreach_dlist(iaddr, hostaddrs) {
      ...
      foreach_dlist(jaddr, addrs) {
        if (iaddr->GetSockaddrLen() == jaddr->GetSockaddrLen() &&
            !memcmp(iaddr->get_sockaddr(), jaddr->get_sockaddr(), iaddr->GetSockaddrLen())) {
          goto skip; /* no price */
        }
      }
...
void InitDefaultAddresses(dlist **out, const char *port) {
  ...
  if (!AddAddress(out, IPADDR::R_DEFAULT, htons(sport), AF_INET, 0, 0, buf, sizeof(buf))) {
...
#ifdef HAVE_IPV6
            sa->sa_family == AF_INET ? (void *)&(((struct sockaddr_in *)sa)->sin_addr)
