...
#define foreach_alist_index(inx, var, list) \
  for ((inx) = 0; list ? ((var) = (typeof((var)))(list)->get((inx))) : NULL; (inx)++)
...
#define foreach_alist_index(inx, var, list) \
  for ((inx) = 0; list ? ((*((void **)&(var)) = (void *)((list)->get((inx))))) : NULL; (inx)++)

#define foreach_alist_rindex(inx, var, list)    \
  for (list ? (inx) = ((list)->size() - 1) : 0; \
       list ? ((*((void **)&(var)) = (void *)((list)->get((inx))))) : NULL; (inx)--)
 ...
 * Second arg of init
 */
enum { owned_by_alist = true, not_owned_by_alist = false };
