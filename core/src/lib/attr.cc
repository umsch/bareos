...
int UnpackAttributesRecord(JobControlRecord *jcr, int32_t stream, char *rec, int32_t reclen,
                           Attributes *attr) {
  char *p;
  int object_len;
  /*
  ...
  Dmsg8(debuglevel,
        "unpack_attr FI=%d Type=%d fname=%s attr=%s lname=%s attrEx=%s datastr=%d delta_seq=%d\n",
        attr->file_index, attr->type, attr->fname, attr->attr, attr->lname, attr->attrEx,
        attr->data_stream, attr->delta_seq);
  *attr->ofname = 0;
  *attr->olname = 0;
  return 1;
}
...
void BuildAttrOutputFnames(JobControlRecord *jcr, Attributes *attr) {
  ...
  if (jcr->where_bregexp) {
    ...
    if (attr->type == FT_LNKSAVED || attr->type == FT_LNK) {
       ...
       *  on user request to soft links
       */
      if (IsPathSeparator(attr->lname[0]) && (attr->type == FT_LNKSAVED || jcr->prefix_links)) {
      ...
      /* Ensure where is terminated with a slash */
      if (add_link && !IsPathSeparator(jcr->where[wherelen - 1]) && !IsPathSeparator(fn[0])) {
...
static const char *attr_stat_to_str(PoolMem &resultbuffer, JobControlRecord *jcr,
                                    Attributes *attr) {
  char buf[5000];
  char ec1[30];
  char en1[30], en2[30];
  ...
  p += sprintf(p, "%-8.8s %-8.8s", guid->uid_to_name(attr->statp.st_uid, en1, sizeof(en1)),
...
static const char *attr_to_ls_output(PoolMem &resultbuffer, JobControlRecord *jcr,
                                     Attributes *attr) {
  attr_stat_to_str(resultbuffer, jcr, attr);
  resultbuffer.strcat("  ");
  attr_file_to_str(resultbuffer, attr);
