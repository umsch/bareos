 ...
 */
void bnet_thread_server_tcp(dlist *addr_list, int max_clients, alist *sockfds, workq_t *client_wq,
                            bool nokeepalive, void *handle_client_request(void *bsock)) {
  ...
  foreach_dlist(ipaddr, addr_list) {
     ...
     * Reuse old sockets
     */
    if (setsockopt(fd_ptr->fd, SOL_SOCKET, SO_REUSEADDR, (sockopt_val_t)&value, sizeof(value)) <
        0) {
      berrno be;
  ...
  while (!quit) {
    ...
    foreach_alist(fd_ptr, sockfds) {
      if (pfds[cnt++].revents & events) {
         ...
         */
        if (setsockopt(newsockfd, SOL_SOCKET, SO_KEEPALIVE, (sockopt_val_t)&value, sizeof(value)) <
            0) {
          berrno be;
