...
void ReCompileInitialize(void) {
  ...
  if (!syntax_table_inited) {
    ...
    for (a = 'A'; a <= 'Z'; a++) re_syntax_table[a] = Sword;
    for (a = '0'; a <= '9'; a++) re_syntax_table[a] = Sword | Sdigit | Shexdigit;
...
static void re_compile_fastmap_aux(regex_t *bufp, unsigned char *code, int pos,
                                   unsigned char *visited, unsigned char *can_be_null,
  ...
  for (;;) {
    switch (code[pos++]) {
      ...
      case Ceol:
        fastmap[(int)'\n'] = 1;
        if (*can_be_null == 0) *can_be_null = 2; /* can match null, but only at end of buffer */
        return;
      case Cset:
        for (a = 0; a < 256 / 8; a++)
          if (code[pos + a] != 0)
...
static int re_do_compile_fastmap(regex_t *bufp, unsigned char *buffer, int used, int pos,
                                 unsigned char *can_be_null, unsigned char *fastmap) {
...
void ReCompileFastmap(regex_t *bufp) {
  ...
  // assert(bufp->used > 0);
  if (!re_do_compile_fastmap(bufp, bufp->buffer, bufp->used, 0, &bufp->can_be_null, bufp->fastmap))
    return;
  if (got_error) return;
...
static int ReOptimizeStarJump(regex_t *bufp, unsigned char *code) {
  ...
  if (!re_do_compile_fastmap(bufp, bufp->buffer, bufp->used, (int)(p2 - bufp->buffer), &can_be_null,
                             map))
...
const char *re_compile_pattern(regex_t *bufp, unsigned char *regex) {
  ...
  while (op != Rend) {
    ...
    if (level > current_level) {
      for (current_level++; current_level < level; current_level++) SET_LEVEL_START;
      SET_LEVEL_START;
    } else if (level < current_level) {
      current_level = level;
      for (; num_jumps > 0 && future_jumps[num_jumps - 1] >= CURRENT_LEVEL_START; num_jumps--)
    ...
    switch (op) {
      ...
      case Reol:
        if (!((pos >= size) ||
              ((regexp_syntax & RE_NO_BK_VBAR)
                   ? (regex[pos] == '\174')
                   : (pos + 1 < size && regex[pos] == '\134' && regex[pos + 1] == '\174')) ||
              ((regexp_syntax & RE_NO_BK_PARENS)
                   ? (regex[pos] == ')')
                   : (pos + 1 < size && regex[pos] == '\134' && regex[pos + 1] == ')')))) {
          if (regexp_context_indep_ops)
            goto op_error;
          else
            goto normal_char;
      ...
      case Roptional:
        ...
        if (CURRENT_LEVEL_START == pattern_offset) break; /* ignore empty patterns for ? */
        ALLOC(3);
      ...
      case Rplus:
        ...
        if (CURRENT_LEVEL_START == pattern_offset) break; /* ignore empty patterns for + and * */
        ALLOC(9);
...
void re_registers_to_regmatch(regexp_registers_t old_regs, regmatch_t pmatch[], size_t nmatch) {
...
int ReMatch(regex_t *bufp, unsigned char *string, int size, int pos, regexp_registers_t old_regs) {
...
continue_matching:
  switch (*code++) {
      ...
      if (old_regs) {
        ...
        if (!bufp->uses_registers) {
          ...
          for (a = 1; a < bufp->num_registers; a++) {
            if ((GET_REG_START(state, a) == NULL) || (GET_REG_END(state, a) == NULL)) {
              old_regs->start[a] = -1;
              old_regs->end[a] = -1;
              continue;
            }
    ...
    case Cmatch_memory: {
      ...
      if ((regstart == NULL) || (regend == NULL)) goto fail; /* or should we just match nothing? */
    ...
    case Cdummy_failure_jump: {
      ...
      if (failuredest < bufp->buffer || bufp->buffer + bufp->used < failuredest) {
        set_error("Regex VM jump out of bounds (Cdummy_failure_jump failuredest)");
        FREE_STATE(state);
        return -2;
      }
    ...
    case Crepeat1: {
      ...
      switch (*pinst++) {
        ...
        case Cexact: {
          ...
          if (translate) {
            while (text < textend && translate[(unsigned char)*text] == ch) text++;
          } else {
    ...
    case Cwordbound: {
      ...
      if ((SYNTAX(text[-1]) & Sword) ^ (SYNTAX(*text) & Sword)) goto continue_matching;
      goto fail;
    }
    case Cnotwordbound: {
...
int ReSearch(regex_t *bufp, unsigned char *str, int size, int pos, int range,
