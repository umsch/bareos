...
class DLL_IMP_EXP BareosSocket : public SmartAlloc {
 ...
 public:
  ...
  TLS_CONNECTION *GetTlsConnection() { return tls_conn; } /* Associated tls connection */
 ...
 protected:
  ...
  virtual void fin_init(JobControlRecord *jcr, int sockfd, const char *who, const char *host,
                        int port, struct sockaddr *lclient_addr) = 0;
  virtual bool open(JobControlRecord *jcr, const char *name, char *host, char *service, int port,
                    utime_t heart_beat, int *fatal) = 0;

 private:
  TLS_CONNECTION *tls_conn; /* Associated tls connection */
  // std::shared_ptr<TLS_CONNECTION> tls_conn;          /* Associated tls connection */
  bool two_way_authenticate(JobControlRecord *jcr, const char *what, const char *identity,
                            s_password &password, TlsResource *tls_configuration,
 ...
 public:
  ...
  bool AuthenticateWithDirector(JobControlRecord *jcr, const char *name, s_password &password,
                                char *response, int response_len, TlsResource *tls_configuration);
  ...
  bool AuthenticateOutboundConnection(JobControlRecord *jcr, const char *what, const char *identity,
                                      s_password &password, TlsResource *tls_configuration);

  bool AuthenticateInboundConnection(JobControlRecord *jcr, const char *what, const char *name,
                                     s_password &password, TlsResource *tls_configuration);
 ...
 */
enum { BNET_SIGNAL = -1, BNET_HARDEOF = -2, BNET_ERROR = -3 };
