...
bool BareosSocketTCP::connect(JobControlRecord *jcr, int retry_interval, utime_t max_retry_time,
                              utime_t heart_beat, const char *name, char *host, char *service,
  ...
  for (i = 0; !open(jcr, name, host, service, port, heart_beat, &fatal); i -= retry_interval) {
    berrno be;
        ...
        default:
          *fatal = 1;
          Pmsg3(000, _("Socket open error. proto=%d port=%d. ERR=%s\n"), ipaddr->GetFamily(),
                ipaddr->GetPortHostOrder(), be.bstrerror());
          break;
      }
      continue;
    }
...
bool BareosSocketTCP::SetKeepalive(JobControlRecord *jcr, int sockfd, bool enable,
                                   int keepalive_start, int keepalive_interval) {
  ...
  if (enable && keepalive_interval) {
    ...
    if (setsockopt(sockfd, SOL_TCP, TCP_KEEPIDLE, (sockopt_val_t)&keepalive_start,
    ...
    if (setsockopt(sockfd, SOL_TCP, TCP_KEEPINTVL, (sockopt_val_t)&keepalive_interval,
  ...
  if (rc != pktsiz) {
    ...
    if (rc < 0) {
      if (!suppress_error_msgs_) {
        Qmsg5(jcr_, M_ERROR, 0, _("Write error sending %d bytes to %s:%s:%d: ERR=%s\n"), msglen,
              who_, host_, port_, this->bstrerror());
      }
    } else {
      Qmsg5(jcr_, M_ERROR, 0, _("Wrote %d bytes to %s:%s:%d, but only %d accepted.\n"), msglen,
            who_, host_, port_, rc);
    }
    ok = false;
  }
...
int32_t BareosSocketTCP::recv() {
  ...
  if (pktsiz < 0 || pktsiz > max_packet_size) {
    if (pktsiz > 0) { /* if packet too big */
      Qmsg3(jcr_, M_FATAL, 0, _("Packet size too big from \"%s:%s:%d. Terminating connection.\n"),
            who_, host_, port_);
...
int BareosSocketTCP::GetPeer(char *buf, socklen_t buflen) {
  ...
  if (!inet_ntop(peer_addr.sin_family, &peer_addr.sin_addr, buf, buflen)) return -1;
...
bool BareosSocketTCP::SetBufferSize(uint32_t size, int rw) {
  ...
  if (rw & BNET_SETBUF_READ) {
    while ((dbuf_size > TAPE_BSIZE) &&
           (setsockopt(fd_, SOL_SOCKET, SO_RCVBUF, (sockopt_val_t)&dbuf_size, sizeof(dbuf_size)) <
            0)) {
      berrno be;
    ...
    if (dbuf_size != start_size) {
      Qmsg1(get_jcr(), M_WARNING, 0, _("Warning network buffer = %d bytes not max size.\n"),
            dbuf_size);
    }
  }
  if (size != 0) {
  ...
  if (rw & BNET_SETBUF_WRITE) {
    while ((dbuf_size > TAPE_BSIZE) &&
           (setsockopt(fd_, SOL_SOCKET, SO_SNDBUF, (sockopt_val_t)&dbuf_size, sizeof(dbuf_size)) <
            0)) {
      berrno be;
      Qmsg1(get_jcr(), M_ERROR, 0, _("sockopt error: %s\n"), be.bstrerror());
      dbuf_size -= TAPE_BSIZE;
...
#ifdef HAVE_WIN32
    /*
     * For Windows, we must simulate Unix errno on a socket error in order to handle errors
     * correctly.
     */
