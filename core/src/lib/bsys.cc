...
void SetSecureEraseCmdline(const char *cmdline) { secure_erase_cmdline = cmdline; }
  ...
  if (stat(fname, &statp) == 0) {
    ...
    *pidbuf = 0;
    if ((pidfd = open(fname, O_RDONLY | O_BINARY, 0)) < 0 ||
        read(pidfd, &pidbuf, sizeof(pidbuf)) < 0 || sscanf(pidbuf, "%d", &oldpid) != 1) {
      berrno be;
...
bool PathContainsDirectory(PoolMem &path) { return PathContainsDirectory(path.c_str()); }
 ...
 */
bool PathAppend(PoolMem &path, PoolMem &extra) { return PathAppend(path, extra.c_str()); }
...
/*
 * based on
 * src/findlib/mkpath.c:bool makepath(Attributes *attr, const char *apath, mode_t mode, mode_t
 * parent_mode, ...
 */
...
bool PathCreate(PoolMem &path, mode_t mode) { return PathCreate(path.c_str(), mode); }
...
static int Addrtosymstr(void *pc, char *buffer, int size) {
  ...
  if ((info.dli_fname != NULL && info.dli_sname != NULL) &&
    ...
    return (Bsnprintf(buffer, size, "%s'%s+0x%x [0x%p]", info.dli_fname, info.dli_sname,
 ...
 */
#if defined(HAVE_BACKTRACE) && defined(HAVE_BACKTRACE_SYMBOLS) && defined(HAVE_GCC)
 ...
 */
    ...
    defined(HAVE_UCONTEXT_H) && defined(HAVE_DEMANGLE_H) && defined(HAVE_CPLUS_DEMANGLE) && \
    defined(__SUNPRO_CC)
