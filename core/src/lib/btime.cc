...
utime_t StrToUtime(const char *str) {
  ...
  if (sscanf(str, "%d-%d-%d %d:%d:%d", &tm.tm_year, &tm.tm_mon, &tm.tm_mday, &tm.tm_hour,
...
fdate_t DateEncode(uint32_t year, uint8_t month, uint8_t day) {
  ...
  return (((int32_t)(365.25 * (y + 4716))) + ((int)(30.6001 * (m + 1))) + day + b - 1524.5);
}
...
/* Deprecated. Do not use. */
ftime_t TimeEncode(uint8_t hour, uint8_t minute, uint8_t second, float32_t second_fraction) {

  ASSERT((second_fraction >= 0.0) || (second_fraction < 1.0));
  return (ftime_t)(((second + 60L * (minute + 60L * hour)) / 86400.0)) + second_fraction;
}
...
/* Deprecated. Do not use. */
void date_time_encode(struct date_time *dt, uint32_t year, uint8_t month, uint8_t day, uint8_t hour,
                      uint8_t minute, uint8_t second, float32_t second_fraction) {
...
/* Deprecated. Do not use. */
void TimeDecode(ftime_t time, uint8_t *hour, uint8_t *minute, uint8_t *second,
...
/* Deprecated. Do not use. */
void date_time_decode(struct date_time *dt, uint32_t *year, uint8_t *month, uint8_t *day,
                      uint8_t *hour, uint8_t *minute, uint8_t *second, float32_t *second_fraction) {
