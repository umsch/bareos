...
static inline void NonCompatibleCompressionAlgorithm(JobControlRecord *jcr,
                                                     uint32_t compression_algorithm) {
  Jmsg(jcr, M_FATAL, 0, _("Illegal compression algorithm %s for compatible mode\n"),
       cmprs_algo_to_text(compression_algorithm));
}

bool SetupCompressionBuffers(JobControlRecord *jcr, bool compatible, uint32_t compression_algorithm,
  ...
  switch (compression_algorithm) {
    ...
    case COMPRESS_LZO1X: {
       ...
       */
      wanted_compress_buf_size =
          jcr->buf_size + (jcr->buf_size / 16) + 64 + 3 + (int)sizeof(comp_stream_header);
      if (wanted_compress_buf_size > *compress_buf_size) {
        *compress_buf_size = wanted_compress_buf_size;
      }
    ...
    case COMPRESS_FZ4H: {
       ...
       *    output_block_size = input_block_size + (input_block_size / 10 + 16 * 2) +
       * sizeof(comp_stream_header)
       *
       ...
       */
      wanted_compress_buf_size =
          jcr->buf_size + (jcr->buf_size / 10 + 16 * 2) + (int)sizeof(comp_stream_header);
      if (wanted_compress_buf_size > *compress_buf_size) {
        *compress_buf_size = wanted_compress_buf_size;
      }
...
#ifdef HAVE_LIBZ
static bool compress_with_zlib(JobControlRecord *jcr, char *rbuf, uint32_t rsize,
                               unsigned char *cbuf, uint32_t max_compress_len,
...
#ifdef HAVE_LZO
static bool compress_with_lzo(JobControlRecord *jcr, char *rbuf, uint32_t rsize,
                              unsigned char *cbuf, uint32_t max_compress_len,
                              uint32_t *compress_len) {
  int lzores;
  lzo_uint len = 0;
  ...
  lzores =
      lzo1x_1_compress((const unsigned char *)rbuf, rsize, cbuf, &len, jcr->compress.workset.pLZO);
  *compress_len = len;
...
#ifdef HAVE_FASTLZ
static bool compress_with_fastlz(JobControlRecord *jcr, char *rbuf, uint32_t rsize,
                                 unsigned char *cbuf, uint32_t max_compress_len,
                                 uint32_t *compress_len) {
  int zstat;
  zfast_stream *pZfastStream;
...
bool CompressData(JobControlRecord *jcr, uint32_t compression_algorithm, char *rbuf, uint32_t rsize,
                  unsigned char *cbuf, uint32_t max_compress_len, uint32_t *compress_len) {
  *compress_len = 0;
  switch (compression_algorithm) {
#ifdef HAVE_LIBZ
    case COMPRESS_GZIP:
...
#ifdef HAVE_LIBZ
static bool decompress_with_zlib(JobControlRecord *jcr, const char *last_fname, char **data,
                                 uint32_t *length, bool sparse, bool with_header,
  ...
  while ((status = uncompress((Byte *)wbuf, &compress_len, (const Byte *)cbuf,
    ...
    jcr->compress.inflate_buffer_size =
        jcr->compress.inflate_buffer_size + (jcr->compress.inflate_buffer_size >> 1);
    jcr->compress.inflate_buffer =
        CheckPoolMemorySize(jcr->compress.inflate_buffer, jcr->compress.inflate_buffer_size);
...
#endif
#ifdef HAVE_LZO
static bool decompress_with_lzo(JobControlRecord *jcr, const char *last_fname, char **data,
                                uint32_t *length, bool sparse, bool want_data_stream) {
  char ec1[50]; /* Buffer printing huge values */
  ...
  real_compress_len = *length - sizeof(comp_stream_header);
  Dmsg2(400, "Comp_len=%d msglen=%d\n", compress_len, *length);
  while ((status = lzo1x_decompress_safe(cbuf, real_compress_len, wbuf, &compress_len, NULL)) ==
...
#ifdef HAVE_FASTLZ
static bool decompress_with_fastlz(JobControlRecord *jcr, const char *last_fname, char **data,
                                   uint32_t *length, uint32_t comp_magic, bool sparse,
                                   bool want_data_stream) {
  int zstat;
  zfast_stream stream;
...
bool DecompressData(JobControlRecord *jcr, const char *last_fname, int32_t stream, char **data,
                    uint32_t *length, bool want_data_stream) {
  ...
  switch (stream) {
    ...
    case STREAM_ENCRYPTED_WIN32_COMPRESSED_DATA: {
      ...
      switch (comp_magic) {
        ...
        case COMPRESS_GZIP:
          switch (stream) {
            case STREAM_SPARSE_COMPRESSED_DATA:
              return decompress_with_zlib(jcr, last_fname, data, length, true, true,
                                          want_data_stream);
            default:
              return decompress_with_zlib(jcr, last_fname, data, length, false, true,
                                          want_data_stream);
          }
#endif
#ifdef HAVE_LZO
        ...
        case COMPRESS_FZ4H:
          switch (stream) {
            case STREAM_SPARSE_COMPRESSED_DATA:
              return decompress_with_fastlz(jcr, last_fname, data, length, comp_magic, true,
                                            want_data_stream);
            default:
              return decompress_with_fastlz(jcr, last_fname, data, length, comp_magic, false,
                                            want_data_stream);
          }
#endif
        default:
    ...
    default:
      ...
      switch (stream) {
        ...
        default:
          return decompress_with_zlib(jcr, last_fname, data, length, false, false,
                                      want_data_stream);
      }
#else
...
#else
const char *cmprs_algo_to_text(uint32_t compression_algorithm) { return "Unknown"; }
