 ...
 * Connection
 */
Connection::Connection(const char *name, int protocol_version, BareosSocket *socket,
                       bool authenticated) {
  tid_ = pthread_self();
...
Connection *ConnectionPool::add_connection(const char *name, int fd_protocol_version,
                                           BareosSocket *socket, bool authenticated) {
...
bool ConnectionPool::exists(const char *name) { return (get_connection(name) != NULL); }
...
Connection *ConnectionPool::get_connection(const char *name) {
  ...
  foreach_alist(connection, connections_) {
    if (connection->check() && connection->authenticated() && connection->bsock() &&
        (!connection->in_use()) && bstrcmp(name, connection->name())) {
