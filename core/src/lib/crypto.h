...
typedef int(CRYPTO_TLS_PSK_SERVER_CB)(const char *identity, unsigned char *psk,
                                      unsigned int max_psk_len);

/** Client TLS-PSK callback */
typedef int(CRYPTO_TLS_PSK_CLIENT_CB)(char *identity, unsigned int max_identity_len,
...
DLL_IMP_EXP SIGNATURE *crypto_sign_decode(JobControlRecord *jcr, const uint8_t *sigData,
                                          uint32_t length);
...
DLL_IMP_EXP crypto_error_t CryptoSessionDecode(const uint8_t *data, uint32_t length,
                                               alist *keypairs, CRYPTO_SESSION **session);
DLL_IMP_EXP CRYPTO_SESSION *CryptoSessionDecode(const uint8_t *data, uint32_t length);
DLL_IMP_EXP CIPHER_CONTEXT *crypto_cipher_new(CRYPTO_SESSION *cs, bool encrypt,
                                              uint32_t *blocksize);
DLL_IMP_EXP bool CryptoCipherUpdate(CIPHER_CONTEXT *cipher_ctx, const uint8_t *data,
                                    uint32_t length, const uint8_t *dest, uint32_t *written);
...
DLL_IMP_EXP int CryptoKeypairLoadKey(X509_KEYPAIR *keypair, const char *file,
