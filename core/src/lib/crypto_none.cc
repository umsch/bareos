...
crypto_error_t CryptoSignGetDigest(SIGNATURE *sig, X509_KEYPAIR *keypair, crypto_digest_t &type,
                                   DIGEST **digest) {
...
int CryptoSignAddSigner(SIGNATURE *sig, DIGEST *digest, X509_KEYPAIR *keypair) { return false; }

int CryptoSignEncode(SIGNATURE *sig, uint8_t *dest, uint32_t *length) { return false; }
...
int CryptoKeypairLoadCert(X509_KEYPAIR *keypair, const char *file) { return false; }
...
bool CryptoSessionEncode(CRYPTO_SESSION *cs, uint8_t *dest, uint32_t *length) { return false; }

crypto_error_t CryptoSessionDecode(const uint8_t *data, uint32_t length, alist *keypairs,
...
bool CryptoCipherUpdate(CIPHER_CONTEXT *cipher_ctx, const uint8_t *data, uint32_t length,
                        const uint8_t *dest, uint32_t *written) {
  return false;
}
...
const char *crypto_digest_name(DIGEST *digest) { return crypto_digest_name(digest->type); }
#endif /* HAVE_CRYPTO */

#if !defined(HAVE_OPENSSL) && !defined(HAVE_GNUTLS) && !defined(HAVE_NSS)
/*
