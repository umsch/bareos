...
ASN1_SEQUENCE(SignerInfo) = {
    ...
    ASN1_SIMPLE(SignerInfo, signature, ASN1_OCTET_STRING)} ASN1_SEQUENCE_END(SignerInfo);
...
ASN1_SEQUENCE(CryptoData) = {
    ...
    ASN1_SET_OF(CryptoData, recipientInfo, RecipientInfo)} ASN1_SEQUENCE_END(CryptoData);
...
#define d2i_ASN1_SET_OF_SignerInfo(st, pp, length, d2i_func, free_func, ex_tag, ex_class)  \
  SKM_ASN1_SET_OF_d2i(SignerInfo, (st), (pp), (length), (d2i_func), (free_func), (ex_tag), \
                      (ex_class))
...
#define d2i_ASN1_SET_OF_RecipientInfo(st, pp, length, d2i_func, free_func, ex_tag, ex_class)  \
  SKM_ASN1_SET_OF_d2i(RecipientInfo, (st), (pp), (length), (d2i_func), (free_func), (ex_tag), \
                      (ex_class))
...
static ASN1_OCTET_STRING *openssl_cert_keyid(X509 *cert) {
  ...
  if (method->it) {
    ...
    keyid = (ASN1_OCTET_STRING *)ASN1_item_d2i(
        NULL, &ext_value_data, X509_EXTENSION_get_data(ext)->length, ASN1_ITEM_ptr(method->it));
  } else {
    /* Old style ASN1 */
...
#else
  keyid =
      (ASN1_OCTET_STRING *)method->d2i(NULL, &ext_value_data, X509_EXTENSION_get_data(ext)->length);
#endif
...
CRYPTO_SESSION *crypto_session_new(crypto_cipher_t cipher, alist *pubkeys) {
  ...
  foreach_alist(keypair, pubkeys) {
    ...
    if ((ekey_len =
             EVP_PKEY_encrypt(ekey, cs->session_key, cs->session_key_len, keypair->pubkey)) <= 0) {
      /* OpenSSL failure */
 ...
 *          NULL on failure.
 * Returns: CRYPTO_ERROR_NONE and a pointer to a newly allocated CRYPTO_SESSION structure in
 * *session on success. A crypto_error_t value on failure.
 */
  ...
  foreach_alist(keypair, keypairs) {
    ...
    for (i = 0; i < sk_RecipientInfo_num(recipients); i++) {
      ...
      if (M_ASN1_OCTET_STRING_cmp(keypair->keyid, ri->subjectKeyIdentifier) == 0) {
        ...
        cs->session_key_len =
            EVP_PKEY_decrypt(cs->session_key, M_ASN1_STRING_data(ri->encryptedKey),
 ...
 *  Returns: A pointer to a CIPHER_CONTEXT object on success. The cipher block size is returned in
 * blocksize. NULL on failure.
 */
...
static void OpensslUpdateDynamicMutex(int mode, struct CRYPTO_dynlock_value *dynlock,
...
static void OpensslDestroyDynamicMutex(struct CRYPTO_dynlock_value *dynlock, const char *file,
                                       int line) {
  int status;
