...
#define foreach_htable(var, tbl) \
  for ((var) = (typeof(var))((tbl)->first()); (var); (var) = (typeof(var))((tbl)->next()))
#else
#define foreach_htable(var, tbl)                               \
...
class DLL_IMP_EXP htable : public SmartAlloc {
 ...
 public:
  htable(void *item, void *link, int tsize = 31, int nr_pages = 0, int nr_entries = 4);
  ~htable() { destroy(); }
  void init(void *item, void *link, int tsize = 31, int nr_pages = 0, int nr_entries = 4);
