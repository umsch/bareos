 ...
 */
void RegisterJobEndCallback(JobControlRecord *jcr, void JobEndCb(JobControlRecord *jcr, void *),
                            void *ctx) {
...
void JobControlRecord::setJobStatus(int newJobStatus) {
  ...
  if (priority > old_priority || (priority == 0 && old_priority == 0)) {
    Dmsg4(800, "Set new stat. old: %c,%d new: %c,%d\n", oldJobStatus, old_priority, newJobStatus,
          priority);
  ...
  foreach_jcr(jcr) {
    ...
    if (bs) {
      ...
      if (timer_start && (watchdog_time - timer_start) > watch_dog_timeout) {
        ...
        bs->SetTimedOut();
        Qmsg(jcr, M_ERROR, 0,
             _("Watchdog sending kill after %d secs to thread stalled reading Director.\n"),
             watchdog_time - timer_start);
        jcr->MyThreadSendSignal(TIMEOUT_SIGNAL);
      }
    }
...
void DbgPrintJcr(FILE *fp) {
  ...
  for (JobControlRecord *jcr = (JobControlRecord *)jcrs->first(); jcr;
       jcr = (JobControlRecord *)jcrs->next(jcr)) {
    fprintf(fp, "threadid=%s JobId=%d JobStatus=%c jcr=%p name=%s\n",
            edit_pthread(jcr->my_thread_id, ed1, sizeof(ed1)), (int)jcr->JobId, jcr->JobStatus, jcr,
            jcr->Job);
