 ...
 */
static inline LEX *lex_add(LEX *lf, const char *filename, FILE *fd, Bpipe *bpipe,
                           LEX_ERROR_HANDLER *ScanError, LEX_WARNING_HANDLER *scan_warning) {
  LEX *nf;
 ...
 * the next field.
 */
LEX *lex_open_file(LEX *lf, const char *filename, LEX_ERROR_HANDLER *ScanError,
  ...
  if (filename[0] == '|') {
    ...
    /*
     * Flag GLOB_NOMAGIC is a GNU extension, therefore manually check if string is a wildcard
     * string.
     */
...
LEX *lex_new_buffer(LEX *lf, LEX_ERROR_HANDLER *ScanError, LEX_WARNING_HANDLER *scan_warning) {
  ...
  switch (expect) {
    ...
    case BCT_PINT32_RANGE:
      if (token == BCT_NUMBER) {
        ...
        if (!p) {
          scan_err2(lf, _("expected an integer or a range, got %s: %s"), lex_tok_to_str(token),
                    lf->str);
          token = BCT_ERROR;
          break;
        }
    ...
    case BCT_INT16:
      if (token != BCT_NUMBER || !Is_a_number(lf->str)) {
        scan_err2(lf, _("expected an integer number, got %s: %s"), lex_tok_to_str(token), lf->str);
        token = BCT_ERROR;
        break;
      }
      errno = 0;
    ...
    case BCT_NAME:
      if (token != BCT_IDENTIFIER && token != BCT_UNQUOTED_STRING && token != BCT_QUOTED_STRING) {
        scan_err2(lf, _("expected a name, got %s: %s"), lex_tok_to_str(token), lf->str);
        token = BCT_ERROR;
    ...
    case BCT_STRING:
      if (token != BCT_IDENTIFIER && token != BCT_UNQUOTED_STRING && token != BCT_QUOTED_STRING) {
        scan_err2(lf, _("expected a string, got %s: %s"), lex_tok_to_str(token), lf->str);
        token = BCT_ERROR;
      } else {
        token = BCT_STRING;
      }
