...
class lmgr_thread_t : public SmartAlloc {
 public:
  ...
  void _dump(FILE *fp) {
    ...
    fprintf(fp, "threadid=%s max=%i current=%i\n", edit_pthread(thread_id, ed1, sizeof(ed1)), max,
            current);

    for (int i = 0; i <= current; i++) {
      fprintf(fp, "   lock=%p state=%s priority=%i %s:%i\n", lock_list[i].lock,
              (lock_list[i].state == 'W') ? "Wanted " : "Granted", lock_list[i].priority,
   ...
   */
  virtual void pre_P(void *m, int priority, const char *f = "*unknown*", int l = 0) {
  ...
  virtual void do_V(void *m, const char *f = "*unknown*", int l = 0) {
    ...
    Lmgr_p(&mutex);
    {
      if (lock_list[current].lock == m) {
        ...
        FPmsg4(0, "ERROR: wrong P/V order pos=%i lock=%p %s:%i\n", current, lock_list[current].lock,
               lock_list[current].file, lock_list[current].line);
...
bool LmgrDetectDeadlock() {
  ...
  {
    lmgr_thread_t *item;
    foreach_dlist(item, global_mgr) { Lmgr_p(&item->mutex); }

    ret = LmgrDetectDeadlockUnlocked();

    foreach_dlist(item, global_mgr) { Lmgr_v(&item->mutex); }
  }
  Lmgr_v(&lmgr_global_mutex);
...
void DbgPrintLock(FILE *fp) {
  ...
  lmgr_thread_t *item;
  foreach_dlist(item, global_mgr) { item->_dump(fp); }
}
...
void LmgrDump() {
  ...
  {
    lmgr_thread_t *item;
    foreach_dlist(item, global_mgr) { item->dump(stderr); }
  }
  Lmgr_v(&lmgr_global_mutex);
}
 ...
 */
int pthread_mutex_destroy(bthread_mutex_t *m) { return pthread_mutex_destroy(&m->mutex); }
...
/* TODO: check this
 */
int BthreadCondWait_p(pthread_cond_t *cond, pthread_mutex_t *m, const char *file, int line) {
...
/* TODO: check this
 */
int BthreadCondTimedwait_p(pthread_cond_t *cond, pthread_mutex_t *m, const struct timespec *abstime,
...
/* TODO: check this
 */
int BthreadCondWait_p(pthread_cond_t *cond, bthread_mutex_t *m, const char *file, int line) {
...
/* TODO: check this
 */
int BthreadCondTimedwait_p(pthread_cond_t *cond, bthread_mutex_t *m, const struct timespec *abstime,
                           const char *file, int line) {
  int ret;
  lmgr_thread_t *self = lmgr_get_thread_info();
  self->do_V(m, file, line);
