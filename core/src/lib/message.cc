...
DLL_IMP_EXP job_code_callback_t message_job_code_callback =
 ...
 * email addresses, ...
 */
void AddMsgDest(MessagesResource *msg, int dest_code, int msg_type, char *where, char *mail_cmd,
  ...
  Dmsg6(850, "add new d=%p msgtype=%d destcode=%d where=%s mailcmd=%s timestampformat=%s\n", d,
        msg_type, dest_code, NPRT(where), NPRT(d->mail_cmd), NPRT(d->timestamp_format));
  msg->dest_chain = d;
}
...
static void MakeUniqueMailFilename(JobControlRecord *jcr, POOLMEM *&name, DEST *d) {
  ...
  if (jcr) {
    Mmsg(name, "%s/%s.%s.%d.mail", working_directory, my_name, jcr->Job, (int)(intptr_t)d);
  } else {
    Mmsg(name, "%s/%s.%s.%d.mail", working_directory, my_name, my_name, (int)(intptr_t)d);
  }
      ...
      switch (d->dest_code) {
        ...
        case MD_DIRECTOR:
          ...
          if (jcr && jcr->dir_bsock && !jcr->dir_bsock->errors) {
            jcr->dir_bsock->fsend("Jmsg Job=%s type=%d level=%lld %s", jcr->Job, type, mtime, msg);
          } else {
...
#ifdef FULL_LOCATION
  if (level >= 0) {
    len = Bsnprintf(buf, sizeof(buf), "%s: %s:%d-%u ", my_name, get_basename(file), line,
                    GetJobidFromTsd());
  }
#endif
...
void e_msg(const char *file, int line, int type, int level, const char *fmt, ...) {
   ...
   */
  if (!daemon_msgs ||
      ((type != M_ABORT && type != M_ERROR_TERM) && !BitIsSet(type, daemon_msgs->SendMsg))) {
 ...
 */
void j_msg(const char *file, int line, JobControlRecord *jcr, int type, utime_t mtime,
           const char *fmt, ...) {
  va_list ap;
  int len, maxlen;
  PoolMem buf(PM_EMSG), more(PM_EMSG);
...
void DequeueMessages(JobControlRecord *jcr) {
  ...
  foreach_dlist(item, jcr->msg_queue) { Jmsg(jcr, item->type, item->mtime, "%s", item->msg); }
 ...
 */
void q_msg(const char *file, int line, JobControlRecord *jcr, int type, utime_t mtime,
           const char *fmt, ...) {
  va_list ap;
  int len, maxlen;
  PoolMem buf(PM_EMSG), more(PM_EMSG);
 ...
 */
void SetLogTimestampFormat(const char *format) { log_timestamp_format = format; }
