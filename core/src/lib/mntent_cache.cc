 ...
 */
static mntent_cache_entry_t *add_mntent_mapping(uint32_t dev, const char *special,
 ...
 * This function should be called with a write lock on the mntent_cache.
 */
static mntent_cache_entry_t *update_mntent_mapping(uint32_t dev, const char *special,
                                                   const char *mountpoint, const char *fstype,
                                                   const char *mntopts) {
...
static void refresh_mount_cache(
    mntent_cache_entry_t *handle_entry(uint32_t dev, const char *special, const char *mountpoint,
  ...
  while ((mnt = getmntent(fp)) != (struct mntent *)NULL) {
    ...
    handle_entry(st.st_dev, mnt->mnt_fsname, mnt->mnt_dir, mnt->mnt_type, mnt->mnt_opts);
  }
  ...
  while (getmntent(fp, &mnt) == 0) {
    ...
    handle_entry(st.st_dev, mnt.mnt_special, mnt.mnt_mountp, mnt.mnt_fstype, mnt.mnt_mntopts);
  }
  ...
  if ((cnt = getmntinfo(&mntinfo, flags)) > 0) {
    while (cnt > 0) {
      if (!SkipFstype(mntinfo->f_fstypename) && stat(mntinfo->f_mntonname, &st) == 0) {
        handle_entry(st.st_dev, mntinfo->f_mntfromname, mntinfo->f_mntonname, mntinfo->f_fstypename,
                     NULL);
      }
      mntinfo++;
      cnt--;
  ...
  while (cnt < n_entries) {
    ...
    handle_entry(st.st_dev, current->f_mntfromname, current->f_mntonname, current->f_fstypename,
                 NULL);
    current++;
    cnt++;
  }
...
mntent_cache_entry_t *find_mntent_mapping(uint32_t dev) {
  ...
  if (!mce) {
    RepopulateMntentCache();
    mce =
        (mntent_cache_entry_t *)mntent_cache_entries->binary_search(&lookup, CompareMntentMapping);
  }
