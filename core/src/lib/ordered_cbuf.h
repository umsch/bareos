...
enum oc_peek_types { PEEK_FIRST = 0, PEEK_LAST, PEEK_LIST, PEEK_CLONE };
...
class ordered_circbuf : public SmartAlloc {
 ...
 public:
  ...
  void destroy();
  void *enqueue(void *data, uint32_t data_size, int compare(void *item1, void *item2),
                void update(void *item1, void *item2), bool use_reserved_slot = false,
                bool no_signal = false);
  void *dequeue(bool reserve_slot = false, bool requeued = false, struct timespec *ts = NULL,
                int timeout = 300);
  void *peek(enum oc_peek_types type, void *data, int callback(void *item1, void *item2));
  int unreserve_slot();
  int flush();
