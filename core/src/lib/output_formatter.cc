...
OutputFormatter::OutputFormatter(SEND_HANDLER *send_func_arg, void *send_ctx_arg,
                                 FILTER_HANDLER *filter_func_arg, void *filter_ctx_arg,
                                 int api_mode) {
  InitializeJson();
    ...
    case API_MODE_JSON:
      ...
      if (name == NULL) {
        ...
        if (json_is_array(json_object_current)) {
           ...
           * We do a workaround here, but this will only keep the last added entry (others will be
           * overwritten).
           */
          Dmsg0(800,
                "Warning: requested to add a nameless object to another object. This does not "
                "match.\n");
...
void OutputFormatter::ObjectKeyValueBool(const char *key, bool value, const char *value_fmt) {
...
void OutputFormatter::ObjectKeyValueBool(const char *key, const char *key_fmt, bool value,
...
void OutputFormatter::ObjectKeyValue(const char *key, uint64_t value, const char *value_fmt) {
...
void OutputFormatter::ObjectKeyValue(const char *key, const char *key_fmt, uint64_t value,
                                     const char *value_fmt) {
  PoolMem string;
...
void OutputFormatter::ObjectKeyValue(const char *key, const char *value, const char *value_fmt,
                                     int wrap) {
  ObjectKeyValue(key, NULL, value, value_fmt, wrap);
}

void OutputFormatter::ObjectKeyValue(const char *key, const char *key_fmt, const char *value,
