...
class DLL_IMP_EXP OutputFormatter : public SmartAlloc {
 ...
 public:
   ...
   * Methods
   */
  OutputFormatter(SEND_HANDLER *send_func, void *send_ctx, FILTER_HANDLER *filter_func,
                  void *filter_ctx, int api_mode = API_MODE_OFF);
  ~OutputFormatter();
  ...
  void ObjectKeyValue(const char *key, const char *key_fmt, const char *value,
                      const char *value_fmt, int wrap = -1);
