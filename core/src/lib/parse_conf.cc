...
void ConfigurationParser::init(const char *cf, LEX_ERROR_HANDLER *ScanError,
                               LEX_WARNING_HANDLER *scan_warning, INIT_RES_HANDLER *init_res,
                               STORE_RES_HANDLER *StoreRes, PRINT_RES_HANDLER *print_res,
                               int32_t err_type, void *vres_all, int32_t res_all_size,
                               int32_t r_first, int32_t r_last, ResourceTable *resources,
...
bool ConfigurationParser::ParseConfigFile(const char *cf, void *caller_ctx,
      ...
      switch (state) {
        ...
        case p_resource:
          switch (token) {
            ...
            case BCT_IDENTIFIER:
              ...
              if (i >= 0) {
                ...
                if (item->flags & CFG_ITEM_DEPRECATED) {
                  scan_warn2(lc, _("using deprecated keyword %s on line %d"), item->name,
                             lc->line_no);
                  /*
                   * As we only want to warn we continue parsing the config. So no goto bail_out
                   * here.
                   */
                }
                ...
                if (!StoreResource(item->type, lc, item, i, pass)) {
                  /*
                   * None of the generic types fired if there is a registered callback call that
                   * now.
                   */
                  if (store_res_) {
            ...
            default:
              scan_err2(lc, _("unexpected token %d %s in resource definition"), token,
                        lex_tok_to_str(token));
              goto bail_out;
          }
          break;
...
const char *ConfigurationParser::get_resource_type_name(int code) { return res_to_str(code); }
...
#ifdef HAVE_SETENV
static inline void set_env(const char *key, const char *value) { setenv(key, value, 1); }
#elif HAVE_PUTENV
...
bool ConfigurationParser::GetConfigFile(PoolMem &full_path, const char *config_dir,
...
bool ConfigurationParser::FindConfigPath(PoolMem &full_path) {
  ...
  if (!cf_) {
    ...
    if (!found) {
      Jmsg2(NULL, M_ERROR, 0, _("Failed to find configuration files at \"%s\" and \"%s\".\n"), cf_,
            full_path.c_str());
    }
  } else {
 ...
 */
void ConfigurationParser::InitResource(int type, ResourceItem *items, int pass,
    ...
    case 1: {
      ...
      for (i = 0; items[i].name; i++) {
        ...
        if (items[i].default_value != NULL && !(items[i].flags & CFG_ITEM_DEFAULT)) {
          Pmsg1(
              000,
              _("Found config item %s which has default value but no CFG_ITEM_DEFAULT flag set\n"),
              items[i].name);
...
bool ConfigurationParser::RemoveResource(int type, const char *name) {
   ...
   * For a general approach, a check if this resource is referenced by other resources must be
   * added. If it is referenced, don't remove it.
   */
  last = NULL;
...
void ConfigurationParser::DumpResources(void sendit(void *sock, const char *fmt, ...), void *sock,
...
bool ConfigurationParser::GetPathOfNewResource(PoolMem &path, PoolMem &extramsg,
                                               const char *component, const char *resourcetype,
                                               const char *name, bool error_if_exists,
