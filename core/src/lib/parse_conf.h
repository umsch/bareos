 ...
 * Parser state
 */
enum parse_state { p_none, p_resource };

/*
 * Password encodings.
 */
enum password_encoding { p_encoding_clear, p_encoding_md5 };
  ...
  {"TlsVerifyPeer",                                                                            \
   ...
   "If enabled, the CN of a certificate must the Address or in the \"TLS Allowed CN\" list."}, \
      ...
      {"TlsKey",                                                                               \
       ...
       NULL,                                                                                   \
       NULL,                                                                                   \
       "Path of a PEM encoded private key. It must correspond to the specified \"TLS "         \
       "Certificate\"."},                                                                      \
  {                                                                                            \
...
typedef void(PRINT_RES_HANDLER)(ResourceItem *items, int i, PoolMem &cfg_str,
                                bool hide_sensitive_data, bool inherited);
...
class DLL_IMP_EXP ConfigurationParser {
 public:
   ...
   * Methods
   */
  void init(const char *cf, LEX_ERROR_HANDLER *ScanError, LEX_WARNING_HANDLER *scan_warning,
            INIT_RES_HANDLER *init_res, STORE_RES_HANDLER *StoreRes, PRINT_RES_HANDLER *print_res,
            int32_t err_type, void *vres_all, int32_t res_all_size, int32_t r_first, int32_t r_last,
            ResourceTable *resources, CommonResourceHeader **res_head);
  ...
  void InitResource(int type, ResourceItem *items, int pass,
                    std::function<void *(void *res)> initres);
  bool RemoveResource(int type, const char *name);
  void DumpResources(void sendit(void *sock, const char *fmt, ...), void *sock,
 ...
 protected:
  const char *
      config_default_filename_; /* default config filename, that is used, if no filename is given */
...
DLL_IMP_EXP void DumpResource(int type, CommonResourceHeader *res,
                              void sendmsg(void *sock, const char *fmt, ...), void *sock,
                              bool hide_sensitive_data = false, bool verbose = false);
DLL_IMP_EXP void IndentConfigItem(PoolMem &cfg_str, int level, const char *config_item,
