...
char *generate_crypto_passphrase(uint16_t length) {
  ...
  if (RAND_bytes(rand_bytes, length) != 1) {
    ...
    error = ERR_get_error();
    Emsg1(M_ERROR, 0, _("Failed to get random bytes from RAND_bytes for passphrase: ERR=%s\n"),
  ...
  if (error != GNUTLS_E_SUCCESS) {
    Emsg1(M_ERROR, 0, _("Failed to get random bytes from gnutls_rnd for passphrase: ERR=%s\n"),
