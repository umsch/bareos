 ...
 */
static bool load_a_plugin(void *binfo, void *bfuncs, const char *plugin_pathname,
                          const char *plugin_name, const char *type, alist *plugin_list,
 ...
 */
bool LoadPlugins(void *binfo, void *bfuncs, alist *plugin_list, const char *plugin_dir,
                 alist *plugin_names, const char *type, bool IsPluginCompatible(Plugin *plugin)) {
  struct stat statp;
  bool found = false;
  ...
  if (plugin_names && plugin_names->size()) {
    ...
    foreach_alist(name, plugin_names) {
       ...
       */
      if (load_a_plugin(binfo, bfuncs, fname.c_str(), plugin_name.c_str(), type, plugin_list,
                        IsPluginCompatible)) {
        found = true;
      }
    }
    ...
    while (1) {
       ...
       * Try to load the plugin and resolve the wanted symbols.
       */
      if (load_a_plugin(binfo, bfuncs, fname.c_str(), result->d_name, type, plugin_list,
                        IsPluginCompatible)) {
        found = true;
      }
    }
...
void DbgPrintPluginAddHook(dbg_print_plugin_hook_t *fct) { dbg_print_plugin_hook = fct; }
