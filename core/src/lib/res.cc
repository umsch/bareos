 ...
 */
static void scan_types(LEX *lc, MessagesResource *msg, int dest_code, char *where, char *cmd,
...
static void StoreName(LEX *lc, ResourceItem *item, int index, int pass) {
   ...
   */
  if (*(item->value)) {
    scan_err2(lc, _("Attempt to redefine name \"%s\" to \"%s\"."), *(item->value), lc->str);
    return;
  }
...
static void StoreRes(LEX *lc, ResourceItem *item, int index, int pass) {
  ...
  if (pass == 2) {
    ...
    if (res == NULL) {
      scan_err3(lc, _("Could not find config resource \"%s\" referenced on line %d: %s"), lc->str,
                lc->line_no, lc->line);
      return;
    }
    if (*(item->resvalue)) {
      scan_err3(lc, _("Attempt to redefine resource \"%s\" referenced on line %d: %s"), item->name,
                lc->line_no, lc->line);
      return;
    }
    *(item->resvalue) = res;
 ...
 * Unit types
 */
enum unit_type { STORE_SIZE, STORE_SPEED };

/*
 * Store a size in bytes
 */
static void store_int_unit(LEX *lc, ResourceItem *item, int index, int pass, bool size32,
                           enum unit_type type) {
  int token;
  uint64_t uvalue;
  char bsize[500];
  ...
  switch (token) {
    ...
    case BCT_UNQUOTED_STRING:
      ...
      break;
    default:
      scan_err2(lc, _("expected a %s, got: %s"), (type == STORE_SIZE) ? _("size") : _("speed"),
                lc->str);
      return;
  }
  if (token != BCT_EOL) {
...
static void StoreAddresses(LEX *lc, ResourceItem *item, int index, int pass) {
  ...
  enum { EMPTYLINE = 0x0, PORTLINE = 0x1, ADDRLINE = 0x2 } next_line = EMPTYLINE;
  ...
  do {
    ...
    if (pass == 1 && !AddAddress(item->dlistvalue, IPADDR::R_MULTIPLE, htons(port), family,
                                 hostname_str, port_str, errmsg, sizeof(errmsg))) {
      scan_err3(lc, _("Can't add hostname(%s) and port(%s) to addrlist (%s)"), hostname_str,
                port_str, errmsg);
    }
...
static void StoreAddressesAddress(LEX *lc, ResourceItem *item, int index, int pass) {
  ...
  if (pass == 1 && !AddAddress(item->dlistvalue, IPADDR::R_SINGLE_ADDR, htons(port), AF_INET,
...
static void StoreAddressesPort(LEX *lc, ResourceItem *item, int index, int pass) {
  ...
  if (pass == 1 && !AddAddress(item->dlistvalue, IPADDR::R_SINGLE_PORT, htons(port), AF_INET, 0,
...
static inline void PrintConfigTime(ResourceItem *item, PoolMem &cfg_str, bool inherited) {
  ...
  static const int32_t multiplier[] = {
      60 * 60 * 24 * 365, 60 * 60 * 24 * 30, 60 * 60 * 24 * 7, 60 * 60 * 24, 60 * 60, 60, 1, 0};
...
static inline bool HasDefaultValue(ResourceItem *item) {
  ...
  if (item->flags & CFG_ITEM_DEFAULT) {
    ...
    switch (item->type) {
      ...
      case CFG_TYPE_BOOL: {
        bool default_value =
            Bstrcasecmp(item->default_value, "true") || Bstrcasecmp(item->default_value, "yes");
