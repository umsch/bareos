...
bool ClearScsiEncryptionKey(int fd, const char *device_name) {
   ...
   */
  return send_scsi_cmd_page(fd, device_name, (void *)&cdb, cdb_len, (void *)&cmd_page,
                            cmd_page_len);
}
 ...
 */
int GetScsiDriveEncryptionStatus(int fd, const char *device_name, POOLMEM *&status, int indent) {
  ...
  if (spd->RDMD) {
    ...
    IndentStatusMsg(status, _("Raw Decryption Mode Disabled (RDMD): Disabled\n"), indent + 3);
  }
  ...
  switch (spd->CEEMS) {
    case SPP_CEEM_NO_ENCR_CHECK:
      IndentStatusMsg(status, _("Check External Encryption Mode Status (CEEMS) : No\n"),
                      indent + 3);
      break;
    case SPP_CEEM_CHECK_EXTERNAL:
      IndentStatusMsg(status, _("Check External Encryption Mode Status (CEEMS) : External\n"),
                      indent + 3);
      break;
    case SPP_CEEM_CHECK_ENCR:
      IndentStatusMsg(status, _("Check External Encryption Mode Status (CEEMS) : Encrypt\n"),
                      indent + 3);
      break;
    default:
      break;
   ...
   */
  if (spd->VCELB) {
    IndentStatusMsg(status, _("Volume Contains Encrypted Logical Blocks (VCELB): Enabled\n"),
                    indent + 3);
  } else {
    IndentStatusMsg(status, _("Volume Contains Encrypted Logical Blocks (VCELB): Disabled\n"),
                    indent + 3);
  }
  ...
  switch (spd->parametersControl) {
    case SPP_PARM_LOG_BLOCK_ENCR_NONE:
      IndentStatusMsg(status, _("Logical Block encryption parameters: No report\n"), indent + 3);
      break;
    case SPP_PARM_LOG_BLOCK_ENCR_AME:
      IndentStatusMsg(status, _("Logical Block encryption parameters: Application Managed\n"),
                      indent + 3);
      break;
    case SPP_PARM_LOG_BLOCK_ENCR_DRIVE:
      IndentStatusMsg(status, _("Logical Block encryption parameters: Drive Managed\n"),
                      indent + 3);
      break;
    ...
    case SPP_PARM_LOG_BLOCK_UNSUP:
      IndentStatusMsg(status, _("Logical Block encryption parameters: Unsupported\n"), indent + 3);
      break;
    default:
      break;
  }
  ...
  if (spd->encryptionMode != SPP_ENCR_MODE_DISABLE &&
    ...
    switch (spd->kadFormat) {
      case SPP_KAD_KEY_FORMAT_NORMAL:
        IndentStatusMsg(status, _("Key Associated Data (KAD) Descriptor: Normal key\n"),
                        indent + 3);
        break;
      ...
      case SPP_KAD_KEY_FORMAT_WRAPPED:
        IndentStatusMsg(status, _("Key Associated Data (KAD) Descriptor: Wrapped public key\n"),
                        indent + 3);
        break;
      case SPP_KAD_KEY_FORMAT_ESP_SCSI:
        IndentStatusMsg(status, _("Key Associated Data (KAD) Descriptor: Key using ESP-SCSI\n"),
                        indent + 3);
        break;
      default:
        break;
 ...
 * The return data is interpreted and a status report is build.
 */
int GetScsiVolumeEncryptionStatus(int fd, const char *device_name, POOLMEM *&status, int indent) {
  ...
  if (spnb->RDMDS) {
    IndentStatusMsg(status, _("Raw Decryption Mode Disabled Status (RDMDS): Enabled\n"),
                    indent + 3);
  } else {
    IndentStatusMsg(status, _("Raw Decryption Mode Disabled Status (RDMDS): Disabled\n"),
                    indent + 3);
  }

  if (spnb->EMES) {
    IndentStatusMsg(status, _("Encryption Mode External Status (EMES): Enabled\n"), indent + 3);
  } else {
    IndentStatusMsg(status, _("Encryption Mode External Status (EMES): Disabled\n"), indent + 3);
  }
  ...
  if (spnb->encryptionStatus == SPP_ENCR_STATUS_ENCRYPTED) {
    switch (spnb->nextBlockKADFormat) {
      case SPP_KAD_KEY_FORMAT_NORMAL:
        IndentStatusMsg(status, _("Next Block Key Associated Data (KAD) Descriptor: Normal key\n"),
                        indent + 3);
        break;
      case SPP_KAD_KEY_FORMAT_REFERENCE:
        IndentStatusMsg(
...
bool SetScsiEncryptionKey(int fd, const char *device_name, char *encryption_key) { return false; }
...
bool NeedScsiCryptoKey(int fd, const char *device_name, bool use_drive_status) { return false; }
