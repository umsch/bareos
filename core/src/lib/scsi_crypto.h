 ...
 * SCSI CDB opcodes
 */
enum { SCSI_SPIN_OPCODE = 0xa2, SCSI_SPOUT_OPCODE = 0xb5 };
...
enum {
  ...
  SPP_NEXUS_SC_ALL_I_T_NEXUS =
      2 /* The data encryption parameters shall be shared with all I_T nexuses. */
};
...
enum {
  ...
  SPP_ENCR_MODE_EXTERNAL = 1, /* The data associated with the WRITE(6) and WRITE(16) commands has
...
enum {
  ...
  SPP_DECR_MODE_DECRYPT = 2, /* The device server shall decrypt all data that is read from the
 ...
 */
enum {
  SPP_KAD_KEY_FORMAT_NORMAL =
      0, /* The KEY field contains the key to be used to encrypt or decrypt data. */
  SPP_KAD_KEY_FORMAT_REFERENCE = 1, /* The KEY field contains a vendor-specific key reference. */
  SPP_KAD_KEY_FORMAT_WRAPPED =
      2, /* The KEY field contains the key wrapped by the device server public key. */
...
enum {
  ...
  SPP_COMP_STATUS_UNAVAIL = 1, /* The device server is capable of determining if the logical
                                  ...
                                  Possible reasons are:
                                     a) the next logical block has not yet been read into the
                                  buffer; b) there was an error reading the next logical block; or
  ...
  SPP_COMP_STATUS_COMPRESSED = 4 /* The device server has determined that the logical object
                                    referenced by the LOGICAL OBJECT NUMBER field is compressed. */
};
...
enum {
  ...
  SPP_ENCR_STATUS_ENCR_ALG_NOT_SUPP =
      4, /* The device server has determined that the logical object referenced by
  ...
  SPP_ENCR_STATUS_ENCRYPTED = 5, /* The device server has determined that the logical object
                                    referenced by the LOGICAL OBJECT NUMBER field is encrypted by an
                                    algorithm that is supported by this device server. The values in
                                    the ALGORITHM INDEX and KEY-ASSOCIATED DATA DESCRIPTORS fields
                                    contain information pertaining to the encrypted block. */
  SPP_ENCR_STATUS_ENCR_NOT_AVAIL =
      6 /* The device server has determined that the logical object referenced by
           the LOGICAL OBJECT NUMBER field is encrypted by an algorithm that is
...
DLL_IMP_EXP int GetScsiDriveEncryptionStatus(int fd, const char *device_name, POOLMEM *&status,
                                             int indent);
DLL_IMP_EXP int GetScsiVolumeEncryptionStatus(int fd, const char *device_name, POOLMEM *&status,
                                              int indent);
