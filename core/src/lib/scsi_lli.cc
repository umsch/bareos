 ...
 */
static inline bool do_scsi_cmd_page(int fd, const char *device_name, void *cdb,
                                    unsigned int cdb_len, void *cmd_page, unsigned int cmd_page_len,
                                    int direction) {
  int rc;
  sg_io_hdr_t io_hdr;
 ...
 */
bool RecvScsiCmdPage(int fd, const char *device_name, void *cdb, unsigned int cdb_len,
 ...
 */
bool send_scsi_cmd_page(int fd, const char *device_name, void *cdb, unsigned int cdb_len,
                        void *cmd_page, unsigned int cmd_page_len) {
...
static inline bool do_scsi_cmd_page(int fd, const char *device_name, void *cdb,
  ...
  if (rc != 0) {
    ...
    Emsg3(M_ERROR, 0, _("Sense Key: %0.2X ASC: %0.2X ASCQ: %0.2X\n"), LOBYTE(sense.senseKey),
          sense.addSenseCode, sense.addSenseCodeQual);
    goto bail_out;
  }
...
static inline bool do_scsi_cmd_page(int fd, const char *device_name, void *cdb,
  ...
  if (cam_send_ccb(cam_dev, ccb) < 0) {
    Emsg2(M_ERROR, 0, _("Failed to send ccb to device %s: %s\n"), device_name,
          cam_error_string(cam_dev, ccb, errbuf, sizeof(errbuf), CAM_ESF_ALL, CAM_EPF_ALL));
    cam_freeccb(ccb);
    goto bail_out;
  }
