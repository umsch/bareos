 ...
 * SCSI CDB opcodes
 */
enum { SCSI_LOG_OPCODE = 0x4d };

enum { SCSI_TAPE_ALERT_FLAGS = 0x2e };
...
typedef struct {
  ...
  uint8_t disable_update : 1; /* DU: Will be set to 0. The library will always update values
                                 reflected by the log parameters. */
  uint8_t disable_save : 1;   /* DS: Will be set to 1. The library does not support saving of log
                                 parameters. */
  uint8_t target_save_disabled : 1; /* TSD: Will be set to 0. The library provides a self-defined
                                       method for saving log parameters. */
  uint8_t enable_threshold_comparison : 1; /* ETC: Will be set to 0. No comparison to threshold
                                              values is made. */
  uint8_t threshold_met_criteria : 2; /* TMC: Will be set to 0. Comparison to threshold values is
                                         not supported. */
  uint8_t list_parameter_binary : 1;  /* LBIN: This field is only valid if LP is set to 1. When LBIN
                                       * is set to 0, the list  parameter is ASCII. When LBIN is set
                                       * to 1, the list parameter is a binary value. */
  uint8_t list_parameter : 1; /* LP: This field will be set to 0 for data counters and set to 1 for
                                 list parameters */
#else
  uint8_t list_parameter : 1; /* LP: This field will be set to 0 for data counters and set to 1 for
                                 list parameters */
...
static tapealert_mapping tapealert_mappings[] = {
     ...
     */
    {0x28,
     (const char *)"Loader Hardware A: Changer having problems communicating with tape drive"},
