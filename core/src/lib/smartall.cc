...
void sm_free(const char *file, int line, void *fp) {
  ...
  if (((unsigned char *)cp)[head->ablen - 1] != ((((intptr_t)cp) & 0xFF) ^ 0xC5)) {
    ...
    SmartAllocMsg(__FILE__, __LINE__,
                  _("Overrun buffer: len=%d addr=%p allocated: %s:%d called from %s:%d\n"),
...
void *actuallycalloc(unsigned int nelem, unsigned int elsize) { return calloc(nelem, elsize); }
...
void *actuallyrealloc(void *ptr, unsigned int size) { return realloc(ptr, size); }
    ...
    if (bad) {
      ...
      if (ap->abfname != NULL) {
        ...
        FPmsg4(0, _("Damaged buffer:  %6u bytes allocated at line %d of %s %s\n"), memsize,
               ap->ablineno, my_name, get_basename(ap->abfname));
        if (bufdump) {
          unsigned llen = 0;
          char *cp = ((char *)ap) + HEAD_SIZE;
