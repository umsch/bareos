...
TLS_CONTEXT *new_tls_context(const char *CaCertfile, const char *CaCertdir, const char *crlfile,
  ...
  if (CaCertfile) {
    error =
        gnutls_certificate_set_x509_trust_file(ctx->gnutls_cred, CaCertfile, GNUTLS_X509_FMT_PEM);
    if (error < GNUTLS_E_SUCCESS) {
      error =
          gnutls_certificate_set_x509_trust_file(ctx->gnutls_cred, CaCertfile, GNUTLS_X509_FMT_DER);
      if (error < GNUTLS_E_SUCCESS) {
  ...
  /*
   * Try to load the certificate and the keyfile, first in PEM format and if that fails in DER
   * format.
   */
  if (certfile && keyfile) {
    error = gnutls_certificate_set_x509_key_file(ctx->gnutls_cred, certfile, keyfile,
                                                 GNUTLS_X509_FMT_PEM);
    if (error != GNUTLS_E_SUCCESS) {
      error = gnutls_certificate_set_x509_key_file(ctx->gnutls_cred, certfile, keyfile,
                                                   GNUTLS_X509_FMT_DER);
...
bool TlsPostconnectVerifyCn(JobControlRecord *jcr, TLS_CONNECTION *tls_conn, alist *verify_list) {
  ...
  for (cnt = 0; error >= 0; cnt++) {
    ...
    error = gnutls_x509_crt_get_dn_by_oid(cert, GNUTLS_OID_X520_COMMON_NAME, cnt, 0, cannonicalname,
                                          &cn_length);
    if (error < 0) {
      break;
    }
