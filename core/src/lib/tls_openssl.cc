...
class TlsConnection {
 ...
 public:
  ...
  TlsConnection(std::shared_ptr<TlsContext> tls_ctx, int fd,
...
static X509_LOOKUP_METHOD *X509_LOOKUP_crl_reloader(void) { return (&x509_crl_reloader); }
...
static std::shared_ptr<TLS_CONTEXT> new_tls_context(const char *CaCertfile, const char *CaCertdir,
  ...
  if (CaCertfile || CaCertdir) {
    ...
    Jmsg0(NULL, M_ERROR, 0,
          _("Either a certificate file or a directory must be"
  ...
  if (VerifyPeer) {
     ...
     */
    SSL_CTX_set_verify(ctx->openssl, SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT,
                       OpensslVerifyPeer);
  } else {
    SSL_CTX_set_verify(ctx->openssl, SSL_VERIFY_NONE, NULL);
  }
...
std::shared_ptr<TLS_CONTEXT> TlsCert::CreateClientContext(
  ...
  return new_tls_context(
      (!CaCertfile || CaCertfile->empty()) ? nullptr : CaCertfile->c_str(),
      ...
      (!keyfile || keyfile->empty()) ? nullptr : keyfile->c_str(), TlsPemCallback,
      (!pem_message || pem_message->empty()) ? nullptr : pem_message->c_str(), nullptr,
      (!cipherlist || cipherlist->empty()) ? nullptr : cipherlist->c_str(), VerifyPeer);
}
  ...
  if (tls_conn == nullptr) {
    ...
    if (cipher) {
      ...
      Qmsg(jcr, M_INFO, 0, _("Secure connection to %s at %s:%d with cipher %s established\n"), who,
           host, port, cipher_name);
    } else {
      Qmsg(jcr, M_WARNING, 0,
           _("Secure connection to %s at %s:%d with UNKNOWN cipher established\n"), who, host,
           port);
    }
  }
}
 ...
 *          NULL on failure;
 */
TLS_CONNECTION *new_tls_connection(std::shared_ptr<TlsContext> tls_ctx, int fd, bool server) {
 ...
 *           false on failure
 */
bool TlsBsockConnect(BareosSocket *bsock) { return OpensslBsockSessionStart(bsock, false); }
 ...
 *           false on failure
 */
bool TlsBsockAccept(BareosSocket *bsock) { return OpensslBsockSessionStart(bsock, true); }
