...
static TREE_NODE *search_and_insert_tree_node(char *fname, int type, TREE_ROOT *root,
                                              TREE_NODE *parent);
 ...
 */
void TreeAddDeltaPart(TREE_ROOT *root, TREE_NODE *node, JobId_t JobId, int32_t FileIndex) {




  struct delta_list *elt = (struct delta_list *)tree_alloc(root, sizeof(struct delta_list));
 ...
 */
TREE_NODE *insert_tree_node(char *path, char *fname, int type, TREE_ROOT *root, TREE_NODE *parent) {
  ...
  if (*fname) {
    if (!parent) { /* if no parent, we need to make one */
      ...
      if (path_len == root->cached_path_len && bstrcmp(path, root->cached_path)) {
