...
#ifndef TEST_RWLOCK
void TestDevlock(void **state) { (void)state; /* unused */ }
#endif
#endif
...
void TestDevlock(void **state) {
  ...
  for (count = 0; count < THREADS; count++) {
    ...
    status =
        pthread_create(&threads[count].thread_id, NULL, thread_routine, (void *)&threads[count]);
    ...
    if ((iteration % self->interval) == 0) {
      ...
      if (status == EBUSY)
        ...
        if (data[element].data != data[element].updates)
          printf("%d: data[%d] %d != %d\n", self->thread_num, element, data[element].data,
                 data[element].updates);
...
void TestDevlock(void **state) {
  ...
  for (count = 0; count < THREADS; count++) {
    ...
    status =
        pthread_create(&threads[count].thread_id, NULL, thread_routine, (void *)&threads[count]);
    if (status != 0) err_abort(status, _("Create thread"));
  }
  ...
  for (count = 0; count < THREADS; count++) {
    status = pthread_join(threads[count].thread_id, NULL);
    if (status != 0) err_abort(status, _("Join thread"));
    thread_updates += threads[count].updates;
    printf(_("%02d: interval %d, updates %d, "
             "r_collisions %d, w_collisions %d\n"),
           count, threads[count].interval, threads[count].updates, threads[count].r_collisions,
