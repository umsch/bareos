...
TEST(scan, scan) {
  ...
  const char *catreq =
      "CatReq Job=NightlySave.2004-06-11_19.11.32 CreateJobMedia FirstIndex=1 LastIndex=114 "
      "StartFile=0 EndFile=0 StartBlock=208 EndBlock=2903248";
  static char Create_job_media[] =
      "CatReq Job=%127s CreateJobMedia "
      "FirstIndex=%u LastIndex=%u StartFile=%u EndFile=%u "
      "StartBlock=%u EndBlock=%u\n";
  static char OK_media[] =
      "1000 OK VolName=%127s VolJobs=%u VolFiles=%u"
      " VolBlocks=%u VolBytes=%" lld
      " VolMounts=%u VolErrors=%u VolWrites=%u"
      " MaxVolBytes=%" lld " VolCapacityBytes=%" lld
      " VolStatus=%20s"
      " Slot=%d MaxVolJobs=%u MaxVolFiles=%u InChanger=%d"
      " VolReadTime=%" lld " VolWriteTime=%" lld;
  const char *media =
      "1000 OK VolName=TestVolume001 VolJobs=0 VolFiles=0 VolBlocks=0 VolBytes=1 VolMounts=0 "
      "VolErrors=0 VolWrites=0 MaxVolBytes=0 VolCapacityBytes=0 VolStatus=Append Slot=0 "
  ...
  *Job = 0;
  cnt = bsscanf(catreq, Create_job_media, &Job, &FirstIndex, &LastIndex, &StartFile, &EndFile,
                &StartBlock, &EndBlock);
  // printf("cnt=%d Job=%s\n", cnt, Job);
  ...
  cnt = bsscanf(media, OK_media, vol.VolCatName, &vol.VolCatJobs, &vol.VolCatFiles,
                ...
                &vol.Slot, &vol.VolCatMaxJobs, &vol.VolCatMaxFiles, &vol.InChanger,
                &vol.VolReadTime, &vol.VolWriteTime, &vol.MaxBlocksize, &vol.MinBlocksize);
  // printf("cnt=%d Vol=%s\n", cnt, vol.VolCatName);
  ...
  const char *catreq_findmedia =
      "CatReq Job=BackupCatalog.2016-12-23_10.52.45_23 FindMedia=1 pool_name=Full media_type=Full "
      "unwanted_volumes=\n";
  cnt =
      bsscanf(catreq_findmedia, Find_media, &Job, &index, &pool_name, &MediaType, unwanted_volumes);
