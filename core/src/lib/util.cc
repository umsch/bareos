 ...
 *                 ->line3
 */
const char *IndentMultilineString(PoolMem &resultbuffer, const char *multilinestring,
...
char *encode_time(utime_t utime, char *buf) {
  ...
  Blocaltime(&time, &tm);
  n = sprintf(buf, "%04d-%02d-%02d %02d:%02d:%02d", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
...
const char *volume_status_to_str(const char *status) {
  ...
  int pos;
  const char *vs[] = {
      NT_("Append"), _("Append"), NT_("Archive"), _("Archive"), NT_("Disabled"),  _("Disabled"),
      NT_("Full"),   _("Full"),   NT_("Used"),    _("Used"),    NT_("Cleaning"),  _("Cleaning"),
      NT_("Purged"), _("Purged"), NT_("Recycle"), _("Recycle"), NT_("Read-Only"), _("Read-Only"),
      NT_("Error"),  _("Error"),  NULL,           NULL};
 ...
 *  to = recipients list
 */
POOLMEM *edit_job_codes(JobControlRecord *jcr, char *omsg, char *imsg, const char *to,
