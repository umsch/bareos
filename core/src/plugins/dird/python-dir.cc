...
#define PLUGIN_USAGE                                                                               \
  "python:instance=<instance_id>:module_path=<path-to-python-modules>:module_name=<python-module-" \
  "to-load>"
...
static pDirFuncs pluginFuncs = {sizeof(pluginFuncs), DIR_PLUGIN_INTERFACE_VERSION,
 ...
 */
bRC DLL_IMP_EXP loadPlugin(bDirInfo *lbinfo, bDirFuncs *lbfuncs, genpInfo **pinfo,
                           pDirFuncs **pfuncs) {
...
static bRC PyLoadModule(bpContext *ctx, void *value) {
  ...
  if (p_ctx->module_name) {
    ...
    Dmsg(ctx, debuglevel, "python-dir: Successfully loaded module with name %s\n",
         p_ctx->module_name);
    ...
    if (pFunc && PyCallable_Check(pFunc)) {
      ...
      Dmsg(ctx, debuglevel, "python-dir: Failed to find function named load_bareos_plugins()\n");
      goto bail_out;
    }
...
static bRC PyParsePluginDefinition(bpContext *ctx, void *value) {
  ...
  if (pFunc && PyCallable_Check(pFunc)) {
    ...
    Dmsg(ctx, debuglevel, "python-dir: Failed to find function named parse_plugin_definition()\n");
    return bRC_Error;
  }
...
static bRC PyGetPluginValue(bpContext *ctx, pDirVariable var, void *value) { return bRC_OK; }

static bRC PySetPluginValue(bpContext *ctx, pDirVariable var, void *value) { return bRC_OK; }
...
static bRC PyHandlePluginEvent(bpContext *ctx, bDirEvent *event, void *value) {
  ...
  if (pFunc && PyCallable_Check(pFunc)) {
    ...
    Dmsg(ctx, debuglevel, "python-dir: Failed to find function named handle_plugin_event()\n");
  }
...
static PyObject *PyBareosGetValue(PyObject *self, PyObject *args) {
  ...
  switch (var) {
    ...
    default:
      ctx = PyGetbpContext(pyCtx);
      Dmsg(ctx, debuglevel, "python-dir: PyBareosGetValue unknown variable requested %d\n", var);
      break;
  }
...
static PyObject *PyBareosSetValue(PyObject *self, PyObject *args) {
  ...
  switch (var) {
    ...
    default:
      ctx = PyGetbpContext(pyCtx);
      Dmsg(ctx, debuglevel, "python-dir: PyBareosSetValue unknown variable requested %d\n", var);
      break;
  }
...
static PyObject *PyBareosRegisterEvents(PyObject *self, PyObject *args) {
  ...
  for (int i = 0; i < len; i++) {
    ...
    if (event >= bDirEventJobStart && event <= bDirEventGetScratch) {
      Dmsg(ctx, debuglevel, "python-dir: PyBareosRegisterEvents registering event %d\n", event);
