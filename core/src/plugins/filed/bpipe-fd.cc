 ...
 */
enum plugin_argument_type { argument_none = 0, argument_file, argument_reader, argument_writer };
...
static bRC newPlugin(bpContext *ctx) {
  ...
  bfuncs->registerBareosEvents(ctx, 6, bEventNewPluginOptions, bEventPluginCommand, bEventJobStart,
...
static bRC parse_plugin_definition(bpContext *ctx, void *value) {
  ...
  while (argument) {
    ...
    if (!plugin_arguments[i].name && !compatible) {
       ...
       */
      Jmsg(ctx, M_FATAL,
           "bpipe-fd: Found mixing of old and new syntax, please fix your plugin definition (%s)\n",
           plugin_definition);
      Dmsg(ctx, debuglevel,
           "bpipe-fd: Found mixing of old and new syntax, please fix your plugin definition (%s)\n",
           plugin_definition);
          ...
          switch (plugin_arguments[i].type) {
            case argument_file:
              if (!PathContainsDirectory(argument_value)) {
                Jmsg(ctx, M_FATAL,
                     "bpipe-fd: file argument (%s) must contain a directory structure. Please fix "
