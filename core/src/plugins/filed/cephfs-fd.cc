 ...
 */
enum plugin_argument_type { argument_none, argument_conffile, argument_basedir };
...
static bRC get_next_file_to_backup(bpContext *ctx) {
  ...
  while (1) {
    ...
    switch (p_ctx->statp.st_mode & S_IFMT) {
      ...
      case S_IFLNK:
        p_ctx->type = FT_LNK;
        status = ceph_readlink(p_ctx->cmount, p_ctx->next_filename, p_ctx->link_target,
        ...
        switch (errno) {
          ...
          default:
            Jmsg(ctx, M_ERROR, "cephfs-fd: ceph_lgetxattr(%s) failed: %s\n", ap->fname,
                 be.bstrerror(-xattr_value_length));
            return bRC_Error;
        }
      }
...
static bRC setAcl(bpContext *ctx, acl_pkt *ap) {
  ...
  while (UnserLength(ap->content) < ap->content_length) {
    ...
    status = ceph_lsetxattr(p_ctx->cmount, ap->fname, acl_name.c_str(), xattr_value.c_str(),
