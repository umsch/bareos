...
static plugin_argument plugin_arguments[] = {{"volume", argument_volume_spec},
...
static gluster_find_mapping gluster_find_mappings[] = {{"NEW ", gf_type_new, 4},
 ...
 */
bRC DLL_IMP_EXP unloadPlugin() { return bRC_OK; }
...
static bRC newPlugin(bpContext *ctx) {
   ...
   */
  bfuncs->registerBareosEvents(ctx, 7, bEventLevel, bEventSince, bEventRestoreCommand,
 ...
 */
static bRC getPluginValue(bpContext *ctx, pVariable var, void *value) { return bRC_OK; }

/**
 * Set a plugin value (none defined)
 */
static bRC setPluginValue(bpContext *ctx, pVariable var, void *value) { return bRC_OK; }
  ...
  while (1) {
    ...
    if (!p_ctx->crawl_fs) {
      ...
      switch (gf_mapping->type) {
        ...
        case gf_type_rename:
          /*
           * RENAME means we clear the seen bitmap for the original name and backup the new
           * filename.
           */
          bstrinlinecpy(p_ctx->next_filename, p_ctx->next_filename + gf_mapping->compare_size);
       ...
       */
      if (p_ctx->basedir &&
          !bstrncmp(p_ctx->basedir, p_ctx->next_filename, strlen(p_ctx->basedir))) {
    ...
    switch (p_ctx->statp.st_mode & S_IFMT) {
      ...
      case S_IFLNK:
        p_ctx->type = FT_LNK;
        status = glfs_readlink(p_ctx->glfs, p_ctx->next_filename, p_ctx->link_target,
      ...
      case S_IFDIR:
        if (!p_ctx->crawl_fs) {
           ...
           * don't recurse into the directory but just process a list of externally provided
           * filenames.
           */
          p_ctx->type = FT_DIREND;
        } else {
...
static inline bool GfapiMakedirs(plugin_ctx *p_ctx, const char *directory) {
   ...
   */
  for (char *p = new_directory.c_str() + (len - 1); (p >= new_directory.c_str()) && *p == '/';
       p--) {
    *p = '\0';
  }

  if (strlen(new_directory.c_str()) && glfs_stat(p_ctx->glfs, new_directory.c_str(), &st) != 0) {
    /*
 ...
 */
static inline bool parse_gfapi_devicename(char *devicename, char **transport, char **servername,
                                          char **volumename, char **dir, int *serverport) {
  char *bp;
...
static bRC connect_to_gluster(bpContext *ctx, bool is_backup) {
  ...
  if (!parse_gfapi_devicename(p_ctx->gfapi_volume_spec, &p_ctx->transport, &p_ctx->servername,
                              &p_ctx->volumename, &p_ctx->basedir, &p_ctx->serverport)) {
    return bRC_Error;
  }
  ...
  if (is_backup && p_ctx->snapdir) {
    status = glfs_set_xlator_option(p_ctx->glfs, "*-snapview-client", "snapdir-entry-path",
                                    p_ctx->snapdir);
    if (status < 0) {
      goto bail_out;
    }
...
static bRC setup_backup(bpContext *ctx, void *value) {
  ...
  if (p_ctx->gf_file_list) {
    ...
    if (p_ctx->is_accurate) {
      /*
       * Mark all files as seen from the previous backup when this is a incremental or differential
       * backup. The entries we get from glusterfind are only the changes since that earlier backup.
       */
      switch (p_ctx->backup_level) {
        case L_INCREMENTAL:
        case L_DIFFERENTIAL:
          if (bfuncs->SetSeenBitmap(ctx, true, NULL) != bRC_OK) {
            Jmsg(ctx, M_FATAL,
                 "Failed to enable all entries in Seen bitmap, not an accurate backup ?\n");
            Dmsg(ctx, debuglevel,
                 "Failed to enable all entries in Seen bitmap, not an accurate backup ?\n");
            goto bail_out;
          }
...
/**
 * This is called during restore to create the file (if necessary) We must return in
 * rp->create_status:
 *
 *  CF_ERROR    -- error
...
static inline uint32_t serialize_acl_stream(PoolMem *buf, uint32_t expected_serialize_len,
                                            uint32_t offset, const char *acl_name,
                                            uint32_t acl_name_length, char *xattr_value,
...
static bRC setAcl(bpContext *ctx, acl_pkt *ap) {
  ...
  while (UnserLength(ap->content) < ap->content_length) {
    ...
    status = glfs_lsetxattr(p_ctx->glfs, ap->fname, acl_name.c_str(), xattr_value.c_str(),
                            xattr_value_length, 0);
    if (status < 0) {
      berrno be;
