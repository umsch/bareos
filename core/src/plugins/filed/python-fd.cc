...
static bRC newPlugin(bpContext *ctx) {
   ...
   */
  bfuncs->registerBareosEvents(ctx, 9, bEventLevel, bEventSince, bEventNewPluginOptions,
...
static inline bool ParseBoolean(const char *argument_value) {
  ...
  if (Bstrcasecmp(argument_value, "yes") || Bstrcasecmp(argument_value, "true")) {
    return true;
  } else {
    return false;
  }
 ...
 */
static inline bRC conv_python_retval(PyObject *pRetVal) { return (bRC)PyInt_AsLong(pRetVal); }
...
static void PyErrorHandler(bpContext *ctx, int msgtype) {
  ...
  if (tracebackModule != NULL) {
    ...
    tbList = PyObject_CallMethod(tracebackModule, (char *)"format_exception", (char *)"OOO", type,
...
static bRC PyGetPluginValue(bpContext *ctx, pVariable var, void *value) { return bRC_OK; }

static bRC PySetPluginValue(bpContext *ctx, pVariable var, void *value) { return bRC_OK; }
  ...
  if (!is_options_plugin) {
    ...
    if (IS_FT_OBJECT(sp->type)) {
      ...
      if (pSavePkt->object_len > 0) {
         ...
         * As this has to linger as long as the backup is running we save it in our plugin context.
         */
        if (pSavePkt->object_name && pSavePkt->object && PyString_Check(pSavePkt->object_name) &&
...
static PyObject *PyRestoreObject_repr(PyRestoreObject *self) {
  ...
  Mmsg(buf,
       ...
       self->object_type, self->object_len, self->object_full_len, self->object_index,
...
static int PyRestoreObject_init(PyRestoreObject *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "|oosiiiiiiI", kwlist, &self->object_name,
                                   &self->object, &self->plugin_name, &self->object_type,
                                   &self->object_len, &self->object_full_len, &self->object_index,
                                   &self->object_compression, &self->stream, &self->JobId)) {
    return -1;
  }
...
static PyObject *PyStatPacket_repr(PyStatPacket *self) {
  ...
  Mmsg(buf,
       ...
       self->dev, self->ino, (self->mode & ~S_IFMT), self->nlink, self->uid, self->gid, self->rdev,
       self->size, self->atime, self->mtime, self->ctime, self->blksize, self->blocks);
...
static int PyStatPacket_init(PyStatPacket *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "|IKHHIIIKIIIIK", kwlist, &self->dev, &self->ino,
...
static PyObject *PySavePacket_repr(PySavePacket *self) {
  ...
  Mmsg(buf,
       ...
       print_flags_bitmap(self->flags), self->no_read, self->portable, self->accurate_found,
       self->cmd, self->save_time, self->delta_seq, PyGetStringValue(self->object_name),
...
static int PySavePacket_init(PySavePacket *self, PyObject *args, PyObject *kwds) {
  ...
  static char *kwlist[] = {(char *)"fname",          (char *)"link",         (char *)"type",
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "|ooiocccsiiooii", kwlist, &self->fname, &self->link,
                                   &self->type, &self->flags, &self->no_read, &self->portable,
                                   &self->accurate_found, &self->cmd, &self->save_time,
                                   &self->delta_seq, &self->object_name, &self->object,
...
static PyObject *PyRestorePacket_repr(PyRestorePacket *self) {
  ...
  Mmsg(buf,
       ...
       self->stream, self->data_stream, self->type, self->file_index, self->LinkFI, self->uid,
       PyGetStringValue(stat_repr), self->attrEx, self->ofname, self->olname, self->where,
...
static int PyRestorePacket_init(PyRestorePacket *self, PyObject *args, PyObject *kwds) {
  ...
  static char *kwlist[] = {(char *)"stream",     (char *)"data_stream",   (char *)"type",
  ...
  if (!PyArg_ParseTupleAndKeywords(
          args, kwds, "|iiiiiIosssssii", kwlist, &self->stream, &self->data_stream, &self->type,
          &self->file_index, &self->LinkFI, &self->uid, &self->statp, &self->attrEx, &self->ofname,
 ...
 * Destructor.
 */
static void PyRestorePacket_dealloc(PyRestorePacket *self) { PyObject_Del(self); }
...
static PyObject *PyIoPacket_repr(PyIoPacket *self) {
  ...
  Mmsg(buf,
       ...
       self->func, self->count, self->flags, (self->mode & ~S_IFMT), PyGetByteArrayValue(self->buf),
       self->fname, self->status, self->io_errno, self->lerror, self->whence, self->offset,
       self->win32);
  s = PyString_FromString(buf.c_str());
...
static int PyIoPacket_init(PyIoPacket *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "|Hiiiosiiiilc", kwlist, &self->func, &self->count,
                                   &self->flags, &self->mode, &self->buf, &self->fname,
                                   &self->status, &self->io_errno, &self->lerror, &self->whence,
                                   &self->offset, &self->win32)) {
    return -1;
  }
...
static int PyAclPacket_init(PyAclPacket *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "|so", kwlist, &self->fname, &self->content)) {
    return -1;
  }
...
static int PyXattrPacket_init(PyXattrPacket *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "|soo", kwlist, &self->fname, &self->name,
                                   &self->value)) {
    return -1;
  }
