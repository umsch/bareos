...
#define PLUGIN_USAGE                                                                          \
  "rados:conffile=<ceph_conf_file>:namespace=<name_space>:clientid=<client_id>:clustername=<" \
  "clustername>:username=<username>:snapshotname=<snapshot_name>:"
...
static plugin_argument plugin_arguments[] = {
    {"conffile", argument_conffile},         {"poolname", argument_poolname},
    {"clientid", argument_clientid},         {"clustername", argument_clustername},
    {"username", argument_username},         {"namespace", argument_namespace},
    {"snapshotname", argument_snapshotname}, {NULL, argument_none}};
...
static bRC get_next_object_to_backup(bpContext *ctx) {
  ...
  while (1) {
    ...
    status =
        rados_stat(p_ctx->ioctx, p_ctx->object_name, &p_ctx->object_size, &p_ctx->object_mtime);
    if (status < 0) {
      berrno be;
...
static bRC connect_to_rados(bpContext *ctx) {
  ...
  if (!p_ctx->cluster_initialized) {
    ...
    status = rados_create2(&p_ctx->cluster, p_ctx->rados_clustername, p_ctx->rados_username,
                           rados_flags);
#endif
    if (status < 0) {
      berrno be;
...
static bRC getXattr(bpContext *ctx, xattr_pkt *xp) {
   ...
   */
  status =
      rados_getxattrs_next(p_ctx->xattr_iterator, &xattr_name, &xattr_value, &xattr_value_length);
  if (status < 0) {
    berrno be;
