...
static pFuncs pluginFuncs = {
    ...
    getPluginValue, setPluginValue, handlePluginEvent, startBackupFile, endBackupFile,
    startRestoreFile, endRestoreFile, pluginIO, createFile, setFileAttributes,
    NULL, /* no checkFile */
    NULL, /* no getAcl */
    NULL, /* no setAcl */
    NULL, /* no getXattr */
...
static bRC newPlugin(bpContext *ctx) {
  ...
  bfuncs->registerBareosEvents(ctx, 3, bEventLevel, bEventRestoreCommand, bEventBackupCommand);
  return bRC_OK;
}
...
static const char *files[] = {"/etc/passwd", "/etc/group", "/etc/hosts", "/etc/services"};
