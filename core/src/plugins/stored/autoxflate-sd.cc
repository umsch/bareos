 ...
 */
bRC DLL_IMP_EXP loadPlugin(bsdInfo *lbinfo, bsdFuncs *lbfuncs, genpInfo **pinfo,
                           psdFuncs **pfuncs) {
...
static bRC newPlugin(bpContext *ctx) {
   ...
   */
  bfuncs->registerBareosEvents(ctx, 4, bsdEventJobEnd, bsdEventSetupRecordTranslation,
...
static bool SetupAutoDeflation(bpContext *ctx, DeviceControlRecord *dcr) {
  ...
  if (!SetupCompressionBuffers(jcr, sd_enabled_compatible, dcr->device->autodeflate_algorithm,
                               &compress_buf_size)) {
    goto bail_out;
  }
    ...
    if (!jcr->compress.inflate_buffer) {
      ...
      if (decompress_buf_size > jcr->compress.inflate_buffer_size) {
        jcr->compress.inflate_buffer =
            ReallocPoolMemory(jcr->compress.inflate_buffer, decompress_buf_size);
        jcr->compress.inflate_buffer_size = decompress_buf_size;
      }
    }
  } else {
...
static bool AutoDeflateRecord(bpContext *ctx, DeviceControlRecord *dcr) {
  ...
  switch (rec->maskedStream) {
    ...
    case STREAM_SPARSE_DATA:
      ...
      max_compression_length =
          dcr->jcr->compress.deflate_buffer_size - OFFSET_FADDR_SIZE - sizeof(comp_stream_header);
      break;
  }
   ...
   */
  if (!CompressData(dcr->jcr, dcr->device->autodeflate_algorithm, rec->data, rec->data_len, data,
                    max_compression_length, &nrec->data_len)) {
...
/**
 * Inflate (uncompress) the content of a read record and return the data as an alternative
 * datastream.
 */
static bool AutoInflateRecord(bpContext *ctx, DeviceControlRecord *dcr) {
  ...
  if (!DecompressData(dcr->jcr, "Unknown", rec->maskedStream, &nrec->data, &nrec->data_len, true)) {
    bfuncs->FreeRecord(nrec);
    goto bail_out;
  }
