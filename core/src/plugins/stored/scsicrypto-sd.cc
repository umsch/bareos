...
static bRC newPlugin(bpContext *ctx) {
   ...
   */
  bfuncs->registerBareosEvents(ctx, 7, bsdEventLabelRead, bsdEventLabelVerified, bsdEventLabelWrite,
  ...
  *StoredVolEncrKey = '\0';
  ...
  if (dcr->jcr && dcr->jcr->director) {
    ...
    if (director->keyencrkey.value) {
      ...
      if (AesUnwrap((unsigned char *)director->keyencrkey.value, DEFAULT_PASSPHRASE_LENGTH / 8,
                    (unsigned char *)WrappedVolEncrKey, (unsigned char *)VolEncrKey) != 0) {
        Emsg1(M_ERROR, 0,
              "scsicrypto-sd: Failed to unwrap encryption key using %s, probably wrong "
...
static bRC do_clear_scsi_encryption_key(void *value) {
  ...
  if (need_to_clear) {
    ...
    Dmsg0(debuglevel,
          "scsicrypto-sd: Not clearing crypto key because encryption is currently not enabled on "
          "drive\n");
    V(crypto_operation_mutex);
    return bRC_OK;
  }
...
static bRC send_device_encryption_status(void *value) {
  ...
  if (dst->device->drive_crypto_enabled) {
    ...
    dst->status_length = GetScsiDriveEncryptionStatus(dst->device->dev->fd(),
                                                      dst->device->dev->dev_name, dst->status, 4);
    V(crypto_operation_mutex);
  }
  return bRC_OK;
}
