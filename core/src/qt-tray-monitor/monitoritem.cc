...
MonitorItem::MonitorItem(QObject* parent) : QObject(parent), d(new MonitorItemPrivate) {
...
bool MonitorItem::doconnect() {
  ...
  switch (d->type) {
    ...
    case R_DIRECTOR:
      ...
      if (!d->DSock->connect(NULL, d->connectTimeout, 0, 0, "Director daemon", dird->address, NULL,
                             dird->DIRport, false)) {
        delete d->DSock;
        d->DSock = NULL;
      } else {
    ...
    case R_CLIENT:
      ...
      emit showStatusbarMessage(message);
      d->DSock = New(BareosSocketTCP);
      if (!d->DSock->connect(NULL, d->connectTimeout, 0, 0, "File daemon", filed->address, NULL,
                             filed->FDport, false)) {
        delete d->DSock;
        d->DSock = NULL;
      } else {
    ...
    case R_STORAGE:
      ...
      emit showStatusbarMessage(message);
      d->DSock = New(BareosSocketTCP);
      if (!d->DSock->connect(NULL, d->connectTimeout, 0, 0, "Storage daemon", stored->address, NULL,
                             stored->SDport, false)) {
        delete d->DSock;
        d->DSock = NULL;
      } else {
...
void MonitorItem::connectToMainWindow(QObject* mainWindow) {
  ...
  connect(this, SIGNAL(clearText(QString)), mainWindow, SLOT(onClearText(QString)));
  connect(this, SIGNAL(statusChanged(QString, int)), mainWindow,
          SLOT(onStatusChanged(QString, int)));
  if (d->type == R_CLIENT) {
    connect(this, SIGNAL(jobIsRunning(bool)), mainWindow, SLOT(onFdJobIsRunning(bool)));
  }
}
