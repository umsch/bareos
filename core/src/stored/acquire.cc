...
bool AcquireDeviceForRead(DeviceControlRecord *dcr) {
  ...
  if (dev->vol && dev->vol->IsSwapping()) {
    ...
    Dmsg3(rdebuglevel, "swapping: slot=%d Vol=%s dev=%s\n", dev->vol->GetSlot(), dev->vol->vol_name,
          dev->print_name());
  }

  InitDeviceWaitTimers(dcr);

  tape_previously_mounted = dev->CanRead() || dev->CanAppend() || dev->IsLabeled();
  ...
  while (1) {
    ...
    if (dev->device->drive_crypto_enabled ||
        (dev->HasCap(CAP_ALWAYSOPEN) && !bstrcmp(dev->VolHdr.VolumeName, dcr->VolumeName))) {
      dev->ClearLabeled();
    }
...
DeviceControlRecord *acquire_device_for_append(DeviceControlRecord *dcr) {
  ...
  Dmsg4(100, "=== nwriters=%d nres=%d vcatjob=%d dev=%s\n", dev->num_writers, dev->NumReserved(),
        dev->VolCatInfo.VolCatJobs, dev->print_name());
...
bool ReleaseDevice(DeviceControlRecord *dcr) {
  ...
  if (!JobCanceled(jcr)) {
    if (!dcr->device->drive_tapealert_enabled && dcr->device->alert_command) {
      ...
      /*
       * If all reservations are cleared for this device raise an event that SD plugins can register
       * to.
       */
...
static void AttachDcrToDev(DeviceControlRecord *dcr) {
  ...
  if (!dcr->attached_to_dev && dev->initiated && jcr && jcr->getJobType() != JT_SYSTEM) {
    dev->Lock();
    Dmsg4(200, "Attach Jid=%d dcr=%p size=%d dev=%s\n", (uint32_t)jcr->JobId, dcr,
          dev->attached_dcrs->size(), dev->print_name());
