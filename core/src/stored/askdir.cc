...
bool StorageDaemonDeviceControlRecord::dir_update_device(JobControlRecord *jcr, Device *dev) {
  ...
  ok = dir->fsend(Device_update, jcr->Job, dev_name.c_str(), dev->CanAppend() != 0,
                  dev->CanRead() != 0, dev->num_writers, dev->IsOpen() != 0, dev->IsLabeled() != 0,
                  dev->IsOffline() != 0, dev->reserved_device, dev->IsTape() ? 100000 : 1,
                  dev->autoselect, 0, ChangerName.c_str(), MediaType.c_str(), VolumeName.c_str());
...
static bool DoGetVolumeInfo(DeviceControlRecord *dcr) {
  ...
  Dmsg1(debuglevel, "<dird %s", dir->msg);
  n = sscanf(dir->msg, OK_media, vol.VolCatName, &vol.VolCatJobs, &vol.VolCatFiles,
             &vol.VolCatBlocks, &vol.VolCatBytes, &vol.VolCatMounts, &vol.VolCatErrors,
             &vol.VolCatWrites, &vol.VolCatMaxBytes, &vol.VolCatCapacityBytes, vol.VolCatStatus,
             &vol.Slot, &vol.VolCatMaxJobs, &vol.VolCatMaxFiles, &InChanger, &vol.VolReadTime,
             &vol.VolWriteTime, &vol.EndFile, &vol.EndBlock, &vol.LabelType, &vol.VolMediaId,
             vol.VolEncrKey, &vol.VolMinBlocksize, &vol.VolMaxBlocksize);
  if (n != 24) {
    Dmsg3(debuglevel, "Bad response from Dir fields=%d, len=%d: %s", n, dir->msglen, dir->msg);
  ...
  if (*vol.VolEncrKey) {
    if (UpdateCryptoCache(vol.VolCatName, vol.VolEncrKey)) {
      WriteCryptoCache(me->working_directory, "bareos-sd", GetFirstPortHostOrder(me->SDaddrs));
    }
  }
...
bool StorageDaemonDeviceControlRecord::DirGetVolumeInfo(enum get_vol_info_rw writing) {
  ...
  dir->fsend(Get_Vol_Info, jcr->Job, getVolCatName(), (writing == GET_VOL_INFO_FOR_WRITE) ? 1 : 0);
  Dmsg1(debuglevel, ">dird %s", dir->msg);
...
bool StorageDaemonDeviceControlRecord::DirUpdateVolumeInfo(bool label, bool update_LastWritten) {
  ...
  dir->fsend(Update_media, jcr->Job, volume_name.c_str(), vol->VolCatJobs, vol->VolCatFiles,
             vol->VolCatBlocks, edit_uint64(vol->VolCatBytes, ed1), vol->VolCatMounts,
             vol->VolCatErrors, vol->VolCatWrites, edit_uint64(vol->VolCatMaxBytes, ed2),
             edit_uint64(vol->VolLastWritten, ed6), vol->VolCatStatus, vol->Slot, label,
  ...
  if (!jcr->IsCanceled()) {
    if (!DoGetVolumeInfo(this)) {
      ...
      Dmsg2(debuglevel, _("Didn't get vol info vol=%s: ERR=%s"), vol->VolCatName, jcr->errmsg);
      goto bail_out;
    }
...
bool StorageDaemonDeviceControlRecord::DirCreateJobmediaRecord(bool zero) {
  ...
  if (zero) {
     ...
     */
    dir->fsend(Create_job_media, jcr->Job, 0, 0, 0, 0, 0, 0, 0, 0, edit_uint64(VolMediaId, ed1));
  } else {
    dir->fsend(Create_job_media, jcr->Job, VolFirstIndex, VolLastIndex, StartFile, EndFile,
               StartBlock, EndBlock, Copy, Stripe, edit_uint64(VolMediaId, ed1));
  }
  Dmsg1(debuglevel, ">dird %s", dir->msg);
...
bool StorageDaemonDeviceControlRecord::DirUpdateFileAttributes(DeviceRecord *record) {
  ...
  dir->msg = CheckPoolMemorySize(dir->msg, sizeof(FileAttributes) + MAX_NAME_LENGTH +
                                               sizeof(DeviceRecord) + record->data_len + 1);
  dir->msglen =
      Bsnprintf(dir->msg, sizeof(FileAttributes) + MAX_NAME_LENGTH + 1, FileAttributes, jcr->Job);
...
bool StorageDaemonDeviceControlRecord::DirAskSysopToMountVolume(int mode) {
  ...
  while (1) {
    ...
    if (!dev->poll && (status == W_TIMEOUT || status == W_MOUNT)) {
      ...
      Jmsg(jcr, M_MOUNT, 0, msg, VolumeName, jcr->Job, dev->print_name(), pool_name, media_type);
...
bool DeviceControlRecord::DirAskSysopToMountVolume(int /*mode*/) {
  ...
  fprintf(stderr, _("Mount Volume \"%s\" on device %s and press return when ready: "), VolumeName,
          dev->print_name());
  dev->close(this);
  getchar();
  return true;
