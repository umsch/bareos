...
bool AuthenticateDirector(JobControlRecord *jcr) {
  ...
  if (!dir->AuthenticateInboundConnection(jcr, "Director", director->name(), director->password,
                                          director)) {
...
bool AuthenticateWithStoragedaemon(JobControlRecord *jcr) {
  ...
  if (!sd->AuthenticateOutboundConnection(jcr, "Storage daemon", identity, password, me)) {
    Jmsg1(jcr, M_FATAL, 0,
          _("Authorization problem: Two way security handshake failed with Storage daemon at %s\n"),
          sd->who());
    return false;
...
bool AuthenticateWithFiledaemon(JobControlRecord *jcr) {
  ...
  if (!fd->AuthenticateOutboundConnection(jcr, "File daemon", jcr->client_name, password, me)) {
    Jmsg1(jcr, M_FATAL, 0,
          _("Authorization problem: Two way security handshake failed with File daemon at %s\n"),
          fd->who());
    return false;
