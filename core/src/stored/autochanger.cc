...
static char *transfer_edit_device_codes(DeviceControlRecord *dcr, POOLMEM *&omsg, const char *imsg,
                                        const char *cmd, slot_number_t src_slot,
  ...
  foreach_res(changer, R_AUTOCHANGER) {
    ...
    foreach_alist(device, changer->device) {
      ...
      if (!device->changer_name) {
        Jmsg(NULL, M_ERROR, 0, _("No Changer Name given for device %s. Cannot continue.\n"),
             device->name());
        OK = false;
      }

      if (!device->changer_command) {
        Jmsg(NULL, M_ERROR, 0, _("No Changer Command given for device %s. Cannot continue.\n"),
             device->name());
        OK = false;
      }
...
int AutoloadDevice(DeviceControlRecord *dcr, int writing, BareosSocket *dir) {
  ...
  if (slot <= 0) {
    ...
    if (!dev->poll) {
      Jmsg(jcr, M_INFO, 0,
           _("No \"Changer Device\" for %s. Manual load of Volume may be required.\n"),
           dev->print_name());
    }
    rtn_stat = 0;
    ...
    if (!dev->poll) {
      Jmsg(jcr, M_INFO, 0,
           _("No \"Changer Command\" for %s. Manual load of Volume may be required.\n"),
           dev->print_name());
    }
    rtn_stat = 0;
  } else {
    ...
    if (loaded != slot) {
      ...
      Jmsg(jcr, M_INFO, 0, _("3304 Issuing autochanger \"load slot %hd, drive %hd\" command.\n"),
           slot, drive);
 ...
 *          slot otherwise
 *
 * Note, this is safe to do without releasing the drive since it does not attempt load/unload a
 * slot.
 */
 ...
 */
bool UnloadAutochanger(DeviceControlRecord *dcr, slot_number_t loaded, bool lock_set) {
  ...
  if (!dev->IsAutochanger() || !dcr->device->changer_name || !dcr->device->changer_command) {
    return false;
  }
  ...
  if (loaded > 0) {
    ...
    Jmsg(jcr, M_INFO, 0, _("3307 Issuing autochanger \"unload slot %hd, drive %hd\" command.\n"),
         loaded, dev->drive);
    slot = dcr->VolCatInfo.Slot;
    dcr->VolCatInfo.Slot = loaded;
    changer = edit_device_codes(dcr, changer, dcr->device->changer_command, "unload");
    dev->close(dcr);
    Dmsg1(100, "Run program=%s\n", changer);
    status = RunProgramFullOutput(changer, timeout, results.addr());
...
bool UnloadDev(DeviceControlRecord *dcr, Device *dev, bool lock_set) {
  ...
  ChangerCmd = edit_device_codes(dcr, ChangerCmd, dcr->device->changer_command, "unload");
  dev->close(dcr);
 ...
 * We assume that it is always the Console that is calling us.
 */
bool AutochangerTransferCmd(DeviceControlRecord *dcr, BareosSocket *dir, slot_number_t src_slot,
                            slot_number_t dst_slot) {
  Device *dev = dcr->dev;
  uint32_t timeout = dcr->device->max_changer_wait;
  POOLMEM *changer;
  ...
  changer = GetPoolMemory(PM_FNAME);
  LockChanger(dcr);
  changer = transfer_edit_device_codes(dcr, changer, dcr->device->changer_command, "transfer",
                                       src_slot, dst_slot);
