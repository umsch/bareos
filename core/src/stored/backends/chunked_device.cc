...
#ifdef HAVE_MMAP
  if (use_mmap_) {
    buffer = (char *)::mmap(NULL, current_chunk_->chunk_size, (PROT_READ | PROT_WRITE),
...
bool chunked_device::EnqueueChunk(chunk_io_request *request) {
   ...
   */
  enqueued_request =
      (chunk_io_request *)cb_->enqueue(new_request, sizeof(chunk_io_request), CompareChunkIoRequest,
...
bool chunked_device::DequeueChunk() {
  ...
  while (1) {
     ...
     */
    new_request =
        (chunk_io_request *)cb_->dequeue(true, /* reserve_slot we may need to enqueue the request */
    ...
    Dmsg3(100, "Flushing chunk %d of volume %s by thread %s\n", new_request->chunk,
          new_request->volname, edit_pthread(pthread_self(), ed1, sizeof(ed1)));
    ...
    if (!FlushRemoteChunk(new_request)) {
      ...
      if (retries_ > 0 && new_request->tries >= retries_) {
        Mmsg4(errmsg,
              _("Unable to flush chunk %d of volume %s to backing store after %d tries, setting "
                "device %s readonly\n"),
      ...
      enqueued_request = (chunk_io_request *)cb_->enqueue(
          new_request, sizeof(chunk_io_request), CompareChunkIoRequest, UpdateChunkIoRequest,
    ...
    if (current_chunk_->start_offset <= offset_ &&
      ...
      while (retval < (ssize_t)count) {
        ...
        if (offset_ < current_chunk_->end_offset) {
          wanted_offset = (offset_ % current_chunk_->chunk_size);
          bytes_left =
              MIN((ssize_t)(count - offset), (ssize_t)(current_chunk_->buflen - wanted_offset));
        ...
        if (!ReadChunk()) {
           ...
           */
          bytes_left = MIN((ssize_t)(count - offset), (ssize_t)(current_chunk_->buflen));
...
static int ListIoRequest(void *request, void *data) {
  ...
  status.bsprintf("   /%s/%04d - %ld\n", io_request->volname, io_request->chunk,
                  io_request->wbuflen);
