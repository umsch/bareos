...
static device_option device_options[] = {{"publish_file=", argument_publishfile, 13},
...
int elasto_device::d_open(const char *pathname, int flags, int mode) {
  ...
  if (min_block_size == 0 || (min_block_size % ELASTO_GRANULARITY) != 0) {
    Emsg2(M_ABORT, 0,
          _("Device minimum block size (%d) not multiple of elasto blocksize size (%d)\n"),
  ...
  if (max_block_size == 0 || (max_block_size % ELASTO_GRANULARITY) != 0) {
    Emsg2(M_ABORT, 0,
          _("Device maximum block size (%d) not multiple of elasto blocksize size (%d)\n"),
