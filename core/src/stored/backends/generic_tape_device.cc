        ...
        if (errno == ENOMEM) { /* tape record exceeds buf len */
          ...
          if (HasCap(CAP_IBMLINTAPE)) {
            ...
            if (CheckScsiAtEod(fd_)) {
              ...
              Dmsg0(100,
                    "Not at EOD, might be a real error. Check sense trace from lin_taped logs.\n");
              SetEot();
              clrerror(-1);
...
static inline bool DevGetOsPos(Device *dev, struct mtget *mt_stat) {
  ...
  return dev->HasCap(CAP_MTIOCGET) && dev->d_ioctl(dev->fd(), MTIOCGET, (char *)mt_stat) == 0 &&
         mt_stat->mt_fileno >= 0;
}
...
bool generic_tape_device::fsr(int num) {
  ...
  if (status == 0) {
    ...
    if (DevGetOsPos(this, &mt_stat)) {
      Dmsg4(100, "Adjust from %d:%d to %d:%d\n", file, block_num, mt_stat.mt_fileno,
            mt_stat.mt_blkno);
...
int generic_tape_device::d_ioctl(int fd, ioctl_req_t request, char *op) { return -1; }

boffset_t generic_tape_device::d_lseek(DeviceControlRecord *dcr, boffset_t offset, int whence) {
