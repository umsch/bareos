 ...
 */
enum device_option_type { argument_none = 0, argument_uri };
...
static device_option device_options[] = {{"uri=", argument_uri, 4}, {NULL, argument_none}};
...
static inline bool GfapiMakedirs(glfs_t *glfs, const char *directory) {
  ...
  if (strlen(new_directory.c_str()) && glfs_stat(glfs, new_directory.c_str(), &st) != 0) {
    /*
...
int gfapi_device::d_open(const char *pathname, int flags, int mode) {
  ...
  if (!gfapi_configstring_) {
    ...
    if (!parse_gfapi_devicename(gfapi_uri_, &transport_, &servername_, &volumename_, &basedir_,
                                &serverport_)) {
  ...
  if (!glfs_) {
    ...
    status =
        glfs_set_volfile_server(glfs_, (transport_) ? transport_ : "tcp", servername_, serverport_);
    if (status < 0) {
      Mmsg3(errmsg,
            _("Unable to initialize Gluster management server for transport %s, servername %s, "
              "serverport %d\n"),
  ...
  /*
   * See if the O_CREAT flag is set as glfs_open doesn't support that flag and you have to call
   * glfs_creat then.
   */
  if (flags & O_CREAT) {
