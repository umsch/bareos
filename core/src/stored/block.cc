...
void DumpBlock(DeviceBlock *b, const char *msg) {
  ...
  BlockCheckSum = bcrc32((uint8_t *)b->buf + BLKHDR_CS_LENGTH, block_len - BLKHDR_CS_LENGTH);
  Pmsg6(000,
  ...
  while (p < (b->buf + block_len + WRITE_RECHDR_LENGTH)) {
    ...
    Pmsg6(000, _("   Rec: VId=%u VT=%u FI=%s Strm=%s len=%d p=%x\n"), VolSessionId, VolSessionTime,
          FI_to_ascii(buf1, FileIndex), stream_to_ascii(buf2, Stream, FileIndex), data_len, p);
    p += data_len + rhl;
  }
}
...
DeviceBlock *new_block(Device *dev) {
  ...
  if (dev->max_block_size == 0) {
    ...
    Dmsg1(100,
          "created new block of blocksize %d (dev->device->label_block_size) as "
          "dev->max_block_size is zero\n",
          block->buf_len);
  } else {
...
static uint32_t SerBlockHeader(DeviceBlock *block, bool DoChecksum) {
   ...
   */
  if (DoChecksum) {
    CheckSum = bcrc32((uint8_t *)block->buf + BLKHDR_CS_LENGTH, block_len - BLKHDR_CS_LENGTH);
  }
...
static inline bool unSerBlockHeader(JobControlRecord *jcr, Device *dev, DeviceBlock *block) {
  ...
  if (Id[3] == '1') {
    ...
    if (!bstrncmp(Id, BLKHDR1_ID, BLKHDR_ID_LENGTH)) {
      dev->dev_errno = EIO;
      Mmsg4(dev->errmsg,
            _("Volume data error at %u:%u! Wanted ID: \"%s\", got \"%s\". Buffer discarded.\n"),
  ...
  if (block_len > MAX_BLOCK_LENGTH) {
    dev->dev_errno = EIO;
    Mmsg3(dev->errmsg,
          _("Volume data error at %u:%u! Block length %u is insane (too large), probably due to a "
            "bad archive.\n"),
  ...
  if (block_len <= block->read_len && dev->DoChecksum()) {
    BlockCheckSum = bcrc32((uint8_t *)block->buf + BLKHDR_CS_LENGTH, block_len - BLKHDR_CS_LENGTH);
    if (BlockCheckSum != CheckSum) {
      dev->dev_errno = EIO;
      Mmsg6(dev->errmsg,
            _("Volume data error at %u:%u!\n"
              "Block checksum mismatch in block=%u len=%d: calc=%x blk=%x\n"),
            dev->file, dev->block_num, (unsigned)BlockNumber, block_len, BlockCheckSum, CheckSum);
      if (block->read_errors == 0 || verbose >= 2) {
        Jmsg(jcr, M_ERROR, 0, "%s", dev->errmsg);
...
bool DeviceControlRecord::WriteBlockToDev() {
   ...
   */
  if ((dev->max_file_size > 0) && (dev->file_size + block->binbuf) >= dev->max_file_size) {
  ...
  if (status != (ssize_t)wlen) {
    ...
    if (dev->dev_errno == ENOSPC) {
      Jmsg(jcr, M_INFO, 0,
           _("End of Volume \"%s\" at %u:%u on device %s. Write of %u bytes got %d.\n"),
      ...
      if (!dcr->ReadBlockFromDev(NO_BLOCK_NUMBER_CHECK)) {
        ...
        if (lblock->BlockNumber != dev->LastBlock) {
          if (dev->LastBlock > (lblock->BlockNumber + 1)) {
            Jmsg(jcr, M_FATAL, 0,
                 _("Re-read of last block: block numbers differ by more than one.\n"
                   "Probable tape misconfiguration and data loss. Read block=%u Want block=%u.\n"),
                 lblock->BlockNumber, dev->LastBlock);
          } else {
            Jmsg(jcr, M_ERROR, 0,
                 _("Re-read of last block OK, but block numbers differ. Read block=%u Want "
                   "block=%u.\n"),
                 lblock->BlockNumber, dev->LastBlock);
          }
        } else {
...
reread:
  ...
  if (block->read_len < BLKHDR2_LENGTH) {
    dev->dev_errno = EIO;
    Mmsg4(dev->errmsg,
          _("Volume data error at %u:%u! Very short block of %d bytes on device %s discarded.\n"),
  ...
  if (block->block_len > block->read_len) {
    dev->dev_errno = EIO;
    Mmsg4(dev->errmsg,
          _("Volume data error at %u:%u! Short block of %d bytes on device %s discarded.\n"),
          dev->file, dev->block_num, block->read_len, dev->print_name());
    Jmsg(jcr, M_ERROR, 0, "%s", dev->errmsg);
    dev->SetShortBlock();
    block->read_len = block->binbuf = 0;
