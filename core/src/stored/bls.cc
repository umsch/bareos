  ...
  for (;;) {
    ...
    if (!MatchBsrBlock(bsr, block)) {
      Dmsg5(100, "reject Blk=%u blen=%u bVer=%d SessId=%u SessTim=%u\n", block->BlockNumber,
            block->block_len, block->BlockVer, block->VolSessionId, block->VolSessionTime);
      continue;
    }
    ...
    if (verbose == 1) {
      ...
      Pmsg9(-1,
            _("File:blk=%u:%u blk_num=%u blen=%u First rec FI=%s SessId=%u SessTim=%u Strm=%s "
              "rlen=%d\n"),
...
/* Do list job records */
static void do_jobs(char *infname) { ReadRecords(dcr, jobs_cb, MountNextReadVolume); }
...
static void GetSessionRecord(Device *dev, DeviceRecord *rec, SESSION_LABEL *sessrec) {
  ...
  Dmsg5(10, "%s Record: VolSessionId=%d VolSessionTime=%d JobId=%d DataLen=%d\n", rtype,
        rec->VolSessionId, rec->VolSessionTime, rec->Stream, rec->data_len);
  if (verbose) {
