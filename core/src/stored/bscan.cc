...
static bool CreateFileAttributesRecord(BareosDb *db, JobControlRecord *mjcr, char *fname,
                                       char *lname, int type, char *ap, DeviceRecord *rec);
...
static JobControlRecord *CreateJobRecord(BareosDb *db, JobDbRecord *mr, SESSION_LABEL *label,
                                         DeviceRecord *rec);
static bool UpdateJobRecord(BareosDb *db, JobDbRecord *mr, SESSION_LABEL *elabel,
                            DeviceRecord *rec);
...
static void usage() {
  ...
  fprintf(stderr,
          _(PROG_COPYRIGHT
            ...
            "       -B <drivername>   specify the database driver name (default NULL) "
...
int main(int argc, char *argv[]) {
  ...
  ReadCryptoCache(me->working_directory, "bareos-sd", GetFirstPortHostOrder(me->SDaddrs));
  ...
  db = db_init_database(NULL, db_driver, db_name, db_user, db_password, db_host, db_port, NULL,
 ...
 *          false if error
 */
static inline bool UnpackRestoreObject(JobControlRecord *jcr, int32_t stream, char *rec,
                                       int32_t reclen, RestoreObjectDbRecord *rop) {
  char *bp;
  uint32_t JobFiles;

  if (sscanf(rec, "%d %d %d %d %d %d ", &JobFiles, &rop->Stream, &rop->object_index,
             &rop->object_len, &rop->object_full_len, &rop->object_compression) != 6) {
    return false;
  }
...
static bool RecordCb(DeviceControlRecord *dcr, DeviceRecord *rec) {
  ...
  if (list_records) {
    Pmsg5(000, _("Record: SessId=%u SessTim=%u FileIndex=%d Stream=%d len=%u\n"), rec->VolSessionId,
          rec->VolSessionTime, rec->FileIndex, rec->Stream, rec->data_len);
  }
    ...
    case STREAM_UNIX_ATTRIBUTES_EX:
      ...
      if (verbose && (num_files & 0x7FFF) == 0) {
        ...
        Pmsg4(000, _("%s file records. At file:blk=%s:%s bytes=%s\n"),
              edit_uint64_with_commas(num_files, ed1), edit_uint64_with_commas(rec->File, ed2),
              edit_uint64_with_commas(rec->Block, ed3), edit_uint64_with_commas(mr.VolBytes, ed4));
      }
 ...
 */
static JobControlRecord *CreateJobRecord(BareosDb *db, JobDbRecord *jr, SESSION_LABEL *label,
                                         DeviceRecord *rec) {
  JobControlRecord *mjcr;
  struct date_time dt;
  struct tm tm;
...
static bool UpdateJobRecord(BareosDb *db, JobDbRecord *jr, SESSION_LABEL *elabel,
  ...
  if (verbose) {
    Pmsg3(000, _("Updated Job termination record for JobId=%u Level=%s TermStat=%c\n"), jr->JobId,
          job_level_to_str(mjcr->getJobLevel()), jr->JobStatus);
  }
  ...
  if (verbose > 1) {
    ...
    Pmsg14(000,
           _("%s\n"
             ...
             "Last Volume Bytes:      %s\n"
             "Termination:            %s\n\n"),
           edt, mjcr->JobId, mjcr->Job, mjcr->fileset_name, job_level_to_str(mjcr->getJobLevel()),
           mjcr->client_name, sdt, edt, edit_uint64_with_commas(mjcr->JobFiles, ec1),
           edit_uint64_with_commas(mjcr->JobBytes, ec2), mjcr->VolSessionId, mjcr->VolSessionTime,
           edit_uint64_with_commas(mr.VolBytes, ec3), TermMsg);
  }
  FreeJcr(mjcr);
