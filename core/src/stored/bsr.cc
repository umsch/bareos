...
static int MatchSesstime(BootStrapRecord *bsr, BsrSessionTime *sesstime, DeviceRecord *rec,
                         bool done);
...
static int MatchJobType(BootStrapRecord *bsr, BsrJobType *job_type, SESSION_LABEL *sessrec,
                        bool done);
static int MatchJobLevel(BootStrapRecord *bsr, BsrJoblevel *job_level, SESSION_LABEL *sessrec,
                         bool done);
...
static int MatchVoladdr(BootStrapRecord *bsr, BsrVolumeAddress *voladdr, DeviceRecord *rec,
                        bool done);
 ...
 *  next block.
 */
void PositionBsrBlock(BootStrapRecord *bsr, DeviceBlock *block) { /* To be implemented */
}
 ...
 */
int MatchBsr(BootStrapRecord *bsr, DeviceRecord *rec, VOLUME_LABEL *volrec, SESSION_LABEL *sessrec,
...
BootStrapRecord *find_next_bsr(BootStrapRecord *root_bsr, Device *dev) {
  ...
  if (!root_bsr->use_positioning || !root_bsr->Reposition || !dev->HasCap(CAP_POSITIONBLOCKS)) {
 ...
 *
 */
static BootStrapRecord *find_smallest_volfile(BootStrapRecord *found_bsr, BootStrapRecord *bsr) {
...
static int MatchAll(BootStrapRecord *bsr, DeviceRecord *rec, VOLUME_LABEL *volrec,
  ...
  if (!MatchFindex(bsr, bsr->FileIndex, rec, 1)) {
    if (bsr->FileIndex) {
      Dmsg3(dbglevel, "Fail on findex=%d. bsr=%d,%d\n", rec->FileIndex, bsr->FileIndex->findex,
            bsr->FileIndex->findex2);
    } else {
...
static int MatchVolfile(BootStrapRecord *bsr, BsrVolumeFile *volfile, DeviceRecord *rec,
                        bool done) {
  if (!volfile) {
    return 1; /* no specification matches all */
  }
...
static int MatchVoladdr(BootStrapRecord *bsr, BsrVolumeAddress *voladdr, DeviceRecord *rec,
  ...
  Dmsg6(dbglevel, "MatchVoladdr: saddr=%llu eaddr=%llu recaddr=%llu sfile=%u efile=%u recfile=%u\n",
        voladdr->saddr, voladdr->eaddr, addr, voladdr->saddr >> 32, voladdr->eaddr >> 32,
        addr >> 32);
