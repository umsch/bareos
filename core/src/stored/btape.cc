...
static void print_total_speed() {
  ...
  Pmsg2(000, _("Total Volume bytes=%sB. Total Write rate = %sB/s\n"),
        edit_uint64_with_suffix(total_size, ec1), edit_uint64_with_suffix(rate, ec2));
}
 ...
 */
typedef enum { FILL_RANDOM, FILL_ZERO } fill_mode_t;
...
static void scan_blocks() {
  ...
  for (;;) {
    ...
    if (verbose == 1) {
      ...
      Pmsg9(-1,
            _("Block=%u file,blk=%u,%u blen=%u First rec FI=%s SessId=%u SessTim=%u Strm=%s "
              "rlen=%d\n"),
...
static void fillcmd() {
  ...
  for (file_index = 0; ok && !JobCanceled(jcr);) {
    ...
    while (!WriteRecordToBlock(dcr, &rec)) {
      ...
      if ((block->BlockNumber % 5000) == 0) {
        ...
        Pmsg5(-1, _("Wrote block=%u, file,blk=%u,%u VolBytes=%s rate=%sB/s\n"), block->BlockNumber,
              dev->file, dev->block_num, edit_uint64_with_commas(dev->VolCatInfo.VolCatBytes, ec1),
              edit_uint64_with_suffix(rate, ec2));
      }
    ...
    Dmsg4(190, "WriteRecord FI=%s SessId=%d Strm=%s len=%d\n", FI_to_ascii(buf1, rec.FileIndex),
          rec.VolSessionId, stream_to_ascii(buf2, rec.Stream, rec.FileIndex), rec.data_len);
...
static int FlushBlock(DeviceBlock *block, int dump) {
  ...
  if (!dcr->WriteBlockToDev()) {
    ...
    Pmsg4(000, _("End of tape %d:%d. Volume Bytes=%s. Write rate = %sB/s\n"), dev->file,
          dev->block_num, edit_uint64_with_commas(dev->VolCatInfo.VolCatBytes, ec1),
          edit_uint64_with_suffix(rate, ec2));
...
static struct cmdstruct commands[] = {
    ...
    {NT_("speed"), speed_test,
     _("[file_size=n(GB)|nb_file=3|skip_zero|skip_random|skip_raw|skip_block] report drive speed")},
...
static void helpcmd() {
  ...
  for (i = 0; i < comsize; i++) printf("  %-10s %s\n", commands[i].key, commands[i].help);
  printf("\n");
}
...
static bool MyMountNextReadVolume(DeviceControlRecord *dcr) {
  ...
  Pmsg3(-1, _("Read block=%u, VolBytes=%s rate=%sB/s\n"), block->BlockNumber,
        edit_uint64_with_commas(VolBytes, ec1), edit_uint64_with_suffix(rate, ec2));
