...
static const char *modes[] = {"CREATE_READ_WRITE", "OPEN_READ_WRITE", "OPEN_READ_ONLY",
                              "OPEN_WRITE_ONLY"};
...
static inline Device *init_dev_(JobControlRecord *jcr, DeviceResource *device, bool new_init) {
  ...
  if (dev->IsFile() && dev->RequiresMount()) {
    ...
    if (!device->mount_command || !device->unmount_command) {
      Jmsg0(jcr, M_ERROR_TERM, 0,
            _("Mount and unmount commands must defined for a device which requires mount.\n"));
    }
  }
  ...
  *dev->errmsg = 0;
  ...
  dev->initiated = true;
  Dmsg3(100, "dev=%s dev_max_bs=%u max_bs=%u\n", dev->dev_name, dev->device->max_block_size,
        dev->max_block_size);
...
void Device::SetBlocksizes(DeviceControlRecord *dcr) {
  ...
  Dmsg4(100,
        "Device %s has dev->device->max_block_size of %u and dev->max_block_size of %u, "
        "dcr->VolMaxBlocksize is %u\n",
...
bool Device::open(DeviceControlRecord *dcr, int omode) {
  ...
  Dmsg4(100, "open dev: type=%d dev_name=%s vol=%s mode=%s\n", dev_type, print_name(),
        getVolCatName(), mode_to_str(omode));
