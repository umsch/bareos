 ...
 */
enum { W_ERROR = 1, W_TIMEOUT, W_POLL, W_MOUNT, W_WAKE };

/**
 * Arguments to open_dev()
 */
enum { CREATE_READ_WRITE = 1, OPEN_READ_WRITE, OPEN_READ_ONLY, OPEN_WRITE_ONLY };
 ...
 * IO directions
 */
enum { IO_DIRECTION_NONE = 0, IO_DIRECTION_IN, IO_DIRECTION_OUT, IO_DIRECTION_INOUT };
...
class Device : public SmartAlloc {
 ...
 public:
  ...
  bool IsFile() const {
    return (dev_type == B_FILE_DEV || dev_type == B_GFAPI_DEV || dev_type == B_DROPLET_DEV ||
            dev_type == B_RADOS_DEV || dev_type == B_CEPHFS_DEV || dev_type == B_ELASTO_DEV);
  }
   ...
   */
  bool CanWrite() const { return IsOpen() && CanAppend() && IsLabeled() && !AtWeot(); }
  bool CanRead() const { return BitIsSet(ST_READREADY, state); }
  bool CanStealLock() const {
    return blocked_ && (blocked_ == BST_UNMOUNTED || blocked_ == BST_WAITING_FOR_SYSOP ||
  ...
  bool IsVolumeToUnload() const { return unload_ && strcmp(VolHdr.VolumeName, UnloadVolName) == 0; }
...
enum get_vol_info_rw { GET_VOL_INFO_FOR_WRITE, GET_VOL_INFO_FOR_READ };
