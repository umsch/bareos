...
static char replicatecmd[] = "replicate Job=%127s address=%s port=%d ssl=%d Authorization=%100s";
...
static void ReadVolumeLabel(JobControlRecord *jcr, DeviceControlRecord *dcr, Device *dev,
                            slot_number_t slot);
static void label_volume_if_ok(DeviceControlRecord *dcr, char *oldname, char *newname,
                               char *poolname, slot_number_t Slot, bool relabel);
static int TryAutoloadDevice(JobControlRecord *jcr, DeviceControlRecord *dcr, slot_number_t slot,
                             const char *VolName);
...
static bool SecureerasereqCmd(JobControlRecord *jcr) {
  ...
  return dir->fsend(OKsecureerase,
                    (me->secure_erase_cmdline ? me->secure_erase_cmdline : "*None*"));
}
...
static bool do_label(JobControlRecord *jcr, bool relabel) {
  ...
  if (relabel) {
    if (sscanf(dir->msg, relabelcmd, dev_name.c_str(), oldname, newname, poolname, mediatype, &slot,
               &drive, &blocksizes.min_block_size, &blocksizes.max_block_size) == 9) {
      ok = true;
    }
  } else {
    *oldname = 0;
    if (sscanf(dir->msg, labelcmd, dev_name.c_str(), newname, poolname, mediatype, &slot, &drive,
  ...
  switch (label_status) {
    ...
    case VOL_NO_LABEL:
      ...
      dir->fsend("3000 OK label. VolBytes=%s Volume=\"%s\" Device=%s\n",
                 edit_uint64(dev->VolCatInfo.VolCatBytes, ed1), newname, dev->print_name());
      break;
    case VOL_NO_MEDIA:
 ...
 */
static DeviceControlRecord *find_device(JobControlRecord *jcr, PoolMem &devname,
                                        drive_number_t drive, BlockSizes *blocksizes) {
...
static bool BootstrapCmd(JobControlRecord *jcr) { return GetBootstrapFile(jcr, jcr->dir_bsock); }
...
static bool ChangerCmd(JobControlRecord *jcr) {
  ...
  if (sscanf(dir->msg, "autochanger listall %127s", devname.c_str()) == 1) {
    ...
    cmd = "drives";
    safe_cmd = ok = true;
  } else if (sscanf(dir->msg, "autochanger transfer %127s %hd %hd", devname.c_str(), &src_slot,
                    &dst_slot) == 3) {
    cmd = "transfer";
    safe_cmd = ok = true;
    is_transfer = true;
...
static void SendDirBusyMessage(BareosSocket *dir, Device *dev) {
  ...
  if (dev->IsBlocked()) {
    switch (dev->blocked()) {
      ...
      case BST_UNMOUNTED_WAITING_FOR_SYSOP:
        dir->fsend(
            _("3932 Device \"%s\" is BLOCKED. user unmounted during wait for media/mount.\n"),
            dev->print_name());
        break;
      case BST_WAITING_FOR_SYSOP:
...
static bool ReplicateCmd(JobControlRecord *jcr) {
  ...
  if (sscanf(dir->msg, replicatecmd, JobName, stored_addr, &stored_port, &enable_ssl,
             sd_auth_key.c_str()) != 5) {
...
static bool PassiveCmd(JobControlRecord *jcr) {
   ...
   */
  if (!fd->connect(jcr, 10, (int)me->FDConnectTimeout, me->heartbeat_interval, _("File Daemon"),
                   filed_addr, NULL, filed_port, 1)) {
    delete fd;
    fd = NULL;
  }
