...
int ReadDevVolumeLabel(DeviceControlRecord *dcr) {
   ...
   */
  want_ansi_label =
      dcr->VolCatInfo.LabelType != B_BAREOS_LABEL || dcr->device->label_type != B_BAREOS_LABEL;
  ...
  if (!dcr->ReadBlockFromDev(NO_BLOCK_NUMBER_CHECK)) {
    ...
    Dmsg1(130, "%s", jcr->errmsg);
  } else if (!bstrcmp(dev->VolHdr.Id, BareosId) && !bstrcmp(dev->VolHdr.Id, OldBaculaId) &&
...
bool WriteNewVolumeLabelToDev(DeviceControlRecord *dcr, const char *VolName, const char *PoolName,
  ...
  /*
   * Reset blocksizes from volinfo to device as we set blocksize to DEFAULT_BLOCK_SIZE to read the
   * label.
   */
  dev->SetBlocksizes(dcr);
...
bool DeviceControlRecord::RewriteVolumeLabel(bool recycle) {
  ...
  if (!dev->HasCap(CAP_STREAM)) {
    ...
    if (recycle) {
      ...
      if (!dev->open(dcr, OPEN_READ_WRITE)) {
        Jmsg2(jcr, M_FATAL, 0, _("Failed to re-open after truncate on device %s: ERR=%s\n"),
              dev->print_name(), dev->print_errmsg());
        return false;
      }
    }
...
bool WriteSessionLabel(DeviceControlRecord *dcr, int label) {
  ...
  Dmsg6(150,
        ...
        stream_to_ascii(buf2, rec->Stream, rec->FileIndex), rec->data_len, rec->remainder);

  FreeRecord(rec);
  Dmsg2(150, "Leave WriteSessionLabel Block=%ud File=%ud\n", dev->GetBlockNum(), dev->GetFile());
  return true;
}
...
bool UnserVolumeLabel(Device *dev, DeviceRecord *rec) {
  ...
  if (rec->FileIndex != VOL_LABEL && rec->FileIndex != PRE_LABEL) {
    Mmsg3(dev->errmsg, _("Expecting Volume Label, got FI=%s Stream=%s len=%d\n"),
          FI_to_ascii(buf1, rec->FileIndex), stream_to_ascii(buf2, rec->Stream, rec->FileIndex),
          rec->data_len);
    if (!forge_on) {
      return false;
    }
...
void DumpVolumeLabel(Device *dev) {
  ...
  Pmsg11(-1,
         _("\nVolume Label:\n"
           ...
           "HostName          : %s\n"
           ""),
         dev->VolHdr.Id, dev->VolHdr.VerNum, dev->VolHdr.VolumeName, dev->VolHdr.PrevVolumeName,
         File, LabelType, dev->VolHdr.LabelSize, dev->VolHdr.PoolName, dev->VolHdr.MediaType,
...
static void DumpSessionLabel(DeviceRecord *rec, const char *type) {
  ...
  Pmsg7(-1,
        _("\n%s Record:\n"
          ...
          "ClientName        : %s\n"
          ""),
        type, label.JobId, label.VerNum, label.PoolName, label.PoolType, label.JobName,
        label.ClientName);
  ...
  if (rec->FileIndex == EOS_LABEL) {
    Pmsg8(
        ...
        _("JobFiles          : %s\n"
          ...
          "JobStatus         : %c\n"
          ""),
        edit_uint64_with_commas(label.JobFiles, ec1), edit_uint64_with_commas(label.JobBytes, ec2),
        edit_uint64_with_commas(label.StartBlock, ec3),
        edit_uint64_with_commas(label.EndBlock, ec4), edit_uint64_with_commas(label.StartFile, ec5),
        edit_uint64_with_commas(label.EndFile, ec6), edit_uint64_with_commas(label.JobErrors, ec7),
        label.JobStatus);
  }
  if (label.VerNum >= 11) {
    char dt[50];
    ...
    switch (rec->FileIndex) {
      ...
      case EOM_LABEL:
        Pmsg7(-1, _("%s Record: File:blk=%u:%u SessId=%d SessTime=%d JobId=%d DataLen=%d\n"), type,
              dev->file, dev->block_num, rec->VolSessionId, rec->VolSessionTime, rec->Stream,
              rec->data_len);
        break;
      case EOT_LABEL:
    ...
    switch (rec->FileIndex) {
      case SOS_LABEL:
        ...
        Pmsg6(-1, _("%s Record: File:blk=%u:%u SessId=%d SessTime=%d JobId=%d\n"), type, dev->file,
              dev->block_num, rec->VolSessionId, rec->VolSessionTime, label.JobId);
        Pmsg4(-1, _("   Job=%s Date=%s Level=%c Type=%c\n"), label.Job, dt, label.JobLevel,
              label.JobType);
        break;
      case EOS_LABEL:
        char ed1[30], ed2[30];
        ...
        Pmsg7(-1, _("   Date=%s Level=%c Type=%c Files=%s Bytes=%s Errors=%d Status=%c\n"), dt,
              label.JobLevel, label.JobType, edit_uint64_with_commas(label.JobFiles, ed1),
              edit_uint64_with_commas(label.JobBytes, ed2), label.JobErrors, (char)label.JobStatus);
        break;
      case EOM_LABEL:
      case PRE_LABEL:
      case VOL_LABEL:
