...
static bool CloneRecordInternally(DeviceControlRecord *dcr, DeviceRecord *rec) {
  ...
  if (rec->FileIndex >= 0) {
    ...
    if (rec->VolSessionId != rec->last_VolSessionId ||
        rec->VolSessionTime != rec->last_VolSessionTime || rec->FileIndex != rec->last_FileIndex) {
      jcr->JobFiles++;
  ...
  while (!WriteRecordToBlock(jcr->dcr, jcr->dcr->after_rec)) {
    Dmsg4(200, "!WriteRecordToBlock blkpos=%u:%u len=%d rem=%d\n", dev->file, dev->block_num,
          jcr->dcr->after_rec->data_len, jcr->dcr->after_rec->remainder);
  ...
  Dmsg5(500, "wrote_record JobId=%d FI=%s SessId=%d Strm=%s len=%d\n", jcr->JobId,
        FI_to_ascii(buf1, jcr->dcr->after_rec->FileIndex), jcr->dcr->after_rec->VolSessionId,
        stream_to_ascii(buf2, jcr->dcr->after_rec->Stream, jcr->dcr->after_rec->FileIndex),
...
bool DoMacRun(JobControlRecord *jcr) {
  ...
  if (jcr->remote_replicate) {
     ...
     */
    if (!AcquireDeviceForRead(jcr->read_dcr) || !acquire_device_for_append(jcr->dcr)) {
      ok = false;
      goto bail_out;
    }
