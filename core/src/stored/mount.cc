...
enum { try_next_vol = 1, try_read_vol, try_error, try_default };

enum { check_next_vol = 1, check_ok, check_read_vol, check_error };
...
read_volume:
  ...
  if (!dev->haveVolCatInfo()) {
    ...
    Dmsg3(200,
          "applying vol block sizes to device %s: dcr->VolMinBlocksize set to %u, "
          "dcr->VolMaxBlocksize set to %u\n",
...
int DeviceControlRecord::CheckVolumeLabel(bool &ask, bool &autochanger) {
  ...
  Dmsg2(150, "Want dirVol=%s dirStat=%s\n", VolumeName, VolCatInfo.VolCatStatus);
      ...
      if (!dcr->DirGetVolumeInfo(GET_VOL_INFO_FOR_WRITE)) {
        ...
        Jmsg(jcr, M_WARNING, 0,
             _("Director wanted Volume \"%s\".\n"
               ...
               "    %s"),
             dcrVolCatInfo.VolCatName, dev->VolHdr.VolumeName, vol_info_msg.c_str());
        ask = true;
...
void DeviceControlRecord::DoSwapping(bool IsWriting) {
  ...
  if (dev->swap_dev) {
    if (dev->swap_dev->MustUnload()) {
      if (dev->vol) {
        dev->swap_dev->SetSlot(dev->vol->GetSlot());
      }
    ...
    if (dev->swap_dev->vol) {
      Dmsg2(100, "Vol=%s on dev=%s\n", dev->swap_dev->vol->vol_name, dev->swap_dev->print_name());
    }
    ...
    if (dev->VolCatInfo.VolCatBytes == (uint64_t)pos) {
      ...
      Jmsg(jcr, M_WARNING, 0,
           _("For Volume \"%s\":\n"
             ...
             "Correcting Catalog\n"),
           VolumeName, edit_uint64(pos, ed1), edit_uint64(dev->VolCatInfo.VolCatBytes, ed2));
...
int DeviceControlRecord::TryAutolabel(bool opened) {
  ...
  if (dev->HasCap(CAP_LABEL) && (VolCatInfo.VolCatBytes == 0 ||
    ...
    if (!WriteNewVolumeLabelToDev(dcr, VolumeName, pool_name, false /* no relabel */)) {
