...
extern "C" int BndmpAuthClear(struct ndm_session *sess, char *name, char *pass) {
  ...
  foreach_res(auth_config, R_NDMP) {
    ...
    if (bstrcmp(name, auth_config->username) && bstrcmp(pass, auth_config->password.value)) {
      /*
 ...
 */
static inline bool bndmp_write_data_to_block(JobControlRecord *jcr, int stream, char *data,
 ...
 * data_length == 0 = EOF
 */
static inline bool bndmp_read_data_from_block(JobControlRecord *jcr, char *data,
                                              uint32_t wanted_data_length, uint32_t *data_length) {
  ...
  while (!done) {
    ...
    if (!IsBlockEmpty(rctx->rec)) {
       ...
       */
      if (!ReadNextBlockFromDevice(dcr, &rctx->sessrec, NULL, MountNextReadVolume, &ok)) {
        return false;
      }
    ...
    switch (rctx->rec->maskedStream) {
      ...
      case STREAM_FILE_DATA:
        if (wanted_data_length < rctx->rec->data_len) {
          Jmsg0(jcr, M_FATAL, 0,
                _("Data read from volume bigger then NDMP databuffer, please increase the NDMP "
                  "blocksize.\n"));
          return false;
        }
...
static inline bool BndmpCreateVirtualFile(JobControlRecord *jcr, char *filename) {
   ...
   */
  size = Mmsg(data, "%ld %d %s%c%s%c%s%c%s%c%d%c", dcr->FileIndex, /* File_index */
              FT_REG,                                              /* File type */
...
extern "C" ndmp9_error bndmp_tape_open(struct ndm_session *sess, char *drive_name, int will_write) {
  JobControlRecord *jcr;
  ...
  *filesystem++ = '\0';
  ...
  ta->tape_state.open_mode = will_write ? NDMP9_TAPE_RDWR_MODE : NDMP9_TAPE_READ_MODE;
...
extern "C" ndmp9_error bndmp_tape_mtio(struct ndm_session *sess, ndmp9_tape_mtio_op op,
...
extern "C" ndmp9_error bndmp_tape_write(struct ndm_session *sess, char *buf, uint32_t count,
                                        uint32_t *done_count) {
  JobControlRecord *jcr;
  ndmp9_error err;
  struct ndmp_session_handle *handle;
...
extern "C" ndmp9_error bndmp_tape_read(struct ndm_session *sess, char *buf, uint32_t count,
                                       uint32_t *done_count) {
  JobControlRecord *jcr;
  ndmp9_error err;
  struct ndmp_session_handle *handle;
...
extern "C" void *handle_ndmp_client_request(void *arg) {
  ...
  if (!handle) {
    Emsg0(M_ABORT, 0, _("Illegal call to handle_ndmp_client_request with NULL session handle\n"));
    return NULL;
  }
...
extern "C" void *ndmp_thread_server(void *arg) {
  ...
  for (ipaddr = (IPADDR *)ntsa->addr_list->first(); ipaddr;
    ...
    for (next = (IPADDR *)ntsa->addr_list->next(ipaddr); next;
      ...
      if (ipaddr->GetSockaddrLen() == next->GetSockaddrLen() &&
          memcmp(ipaddr->get_sockaddr(), next->get_sockaddr(), ipaddr->GetSockaddrLen()) == 0) {
  ...
  foreach_dlist(ipaddr, ntsa->addr_list) {
     ...
     * Reuse old sockets
     */
    if (setsockopt(fd_ptr->fd, SOL_SOCKET, SO_REUSEADDR, (sockopt_val_t)&turnon, sizeof(turnon)) <
        0) {
      berrno be;
    ...
    for (tlog = 0; bind(fd_ptr->fd, ipaddr->get_sockaddr(), ipaddr->GetSockaddrLen()) < 0;
         tlog -= 5) {
      berrno be;
      if (tlog <= 0) {
      ...
      if (pfds[cnt++].revents & POLLIN) {
        ...
        if (!HostsAccess(&request)) {
          ...
          Jmsg2(NULL, M_SECURITY, 0, _("Connection from %s:%d refused by hosts.access\n"),
                sockaddr_to_ascii(&cli_addr, buf, sizeof(buf)), SockaddrGetPort(&cli_addr));
          close(new_sockfd);
          continue;
        }
        V(mutex);
         ...
         */
        if (setsockopt(new_sockfd, SOL_SOCKET, SO_KEEPALIVE, (sockopt_val_t)&turnon,
                       sizeof(turnon)) < 0) {
          berrno be;
