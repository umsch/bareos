...
static const char *compression_to_str(PoolMem &resultbuffer, const char *compression_algorithm,
                                      uint32_t data_length, uint16_t compression_level,
                                      uint16_t compression_algorithm_version) {
  PoolMem tmp(PM_MESSAGE);
  tmp.bsprintf("%s, level=%u, version=%u, length=%u", compression_algorithm, compression_level,
...
static const char *record_compression_to_str(PoolMem &resultbuffer, const DeviceRecord *rec) {
  ...
  if (maskedStream == STREAM_SPARSE_GZIP_DATA || maskedStream == STREAM_SPARSE_COMPRESSED_DATA) {
    uint64_t faddr = 0;
...
static const char *record_unix_attributes_to_str(PoolMem &resultbuffer, JobControlRecord *jcr,
...
static const char *get_record_short_info(PoolMem &resultbuffer, JobControlRecord *jcr,
                                         const DeviceRecord *rec) {
  switch (rec->maskedStream) {
    case STREAM_UNIX_ATTRIBUTES:
    case STREAM_UNIX_ATTRIBUTES_EX:
...
const char *record_to_str(PoolMem &resultbuffer, JobControlRecord *jcr, const DeviceRecord *rec) {
  ...
  resultbuffer.bsprintf("FileIndex=%-5d Stream=%-2d %-25s DataLen=%-5d", rec->FileIndex,
...
static inline ssize_t WriteHeaderToBlock(DeviceBlock *block, const DeviceRecord *rec,
                                         int32_t Stream) {
  ser_declare;
...
static inline ssize_t WriteDataToBlock(DeviceBlock *block, const DeviceRecord *rec) {
  ...
  memcpy(block->bufp, ((unsigned char *)rec->data) + (rec->data_len - rec->remainder), len);
  block->bufp += len;
  block->binbuf += len;
  ...
  if (!sm_check_rtn(__FILE__, __LINE__, False)) {
    ...
    Dmsg7(0,
          ...
          stream_to_ascii(buf2, rec->Stream, rec->FileIndex), rec->data_len, len,
          BlockWriteNavail(block), rec->remainder);
    Dmsg5(0, "Damaged block: bufp=%x binbuf=%d buf_len=%d rem=%d moved=%d\n", block->bufp,
          block->binbuf, block->buf_len, BlockWriteNavail(block), len);
...
bool ReadRecordFromBlock(DeviceControlRecord *dcr, DeviceRecord *rec) {
  ...
  if (remlen >= rhl) {
    Dmsg4(450, "Enter read_record_block: remlen=%d data_len=%d rem=%d blkver=%d\n", remlen,
          rec->data_len, rec->remainder, dcr->block->BlockVer);
     ...
     */
    if (rec->remainder &&
        (rec->VolSessionId != VolSessionId || rec->VolSessionTime != VolSessionTime)) {
    ...
    Dmsg6(450,
          ...
          "remlen=%d data_len=%d\n",
          FI_to_ascii(buf1, rec->FileIndex), rec->VolSessionId,
          stream_to_ascii(buf2, rec->Stream, rec->FileIndex), data_bytes, remlen, rec->data_len);
  } else {
    /*
