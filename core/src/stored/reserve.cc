...
static bool UseDeviceCmd(JobControlRecord *jcr) {
  ...
  do {
    ...
    ok = sscanf(dir->msg, use_storage, StoreName.c_str(), media_type.c_str(), pool_name.c_str(),
  ...
  if (debug_level >= debuglevel) {
    foreach_alist(store, dirstore) {
      Dmsg5(debuglevel, "Storage=%s media_type=%s pool=%s pool_type=%s append=%d\n", store->name,
            store->media_type, store->pool_name, store->pool_type, store->append);
...
bool FindSuitableDeviceForJob(JobControlRecord *jcr, ReserveContext &rctx) {
  ...
  Dmsg5(debuglevel, "Start find_suit_dev PrefMnt=%d exact=%d suitable=%d chgronly=%d any=%d\n",
        rctx.PreferMountedVols, rctx.exact_match, rctx.suitable_device, rctx.autochanger_only,
        rctx.any_drive);
...
static int ReserveDevice(ReserveContext &rctx) {
  ...
  if (!rctx.device->dev) {
    if (rctx.device->changer_res) {
      Jmsg(rctx.jcr, M_WARNING, 0,
           _("\n"
             "     Device \"%s\" in changer \"%s\" requested by DIR could not be opened or does "
             "not exist.\n"),
           rctx.device->name(), rctx.device_name);
    } else {
      Jmsg(rctx.jcr, M_WARNING, 0,
  ...
  if (rctx.store->append == SD_APPEND) {
    ...
    Dmsg5(debuglevel, "Reserved=%d dev_name=%s mediatype=%s pool=%s ok=%d\n",
          dcr->dev->NumReserved(), dcr->dev_name, dcr->media_type, dcr->pool_name, ok);
...
static bool ReserveDeviceForRead(DeviceControlRecord *dcr) {
  ...
  if (dev->IsBusy()) {
    Dmsg4(debuglevel, "Device %s is busy ST_READREADY=%d num_writers=%d reserved=%d.\n",
          dev->print_name(), BitIsSet(ST_READREADY, dev->state) ? 1 : 0, dev->num_writers,
          dev->NumReserved());
...
static int IsPoolOk(DeviceControlRecord *dcr) {
   ...
   */
  if (bstrcmp(dev->pool_name, dcr->pool_name) && bstrcmp(dev->pool_type, dcr->pool_type)) {
    /*
    ...
    Mmsg(jcr->errmsg,
         _("3608 JobId=%u wants Pool=\"%s\" but have Pool=\"%s\" nreserve=%d on drive %s.\n"),
         (uint32_t)jcr->JobId, dcr->pool_name, dev->pool_name, dev->NumReserved(),
         dev->print_name());
    Dmsg1(debuglevel, "Failed: %s", jcr->errmsg);
    QueueReserveMessage(jcr);
  }
...
static bool IsMaxJobsOk(DeviceControlRecord *dcr) {
  ...
  Dmsg5(debuglevel, "MaxJobs=%d Jobs=%d reserves=%d Status=%s Vol=%s\n",
        dcr->VolCatInfo.VolCatMaxJobs, dcr->VolCatInfo.VolCatJobs, dev->NumReserved(),
        dcr->VolCatInfo.VolCatStatus, dcr->VolumeName);

  /*
   * Limit max concurrent jobs on this drive
   */
  if (dev->max_concurrent_jobs > 0 &&
      dev->max_concurrent_jobs <= (uint32_t)(dev->num_writers + dev->NumReserved())) {
    /*
...
static int CanReserveDrive(DeviceControlRecord *dcr, ReserveContext &rctx) {
  ...
  Dmsg5(debuglevel, "PrefMnt=%d exact=%d suitable=%d chgronly=%d any=%d\n", rctx.PreferMountedVols,
        rctx.exact_match, rctx.suitable_device, rctx.autochanger_only, rctx.any_drive);
   ...
   */
  if (rctx.autochanger_only && !dev->IsBusy() && dev->VolHdr.VolumeName[0] == 0) {
    /*
