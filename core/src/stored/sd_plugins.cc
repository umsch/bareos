...
static bRC bareosDebugMsg(bpContext *ctx, const char *file, int line, int level, const char *fmt,
                          ...);
static char *bareosEditDeviceCodes(DeviceControlRecord *dcr, POOLMEM *&omsg, const char *imsg,
                                   const char *cmd);
 ...
 */
char *edit_device_codes(DeviceControlRecord *dcr, POOLMEM *&omsg, const char *imsg,
                        const char *cmd) {
  const char *p;
  const char *str;
  char ed1[50];
...
static inline bool trigger_plugin_event(JobControlRecord *jcr, bsdEventType eventType,
                                        bsdEvent *event, bpContext *ctx, void *value,
...
void LoadSdPlugins(const char *plugin_dir, alist *plugin_names) {
  ...
  if (!LoadPlugins((void *)&binfo, (void *)&bfuncs, sd_plugin_list, plugin_dir, plugin_names,
...
static bool IsPluginCompatible(Plugin *plugin) {
  ...
  if (info->version != SD_PLUGIN_INTERFACE_VERSION) {
    ...
    Dmsg3(50, "Plugin version incorrect. Plugin=%s wanted=%d got=%d\n", plugin->file,
          SD_PLUGIN_INTERFACE_VERSION, info->version);
    return false;
  }
    ...
    foreach_alist_index(i, plugin_options, jcr->plugin_options) {
      ...
      *bp++ = '\0';
      ...
      if (instance < LOWEST_PLUGIN_INSTANCE || instance > HIGHEST_PLUGIN_INSTANCE) {
        Jmsg(NULL, M_ERROR, 0,
             _("Illegal SD plugin options encountered, %s instance %d skipping\n"), plugin_options,
             instance);
        continue;
      }
...
static DeviceRecord *bareosNewRecord(bool with_data) { return new_record(with_data); }
