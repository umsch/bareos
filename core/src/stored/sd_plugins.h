...
typedef struct s_sdbareosFuncs {
  ...
  char *(*EditDeviceCodes)(DeviceControlRecord *dcr, POOLMEM *&omsg, const char *imsg,
                           const char *cmd);
...
bRC GeneratePluginEvent(JobControlRecord *jcr, bsdEventType event, void *value = NULL,
                        bool reverse = false);
#endif
...
typedef enum { psdVarName = 1, psdVarDescription = 2 } psdVariable;
