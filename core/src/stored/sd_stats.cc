...
static char DevStats[] =
    "Devicestats [%lld]: Device=%s Read=%llu, Write=%llu, SpoolSize=%llu, NumWaiting=%ld, "
    "NumWriters=%ld, "
...
void UpdateDeviceTapealert(const char *devname, uint64_t flags, utime_t now) {
  ...
  Dmsg3(200, "New stats [%lld]: Device %s TapeAlert %llu\n", tape_alert->timestamp,
        dev_stats->DevName, tape_alert->flags);
}
...
static inline void UpdateDeviceStatistics(const char *devname, Device *dev, utime_t now) {
  ...
  if (found && dev_stats->cached) {
    ...
    if (dev_stat->DevReadBytes == dev->DevReadBytes &&
        dev_stat->DevWriteBytes == dev->DevWriteBytes && dev_stat->spool_size == dev->spool_size) {
      return;
    }
  } else if (!found) {
    dev_stats = (struct device_statistics *)malloc(sizeof(struct device_statistics));
  ...
  Dmsg5(200, "New stats [%lld]: Device %s Read %llu, Write %llu, Spoolsize %llu,\n",
        dev_stat->timestamp, dev_stats->DevName, dev_stat->DevReadBytes, dev_stat->DevWriteBytes,
        dev_stat->spool_size);
  ...
  Dmsg4(200, "MediaId=%ld VolBytes=%llu, VolFiles=%llu, VolBlocks=%llu\n", dev_stat->MediaId,
        dev_stat->VolCatBytes, dev_stat->VolCatFiles, dev_stat->VolCatBlocks);
}
...
void UpdateJobStatistics(JobControlRecord *jcr, utime_t now) {
  ...
  if (found && job_stats->cached) {
    ...
    if (job_stat->JobFiles == jcr->JobFiles && job_stat->JobBytes == jcr->JobBytes) {
      return;
    }
  } else if (!found) {
  ...
  Dmsg5(200, "New stats [%lld]: JobId %ld, JobFiles %lu, JobBytes %llu, DevName %s\n",
        job_stat->timestamp, job_stats->JobId, job_stat->JobFiles, job_stat->JobBytes,
        job_stat->DevName);
}
      ...
      if (dev_stats->statistics) {
        ...
        while (dev_stat) {
          ...
          if (!dev_stat->collected) {
            ...
            BashSpaces(dev_tmp);
            Mmsg(msg, DevStats, dev_stat->timestamp, dev_tmp.c_str(), dev_stat->DevReadBytes,
                 dev_stat->DevWriteBytes, dev_stat->spool_size, dev_stat->num_waiting,
                 dev_stat->num_writers, dev_stat->DevReadTime, dev_stat->DevWriteTime,
      ...
      if (job_stats->statistics) {
        ...
        while (job_stat) {
          ...
          if (!job_stat->collected) {
            ...
            BashSpaces(dev_tmp);
            Mmsg(msg, JobStats, job_stat->timestamp, job_stats->JobId, job_stat->JobFiles,
                 job_stat->JobBytes, dev_tmp.c_str());
            Dmsg1(100, ">dird: %s", msg.c_str());
            dir->fsend(msg.c_str());
          }
