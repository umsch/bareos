...
void ListSpoolStats(void sendit(const char *msg, int len, void *sarg), void *arg) {
  ...
  if (spool_stats.data_jobs || spool_stats.max_data_size) {
    len =
        Mmsg(msg, _("Data spooling: %u active jobs, %s bytes; %u total jobs, %s max bytes/job.\n"),
  ...
  if (spool_stats.attr_jobs || spool_stats.max_attr_size) {
    len =
        Mmsg(msg, _("Attr spooling: %u active jobs, %s bytes; %u total jobs, %s max bytes.\n"),
...
static void MakeUniqueDataSpoolFilename(DeviceControlRecord *dcr, POOLMEM *&name) {
  ...
  Mmsg(name, "%s/%s.data.%u.%s.%s.spool", dir, my_name, dcr->jcr->JobId, dcr->jcr->Job,
       dcr->device->name());
}
...
static bool DespoolData(DeviceControlRecord *dcr, bool commit) {
  ...
  if (commit) {
    Jmsg(jcr, M_INFO, 0, _("Committing spooled data to Volume \"%s\". Despooling %s bytes ...\n"),
         jcr->dcr->VolumeName, edit_uint64_with_commas(jcr->dcr->job_spool_size, ec1));
  ...
  *rdev->errmsg = 0;
  ...
  Jmsg(jcr, M_INFO, 0,
       _("Despooling elapsed time = %02d:%02d:%02d, Transfer rate = %s Bytes/second\n"),
