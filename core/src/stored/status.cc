 ...
 */
static void trigger_device_status_hook(JobControlRecord *jcr, DeviceResource *device,
                                       StatusPacket *sp, bsdEventType eventType) {
  bsdDevStatTrig dst;
    ...
    if (dev && dev->IsOpen()) {
      if (dev->IsLabeled()) {
        ...
        len = Mmsg(msg,
                   ...
                   dev->pool_name[0] ? dev->pool_name : "*unknown*", dev->device->media_type);
        sendit(msg, len, sp);
      } else {
      ...
      len =
          Mmsg(msg, _("    Positioned at File=%s Block=%s\n"),
  ...
  foreach_vol(vol) {
    ...
    if (dev) {
      ...
      len = Mmsg(msg, "    Reader=%d writers=%d reserves=%d volinuse=%d\n", dev->CanRead() ? 1 : 0,
                 dev->num_writers, dev->NumReserved(), vol->IsInUse());
      sendit(msg.c_str(), len, sp);
    } else {
  ...
  foreach_read_vol(vol) {
    ...
    if (dev) {
      ...
      len = Mmsg(msg, "    Reader=%d writers=%d reserves=%d volinuse=%d JobId=%d\n",
                 dev->CanRead() ? 1 : 0, dev->num_writers, dev->NumReserved(), vol->IsInUse(),
                 vol->GetJobid());
      sendit(msg.c_str(), len, sp);
    } else {
...
static void ListStatusHeader(StatusPacket *sp) {
  ...
  len = Mmsg(msg, _("%s Version: %s (%s) %s %s %s\n"), my_name, VERSION, BDATE, HOST_OS, DISTNAME,
             DISTVER);
  sendit(msg, len, sp);
  ...
  len =
      Mmsg(msg, _("Daemon started %s. Jobs: run=%d, running=%d.\n"), dt, num_jobs_run, JobCount());
  sendit(msg, len, sp);
  ...
  if (debug_level > 0) {
    len = Mmsg(msg, "APIs=%sOPT,%sATP,%sLPV,%sCFA,%sCFW,\n", p_OpenProcessToken ? "" : "!",
               p_AdjustTokenPrivileges ? "" : "!", p_LookupPrivilegeValue ? "" : "!",
    ...
    len = Mmsg(msg, " %sWUL,%sWMKD,%sGFAA,%sGFAW,%sGFAEA,%sGFAEW,%sSFAA,%sSFAW,%sBR,%sBW,%sSPSP,\n",
               ...
               p_GetFileAttributesW ? "" : "!", p_GetFileAttributesExA ? "" : "!",
               p_GetFileAttributesExW ? "" : "!", p_SetFileAttributesA ? "" : "!",
               p_SetFileAttributesW ? "" : "!", p_BackupRead ? "" : "!", p_BackupWrite ? "" : "!",
               p_SetProcessShutdownParameters ? "" : "!");
    sendit(msg.c_str(), len, sp);
    len = Mmsg(msg, " %sWC2MB,%sMB2WC,%sFFFA,%sFFFW,%sFNFA,%sFNFW,%sSCDA,%sSCDW,\n",
               p_WideCharToMultiByte ? "" : "!", p_MultiByteToWideChar ? "" : "!",
               p_FindFirstFileA ? "" : "!", p_FindFirstFileW ? "" : "!", p_FindNextFileA ? "" : "!",
               p_FindNextFileW ? "" : "!", p_SetCurrentDirectoryA ? "" : "!",
  ...
  len = Mmsg(msg, _(" Heap: heap=%s smbytes=%s max_bytes=%s bufs=%s max_bufs=%s\n"),
             ...
             edit_uint64_with_commas(sm_bytes, b2), edit_uint64_with_commas(sm_max_bytes, b3),
             edit_uint64_with_commas(sm_buffers, b4), edit_uint64_with_commas(sm_max_buffers, b5));
  sendit(msg, len, sp);
  len =
      Mmsg(msg,
           " Sizes: boffset_t=%d size_t=%d int32_t=%d int64_t=%d "
           "mode=%d bwlimit=%skB/s\n",
           (int)sizeof(boffset_t), (int)sizeof(size_t), (int)sizeof(int32_t), (int)sizeof(int64_t),
           (int)DEVELOPER_MODE, edit_uint64_with_commas(me->max_bandwidth_per_job / 1024, b1));
  sendit(msg, len, sp);
  ...
  if (dev->IsAutochanger()) {
    if (dev->GetSlot() > 0) {
      len = Mmsg(msg, _("    Slot %hd %s loaded in drive %hd.\n"), dev->GetSlot(),
                 dev->IsOpen() ? "is" : "was last", dev->drive);
      sendit(msg, len, sp);
...
static void SendDeviceStatus(Device *dev, StatusPacket *sp) {
  ...
  if (debug_level > 5) {
    ...
    len = Mmsg(msg,
               "  %sEOF %sBSR %sBSF %sFSR %sFSF %sEOM %sREM %sRACCESS %sAUTOMOUNT %sLABEL "
               "%sANONVOLS %sALWAYSOPEN\n",
               ...
               dev->HasCap(CAP_AUTOMOUNT) ? "" : "!", dev->HasCap(CAP_LABEL) ? "" : "!",
               dev->HasCap(CAP_ANONVOLS) ? "" : "!", dev->HasCap(CAP_ALWAYSOPEN) ? "" : "!");
    sendit(msg, len, sp);
  }
  ...
  len = Mmsg(msg,
             "  %sOPENED %sTAPE %sLABEL %sMALLOC %sAPPEND %sREAD %sEOT %sWEOT %sEOF %sNEXTVOL "
             "%sSHORT %sMOUNTED\n",
             dev->IsOpen() ? "" : "!", dev->IsTape() ? "" : "!", dev->IsLabeled() ? "" : "!",
             BitIsSet(ST_MALLOC, dev->state) ? "" : "!", dev->CanAppend() ? "" : "!",
             dev->CanRead() ? "" : "!", dev->AtEot() ? "" : "!",
             BitIsSet(ST_WEOT, dev->state) ? "" : "!", dev->AtEof() ? "" : "!",
             BitIsSet(ST_NEXTVOL, dev->state) ? "" : "!", BitIsSet(ST_SHORT, dev->state) ? "" : "!",
  ...
  foreach_jcr(jcr) {
    ...
    if ((dcr && dcr->device) || (rdcr && rdcr->device)) {
      ...
      if (rdcr && rdcr->device) {
        len = Mmsg(msg,
                   _("Reading: %s %s job %s JobId=%d Volume=\"%s\"\n"
                     "    pool=\"%s\" device=%s\n"),
                   job_level_to_str(jcr->getJobLevel()), job_type_to_str(jcr->getJobType()),
                   JobName, jcr->JobId, rdcr->VolumeName, rdcr->pool_name,
      ...
      len = Mmsg(msg, _("    Files=%s Bytes=%s AveBytes/sec=%s LastBytes/sec=%s\n"),
                 ...
                 edit_uint64_with_commas(jcr->JobBytes, b2), edit_uint64_with_commas(avebps, b3),
      ...
      if (jcr->file_bsock) {
        len = Mmsg(msg, _("    FDReadSeqNo=%s in_msg=%u out_msg=%d fd=%d\n"),
                   ...
                   jcr->file_bsock->in_msg_no, jcr->file_bsock->out_msg_no, jcr->file_bsock->fd_);
        sendit(msg, len, sp);
      } else {
...
static void sendit(const char *msg, int len, void *sp) { sendit(msg, len, (StatusPacket *)sp); }
