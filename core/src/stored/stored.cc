...
int main(int argc, char *argv[]) {
  ...
  CreatePidFile(me->pid_directory, "bareos-sd", GetFirstPortHostOrder(me->SDaddrs));
  ReadStateFile(me->working_directory, "bareos-sd", GetFirstPortHostOrder(me->SDaddrs));
...
static int check_resources() {
  ...
  foreach_res(director, R_DIRECTOR) {
    ...
    if ((director->tls_cert.keyfile == nullptr || director->tls_cert.keyfile->empty()) &&
        tls_needed) {
  ...
  foreach_res(device, R_DEVICE) {
    if (device->drive_crypto_enabled && BitIsSet(CAP_LABEL, device->cap_bits)) {
      Jmsg(NULL, M_FATAL, 0,
           _("LabelMedia enabled is incompatible with tape crypto on Device \"%s\" in %s.\n"),
