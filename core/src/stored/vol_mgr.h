...
#define foreach_vol(vol) for (vol = vol_walk_start(); vol; (vol = VolWalkNext(vol)))

#define endeach_vol(vol) VolWalkEnd(vol)

#define foreach_read_vol(vol) for (vol = read_vol_walk_start(); vol; (vol = ReadVolWalkNext(vol)))
