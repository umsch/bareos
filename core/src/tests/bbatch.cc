...
int main(int argc, char *argv[]) {
  ...
  if (restore_list) {
    ...
    if ((db = db_init_database(NULL, db_driver, db_name, db_user, db_password, db_host, 0, NULL,
                               false, disable_batch, false, false)) == NULL) {
...
static void *do_batch(void *jcr) {
  ...
  printf("Create %u files at %.2f/s\n", lineno, (lineno / ((float)((end - begin) / 1000000))));
  nb--;
  V(mutex);
  pthread_exit(NULL);
  return NULL;
