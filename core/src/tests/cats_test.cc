...
int main(int argc, char *argv[]) {
  ...
  if (full_test) {
    db = db_init_database(jcr, db_driver, db_name, db_user, db_password, db_address, db_port + 100,
                          NULL);
  ...
  ok(db_sql_query(db, "SELECT VersionId FROM Version", DbIntHandler, &j), "SELECT VersionId");

  ok(UPDATE_DB(jcr, db, (char *)"UPDATE Version SET VersionId = 1"), "Update VersionId");
  ...
  ok(db_sql_query(db, "SELECT MIN(StorageId) FROM Storage", DbIntHandler, &storageid),
     "Get StorageId");
  ...
  ok(db_sql_query(db, "SELECT 1,2,3,4,5,'a','b','c','d','e'", CountCol, &j), "Count 10 rows");
  ...
  ok(db_sql_query(db, "SELECT 1 FROM Media WHERE VolumeName='missing'", DbIntHandler, &j),
     "Good empty SELECT query");
  ...
  db_list_ctx lctx;
  ok(db_sql_query(db, "SELECT FileId FROM File ORDER By FileId LIMIT 10", DbListHandler, &lctx),
     "db_list_ctx");
