...
extern "C" {
typedef int (*loadPlugin)(void *binfo, void *bfuncs, void **pinfo, void **pfuncs);
typedef int (*unloadPlugin)(void);
}
#define DEFAULT_API_VERSION 1

enum plugintype { DIRPLUGIN, FDPLUGIN, SDPLUGIN, ERRORPLUGIN };
...
struct _bareosfuncs {
  ...
  int (*JobMessage)(void *ctx, const char *file, int line, int type, int64_t mtime, const char *fmt,
                    ...);
...
void *bareosMalloc(void *ctx, const char *file, int line, size_t size) { return MALLOC(size); };

void bareosFree(void *ctx, const char *file, int line, void *mem) { FREE(mem); };
...
int Getplugintype(progdata *pdata) {
  ...
  if (pinfo->plugin_magic && bstrcmp(pinfo->plugin_magic, DIR_PLUGIN_MAGIC)) {
    ...
    if (pinfo->plugin_magic && bstrcmp(pinfo->plugin_magic, FD_PLUGIN_MAGIC)) {
      ...
      if (pinfo->plugin_magic && bstrcmp(pinfo->plugin_magic, SD_PLUGIN_MAGIC)) {
        return SDPLUGIN;
      } else {
        return ERRORPLUGIN;
      }
