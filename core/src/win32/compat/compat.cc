 ...
 * as the security default. If a process does not call CoInitializeSecurity, COM calls it
 * automatically the first time an interface is marshaled or unmarshaled, registering the system
 * default security. No default security packages are registered until then.
 *
...
static void VSSPathConvertCleanup(void *arg) {
  ...
  Dmsg1(debuglevel,
        "VSSPathConvertCleanup: Cleanup thread specific conversion pointers at address %p\n", tvpc);
...
static inline POOLMEM *make_wchar_win32_path(POOLMEM *pszUCSPath, BOOL *pBIsRawPath /*= NULL*/) {
  ...
  if (bAddDrive || bAddCurrentPath) {
    ...
    if (dwCurDirPathSize > 0) {
       ...
       */
      pwszCurDirBuf = (wchar_t *)CheckPoolMemorySize((POOLMEM *)pwszCurDirBuf,
  ...
  if (bAddCurrentPath) {
    ...
    pwszBuf =
        (wchar_t *)CheckPoolMemorySize((POOLMEM *)pwszBuf, dwBufCharsNeeded * sizeof(wchar_t));
  ...
  *win32_name = 0;
  ...
  /*
   * Here we convert to VSS specific file name which can get longer because VSS will make something
   * like
   * \\\\?\\GLOBALROOT\\Device\\HarddiskVolumeShadowCopy1\\bareos\\uninstall.exe from
   * c:\bareos\uninstall.exe
   */
...
int make_win32_path_UTF8_2_wchar(POOLMEM *&pszUCS, const char *pszUTF,
  ...
  if (tcc) {
    ...
    tcc->pWin32ConvUTF8Cache =
        CheckPoolMemorySize(tcc->pWin32ConvUTF8Cache, tcc->dwWin32ConvUTF8strlen + 2);
...
bool CreateJunction(const char *szJunction, const char *szPath) {
   ...
   */
  hDir = p_CreateFileW((LPCWSTR)szJunctionW, GENERIC_WRITE, 0, NULL, OPEN_EXISTING,
...
#define MOUNTPOINTREPARSEBUFFER_FIXED_HEADER_SIZE 8
  ...
  wcsncpy((LPWSTR)rdb->MountPointReparseBuffer.PathBuffer, (LPWSTR)szSubstituteName,
   ...
   */
  if (!DeviceIoControl(hDir, FSCTL_SET_REPARSE_POINT, (LPVOID)buf, data_length, NULL, 0, &dwRet,
                       0)) {
...
const char *errorString(void) {
  ...
  FormatMessage(
      ...
      NULL, GetLastError(), MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), /* Default lang */
...
static inline bool GetVolumeMountPointData(const char *filename, POOLMEM *&devicename) {
  ...
  if (p_GetFileAttributesW) {
    ...
    if (p_CreateFileW) {
      h = CreateFileW((LPCWSTR)pwszBuf, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
    ...
    h = CreateFileA(win32_fname, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
                    FILE_FLAG_BACKUP_SEMANTICS | FILE_FLAG_OPEN_REPARSE_POINT, NULL);
  ...
  if (h != INVALID_HANDLE_VALUE) {
    ...
    memset(buf, 0, buf_length);
    rdb->ReparseTag = IO_REPARSE_TAG_MOUNT_POINT;
    ok = DeviceIoControl(h, FSCTL_GET_REPARSE_POINT, NULL, 0, /* in buffer, bytes */
  ...
  if (h != INVALID_HANDLE_VALUE) {
    ...
    if (ok) {
      ...
      *(wchar_t *)(rdb->SymbolicLinkReparseBuffer.PathBuffer + offset + len) = L'\0';
      ...
      nrconverted =
          wchar_2_UTF8(path, (wchar_t *)(rdb->SymbolicLinkReparseBuffer.PathBuffer + offset));
      ...
      if (bstrncasecmp(
              path, "?\\",
              2)) { /* skip ?\\ if exists, this is a MountPoint that we opened as Symlink */
        ofs = 2;
        Dmsg0(debuglevel,
              "?\\ was in filename, skipping, use of MountPointReparseDataBuffer is needed\n");
      }
...
static int GetWindowsFileInfo(const char *filename, struct stat *sb, bool is_directory) {
  ...
  if (p_FindFirstFileW) { /* use unicode */
    ...
    if (fh != INVALID_HANDLE_VALUE) {
      h = p_CreateFileW((LPCWSTR)pwszBuf, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
...
int fstat(intptr_t fd, struct stat *sb) {
  ...
  if (p_GetFileInformationByHandleEx) {
    ...
    if (p_GetFileInformationByHandleEx((HANDLE)_get_osfhandle(fd), FileBasicInfo, &binfo,
                                       sizeof(binfo))) {
...
static int stat2(const char *filename, struct stat *sb) {
  ...
  if (p_GetFileAttributesW) {
    ...
    if (p_CreateFileW) {
      h = CreateFileW((LPCWSTR)pwszBuf, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0,
                      NULL);
    }
    FreePoolMemory(pwszBuf);
  } else if (p_GetFileAttributesA) {
    ...
    h = CreateFileA(win32_fname, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL);
  ...
  if (((data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) ||
    ...
    if (p_GetFileInformationByHandleEx) {
      ...
      if (p_CreateFileW) {
        ...
        h = p_CreateFileW((LPCWSTR)pwszBuf, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0,
                          NULL);
...
#else
int gettimeofday(struct timeval *tv, struct timezone *tz) { return mingw_gettimeofday(tv, tz); }
#endif  // HAVE_MINGW
...
int win32_fputs(const char *string, FILE *stream) {
  ...
  if (hOut && (hOut != INVALID_HANDLE_VALUE) && p_WideCharToMultiByte && p_MultiByteToWideChar &&
      (stream == stdout)) {
    POOLMEM *pwszBuf;
    POOLMEM *pszBuf;
    ...
    dwChars = p_WideCharToMultiByte(GetConsoleOutputCP(), 0, (LPCWSTR)pwszBuf, -1, pszBuf, dwChars,
                                    NULL, NULL);
    FreePoolMemory(pwszBuf);
...
bool win32_restore_file_attributes(POOLMEM *ofname, HANDLE handle,
  ...
  if (handle != INVALID_HANDLE_VALUE) {
    ...
    if (atts->dwFileAttributes & FILE_ATTRIBUTE_COMPRESSED) {
      ...
      if (!DeviceIoControl(handle, FSCTL_SET_COMPRESSION, &format, sizeof(format), NULL, 0,
                           &bytesreturned, NULL)) {
        goto bail_out;
      }
    }
...
winver::winver(void) {
  ...
  const int ver = _mkversion(osvinfo.dwPlatformId, osvinfo.dwMajorVersion, osvinfo.dwMinorVersion);
  ...
  snprintf(WIN_VERSION, sizeof(WIN_VERSION), "%s %lu.%lu.%lu", platform, osvinfo.dwMajorVersion,
  ...
  if (pExtension == NULL) {
    ...
    for (int index = 0; index < (int)(sizeof(ExtensionList) / sizeof(ExtensionList[0])); index++) {
      ...
      if (!bHasPathSeparators) {
         ...
         */
        dwAltNameLength =
            SearchPath(NULL, pPathname, ExtensionList[index], MAX_PATHLENGTH, pAltPathname, NULL);
...
static BOOL CreateChildProcessW(const char *comspec, const char *cmdLine,
                                PROCESS_INFORMATION *hProcInfo, HANDLE in, HANDLE out, HANDLE err) {
   ...
   */
  bFuncRetn =
      p_CreateProcessW((wchar_t *)comspec_wchar, (wchar_t *)cmdLine_wchar, /* Command line */
 ...
 */
static BOOL CreateChildProcessA(const char *comspec, char *cmdLine, PROCESS_INFORMATION *hProcInfo,
...
HANDLE CreateChildProcess(const char *cmdline, HANDLE in, HANDLE out, HANDLE err) {
  ...
  if (p_CreateProcessW && p_MultiByteToWideChar) {
    bFuncRetn = CreateChildProcessW(comspec, cmdLine.c_str(), &piProcInfo, in, out, err);
  } else {
    bFuncRetn = CreateChildProcessA(comspec, cmdLine.c_str(), &piProcInfo, in, out, err);
  }
...
Bpipe *open_bpipe(char *prog, int wait, const char *mode, bool dup_stderr) {
  ...
  if (mode_read) {
     ...
     */
    fSuccess = DuplicateHandle(GetCurrentProcess(), hChildStdoutRd, GetCurrentProcess(),
  ...
  if (mode_write) {
     ...
     */
    fSuccess = DuplicateHandle(GetCurrentProcess(), hChildStdinWr, GetCurrentProcess(),
                               &hChildStdinWrDup, 0,
                               FALSE,  // not inherited
 ...
 */
void PreventOsSuspensions() { SetThreadExecutionState(ES_CONTINUOUS | ES_SYSTEM_REQUIRED); }
