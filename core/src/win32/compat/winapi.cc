...
void InitWinAPIWrapper() {
  ...
  if (hLib) {
    ...
    p_GetLogicalDriveStringsA =
        (t_GetLogicalDriveStringsA)GetProcAddress(hLib, "GetLogicalDriveStringsA");
    p_GetLogicalDriveStringsW =
        (t_GetLogicalDriveStringsW)GetProcAddress(hLib, "GetLogicalDriveStringsW");
    ...
    p_GetFileInformationByHandleEx =
        (t_GetFileInformationByHandleEx)GetProcAddress(hLib, "GetFileInformationByHandleEx");
#endif
    ...
    p_SetProcessShutdownParameters =
        (t_SetProcessShutdownParameters)GetProcAddress(hLib, "SetProcessShutdownParameters");
    ...
    if (g_platform_id != VER_PLATFORM_WIN32_WINDOWS) {
      ...
      p_GetVolumeNameForVolumeMountPointW = (t_GetVolumeNameForVolumeMountPointW)GetProcAddress(
  ...
  if (g_platform_id != VER_PLATFORM_WIN32_WINDOWS) {
    ...
    if (hLib) {
      ...
      p_AdjustTokenPrivileges =
          (t_AdjustTokenPrivileges)GetProcAddress(hLib, "AdjustTokenPrivileges");
       ...
       * EFS calls
       */
      p_OpenEncryptedFileRawA =
          (t_OpenEncryptedFileRawA)GetProcAddress(hLib, "OpenEncryptedFileRawA");
      p_OpenEncryptedFileRawW =
          (t_OpenEncryptedFileRawW)GetProcAddress(hLib, "OpenEncryptedFileRawW");
      p_ReadEncryptedFileRaw = (t_ReadEncryptedFileRaw)GetProcAddress(hLib, "ReadEncryptedFileRaw");
      p_WriteEncryptedFileRaw =
          (t_WriteEncryptedFileRaw)GetProcAddress(hLib, "WriteEncryptedFileRaw");
      p_CloseEncryptedFileRaw =
          (t_CloseEncryptedFileRaw)GetProcAddress(hLib, "CloseEncryptedFileRaw");
    }
  }
