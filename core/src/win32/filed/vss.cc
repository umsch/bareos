...
bool VSSClient::GetShadowPath(const char *szFilePath, char *szShadowPath, int nBuflen) {
  ...
  if (bIsValidName)
    bIsValidName &= isalpha(szFilePath[0]) && szFilePath[1] == ':' && szFilePath[2] == '\\';

  if (bIsValidName) {
    int nDriveIndex = toupper(szFilePath[0]) - 'A';
    if (szShadowCopyName_[nDriveIndex][0] != 0) {

      if (WideCharToMultiByte(CP_UTF8, 0, szShadowCopyName_[nDriveIndex], -1, szShadowPath,
                              nBuflen - 1, NULL, NULL)) {
...
bool VSSClient::GetShadowPathW(const wchar_t *szFilePath, wchar_t *szShadowPath, int nBuflen) {
  ...
  if (bIsValidName)
    bIsValidName &= iswalpha(szFilePath[0]) && szFilePath[1] == ':' && szFilePath[2] == '\\';
...
const size_t VSSClient::GetWriterCount() { return pAlistWriterInfoText_->size(); }
