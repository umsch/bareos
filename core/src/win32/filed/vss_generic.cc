...
static void JmsgVssApiStatus(JobControlRecord *jcr, int msg_status, HRESULT hr,
                             const char *apiName) {
  const char *errmsg;
  ...
  switch (hr) {
    ...
    case VSS_E_BAD_STATE:
      errmsg =
          "Object is not initialized; called during restore or not called in correct sequence.";
      break;
    case VSS_E_WRITER_INFRASTRUCTURE:
      errmsg =
          "The writer infrastructure is not operating properly. Check that the Event Service and "
          "VSS have been started, and check for errors associated with those services in the error "
          "log.";
      break;
    ...
    case RPC_E_CHANGED_MODE:
      errmsg =
          "Previous call to CoInitializeEx specified the multithread apartment (MTA). This call "
          "indicates single-threaded apartment has occurred.";
      break;
    case S_FALSE:
...
static void JmsgVssWriterStatus(JobControlRecord *jcr, int msg_status,
                                VSS_WRITER_STATE eWriterStatus, char *writer_name) {
  const char *errmsg;

  /*
   * The following are normal states
   */
  if (eWriterStatus == VSS_WS_STABLE || eWriterStatus == VSS_WS_WAITING_FOR_BACKUP_COMPLETE) {
    return;
  }
  ...
  switch (eWriterStatus) {
    ...
    case VSS_WS_FAILED_AT_BACKUP_COMPLETE:
      errmsg =
          "The shadow copy has been created and the writer failed during the BackupComplete state.";
      break;
...
static inline wstring GetUniqueVolumeNameForPath(wstring path) {
  ...
  if (!p_GetVolumeNameForVolumeMountPointW ||
      !p_GetVolumeNameForVolumeMountPointW(volumeRootPath, volumeName, MAX_PATH)) {
    return L"";
  }
...
static inline bool HandleVolumeMountPoint(VSSClientGeneric *pVssClient,
                                          IVssBackupComponents *pVssObj, POOLMEM *&volumepath,
                                          POOLMEM *&mountpoint) {
  bool retval = false;
  HRESULT hr;
  POOLMEM *vol = NULL;
...
VSSClientGeneric::VSSClientGeneric() {
  ...
  if (hLib_) {
    ...
    VssFreeSnapshotProperties_ =
        (t_VssFreeSnapshotProperties)GetProcAddress(hLib_, "VssFreeSnapshotProperties");
  }
}
...
bool VSSClientGeneric::Initialize(DWORD dwContext, bool bDuringRestore) {
  ...
  if (!(CreateVssBackupComponents_ && VssFreeSnapshotProperties_)) {
    ...
    Jmsg(jcr_, M_FATAL, 0,
         "Entry point CreateVssBackupComponents or VssFreeSnapshotProperties missing.\n");
    return false;
  }
  ...
  if (!bDuringRestore) {
    ...
    if (FAILED(hr)) {
      Dmsg1(0,
            "VSSClientGeneric::Initialize: IVssBackupComponents->InitializeForBackup returned "
            "0x%08X\n",
            hr);
    ...
    if (GeneratePluginEvent(jcr_, bEventVssSetBackupState) != bRC_Skip) {
      ...
      if (FAILED(hr)) {
        Dmsg1(
            0,
            "VSSClientGeneric::Initialize: IVssBackupComponents->SetBackupState returned 0x%08X\n",
            hr);
    ...
    if (FAILED(hr)) {
      Dmsg1(0,
            "VSSClientGeneric::Initialize: IVssBackupComponents->GatherWriterMetadata returned "
            "0x%08X\n",
            hr);
...
bool VSSClientGeneric::CloseBackup() {
  ...
  /*
   * FIXME?: The docs http://msdn.microsoft.com/en-us/library/aa384582%28v=VS.85%29.aspx say this
   * isn't required...
   */
  if (uidCurrentSnapshotSet_ != GUID_NULL) {
    ...
    pVssObj->DeleteSnapshots(uidCurrentSnapshotSet_, VSS_OBJECT_SNAPSHOT_SET, false, &lSnapshots,
...
bool VSSClientGeneric::CheckWriterStatus() {
  ...
  for (unsigned iWriter = 0; iWriter < cWriters; iWriter++) {
     ...
     * Get writer status
     */
    hr = pVssObj->GetWriterStatus(iWriter, &idInstance, &idWriter, &bstrWriterName, &eWriterStatus,
                                  &hrWriterFailure);
    if (FAILED(hr)) {
      /*
       * Api failed
