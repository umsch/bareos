...
bool exclude_win32_not_to_backup_registry_entries(JobControlRecord *jcr, FindFilesPacket *ff) {
  ...
  if (CountIncludeListFileEntries(ff) == 0) {
    Qmsg(jcr, M_INFO, 1,
         _("Fileset has no \"File=\" directives, ignoring FilesNotToBackup Registry key\n"));
    return true;
  }
  ...
  for (int i = 0; i < ff->fileset->include_list.size(); i++) {
    ...
    for (int j = 0; j < incexe->opts_list.size(); j++) {
      ...
      if (BitIsSet(FO_NO_AUTOEXCL, fo->flags)) {
        Qmsg(jcr, M_INFO, 1,
             _("Fileset has autoexclude disabled, ignoring FilesNotToBackup Registry key\n"));
        return true;
      }
    }
  }
    ...
    if (cValues) {
      ...
      for (unsigned int i = 0; i < cValues; i++) {
        ...
        if (retCode == ERROR_SUCCESS) {
          ...
          dwLen = dwKeyEn.size();
          retCode =
              RegQueryValueEx(hKey, achValue.c_str(), 0, NULL, (LPBYTE)dwKeyEn.c_str(), &dwLen);
          if (retCode == ERROR_SUCCESS) {
            char *lpValue;
            ...
            for (lpValue = dwKeyEn.c_str(); lpValue && *lpValue;
              ...
              /*
               * Never add single character entries. Probably known buggy DRM Entry in Windows XP /
               * 2003
               */
              if (strlen(expandedKey.c_str()) <= 1) {
                Dmsg0(100, TEXT("Single character entry ignored. Probably reading buggy DRM entry "
                                "on Windows XP/2003 SP 1 and later\n"));
              } else {
...
static void *copy_thread(void *data) {
  ...
  while (1) {
    ...
    /*
     * Need to synchronize the main thread and this one so the main thread cannot miss the
     * conditional signal.
     */
    if (pthread_mutex_lock(&context->lock) != 0) {
      goto bail_out;
...
static inline bool SetupCopyThread(JobControlRecord *jcr, BareosWinFilePacket *bfd) {
  ...
  new_context->save_data =
      (CopyThreadSaveData *)malloc(nr_save_elements * sizeof(CopyThreadSaveData));
 ...
 */
int win32_send_to_copy_thread(JobControlRecord *jcr, BareosWinFilePacket *bfd, char *data,
