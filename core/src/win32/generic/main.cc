...
DLL_IMP_EXP int WINAPI WinMain(HINSTANCE Instance, HINSTANCE /*PrevInstance*/, PSTR CmdLine,
  ...
  if (GetVersionEx(&osversioninfo) && osversioninfo.dwPlatformId == VER_PLATFORM_WIN32_NT) {
  ...
  INITCOMMONCONTROLSEX initCC = {sizeof(INITCOMMONCONTROLSEX), ICC_STANDARD_CLASSES};
...
void *Main_Msg_Loop(LPVOID lpwThreadParam) {
  ...
  if (CreateWindow(APP_NAME, APP_NAME, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 0, 0,
