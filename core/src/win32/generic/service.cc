...
void WINAPI serviceStartCallback(DWORD argc, char **argv) {
  ...
  if (!service_handle) {
    ...
    MessageBox(NULL, _("Failure contacting the Service Handler"), APP_DESC, MB_OK);
    return;
  }
...
/*
 * Service Control callback handler. The OS can call us here at any time, most often to stop the
 * service.
 */
void WINAPI serviceControlCallback(DWORD ctrlcode) {
...
int bareosServiceMain() {
  ...
  if (have_service_api) { /* New style service API */
    ...
    SERVICE_TABLE_ENTRY dispatchTable[] = {
        {(char *)APP_NAME, (LPSERVICE_MAIN_FUNCTION)serviceStartCallback}, {(char *)NULL, NULL}};
    ...
    if (kerneldll == NULL) {
      MessageBox(NULL, _("KERNEL32.DLL not found: Bareos service not started"), APP_DESC, MB_OK);
      return 1;
    }
    ...
    RegisterService =
        (DWORD(WINAPI *)(DWORD, DWORD))GetProcAddress(kerneldll, "RegisterServiceProcess");
    if (RegisterService == NULL) {
      MessageBox(NULL, _("Registry service not found: Bareos service not started"), APP_DESC,
                 MB_OK);
...
int installService(const char *cmdOpts) {
  ...
  if ((int)strlen(path) + (int)strlen(cmdOpts) + 30 < maxlen) {
    ...
    MessageBox(NULL, _("Service command length too long. Service not registered."), APP_DESC,
  ...
  if (have_service_api) {
     ...
     */
    bareosService =
        CreateService(serviceManager, APP_NAME, /* Our service name */
    ...
    if (!bareosService) {
      ...
      MessageBox(NULL, _("The Bareos service: " APP_NAME " could not be installed"), APP_DESC,
                 MB_ICONEXCLAMATION | MB_OK);
      return 0;
    }
     ...
     */
    SetServiceDescription(serviceManager, bareosService, (char *)_(SERVICE_DESC));
...
int removeService() {
  ...
  if (have_service_api) { /* Newer Windows platform (NT, Win2K, ...) */
      ...
      if (bareosService) {
        ...
        if (ControlService(bareosService, SERVICE_CONTROL_STOP, &status)) {
          ...
          if (status.dwCurrentState != SERVICE_STOPPED) {
            if (opt_debug) {
              MessageBox(NULL, _("The Bareos service: " APP_NAME " could not be stopped"), APP_DESC,
                         MB_ICONEXCLAMATION | MB_OK);
            }
          }
        }
        ...
        if (DeleteService(bareosService)) {
          if (opt_debug) {
            MessageBox(NULL, _("The Bareos service: " APP_NAME " has been removed"), APP_DESC,
                       MB_ICONINFORMATION | MB_OK);
          }
        } else {
          MessageBox(NULL, _("The Bareos service: " APP_NAME " could not be removed"), APP_DESC,
                     MB_ICONEXCLAMATION | MB_OK);
          stat = 1; /* error */
        }
        CloseServiceHandle(bareosService);
     ...
     */
    HKEY runservices;
    if (RegOpenKey(HKEY_LOCAL_MACHINE, "Software\\Microsoft\\Windows\\CurrentVersion\\RunServices",
                   &runservices) != ERROR_SUCCESS) {
      if (opt_debug) {
        MessageBox(NULL,
                   _("Could not find registry entry.\nService probably not registerd - the Bareos "
    ...
    if (!stopRunningBareos()) {
      if (opt_debug) {
        MessageBox(NULL, _("Bareos could not be contacted, probably not running"), APP_DESC,
                   MB_ICONEXCLAMATION | MB_OK);
      }
...
void LogLastErrorMsg(const char *message, const char *fname, int lineno) {
  ...
  FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, NULL, service_error, 0,
                (LPTSTR)&msg, 0, NULL);
  ...
  Bsnprintf(msgbuf, sizeof(msgbuf), _("\n\n%s error: %ld at %s:%d"), APP_NAME, service_error, fname,
            lineno);
...
/*
 * This is amazingly complicated just to get a bit of English explanation in the service manager's
 * dialog box.
 */
static void SetServiceDescription(SC_HANDLE hSCManager, SC_HANDLE hService, LPSTR lpDesc) {
  ...
  if (sclLock == NULL) {
     ...
     */
    lpqslsBuf =
        (LPQUERY_SERVICE_LOCK_STATUS)LocalAlloc(LPTR, sizeof(QUERY_SERVICE_LOCK_STATUS) + 256);
    if (lpqslsBuf == NULL) {
     ...
     */
    if (!QueryServiceLockStatus(hSCManager, lpqslsBuf, sizeof(QUERY_SERVICE_LOCK_STATUS) + 256,
                                &dwBytesNeeded)) {
