...
static bRC newPlugin(bpContext *ctx) {
   ...
   */
  bfuncs->registerBareosEvents(ctx, 6, bEventLevel, bEventRestoreCommand, bEventBackupCommand,
...
static void SetAdoConnectString(bpContext *ctx) {
  ...
  if (Bstrcasecmp(p_ctx->instance, DEFAULT_INSTANCE)) {
    Mmsg(ado_connect_string, "Provider=SQLOLEDB.1;Data Source=%s;Initial Catalog=master",
         p_ctx->server_address);
  } else {
    Mmsg(ado_connect_string, "Provider=SQLOLEDB.1;Data Source=%s\\%s;Initial Catalog=master",
...
static inline void PerformAdoBackup(bpContext *ctx) {
  ...
  switch (p_ctx->backup_level) {
    case L_INCREMENTAL:
      Mmsg(ado_query,
           "BACKUP LOG [%s] TO VIRTUAL_DEVICE='%s' WITH BLOCKSIZE=%d, BUFFERCOUNT=%d, "
           "MAXTRANSFERSIZE=%d",
           p_ctx->database, vdsname, DEFAULT_BLOCKSIZE, DEFAULT_BUFFERS, DEFAULT_BLOCKSIZE);
      break;
    case L_DIFFERENTIAL:
      Mmsg(ado_query,
           "BACKUP DATABASE [%s] TO VIRTUAL_DEVICE='%s' WITH DIFFERENTIAL, BLOCKSIZE=%d, "
           "BUFFERCOUNT=%d, MAXTRANSFERSIZE=%d",
           p_ctx->database, vdsname, DEFAULT_BLOCKSIZE, DEFAULT_BUFFERS, DEFAULT_BLOCKSIZE);
      break;
    default:
      Mmsg(ado_query,
           "BACKUP DATABASE [%s] TO VIRTUAL_DEVICE='%s' WITH BLOCKSIZE=%d, BUFFERCOUNT=%d, "
           "MAXTRANSFERSIZE=%d",
           p_ctx->database, vdsname, DEFAULT_BLOCKSIZE, DEFAULT_BUFFERS, DEFAULT_BLOCKSIZE);
      break;
  }
...
static inline void perform_aDoRestore(bpContext *ctx) {
  ...
  switch (p_ctx->backup_level) {
    case L_INCREMENTAL:
      Mmsg(ado_query,
           "RESTORE LOG [%s] FROM VIRTUAL_DEVICE='%s' WITH BLOCKSIZE=%d, BUFFERCOUNT=%d, "
           "MAXTRANSFERSIZE=%d, %s",
           p_ctx->database, vdsname, DEFAULT_BLOCKSIZE, DEFAULT_BUFFERS, DEFAULT_BLOCKSIZE,
    ...
    default:
      Mmsg(ado_query,
           "RESTORE DATABASE [%s] FROM VIRTUAL_DEVICE='%s' WITH BLOCKSIZE=%d, BUFFERCOUNT=%d, "
           "MAXTRANSFERSIZE=%d, %s",
           p_ctx->database, vdsname, DEFAULT_BLOCKSIZE, DEFAULT_BUFFERS, DEFAULT_BLOCKSIZE,
           p_ctx->DoNoRecovery ? "NORECOVERY" : "RECOVERY");
      break;
...
static inline bool SetupVdiDevice(bpContext *ctx, struct io_pkt *io) {
  ...
  if ((p_ctx->username && !p_ctx->password) || (!p_ctx->username && p_ctx->password)) {
   ...
   */
  hr = CoCreateInstance(CLSID_MSSQL_ClientVirtualDeviceSet, NULL, CLSCTX_INPROC_SERVER,
                        IID_IClientVirtualDeviceSet2, (void **)&p_ctx->VDIDeviceSet);
  if (!SUCCEEDED(hr)) {
    comReportError(ctx, hr);
    return false;
  }
 ...
 */
static bRC setFileAttributes(bpContext *ctx, struct restore_pkt *rp) { return bRC_OK; }
