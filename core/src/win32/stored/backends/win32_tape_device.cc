...
static inline LONG Read24BitSigned(const unsigned char *pValue) {
  return ((LONG)(((ULONG)pValue[0] << 16) | ((ULONG)pValue[1] << 8) | (ULONG)pValue[2])) << 8 >> 8;
}
...
static inline LONG Read32BitSigned(const unsigned char *pValue) {
  return (LONG)(((ULONG)pValue[0] << 24) | ((ULONG)pValue[1] << 16) | ((ULONG)pValue[2] << 8) |
                (ULONG)pValue[3]);
}

static inline ULONG Read32BitUnsigned(const unsigned char *pValue) {
  return (((ULONG)pValue[0] << 24) | ((ULONG)pValue[1] << 16) | ((ULONG)pValue[2] << 8) |
          (ULONG)pValue[3]);
}
...
int win32_tape_device::d_open(const char *pathname, int flags, int mode) {
  ...
  if (hDevice != INVALID_HANDLE_VALUE) {
    ...
    dwResult = GetTapeParameters(pHandleInfo->OSHandle, GET_TAPE_DRIVE_INFORMATION, &dwSize,
                                 &TapeDriveParameters);
...
int win32_tape_device::TapeOp(struct mtop *mt_com) {
  ...
  switch (mt_com->mt_op) {
    ...
    case MTBSF:
      for (index = 0; index < mt_com->mt_count; index++) {
        result =
            SetTapePosition(pHandleInfo->OSHandle, TAPE_SPACE_FILEMARKS, 0, (DWORD)-1, ~0, FALSE);
        if (result == NO_ERROR) {
          pHandleInfo->ulFile--;
      ...
      break;
    case MTFSR:
      result = SetTapePosition(pHandleInfo->OSHandle, TAPE_SPACE_RELATIVE_BLOCKS, 0,
    ...
    case MTFSFM:
      for (index = 0; index < mt_com->mt_count; index++) {
        result = SetTapePosition(pHandleInfo->OSHandle, TAPE_SPACE_FILEMARKS, 0, mt_com->mt_count,
                                 0, FALSE);
        if (result == NO_ERROR) {
          result =
              SetTapePosition(pHandleInfo->OSHandle, TAPE_SPACE_FILEMARKS, 0, (DWORD)-1, ~0, FALSE);
    ...
    case MTSETBLK: {
      ...
      result =
          SetTapeParameters(pHandleInfo->OSHandle, SET_TAPE_MEDIA_INFORMATION, &SetMediaParameters);
      break;
    }
    case MTSEEK: {
      TAPE_POSITION_INFO TapePositionInfo;

      result = SetTapePosition(pHandleInfo->OSHandle, TAPE_ABSOLUTE_BLOCK, 0, mt_com->mt_count, 0,
                               FALSE);
    ...
    case MTTELL: {
      ...
      result = GetTapePosition(pHandleInfo->OSHandle, TAPE_ABSOLUTE_BLOCK, &partition, &offset,
                               &offsetHi);
      if (result == NO_ERROR) {
        return offset;
      }
    ...
    case MTFSS:
      result = SetTapePosition(pHandleInfo->OSHandle, TAPE_SPACE_SETMARKS, 0, mt_com->mt_count, 0,
                               FALSE);
      break;
    case MTBSS:
      result = SetTapePosition(pHandleInfo->OSHandle, TAPE_SPACE_SETMARKS, 0, -mt_com->mt_count, ~0,
    ...
    case MTCOMPRESSION: {
      ...
      result = GetTapeParameters(pHandleInfo->OSHandle, GET_TAPE_DRIVE_INFORMATION, &size,
                                 &GetDriveParameters);
      ...
      if (result == NO_ERROR) {
        ...
        result = SetTapeParameters(pHandleInfo->OSHandle, SET_TAPE_DRIVE_INFORMATION,
                                   &SetDriveParameters);
      }
      break;
    }
    case MTSETPART:
      result =
          SetTapePosition(pHandleInfo->OSHandle, TAPE_LOGICAL_BLOCK, mt_com->mt_count, 0, 0, FALSE);
      break;
    case MTMKPART:
      if (mt_com->mt_count == 0) {
        result = CreateTapePartition(pHandleInfo->OSHandle, TAPE_INITIATOR_PARTITIONS, 1, 0);
      } else {
        result = CreateTapePartition(pHandleInfo->OSHandle, TAPE_INITIATOR_PARTITIONS, 2,
                                     mt_com->mt_count);
      }
      break;
    default:
...
int win32_tape_device::TapeGet(struct mtget *mt_get) {
  ...
  if (result != NO_ERROR) {
    ...
    result =
        GetTapeParameters(pHandleInfo->OSHandle, GET_TAPE_DRIVE_INFORMATION, &size, &drive_params);
    if (result == NO_ERROR) {
  ...
  result =
      GetTapeParameters(pHandleInfo->OSHandle, GET_TAPE_MEDIA_INFORMATION, &size, &media_params);
   ...
   * Block Number
   */
  mt_get->mt_blkno = (__daddr_t)(
      pHandleInfo->bBlockValid ? pos_info.BlockNumber - pHandleInfo->ullFileStart : (ULONGLONG)-1);
...
static DWORD GetTapePositionInfo(HANDLE hDevice, PTAPE_POSITION_INFO TapePositionInfo) {
  ...
  for (int pass = 0; pass < 2; pass++) {
    ...
    bResult = DeviceIoControl(hDevice, IOCTL_SCSI_PASS_THROUGH, ScsiPassThrough,
                              sizeof(SCSI_PASS_THROUGH), ScsiPassThrough, dwBufferSize,
    ...
    if (bResult && dwBytesReturned >= (offsetof(SCSI_PASS_THROUGH, ScsiStatus) +
      ...
      if (ScsiPassThrough->ScsiStatus == SCSISTAT_GOOD) {
        PREAD_POSITION_RESULT pPosResult =
            (PREAD_POSITION_RESULT)((PUCHAR)ScsiPassThrough + ScsiPassThrough->DataBufferOffset);
...
static DWORD GetDensityBlockSize(HANDLE hDevice, DWORD *pdwDensity, DWORD *pdwBlockSize) {
  ...
  do {
    ...
    bResult = DeviceIoControl(hDevice, IOCTL_STORAGE_GET_MEDIA_TYPES_EX, NULL, 0,
                              (LPVOID)pGetMediaTypes, dwBufferSize, &dwBytesReturned, NULL);
  ...
  for (DWORD idxMedia = 0; idxMedia < pGetMediaTypes->MediaInfoCount; idxMedia++) {
    if (pGetMediaTypes->MediaInfo[idxMedia].DeviceSpecific.TapeInfo.MediaCharacteristics &
...
int win32_tape_device::TapePos(struct mtpos *mt_pos) {
  ...
  result =
      GetTapePosition(pHandleInfo->OSHandle, TAPE_ABSOLUTE_BLOCK, &partition, &offset, &offsetHi);
  if (result == NO_ERROR) {
